...
/* Match the original FSP lwIP build.  Its socket API runs through the
 * tcpip_thread mailbox without taking the global core mutex.  lwIP 2.1.2's
 * default is 1, which adds a mutex operation to every socket send and also
 * changes the interaction between the application and tcpip_thread. */
...
/* lwIP 动态堆（PBUF_RAM）：TCP 发送、ICMP 回包、分片重组都靠它。
 * lwIP 默认只有 1600 字节，连两个 1500 字节的发送 pbuf 都放不下。
 * 可用 menuconfig 的 RT_LWIP_MEM_SIZE 覆盖。 */
#ifdef RT_LWIP_MEM_SIZE
#define MEM_SIZE                    RT_LWIP_MEM_SIZE
#elif !defined MEM_SIZE
 ...
 * socket 层每把一个数据报交给应用就要占一个 netbuf，默认 2 远远不够。 */
...
/* 每收一个数据包都要占一个 tcpip_msg，默认 16 在高速下行时容易耗尽 */
...
/* MEMP_NUM_TCP_SEG 必须 >= TCP_SND_QUEUELEN，否则 lwIP 的 sanity check 直接
 * #error。两者来源不同（前者来自 RT_LWIP_TCP_SEG_NUM，后者由 TCP_SND_BUF
 * 推出），很容易改了发送窗口却忘了同步段池，这里自动抬到下限并给出警告。 */
#if (MEMP_NUM_TCP_SEG < TCP_SND_QUEUELEN)
#warning "MEMP_NUM_TCP_SEG < TCP_SND_QUEUELEN: auto-raised (sync RT_LWIP_TCP_SEG_NUM)"
...
#define UDP_TTL                     255
/* UDP socket 接收邮箱深度。默认 1 意味着“应用没来得及取走的包全丢”。
 * 这是无条件定义，命令行 -D 压不过它，只能走 rtconfig.h 的 RT_LWIP_*。 */
 ...
 * lwIP 原版是 INT_MAX(不限)，RT-Thread port 压到了 8192 —— 对 1472 字节的
 * 数据报只能积压 5 个，超出的部分在 recv_udp() 里被静默丢弃。 */
#ifdef RT_LWIP_RECV_BUFSIZE_DEFAULT
#define RECV_BUFSIZE_DEFAULT            RT_LWIP_RECV_BUFSIZE_DEFAULT
#elif !defined RECV_BUFSIZE_DEFAULT
