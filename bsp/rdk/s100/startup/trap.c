...
#endif
extern void rt_unwind(struct rt_hw_exp_stack *regs, unsigned int pc_adj);
void (*rt_trap_hook)(struct rt_hw_exp_stack *regs, const char *ex, unsigned int exception_type);

static void rt_hw_show_backtrace(struct rt_hw_exp_stack *regs)
{
    rt_unwind(regs, 0U);
}
...
void rt_hw_trap_el2(struct rt_hw_exp_stack *regs, unsigned int trap_id)
{
    const char *ex;
    ...
    switch (trap_id)
    {
    case RT_HW_EL2_TRAP_UNDEF:
        ex = "undefined instruction";
        exception_type = UND_EXCEPTION;
        break;
    case RT_HW_EL2_TRAP_PABT:
        ex = "prefetch abort";
        exception_type = PABT_EXCEPTION;
        break;
    case RT_HW_EL2_TRAP_DABT:
        ex = "data abort";
        exception_type = DABT_EXCEPTION;
        break;
    case RT_HW_EL2_TRAP_IRQ:
        ex = "unexpected IRQ";
        exception_type = RESV_EXCEPTION;
        break;
    case RT_HW_EL2_TRAP_FIQ:
        ex = "unexpected FIQ";
        exception_type = RESV_EXCEPTION;
        break;
    case RT_HW_EL2_TRAP_HVC:
        ex = "unexpected HVC";
        exception_type = RESV_EXCEPTION;
        break;
    ...
    {
        rt_trap_hook(regs, ex, exception_type);
    }
}
