...
static int wifi_speed_test(int argc, char **argv)
{
    ...
    if (udp && (direction == 0))
    {
        ...
        printf("[speed] rx_drv: pkts=%u bytes=%u dropped=%u\n",
               (unsigned)stats.rx_packets, (unsigned)stats.rx_bytes,
               (unsigned)stats.rx_dropped);
        printf("[speed] rx_app: pkts=%u bytes=%u\n",
               (unsigned)(meter.total / (rt_uint64_t)payload),
               (unsigned)meter.total);
    }
    ...
    if (udp && (direction != 0))
    {
        struct wifi_wlan_stats stats;
        wifi_wlan_stats_get(&stats);
        printf("[speed] tx_wlan: pkts=%u bytes=%u queue_reject=%u "
