...
#include "../../wpa_supplicant/supp_def.h"
...
#define HOSTAPD_DFS_REGION_KR	4
#define HOSTAPD_DFS_REGION_CH	5
...
#define WPA_SCAN_AUTHENTICATED      BIT(4)	//MODIFY_SUPPLICANT_FOR_FREERTOS
...
#ifdef CONFIG_SCAN_UMAC_HEAP_ALLOC	//MODIFY_SUPPLICANT_FOR_FREERTOS
...
#define WPA_DRIVER_MESH_CONF_FLAG_FORWARDING		0x00000020
	 ...
	 * See FC80211_MESHCONF_* for all the mesh config parameters.
	 */
	unsigned int flags;
	int auto_plinks;
 ...
 */
#define WPA_DRIVER_FLAGS_SANE_ERROR_CODES		WPA_DRIVER_FLAGS_VALID_ERROR_CODES	//MODIFY_SUPPLICANT_FOR_FREERTOS
...
//struct mac_address {
struct hostapd_mac_address {				//MODIFY_SUPPLICANT_FOR_FREERTOS
	u8 addr[ETH_ALEN];
};
...
struct hostapd_acl_params {
	...
	struct hostapd_mac_address mac_acl[0];	//MODIFY_SUPPLICANT_FOR_FREERTOS
};
...
enum wnm_oper {
	...
	WNM_SLEEP_TFS_REQ_IE_SET,	/* AP requests driver to set TFS req IE for a STA */
	WNM_SLEEP_TFS_RESP_IE_ADD,  /* AP requests driver to add TFS resp IE for a STA */
	WNM_SLEEP_TFS_RESP_IE_NONE, /* AP requests empty TFS resp IE */
	WNM_SLEEP_TFS_RESP_IE_SET,  /* AP requests driver to set TFS resp IE for a STA */
 ...
 */
struct beacon_data {
//struct hostapd_beacon_data {		//MODIFY_SUPPLICANT_FOR_FREERTOS
	u8 *head, *tail;
	u8 *beacon_ies;
	u8 *proberesp_ies;
	u8 *assocresp_ies;
...
struct csa_settings {
	...
	//struct hostapd_beacon_data beacon_csa;		//MODIFY_SUPPLICANT_FOR_FREERTOS
	struct beacon_data beacon_after;
	//struct hostapd_beacon_data beacon_after;	//MODIFY_SUPPLICANT_FOR_FREERTOS
...
struct wpa_driver_ops {
	 ...
	 */
	//MODIFY_SUPPLICANT_FOR_FREERTOS - chg CHG_SET_KEY
	...
	struct hostapd_hw_modes * (*get_hw_feature_data)(void *priv,
							 u16 *num_modes,
							 u16 *flags 
#if 0 /* FC9000 Only */	//MODIFY_SUPPLICANT_FOR_FREERTOS
							 ,u16 u8 *dfs
#endif	// 0
	);
	...
	/**
	 * free_umac_scan_results - Free umac scan results
	 * @priv: private driver interface data
	 */
	//MODIFY_SUPPLICANT_FOR_FREERTOS - add
	int (*free_umac_scan_results)(void *priv);
	...
	/**
	 * none_station - Notice to none station
	 * @priv: Private driver interface data
	 */
	int (*none_station)(void *priv);
#endif /* CONFIG_AP_NONE_STA */

#ifdef CONFIG_AP_POWER
	/**
	 * set_ap_power - set power of AP
	 * @priv: Private driver interface data
	 */
	int (*set_ap_power)(void *priv, int type, int power, int *get_power);
#endif /* CONFIG_AP_POWER */

#ifdef CONFIG_AP_ISOLATION
	/**
	 * set_ap_isolate - Bridge Control
	 * @priv: Private driver interface data
	 * @isolate: default 0 - bridge / 1 - Not bridge
	 */
	int (*set_ap_isolate)(void *priv, int isolate);
#endif /* CONFIG_AP_ISOLATION */

	/**
	 * addba_reject - for sigma test
	 * @priv: Private driver interface data
	 * @addba_reject: 1: Decline Response Enable / 0: AMPDU RX Normal
	 * Returns: 0 on success, -1 on failure
	 */
int (*addba_reject)(void *priv, u8 addba_reject);
#endif /* 1 */
	...
	/**
	 * chk_keep_alive - Send keep-alive (NULL) packet and check ACK
	 * @priv: Private driver interface data
	 * @addr: MAC address of the station
	 * Returns: 0 on success, -1 on failure
	 */
	//MODIFY_SUPPLICANT_FOR_FREERTOS
	int (*chk_keep_alive)(void *priv, const u8 *addr);
#endif  /* 1 */
...
#if 1	/* by Shingu 20160618 (P2P GO Inactivity) */
	/**
	 * chk_deauth_send_done - Check whether Disassoc or Deauth is sent
	 * or not (AP only)
	 * @priv: Private driver interface data
	 * Returns: 0 on success, -1 on failure
	 *
	 */
	//MODIFY_SUPPLICANT_FOR_FREERTOS
	int (*chk_deauth_send_done)(void *priv);
#endif  /* 1 */

#ifdef  CONFIG_P2P_POWER_SAVE /* by Shingu 20170524 (P2P_PS) */
#ifdef  CONFIG_P2P_UNUSED_CMD
	/**
	 * p2p_ps_from_sp - Change status of the P2P Power-save (NoA or Opps)
	 * @priv: Private driver interface data
	 * @p2p_ps_status: 1 or 0 (on / off)
	 * Returns: 0 on success, -1 on failure
	 */
	//MODIFY_SUPPLICANT_FOR_FREERTOS
	int (*p2p_ps_from_sp)(void *priv, int p2p_ps_status);
#endif
...
#if 1 /* FC9000 Only ?? */	//MODIFY_SUPPLICANT_FOR_FREERTOS
	/**
	 * set_retry - Set retry long or retry short
	 * @priv: Private driver interface data
	 * @retry: retry long or retry short value
	 * @retry_long: if 1 retry long, if 0 retry short
	 * Returns: 0 on success, -1 on failure
	 */
	int (*set_retry)(void *priv, u8 retry, u8 retry_long);

	int (*get_retry)(void *priv, u8 retry_long);

	int (*get_rts)(void *priv);
#endif /* 1 */
	...
	/**
	 * send_ft_action - Send ft action
	 */
	void (*send_ft_action) (void *priv, UCHAR action, const UCHAR *target_ap, const UCHAR *ies, size_t ies_len);
	...
	int (*ap_pwrsave)(void *priv, int ps_state, int timeout); // FC9000 only ??	//MODIFY_SUPPLICANT_FOR_FREERTOS
#endif  /* CONFIG_AP */
...
// FC9000 Only Start	//MODIFY_SUPPLICANT_FOR_FREERTOS
#ifdef CONFIG_STA_POWER_SAVE
	int (*sta_power_save)(void *priv, int pwrsave_mode);
#endif /* CONFIG_STA_POWER_SAVE */
	int (*get_rtctime_gap)(void *priv, u64 *rtctimegap);
// FC9000 Only End
...
enum wpa_event_type {
	 ...
	 */
	EVENT_AUTH,
	EVENT_CONN,		// CONN_TEMPORARY
	EVENT_DISCONN,	// CONN_TEMPORARY
...
#if 1 //def CONFIG_RECEIVE_DHCP_EVENT /* FC9000 Only */	//MODIFY_SUPPLICANT_FOR_FREERTOS
	EVENT_DHCP_NO_RESPONSE,
	EVENT_DHCP_ACK_OK,
...
union wpa_event_data {
	...
	struct conn_info {		// CONN_TEMPORARY
		u8 peer[ETH_ALEN];
		u8 bssid[ETH_ALEN];
		u16 conn_type;
		u16 conn_transaction;
		u16 status_code;
		const u8 *ies;
		size_t ies_len;
...
struct i802_bss {	//MODIFY_SUPPLICANT_FOR_FREERTOS
