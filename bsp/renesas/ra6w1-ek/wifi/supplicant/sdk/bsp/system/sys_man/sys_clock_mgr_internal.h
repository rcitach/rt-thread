 ...
 */
void cm_check_xtalm_startup(void);

#ifdef OS_FREERTOS
/**
 * \brief The fast XTAL clock (XTALxxM) has never stopped! Inform the Adapters upon "wake-up".
 *
 * \details The debugger did not detach and the chip did not enter into sleep. Thus, the fast XTAL
 *        clock (XTALxxM) is still running. Use this to inform Adapters that the fast XTAL clock
 *        (XTALxxM) is available.
 *
 * \warning It is provided only for debugging purposes since when the debugger is attached, the
 *        fast XTAL clock (XTALxxM) does not stop and no XTALxxM Ready interrupt hits! It will
 *        never be called in normal operation.
 */
void cm_sys_xtalm_running(void);
#endif
