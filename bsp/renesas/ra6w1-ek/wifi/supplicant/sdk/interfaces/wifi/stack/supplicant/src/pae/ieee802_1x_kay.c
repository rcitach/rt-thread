...
#define MKA_ALIGN_LENGTH(len) (((len) + 0x3U) & ~0x3U)
...
static unsigned int get_mka_param_body_len(const void *body)
{
	...
	return (unsigned int) ((hdr->length << 8) | hdr->length1);
}
...
ieee802_1x_mka_dump_sak_use_body(struct ieee802_1x_mka_sak_use_body *body)
{
	...
	body_len = (int) get_mka_param_body_len(body);
...
ieee802_1x_mka_basic_body_length(struct ieee802_1x_mka_participant *participant)
{
	...
	length += (int) participant->ckn.len;
	return (int) MKA_ALIGN_LENGTH((unsigned int) length);
}
				 ...
				 size_t msg_len)
{
	...
	if (!peer) {
		...
		if (peer) {
			...
			new_expire = time(NULL) + (time_t) (MKA_HELLO_TIME * 1.5) / 1000U;
	...
	struct ieee802_1x_mka_participant *participant)
{
	...
	dl_list_for_each(peer, &participant->live_peers,
			 struct ieee802_1x_kay_peer, list)
		len = len + (int) sizeof(struct ieee802_1x_mka_peer_id);

	return (int) MKA_ALIGN_LENGTH((unsigned int) len);
}
	...
	struct wpabuf *buf)
{
	...
	length = (unsigned int) ieee802_1x_mka_get_live_peer_length(participant);
	...
	struct wpabuf *buf)
{
	...
	length = (unsigned int) ieee802_1x_mka_get_potential_peer_length(participant);
	body = wpabuf_put(buf, sizeof(struct ieee802_1x_mka_peer_body));
	...
	struct wpabuf *buf)
{
	...
	length = (unsigned int) ieee802_1x_mka_get_sak_use_length(participant);
	...
	body->lan = participant->lan & 0x03U;
	...
	body->oan = participant->oan & 0x03U;
	...
	struct wpabuf *buf)
{
	...
	length = (unsigned int) ieee802_1x_mka_get_dist_sak_length(participant);
	body = wpabuf_put(buf, length);
	...
	const u8 *mka_msg, size_t msg_len)
{
	...
	unwrap_sak = os_zalloc((size_t) sak_len);
	if (!unwrap_sak) {
		wpa_printf(MSG_ERROR, "KaY-%s: Out of memory", __func__);
		return -1;
	}
...
ieee802_1x_mka_get_icv_length(struct ieee802_1x_mka_participant *participant)
{
	...
	if (mka_alg_tbl[participant->kay->mka_algindex].icv_len !=
		...
		length = 0;
	length = length + (int) mka_alg_tbl[participant->kay->mka_algindex].icv_len;
			       ...
			       struct wpabuf *buf)
{
	...
	length = (unsigned int) ieee802_1x_mka_get_icv_length(participant);
	if (mka_alg_tbl[participant->kay->mka_algindex].icv_len !=
	    DEFAULT_ICV_LEN)  {
...
ieee802_1x_kay_generate_new_sak(struct ieee802_1x_mka_participant *participant)
{
	...
	cs = &cipher_suite_tbl[kay->macsec_csindex];
	key_len = (unsigned int) cs->sak_len;
	...
	ctx_len = (int) (key_len + sizeof(kay->dist_kn));
	dl_list_for_each(peer, &participant->live_peers,
			 struct ieee802_1x_kay_peer, list)
		ctx_len = ctx_len + (int) sizeof(peer->mi);
	ctx_len = ctx_len + (int) sizeof(participant->mi);

	context = os_zalloc((size_t) ctx_len);
	if (!context)
		goto fail;
			    ...
			    struct wpabuf *pbuf)
{
	...
	eapol_hdr->length = host_to_be16((u16) wpabuf_tailroom(pbuf));
	wpa_printf(MSG_DEBUG,
	...
	struct ieee802_1x_mka_participant *participant)
{
	...
	for (i = 0; i < ARRAY_SIZE(mka_body_handler); i++) {
		if (mka_body_handler[i].body_present &&
		    mka_body_handler[i].body_present(participant))
			length = length + (size_t) mka_body_handler[i].body_length(participant);
	}
...
int ieee802_1x_kay_enable_new_info(struct ieee802_1x_kay *kay)
{
	...
	if (principal->retry_count < MAX_RETRY_CNT || principal->mode == _PSK) {
			      ...
			      size_t buflen)
{
	...
	res = os_snprintf(pos, (size_t) (end - pos),
			  "PAE KaY status=%s\n"
			  "Authenticated=%s\n"
			  "Secured=%s\n"
			  "Failed=%s\n"
	...
	dl_list_for_each(p, &kay->participant_list,
		...
		res = os_snprintf(pos2, (size_t) (end - pos2), "participant_idx=%d\nckn=",
			count);
		if (os_snprintf_error(buflen, res))
			return end - pos;
		pos2 += res;
		...
		pos2 += wpa_snprintf_hex(pos2, (size_t) (end - pos2), p->ckn.name,
					 p->ckn.len);

		res = os_snprintf(pos2, (size_t) (end - pos2),
				  "\nmi=%s\n"
				  "mn=%u\n"
				  "active=%s\n"
				  "participant=%s\n"
			   ...
			   char *end)
{
	...
	dl_list_for_each(p, peers, struct ieee802_1x_kay_peer, list) {
		res = os_snprintf(pos, (size_t) (end - pos),
				  ...
				  live ? 1 : 2,
				  sci_txt(&p->sci));
		if (os_snprintf_error((size_t) (end - pos), res))
			return pos;
		pos += res;
	}
			   ...
			   size_t buflen)
{
	...
	dl_list_for_each(p, &kay->participant_list,
		...
		res = os_snprintf(pos2, (size_t) (end - pos2), "ieee8021XKayMkaPartCKN=");
		if (os_snprintf_error(buflen, res))
			return end - pos;
		pos2 += res;
