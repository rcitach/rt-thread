...
#if 0 // FC9000 inline function	// MODIFY_SUPPLICANT_FOR_FREERTOS
...
#if 1 // FC9000		// MODIFY_SUPPLICANT_FOR_FREERTOS
...
int hostapd_drv_sta_disassoc(struct hostapd_data *hapd,
			     const u8 *addr, int reason);
#if 1	/* by Shingu 20160622 (P2P GO Inactivity) */	// MODIFY_SUPPLICANT_FOR_FREERTOS
...
#ifdef  CONFIG_P2P_POWER_SAVE /* by Shingu 20170524 (P2P_PS) */	// MODIFY_SUPPLICANT_FOR_FREERTOS
#ifdef  CONFIG_P2P_UNUSED_CMD
static inline int hostapd_drv_p2p_ps_from_sp(struct hostapd_data *hapd,
						int p2p_ps_status)
{
	if (hapd->driver == NULL || hapd->driver->p2p_ps_from_sp == NULL)
		return 0;
	return hapd->driver->p2p_ps_from_sp(hapd->drv_priv, p2p_ps_status);
#endif
...
static inline int hostapd_drv_none_station( struct hostapd_data *hapd)
{
	if (hapd->driver == NULL || hapd->driver->none_station == NULL)
		return -1;
	return hapd->driver->none_station(hapd->drv_priv);
}
#endif /* CONFIG_AP_NONE_STA */

#ifdef CONFIG_AP_POWER
static inline int hostapd_drv_set_ap_power( struct hostapd_data *hapd, int type, int power, int *get_power)
{
	if (hapd->driver == NULL || hapd->driver->set_ap_power == NULL)
		return -1;
	return hapd->driver->set_ap_power(hapd->drv_priv, type, power, get_power);
}
...
#if 1	/* by Shingu 20161010 (Keep-alive) */	// MODIFY_SUPPLICANT_FOR_FREERTOS
static inline int hostapd_drv_chk_keep_alive(struct hostapd_data *hapd, const u8 *addr)
{
	if (hapd->driver == NULL || hapd->driver->chk_keep_alive == NULL)
		return -1;
	return hapd->driver->chk_keep_alive(hapd->drv_priv, addr);
}
#endif	/* 1 */
...
static inline int hostapd_drv_set_ap_isolate(   struct hostapd_data *hapd, int isolate)
{
	if (hapd->driver == NULL || hapd->driver->set_ap_isolate == NULL)
		return -1;
	hapd->conf->isolate = isolate;
	return hapd->driver->set_ap_isolate(hapd->drv_priv, isolate);
}
				     ...
				     struct wpa_driver_ap_params *params)
{
	if (hapd->driver == NULL || hapd->driver->set_ap == NULL)
		return 0;
	return hapd->driver->set_ap(hapd->drv_priv, params);		// wpa_driver_fc80211_set_ap
}
					 ...
					 unsigned int freq)
{
	...
	if (!hapd->driver->get_survey)
		return -1;
	return hapd->driver->get_survey(hapd->drv_priv, freq);	// fc80211_get_dump_survey
}
#endif /* CONFIG_AP */
...
#ifdef CONFIG_AP
// FC9000 only ??	// MODIFY_SUPPLICANT_FOR_FREERTOS - start add
static inline int hostapd_drv_ap_pwrsave(struct hostapd_data *hapd, int ps_state, int timeout)
{
	if (hapd->driver == NULL || hapd->driver->ap_pwrsave == NULL)
		return -1;

	return hapd->driver->ap_pwrsave(hapd->drv_priv, ps_state, timeout);
}
#endif /* CONFIG_AP */

static inline int hostapd_set_rts(struct hostapd_data *hapd, int rts)
{
...
static inline int hostapd_set_frag(struct hostapd_data *hapd, int frag)
{
...
static inline int hostapd_addba_reject(struct hostapd_data *hapd, u8 addba_reject)
{
	if (hapd->driver == NULL || hapd->driver->addba_reject == NULL)
		return -1;
	return hapd->driver->addba_reject(hapd->drv_priv, addba_reject);
}
// FC9000 only ??	// MODIFY_SUPPLICANT_FOR_FREERTOS - end add
