...
extern void eapol_callback_init(void);
						       ...
						       u8 id)
{
	...
	if (domain) {
		...
		wpabuf_put_u8(msg, (u8) domain_len);
...
void eap_log_msg(struct eap_sm *sm, const char *fmt, ...)
{
	...
	buf = os_malloc((size_t) buflen);
	if (buf == NULL)
		return;
	va_start(ap, fmt);
	vsnprintf(buf, (size_t) buflen, fmt, ap);
	va_end(ap);
...
SM_STATE(EAP, FAILURE)
{
	...
	wpabuf_free(sm->eap_if.eapReqData);
	sm->eap_if.eapReqData = eap_sm_buildFailure(sm, (u8) sm->currentId);
...
SM_STATE(EAP, SUCCESS)
{
	...
	wpabuf_free(sm->eap_if.eapReqData);
	sm->eap_if.eapReqData = eap_sm_buildSuccess(sm, (u8) sm->currentId);
	wpabuf_free(sm->lastReqData);
	sm->lastReqData = NULL;
...
static void eap_sm_parseEapResp(struct eap_sm *sm, const struct wpabuf *resp)
{
	...
	if (plen > sizeof(*hdr)) {
		...
		if (sm->respMethod == EAP_TYPE_EXPANDED) {
			...
			sm->respVendor = (int) WPA_GET_BE24(pos);
			pos += 3;
