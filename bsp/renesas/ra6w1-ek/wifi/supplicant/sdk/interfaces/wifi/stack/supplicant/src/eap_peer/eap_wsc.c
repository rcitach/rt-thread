...
static void * eap_wsc_init(struct eap_sm *sm)
{
	...
	if (pos) {
		...
		while (*pos != '\0' && *pos != ' ')
			pos++;
		cfg.pin_len = (size_t) (pos - (const char *) cfg.pin);
		if (cfg.pin_len == 6 &&
					 ...
					 struct eap_method_ret *ret, u8 id)
{
	...
	if (flags & WSC_FLAGS_LF)
		wpabuf_put_be16(resp, (u16) wpabuf_len(data->out_buf));
				       ...
				       const struct wpabuf *reqData)
{
	...
	if (data->in_buf &&
	    eap_wsc_process_cont(data, pos, (size_t) (end - pos), op_code) < 0) {
		ret->ignore = true;
		return NULL;
	}
	...
	if (data->in_buf == NULL) {
		...
		wpabuf_set(&tmpbuf, pos, (size_t) (end - pos));
		data->in_buf = &tmpbuf;
	}
