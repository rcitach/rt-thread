			  ...
			  unsigned int len)
{
	...
	num = get_shared_radio_freqs(wpa_s, &freqs->freq, wpa_s->num_multichan_concurrent);
					     ...
					     int freq)
{
	...
	if (
		...
		wpa_s->conf->p2p_ignore_shared_freq &&
...
#endif	// defined ( __SUPP_27_SUPPORT__ )
...
#endif	/* by Bright */	//MODIFY_SUPPLICANT_FOR_FREERTOS
}
#endif // CONFIG_P2P_UNUSED_CMD
...
{
	if (
		wpa_s->global->p2p_24ghz_social_channels &&
			 ...
			 bool include_6ghz)
{
	...
	case P2P_SCAN_FULL:
		...
		update_freqs_with_country_code(wpa_s, &params->freqs, 0);
#else
				 ...
				 enum p2p_group_removal_reason removal_reason)
{
	...
	if (ssid == NULL) {
		...
		if (ssid == NULL &&
			wpa_s->p2p_group_interface == NOT_P2P_GROUP_INTERFACE)
		{
			wpa_printf(MSG_DEBUG, "P2P: P2P group interface "
				   "not found");
			return -1;
		}
	}
...
#ifdef CONFIG_SCAN_RESULT_OPTIMIZE
		ra6wx_dump("P2P: Probe Response IEs",
				(u8 *) (bss->ie), bss->ie_len);
		ra6wx_dump("P2P: Beacon IEs",
					   ...
					   const u8 *go_dev_addr)
{
	...
	if (s) {
		...
		changed = 1;
#endif	// 0
		s = wpa_config_add_network(wpa_s->conf, wpa_s->ifname, FIXED_NETWORK_ID_NONE);	//MODIFY_SUPPLICANT_FOR_FREERTOS - chg
		if (s == NULL)
			return -1;
...
#ifdef  CONFIG_P2P_OPTION
		ra6wx_notice_prt(P2P_EVENT_GROUP_STARTED
				  ...
				  struct wpa_ssid *ssid)
{
	...
	persistent = wpas_p2p_get_persistent(
#ifdef  CONFIG_P2P_UNUSED_CMD
						wpa_s->p2pdev,
#else
						wpa_s->parent,
			      ...
			      int group_formation)
{
	...
	ssid = wpa_config_add_network(wpa_s->conf, wpa_s->ifname, FIXED_NETWORK_ID_P2P);	//MODIFY_SUPPLICANT_FOR_FREERTOS - chg
	if (ssid == NULL) {
...
#else
	ssid->ap_max_inactivity = wpa_s->conf->p2p_go_max_inactivity;
#endif
...
static void wpas_go_neg_completed(void *ctx, struct p2p_go_neg_results *res)
{
...
#if 1   /* by Shingu */	//MODIFY_SUPPLICANT_FOR_FREERTOS
	wpa_bss_flush(wpa_s);
#endif  /* 1 */
...
#if 1   /* by Shingu 20160707 (P2P Optimize) */	//MODIFY_SUPPLICANT_FOR_FREERTOS
	...
	if (get_run_mode() == WIFI_DEVICE_MODE_EXT_P2P_STATION) {
		...
		if (!res->role_go &&
			...
			ra6wx_notice_prt("P2P GO (peer) channel(%d) is different"
			       ...
			       u8 go_intent)
{
	...
	struct wpa_supplicant *wpa_s = ctx;
#endif	// 0
	ra6wx_p2p_prt("[%s] " P2P_EVENT_GO_NEG_REQUEST MACSTR
			" dev_passwd_id=%u\n",
			   ...
			   int new_device)
{
		...
		ra6wx_notice_prt("["P2P_EVENT_DEVICE_FOUND"]\nAddr: " MACSTR2 "\n"
				"Name: %s\n"
#ifdef	CONFIG_WIFI_DISPLAY
				"p2p_dev_addr=" MACSTR "\n"
				"pri_dev_type=%s\n"
...
#else
	ra6wx_p2p_prt("[%s] " P2P_EVENT_FIND_STOPPED "\n", __func__);
#endif
}
			     ...
			     const struct wpabuf *probe_resp_ie)
{
	struct wpa_supplicant *wpa_s = ctx;
	if (wpa_drv_probe_req_report(wpa_s, 1) < 0) {
		ra6wx_p2p_prt("[%s] P2P: Failed to request the driver to "
	...
	if (wpa_drv_remain_on_channel(wpa_s, freq, duration) < 0) {
		ra6wx_p2p_prt("[%s] P2P: Failed to request the driver "
...
/* by Shingu */	//MODIFY_SUPPLICANT_FOR_FREERTOS
					...
					struct p2p_channels *cli_chan)
{
...
#if 0 // Modify to use SOCIAL CHANNEL on 2.4G

    int min_chan, max_chan, resul;

    resul = ra6w1_regdb_get_ch_range_by_country_n_band(wpa_s->conf->country, 2, &min_ch, &max_chan, NULL, 0); 
    if (resul == pdTRUE) {        
    ...
    if (!ra6w1_regdb_is_valid_freq(wpa_s->conf->country, 5000 + 36 * 5, 0)) {
        wpas_p2p_add_chan(&chan->reg_class[cla], 36);
    }
    if (!ra6w1_regdb_is_valid_freq(wpa_s->conf->country, 5000 + 40 * 5, 0)) {
        wpas_p2p_add_chan(&chan->reg_class[cla], 40);
    }
    if (!ra6w1_regdb_is_valid_freq(wpa_s->conf->country, 5000 + 44 * 5, 0)) {
        wpas_p2p_add_chan(&chan->reg_class[cla], 44);
    }
    if (!ra6w1_regdb_is_valid_freq(wpa_s->conf->country, 5000 + 48 * 5, 0)) {
    ...
    if (!ra6w1_regdb_is_valid_freq(wpa_s->conf->country, 5000 + 149 * 5, 0)) {
        wpas_p2p_add_chan(&chan->reg_class[cla], 149);
    }
    if (!ra6w1_regdb_is_valid_freq(wpa_s->conf->country, 5000 + 153 * 5, 0)) {
        wpas_p2p_add_chan(&chan->reg_class[cla], 153);
    }
    if (!ra6w1_regdb_is_valid_freq(wpa_s->conf->country, 5000 + 156 * 5, 0)) {
        wpas_p2p_add_chan(&chan->reg_class[cla], 156);
    }
    if (!ra6w1_regdb_is_valid_freq(wpa_s->conf->country, 5000 + 161 * 5, 0)) {
...
#if 1   /* by Shingu 20180208 (P2P PS opt.) */	//MODIFY_SUPPLICANT_FOR_FREERTOS
#ifdef  CONFIG_P2P_UNUSED_CMD
...
static void wpas_p2p_ps_ctrl(void *ctx)
{
	...
	hostapd_p2p_default_noa(wpa_s->ap_iface->bss[0], 0);
	eloop_cancel_timeout(p2p_ps_resume, wpa_s, NULL);
	eloop_register_timeout(5, 0, p2p_ps_resume, wpa_s, NULL);
}
#endif
#endif  /* 1 */
				  ...
				  const char *conf_p2p_dev)
{
	...
	char ifname[32];		//MODIFY_SUPPLICANT_FOR_FREERTOS 100->32
	char force_name[32];	//MODIFY_SUPPLICANT_FOR_FREERTOS 100->32
	int ret = 0;
	...
	if (ret < 0) {
		//MODIFY_SUPPLICANT_FOR_FREERTOS
		ra6wx_p2p_prt("[%s] P2P: Failed to create P2P Device "
			"interface\n", __func__);
		return NULL;
	}
	os_strlcpy(wpa_s->pending_interface_name, ifname,
...
#endif	// CONFIG_P2P_UNUSED_CMD
	p2pdev_wpa_s->parent = wpa_s;	//MODIFY_SUPPLICANT_FOR_FREERTOS-add
	wpa_s->pending_interface_name[0] = '\0';
#if 0	//MODIFY_SUPPLICANT_FOR_FREERTOS
	return 0;
#else
	...
	if (conncap == P2PS_SETUP_GROUP_OWNER) {
		...
		if (!go_wpa_s) {
			...
			if (response_done && persistent_go) {
				...
				_wpas_p2p_group_add(wpa_s, 1, freq,
						   0, 0, 0, 0, 0, 0, false);
			}
...
static int wpas_prov_disc_resp_cb(void *ctx)
{
	...
	if (persistent_go) {
		...
		_wpas_p2p_group_add(wpa_s, 1, freq, 0, 0, 0, 0, 0, 0,
				   is_p2p_allow_6ghz(wpa_s->global->p2p));
	}
...
int wpas_p2p_init(struct wpa_global *global, struct wpa_supplicant *wpa_s)
{
	...
	if (get_run_mode() != WIFI_DEVICE_MODE_EXT_P2P_GO) {	//MODIFY_SUPPLICANT_FOR_FREERTOS - add
		if (wpa_s->conf->p2p_listen_reg_class &&
	    	wpa_s->conf->p2p_listen_channel) {
			p2p.reg_class = wpa_s->conf->p2p_listen_reg_class;
			p2p.channel = wpa_s->conf->p2p_listen_channel;
			p2p.channel_forced = 1;
#if 0   /* by Shingu 20170418 (P2P ACS) */ //MODIFY_SUPPLICANT_FOR_FREERTOS - del
		} else {
			/*
	...
	if (wpa_s->conf->p2p_oper_reg_class &&
		...
		if (get_run_mode() == WIFI_DEVICE_MODE_EXT_P2P_GO &&
			...
			ra6wx_p2p_prt("[%s] P2P: Failed to select random "
...
#if 1   /* by Shingu 20160726 (P2P Config) */	//MODIFY_SUPPLICANT_FOR_FREERTOS - add
	if (wpa_s->conf->ssid) {
...
#if 1//def  CONFIG_P2P_UNUSED_CMD
...
#if 1   /* by Shingu 20170418 (P2P ACS) */	//MODIFY_SUPPLICANT_FOR_FREERTOS
					...
					struct wpa_scan_results *scan_res)
{
	...
	for (int i = 0; i < (int) scan_res->num; i++) {
	...
	if (!wpa_s->conf->p2p_oper_channel) {
		...
		ra6wx_notice_prt("P2P operating channel to %d\n", min_channel);
	}
	if (!wpa_s->conf->p2p_listen_channel) {
		wpa_s->conf->p2p_listen_channel = min_channel;
		p2p_set_listen_channel(wpa_s->global->p2p, 81, min_channel, 0);
		ra6wx_notice_prt("P2P listen channel to %d\n", min_channel);
	}
			       ...
			       const u8 *ssid, size_t ssid_len)
{
	...
	p2p_freq_to_channel(res.freq, &op_reg, &freq_to_chan);
	wpa_s->global->p2p->cfg->op_channel = freq_to_chan;
		     ...
		     int join, int go_intent, int freq, int pd)
{
...
#else
		ret = wps_generate_pin(&_pin);

#endif	/* 0 */
		os_snprintf(wpa_s->p2p_pin, sizeof(wpa_s->p2p_pin), "%08d",
			    ret);

		ra6wx_p2p_prt("[%s] P2P: Randomly generated PIN: %s\n",
	...
	if (join) {
...
#ifdef	CONFIG_P2P_OPTION
		if (auth) {
			ra6wx_p2p_prt("[%s] P2P: Authorize invitation to "
		...
		if (auto_join) {
			...
			ra6wx_p2p_prt("[%s] P2P: Auto join started at "
				     "%ld.%06ld\n", __func__,
		...
		if (wpas_p2p_join(wpa_s, iface_addr, dev_addr, wps_method,
							0,	//MODIFY_SUPPLICANT_FOR_FREERTOS - add
							freq, NULL, 0) < 0)
			return -1;
		return ret;
	}
	...
	if (wpa_s->create_p2p_iface) {
		...
		if (wpas_p2p_add_group_interface(wpa_s, iftype) < 0) {
			ra6wx_p2p_prt("[%s] P2P: Failed to allocate a new "
...
// 2_10
int _wpas_p2p_connect(struct wpa_supplicant *wpa_s, const u8 *peer_addr,
		     const char *pin, enum p2p_wps_method wps_method,
...
#ifdef CONFIG_5G_SUPPORT		//MODIFY_SUPPLICANT_FOR_FREERTOS - chg	
	num = get_shared_radio_freqs_data(wpa_s, freqs,
					  wpa_s->num_multichan_concurrent);
#else
	num = get_shared_radio_freqs(wpa_s, &freqs->freq,
					  wpa_s->num_multichan_concurrent);
#endif	// CONFIG_5G_SUPPORT
 ...
 */
int _wpas_p2p_group_add(struct wpa_supplicant *wpa_s, int persistent_group,
		       ...
		       int freq, int ht40, int vht)
{
	...
	/* Make sure we are not running find during connection establishment */
	ra6wx_p2p_prt("[%s] P2P: Stop any on-going P2P FIND\n", __func__);

	wpas_p2p_stop_find_oper(wpa_s);

	//MODIFY_SUPPLICANT_FOR_FREERTOS - chg	
	if (wpas_p2p_init_go_params(wpa_s, &params, freq, ht40, vht, NULL, 0, 0, 0, NULL))
		return -1;
	if (params.freq &&
	    !p2p_supported_freq_go(wpa_s->global->p2p, params.freq)) {
		ra6wx_p2p_prt("[%s] P2P: The selected channel for GO "
			   "(%u MHz) is not supported for P2P uses\n",
				  ...
				  bool allow_6ghz)
{
	...
	if (
#ifdef  CONFIG_P2P_OPTION
		wpas_get_p2p_group(wpa_s, ssid->ssid, ssid->ssid_len, &go) &&
#endif	// CONFIG_P2P_OPTION
...
static void wpas_p2p_idle_update(void *ctx, int idle)
{
	...
	if (idle) {
		if (
			wpa_s->global->p2p_fail_on_wps_complete &&
...
int _wpas_p2p_find(struct wpa_supplicant *wpa_s, unsigned int timeout,
		  enum p2p_discovery_type type,
		  ...
		  bool include_6ghz)
{
	...
	return _p2p_find(wpa_s->global->p2p, timeout, type,
...
int wpas_p2p_stop_find(struct wpa_supplicant *wpa_s)
{
	...
	return 0;	//MODIFY_SUPPLICANT_FOR_FREERTOS
}
...
void p2p_reconfig_net(void)
{
	...
	/* P2P_NET_DEVICE */
	ra6wx_assoc_prt(">>> [%s] P2P_NET_DEVICE\n", __FUNCTION__);
	os_memcpy(wifi_monitor_msg->data, "2", 1); // P2P_NET_DEVICE
...
#endif
	if (status != pdTRUE) {
		ra6wx_iface_prt("[%s] tx_queue_send error !!! (%d)\n",
					__func__, status);
	...
	if (status != ERR_OK) {
		ra6wx_err_prt("[%s] event set error !!! (%d)\n", __func__,
			     status);
		os_free(wifi_monitor_msg);
		return;
	}
...
#else
	ra6wx_p2p_prt("[%s] What do I do for this one ???\n", __func__);
	return 0;
#endif  /* CONFIG_AP */
}
#endif
...
void wpas_p2p_group_add_cb(void *eloop_ctx, void *timeout_ctx)	//MODIFY_SUPPLICANT_FOR_FREERTOS-add
{
	struct wpa_supplicant *wpa_s = eloop_ctx;
...
#else
		_wpas_p2p_group_add(wpa_s, 0, 0, 0, 0,0,0,0,0,0);	
#endif		
	} else {
		ra6wx_notice_prt("[%s] supplicant not initialized.\n", __func__);
	}
}

void wpas_p2p_find_cb(void *eloop_ctx, void *timeout_ctx)	//MODIFY_SUPPLICANT_FOR_FREERTOS-add
{
	struct wpa_supplicant *wpa_s = eloop_ctx;

	extern int p2p_ctrl_find(struct wpa_supplicant *wpa_s, char *cmd);
				       ...
				       int group_added)
{
	...
	_wpas_p2p_connect(wpa_s, wpa_s->pending_join_dev_addr, wpa_s->p2p_pin,
			wpa_s->p2p_wps_method,
#ifdef	CONFIG_P2P_OPTION
			wpa_s->p2p_persistent_group,
#else
			0,
#endif	// CONFIG_P2P_OPTION
			0,
...
#if defined ( __SUPP_27_SUPPORT__ )		//MODIFY_SUPPLICANT_FOR_FREERTOS
	struct wpa_supplicant *ifs;
...
static void wpas_p2p_remove_psk(struct wpa_supplicant *wpa_s,
				...
				int iface_addr)
{
    wpas_p2p_remove_psk_entry(wpa_s, s, addr, iface_addr);
#ifdef  CONFIG_P2P_UNUSED_CMD
    int res;
...
#else
	return _wpas_p2p_connect(wpa_s, params->peer->p2p_device_addr, NULL,
				WPS_NFC, 0, 0, 1, 0, wpa_s->conf->p2p_go_intent,
				params->go_freq, 
#if 1//def  CONFIG_P2P_UNUSED_CMD
				wpa_s->p2p_go_vht_center_freq2,
				    ...
				    int forced_freq)
{
	...
	res = wpas_p2p_connect(wpa_s, params->peer->p2p_device_addr, NULL,
				...
				NULL, 0, false);
#else
	res = _wpas_p2p_connect(wpa_s, params->peer->p2p_device_addr, NULL,
				WPS_NFC, 0, 0, 0, 1, wpa_s->conf->p2p_go_intent,
				forced_freq, 
#ifdef  CONFIG_P2P_UNUSED_CMD
				wpa_s->p2p_go_vht_center_freq2,
					 ...
					 enum wpas_p2p_channel_update_trig trig)
{
	 ...
	 */
	//MODIFY_SUPPLICANT_FOR_FREERTOS-chg
	for (ifs = wpa_s->global->ifaces; wpa_s ; wpa_s = wpa_s->next)
	{
