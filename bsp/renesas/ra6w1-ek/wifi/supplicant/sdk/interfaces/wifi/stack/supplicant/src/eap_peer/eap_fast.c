				  ...
				  const char *phase1)
{
	...
	if (pos) {
		data->max_pac_list_len = (size_t) atoi(pos + 22);
...
static void * eap_fast_init(struct eap_sm *sm)
{
	...
	if (config->phase1) {
		...
		if (data->provisioning_allowed == 1) {
			ra6wx_err_prt("EAP-FAST: fast_provisioning=1 is not supported.\n");
			os_free(data);
			return NULL;
		}
	}
...
static struct wpabuf * eap_fast_tlv_nak(int vendor_id, int tlv_type)
{
	...
	nak->vendor_id = host_to_be32((unsigned int) vendor_id);
	nak->nak_type = host_to_be16((unsigned short) tlv_type);
	return buf;
}
...
static struct wpabuf * eap_fast_tlv_result(int status, int intermediate)
{
	...
	result->status = host_to_be16((unsigned short) status);
	return buf;
}
...
static struct wpabuf * eap_fast_pac_request(void)
{
	...
	wpabuf_put(tmp, (size_t) (pos2 - pos));
	return tmp;
}
	...
	*id_len = len;
	if (len > sizeof(*hdr)) {
		...
		if (be_to_host16(hdr->type) == PAC_TYPE_A_ID &&
		    sizeof(*hdr) + (unsigned int) tlen <= len) {
				   ...
				   struct eap_fast_pac *pac)
{
	...
	if (tlv) {
		...
		ehdr->length = host_to_be16((unsigned short) olen);
