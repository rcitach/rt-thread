...
#endif /* CONFIG_FILS */
#ifdef CONFIG_OWE
	} else if ((unsigned int) key_mgmt & WPA_KEY_MGMT_OWE) {
...
int wpa_gen_wpa_ie(struct wpa_sm *sm, u8 *wpa_ie, size_t wpa_ie_len)
{
	if (sm->proto == WPA_PROTO_RSN)
		return wpa_gen_wpa_ie_rsn(wpa_ie, wpa_ie_len,
					  (int) sm->pairwise_cipher,
					  (int) sm->group_cipher,
					  (int) sm->key_mgmt, (int) sm->mgmt_group_cipher,
					  sm);
#ifdef CONFIG_HS20
...
int wpa_gen_rsnxe(struct wpa_sm *sm, u8 *rsnxe, size_t rsnxe_len)
{
	...
	if (wpa_key_mgmt_sae((int) sm->key_mgmt) &&
	...
	if (rsnxe_len < 2 + flen)
		return -1;
	capab = (u16) (capab | (flen - 1)); /* bit 0-3 = Field length (n - 1) */
