		   ...
		   int ctx_bits, int ret_bits, u8 *ret)
{
	...
	lab_len = (int) os_strlen(label);
	ctx_len = (ctx_bits + 7) / 8;
	ret_len = ((ret_bits & 0xffff) + 7) / 8;
	buf_len = lab_len + ctx_len + 4;

	os_memset(ret, 0, (size_t) ret_len);

	n = (ret_bits + h - 1) / h;
	if (n > ((0x1 << r) - 1))
		return -1;

	buf = os_zalloc((size_t) buf_len);
	if (buf == NULL)
		return -1;

	os_memcpy(buf + 1, label, (size_t) lab_len);
	os_memcpy(buf + lab_len + 2, context, (size_t) ctx_len);
	WPA_PUT_BE16(&buf[buf_len - 2], (u16) ret_bits);
	...
	for (i = 0; i < n; i++) {
		...
		buf[0] = (u8) (i + 1);
		if (kdk_bits == 128)
			res = omac1_aes_128(kdk, buf, (size_t) buf_len, ret);
		else
			res = omac1_aes_256(kdk, buf, (size_t) buf_len, ret);
		if (res) {
			os_free(buf);
			return -1;
		}
			    ...
			    const u8 *mac2, u8 *cak, size_t cak_bytes)
{
	...
	return aes_kdf(msk, 8 * msk_bytes, "IEEE8021 EAP CAK",
		       context, sizeof(context) * 8, 8 * (int) cak_bytes, cak);
}
...
{
	return aes_kdf(cak, cak_bytes * 8, "IEEE8021 SAK", ctx, (int) ctx_bytes * 8,
		       (int) sak_bytes * 8, sak);
}
