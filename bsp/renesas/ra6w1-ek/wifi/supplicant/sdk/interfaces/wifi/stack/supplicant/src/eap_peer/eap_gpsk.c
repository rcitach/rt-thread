...
static void * eap_gpsk_init(struct eap_sm *sm)
{
	...
	if (phase1) {
		...
		if (pos) {
			data->forced_cipher = (u16) atoi(pos + 7);
				  ...
				  size_t csuite_list_len)
{
	...
	count = (int) (csuite_list_len / sizeof(struct eap_gpsk_csuite));
	...
	for (i = 0; i < count; i++) {
		...
		vendor = (int) WPA_GET_BE32(csuite->vendor);
					    ...
					    size_t csuite_list_len)
{
	...
	wpabuf_put_be16(resp, (u16) data->id_peer_len);
	wpabuf_put_data(resp, data->id_peer, data->id_peer_len);

	wpabuf_put_be16(resp, (u16) data->id_server_len);
	...
	wpabuf_put_be16(resp, (u16) csuite_list_len);
	wpabuf_put_data(resp, csuite_list, csuite_list_len);

	csuite = wpabuf_put(resp, sizeof(*csuite));
	WPA_PUT_BE32(csuite->vendor, (u32) data->vendor);
	WPA_PUT_BE16(csuite->specifier, (u16) data->specifier);
	...
	if (eap_gpsk_compute_mic(data->sk, data->sk_len, data->vendor,
				 data->specifier, start, (size_t) (rpos - start), rpos) <
	    0) {
		eap_gpsk_state(data, FAILURE);
		wpabuf_free(resp);
		return NULL;
					       ...
					       const u8 *pos, const u8 *end)
{
	...
	if (eap_gpsk_compute_mic(data->sk, data->sk_len, data->vendor,
				 data->specifier, payload, (size_t) (pos - payload), mic)
	    < 0) {
