	...
	TNC_MessageType messageType)
{
	...
	b64 = (char *) base64_encode(message, messageLength, &b64len);
	if (b64 == NULL)
		return TNC_RESULT_FATAL;
	...
	imc->imc_send_len =
		(size_t) os_snprintf((char *) imc->imc_send, b64len + 100U,
...
#ifndef OS_FREERTOS	//MODIFY_SUPPLICANT_FOR_FREERTOS
...
static unsigned char * tncc_get_base64(char *start, size_t *decoded_len)
{
	...
	*pos2 = '\0';
	...
	decoded = (unsigned char *) base64_decode(pos, os_strlen(pos), decoded_len);
	*pos2 = '<';
	if (decoded == NULL) {
					    ...
					    const u8 *msg, size_t len)
{
	...
	*end = '\0';
	...
	if (*pos == '"')
		pos++;
	batch_id = (unsigned int) atoi(pos);
...
static struct wpabuf * tncc_build_soh(int ver)
{
	...
	wpabuf_put_be16(buf, (u16) ver); /* Inner Type */
	...
	wpabuf_put_be16(buf, (u16) (os_strlen(machinename) + 1));
	...
	WPA_PUT_BE16(ssoh_len, (u16) (end - ssoh_len - 2));

	/* TODO: SoHReportEntry TLV (zero or more) */

	/* Update length fields */
	end = wpabuf_put(buf, 0);
	WPA_PUT_BE16(tlv_len, (u16) (end - tlv_len - 2));
	WPA_PUT_BE16(tlv_len2, (u16) (end - tlv_len2 - 2));
	WPA_PUT_BE16(outer_len, (u16) (end - outer_len - 2));
	WPA_PUT_BE16(inner_len, (u16) (end - inner_len - 2));
