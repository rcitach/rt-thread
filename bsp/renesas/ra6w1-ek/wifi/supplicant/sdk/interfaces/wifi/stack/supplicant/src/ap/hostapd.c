...
#if 1	/* by Shingu 20161107 (No disassoc) */
...
#ifdef	CONFIG_NEIGHBOR_AP_DB
#include "neighbor_db.h"
#endif	// CONFIG_NEIGHBOR_AP_DB
#ifdef CONFIG_RADIO_MEASUREMENT
#include "rrm.h"
#endif	// CONFIG_RADIO_MEASUREMENT
#ifdef CONFIG_FILES
#include "fils_hlp.h"
#endif	// CONFIG_FILES
#ifdef CONFIG_ACS
...
void hostapd_free_hapd_data(struct hostapd_data *hapd)
{
	...
	if (!hapd->started) {
		wpa_printf(MSG_WARNING, "%s: Interface %s wasn't started",
...
{
	int ret = 0;
#if 0   // Supplicant 2.7	//MODIFY_SUPPLICANT_FOR_FREERTOS
	u8 addr[ETH_ALEN];
#endif
	...
	if (hapd->conf && hapd->conf->broadcast_deauth) {
		...
		if (hapd->p2p == NULL) {
#endif  /* CONFIG_P2P */
			hostapd_ctrl_iface_disassociate(hapd, "all");
#ifdef  CONFIG_P2P
		} else {
			hostapd_ctrl_iface_deauthenticate(hapd, "all reason=3");

			if (hapd->num_sta &&
				hostapd_drv_chk_deauth_send_done(hapd)) {
				ra6wx_ap_prt("[%s] Sending DEAUTH FAIL!\n", __func__);
			}
		}
#endif  /* CONFIG_P2P */
	}
...
{
#if 0   /* by Trinity 20160624 (P2P GO Inactivity) */
	hostapd_free_stas(hapd);
#endif	// 0
...
static int hostapd_setup_bss(struct hostapd_data *hapd, int first)
{
	...
	struct hostapd_bss_config *conf_temp = hapd->iface->bss[0]->conf;
	int second = 0;
	...
	if (!first || first == -1) {
...
#ifdef CONFIG_OWE_TRANS		
		memcpy(&own_addr[0], addr,ETH_ALEN);
#endif // CONFIG_OWE_TRANS
		...
		if (!is_zero_ether_addr(conf->bssid)) {
			...
			do {
#ifdef CONFIG_OWE_TRANS
				disc_byte_array(hapd->own_addr, ETH_ALEN);
#else
...
#ifdef	CONFIG_AP_BSS_LOAD_UPDATE
		hapd->conf->bss_load_update_period  = 0;
#endif	/* CONFIG_AP_PERIOD */
		second = 1;
		conf->owe_transition_mode = conf_temp->owe_transition_mode;

		conf->ssid.ssid_len =conf_temp->ssid.ssid_len;
		memset(conf->ssid.ssid, 0, SSID_MAX_LEN);
		memcpy(conf->ssid.ssid, conf_temp->ssid.ssid, conf_temp->ssid.ssid_len);
		conf->ssid.ssid_set = conf_temp->ssid.ssid_set;
		
		os_strlcpy(conf->owe_transition_ifname, /* SOFTAP_DEVICE_NAME*/"softap1" , sizeof(conf->iface));
		os_memcpy(conf->owe_transition_ssid, conf->ssid.ssid, conf->ssid.ssid_len); 
		os_memcpy(&conf->owe_transition_ssid[conf->ssid.ssid_len], "-owe", 4);
		conf->owe_transition_ssid_len = conf->ssid.ssid_len + 4 /* "_trans" */;
		os_memcpy(conf->owe_transition_bssid,own_addr,ETH_ALEN);
		memcpy(hapd->iface->bss[0]->own_addr, own_addr, ETH_ALEN);

		conf_temp->ssid.ssid_len = conf->owe_transition_ssid_len;
		memset(conf_temp->ssid.ssid, 0, SSID_MAX_LEN);
		os_memcpy(conf_temp->ssid.ssid, conf->owe_transition_ssid, conf_temp->ssid.ssid_len);
#endif // CONFIG_OWE_TRANS
...
#ifdef CONFIG_OWE_TRANS
		if (wpa_driver_fc80211_set_mode(/*bss*/hapd->drv_priv, FC80211_IFTYPE_AP) < 0)
			ra6wx_drv_prt("[%s] Could not configure "
					"driver mode\n", __func__);
		
		fc80211_get_macaddr(hapd->drv_priv);
#endif // CONFIG_OWE_TRANS
	}
...
#if 0 // JMC TEST	//MODIFY_SUPPLICANT_FOR_FREERTOS
	if (!hostapd_drv_none(hapd)) {
...
void hostapd_tx_queue_params(struct hostapd_iface *iface)
{
	struct hostapd_data *hapd = iface->bss[0];
	int i;
...
#define CH_LIST_UPDATE_TIMEOUT 5
static int setup_interface(struct hostapd_iface *iface)
{
...
#ifdef CONFIG_OWE_TRANS
	hapd = iface->bss[0];
	if (hapd->conf->owe_transition_mode != 1)
#endif // CONFIG_OWE_TRANS
	{
	...
	if (hapd->iconf->country[0] && hapd->iconf->country[1]) {
        ...
        memset(country, 0x0, 4);
        memset(previous_country, 0x0, 4);
		...
		wpa_printf(MSG_DEBUG, "Previous country code %s, new country code %s",
			   previous_country[0]?previous_country:"empty", country[0]?country:"empty");

        if (country[0] == '\0') {
            printf("[%s:%d] country is empty but continue instead of return ... \n", 
                   __func__, __LINE__);
        } else if (os_strncmp(previous_country, country, 2) != 0) {
			printf("Continue interface setup after channel list update is invoked in %d seconds... \n", 
                    CH_LIST_UPDATE_TIMEOUT);
			iface->wait_channel_update = 1;
			eloop_register_timeout(CH_LIST_UPDATE_TIMEOUT, 0,
		     ...
		     const char *ctrl_iface, const char *driver)
{
	...
	if (driver) {
		...
		if (os_strcmp(driver, wpa_drivers->name) == 0) {
			conf->driver = wpa_drivers;
			goto skip;
		}
#else
		int j;
...
#ifdef UNUSED	/* by Bright : Code sync with Supp 2.4 */
...
const char * hostapd_get_state(struct hostapd_data *hapd)
{
	...
	return (hostapd_state_text(iface->state));
}
				     ...
				     struct beacon_data *beacon)
{
	...
	os_memset(beacon, 0, sizeof(struct beacon_data));
#else
				      ...
				      struct hostapd_freq_params *old_params)
{
	...
	if (old_params &&
	    hostapd_set_freq_params(old_params,
								conf->hw_mode,
