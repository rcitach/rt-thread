...
int sae_set_group(struct sae_data *sae, int group)
{
	...
	if (tmp->ec) {
		wpa_printf(MSG_DEBUG, "SAE: Selecting supported ECC group %d",
			   group);
		sae->group = group;
		tmp->prime_len = (int) crypto_ec_prime_len(tmp->ec);
		tmp->prime = crypto_ec_get_prime(tmp->ec);
		tmp->order_len = (int) crypto_ec_order_len(tmp->ec);
	...
	if (tmp->dh) {
		wpa_printf(MSG_DEBUG, "SAE: Selecting supported FFC group %d",
			   group);
		sae->group = group;
		tmp->prime_len = (int) tmp->dh->prime_len;
		...
		tmp->order_len = (int) tmp->dh->order_len;
				 ...
				 u8 *pwd_value)
{
	...
	if (sha256_prf_bits(pwd_seed, SHA256_MAC_LEN, "SAE Hunting and Pecking",
			    prime, (size_t) sae->tmp->prime_len, pwd_value, bits) < 0)
		return -1;
	if (bits % 8)
		buf_shift_right(pwd_value, (size_t) sae->tmp->prime_len, 8 - bits % 8);
	wpa_hexdump_key(MSG_DEBUG, "SAE: pwd-value",
			pwd_value, (size_t) sae->tmp->prime_len);
	...
	x_cand = crypto_bignum_init_set(pwd_value, (size_t) sae->tmp->prime_len);
	if (!x_cand)
		return -1;
...
#if 0 /* disabled because of performance. */
	wpa_hexdump_key(MSG_DEBUG, "SAE: pwd-seed", pwd_seed, SHA256_MAC_LEN);
#endif
				 ...
				 struct crypto_bignum *pwe)
{
	...
	size_t bits = (size_t) (sae->tmp->prime_len * 8);
	u8 exp[1];
	...
	/* pwd-value = KDF-z(pwd-seed, "SAE Hunting and Pecking", p) */
	if (sha256_prf_bits(pwd_seed, SHA256_MAC_LEN, "SAE Hunting and Pecking",
			    sae->tmp->dh->prime, (size_t) sae->tmp->prime_len, pwd_value,
			    bits) < 0)
		return -1;
	wpa_hexdump_key(MSG_DEBUG, "SAE: pwd-value", pwd_value,
			(size_t) sae->tmp->prime_len);
	 ...
	 * selection */
	pwd_value_valid = (u8) const_time_fill_msb((unsigned int) res);
	...
	res = -1;
	a = crypto_bignum_init_set(pwd_value, (size_t) sae->tmp->prime_len);
	if (!a)
		goto fail;
	...
	res = (int) const_time_select_u8((u8) const_time_is_zero((unsigned int) is_val), (u8) res, 0);
			      ...
			      size_t password_len)
{
	...
	prime_len = (size_t) sae->tmp->prime_len;
	 ...
	 * in the loop.
	 */
	k = (u8) dragonfly_min_pwe_loop_iter(sae->group);
			      ...
			      size_t password_len)
{
	...
	u8 mask;
	struct crypto_bignum *pwe;
	size_t prime_len = (size_t) (sae->tmp->prime_len * 8);
	u8 *pwe_buf;
	...
	for (counter = 1; counter <= k || !found; counter++) {
		...
		mask = const_time_eq_u8((unsigned int) res, 1);
				     ...
				     const struct crypto_bignum *u)
{
	...
	uint8_t buf[32] = {0xff,0xff,0xff,0xff,0x00,0x00,0x00,0x01,0x00,0x00,
	...
	z = crypto_bignum_init_uint((unsigned int) abs(z_int));
	...
	m_is_zero = const_time_eq((unsigned int) crypto_bignum_is_zero(t1), 1);
	/* t = m^(p-2) modulo p */
	...
	if (crypto_bignum_to_bin(x1a, bin1, sizeof(bin1), prime_len) < 0 ||
		...
		goto fail;
	const_time_select_bin((u8) m_is_zero, bin1, bin2, prime_len, bin);
	...
	if (crypto_bignum_sub(prime, one, tmp1) < 0 ||
	    crypto_bignum_rshift(tmp1, 1, tmp1) < 0 ||
	    crypto_bignum_exptmod(gx1, tmp1, prime, t1) < 0)
		goto fail;
	debug_print_bignum("SSWU: gx1^((p-1)/2) modulo p", t1, prime_len);
	is_qr = const_time_eq((unsigned int) (crypto_bignum_is_zero(t1) |
	...
	if (crypto_bignum_to_bin(gx1, bin1, sizeof(bin1), prime_len) < 0 ||
		...
		goto fail;
	const_time_select_bin((u8) is_qr, bin1, bin2, prime_len, bin);
	...
	if (crypto_bignum_to_bin(x1, bin1, sizeof(bin1), prime_len) < 0 ||
		...
		goto fail;
	const_time_select_bin((u8) is_qr, bin1, bin2, prime_len, x_y);
	...
	if (crypto_bignum_to_bin(y, bin1, sizeof(bin1), prime_len) < 0 ||
		...
		goto fail;
	const_time_select_bin((u8) is_eq, bin1, bin2, prime_len, &x_y[prime_len]);
			  ...
			  int *rejected_groups, const struct sae_pk *pk)
{
	...
	if (rejected_groups) {
		...
		groups = wpabuf_alloc((size_t) (count * 2));
		if (!groups)
			return -1;
		for (i = 0; i < count; i++)
			wpabuf_put_le16(groups, (u16) rejected_groups[i]);
...
static int sae_derive_k_ecc(struct sae_data *sae, u8 *k)
{
	...
	wpa_hexdump_key(MSG_DEBUG, "SAE: k", k, (size_t) sae->tmp->prime_len);
...
static int sae_derive_k_ffc(struct sae_data *sae, u8 *k)
{
	...
	if (crypto_bignum_exptmod(sae->tmp->pwe_ffc, sae->peer_commit_scalar,
	    ...
	    crypto_bignum_to_bin(K, k, SAE_MAX_PRIME_LEN, (size_t) sae->tmp->prime_len) <
	    0) {
		wpa_printf(MSG_DEBUG, "SAE: Failed to calculate K and k");
		goto fail;
	}
...
static int sae_derive_keys(struct sae_data *sae, const u8 *k)
{
	...
	int ret = -1;
	size_t hash_len, salt_len, prime_len = (size_t) sae->tmp->prime_len;
	const u8 *addr[1];
	size_t len[1];
	 ...
	 * octets). */
	crypto_bignum_to_bin(tmp, val, sizeof(val), (size_t) sae->tmp->order_len);
...
#if 0 /* disabled because of performance. */
	wpa_hexdump_key(MSG_DEBUG, "SAE: Val(512)", sae->pmk, SAE_MAX_PRIME_LEN);
#endif
		     ...
		     const struct wpabuf *token, const char *identifier)
{
	...
	wpabuf_put_le16(buf, (u16) sae->group); /* Finite Cyclic Group */
	if (!sae->h2e && token) {
		wpabuf_put_buf(buf, token);
		wpa_hexdump(MSG_DEBUG, "SAE: Anti-clogging token",
			    wpabuf_head(token), wpabuf_len(token));
	}
	pos = wpabuf_put(buf, (size_t) sae->tmp->prime_len);
	if (crypto_bignum_to_bin(sae->tmp->own_commit_scalar, pos,
				 (size_t) sae->tmp->prime_len, (size_t) sae->tmp->prime_len) < 0)
		return -1;
	wpa_hexdump(MSG_DEBUG, "SAE: own commit-scalar",
		    pos, (size_t) sae->tmp->prime_len);
	if (sae->tmp->ec) {
		pos = wpabuf_put(buf, (size_t) (2 * sae->tmp->prime_len));
		...
		wpa_hexdump(MSG_DEBUG, "SAE: own commit-element(y)",
			    pos + sae->tmp->prime_len, (size_t) sae->tmp->prime_len);
	} else {
		pos = wpabuf_put(buf, (size_t) sae->tmp->prime_len);
		if (crypto_bignum_to_bin(sae->tmp->own_commit_element_ffc, pos,
					 (size_t) sae->tmp->prime_len,
					 (size_t) sae->tmp->prime_len) < 0)
			return -1;
	...
	if (identifier) {
		...
		wpabuf_put_u8(buf, (u8) (1 + os_strlen(identifier)));
	...
	if (sae->h2e && sae->tmp->own_rejected_groups) {
		...
		wpabuf_put_u8(buf, WLAN_EID_EXTENSION);
		wpabuf_put_u8(buf,
			      (u8) (1 + wpabuf_len(sae->tmp->own_rejected_groups)));
	...
	if (sae->h2e && token) {
		wpabuf_put_u8(buf, WLAN_EID_EXTENSION);
		wpabuf_put_u8(buf, (u8) (1 + wpabuf_len(token)));
				   ...
				   size_t *token_len, int h2e)
{
	...
	scalar_elem_len = (size_t) ((sae->tmp->ec ? 3 : 2) * sae->tmp->prime_len);
	if (scalar_elem_len >= (size_t) (end - *pos))
		return; /* No extra data beyond peer scalar and element */

	tlen = (size_t) (end - (*pos + scalar_elem_len));
				   ...
				   const u8 *end)
{
	...
	peer_scalar = crypto_bignum_init_set(*pos, (size_t) sae->tmp->prime_len);
	if (peer_scalar == NULL)
		return WLAN_STATUS_UNSPECIFIED_FAILURE;
					...
					const u8 *end)
{
	...
	if (os_memcmp(*pos, prime, (size_t) sae->tmp->prime_len) >= 0 ||
					...
					const u8 *end)
{
	...
	sae->tmp->peer_commit_element_ffc =
		crypto_bignum_init_set(*pos, (size_t) sae->tmp->prime_len);
		     ...
		     int h2e)
{
	...
	res = (u16) sae_parse_password_identifier(sae, &pos, end);
	if (res != WLAN_STATUS_SUCCESS)
		return res;

	/* Conditional Rejected Groups element */
	if (h2e) {
		res = (u16) sae_parse_rejected_groups(sae, &pos, end);
		if (res != WLAN_STATUS_SUCCESS)
			return res;
	}
			  ...
			  u8 *confirm)
{
	...
	len[0] = 2;
	addr[1] = scalar_b1;
	len[1] = (size_t) sae->tmp->prime_len;
	addr[2] = element1;
	len[2] = element1_len;
	addr[3] = scalar_b2;
	len[3] = (size_t) sae->tmp->prime_len;
	addr[4] = element2;
	len[4] = element2_len;
			      ...
			      u8 *confirm)
{
	...
	if (crypto_ec_point_to_bin(sae->tmp->ec, element1, element_b1,
	    ...
	    sae_cn_confirm(sae, sc, scalar1, element_b1,
			   (size_t) (2 * sae->tmp->prime_len),
			   scalar2, element_b2, (size_t) (2 * sae->tmp->prime_len),
			   confirm) < 0)
		return -1;
	return 0;
}
			      ...
			      u8 *confirm)
{
	...
	if (crypto_bignum_to_bin(element1, element_b1, sizeof(element_b1),
	    ...
	    crypto_bignum_to_bin(element2, element_b2, sizeof(element_b2),
				 (size_t) sae->tmp->prime_len) < 0 ||
	    sae_cn_confirm(sae, sc, scalar1, element_b1, (size_t) sae->tmp->prime_len,
			   scalar2, element_b2, (size_t) sae->tmp->prime_len,
			   confirm) < 0)
		return -1;
	return 0;
}
