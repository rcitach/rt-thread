...
#endif	// CONFIG_RADIUS
#ifdef  UNUSED_CODE_EAP_SIM_DB
	bss->eap_sim_db_timeout = 1;
#endif	// UNUSED_CODE_EAP_SIM_DB

	//bss->eap_sim_id = 3;		//MODIFY_SUPPLICANT_FOR_FREERTOS
...
#ifdef CONFIG_SAE
	bss->anti_clogging_threshold = 1;		// 5->1 MODIFY_SUPPLICANT_FOR_FREERTOS
	bss->sae_sync = 5;
#endif	// CONFIG_SAE
...
#undef ecw2cw
    ...
    memset(conf->country, 0x00, sizeof(conf->country));
	...
	conf->acs_num_scans = 2;	// CHG by sjlee
#endif /* CONFIG_ACS */
...
#else	// CONFIG_SUPP27_STR_CLR_FREE
...
#ifdef CONFIG_SAE_PK
static int hostapd_sae_pk_password_without_pk(struct hostapd_bss_config *bss)
{
	struct sae_password_entry *pw;
	int res = false;
...
static int hostapd_config_check_bss_6g(struct hostapd_bss_config *bss)
{
...
int hostapd_sae_pk_in_use(struct hostapd_bss_config *conf)
{
#ifdef CONFIG_SAE_PK
	struct sae_password_entry *pw;
...
#ifdef CONFIG_SAE_PK
int hostapd_sae_pk_exclusively(struct hostapd_bss_config *conf)
{
	int with_pk = false;
	struct sae_password_entry *pw;
