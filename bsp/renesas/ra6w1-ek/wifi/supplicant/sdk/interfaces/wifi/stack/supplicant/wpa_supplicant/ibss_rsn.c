...
static int supp_ether_send(void *ctx, const u8 *dest, u16 proto, const u8 *buf, size_t len)
{
	...
	wpa_printf_dbg(MSG_DEBUG, "SUPP: %s(dest=" MACSTR " proto=0x%04x "
			"len=%lu)",
...
printf(" [%s] dest=" MACSTR " proto=0x%04x " "len=%lu \n",
			__func__, MAC2STR(dest), proto, (unsigned long) len);
	}
...
printf(" [%s] SUPP: type=%d data_len=%d \n", __func__, type, data_len);
...
printf(" [%s] (addr=" MACSTR " data_len=%lu " "encrypt=%d) \n",
		   __func__, MAC2STR(addr), (unsigned long) data_len, encrypt);
	}
