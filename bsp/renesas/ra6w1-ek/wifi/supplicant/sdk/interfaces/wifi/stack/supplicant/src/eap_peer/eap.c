...
SM_STATE(EAP, INITIALIZE)
{
	...
	eapol_set_int(sm, EAPOL_idleWhile, (unsigned int) sm->ClientTimeout);
...
SM_STATE(EAP, METHOD)
{
	...
	if (!eap_hdr_len_valid(eapReqData, (size_t) min_len))
		return;
	...
	size_t count)
{
	...
	resp = eap_msg_alloc(EAP_VENDOR_IETF, EAP_TYPE_EXPANDED,
			     8 + 8 * (count + 1), EAP_CODE_RESPONSE, (u8) id);
	if (resp == NULL)
		return NULL;
	...
	for (m = methods; m; m = m->next) {
		...
		if (eap_allowed_method(sm, m->vendor, m->method)) {
			...
			wpabuf_put_be24(resp, (u32) m->vendor);
...
static struct wpabuf * eap_sm_buildNak(struct eap_sm *sm, int id)
{
	...
	wpa_hexdump(MSG_DEBUG, "EAP: allowed methods", start, (size_t) found);
...
static void eap_sm_parseEapReq(struct eap_sm *sm, const struct wpabuf *req)
{
	...
	case EAP_CODE_REQUEST:
		...
		if (sm->reqMethod == EAP_TYPE_EXPANDED) {
			...
			sm->reqVendor = (int) WPA_GET_BE24(pos);
			pos += 3;
...
u32 eap_get_phase2_type(const char *name, int *vendor)
{
	...
	if (eap_allowed_phase2_type(v, (int) type)) {
		*vendor = v;
		return type;
	}
					      ...
					      size_t *count)
{
	...
	*count = 0;
	...
	for (m = methods; m; m = m->next) {
		vendor = m->vendor;
		method = m->method;
		if (eap_allowed_phase2_type(vendor, (int) method)) {
			if (vendor == EAP_VENDOR_IETF &&
