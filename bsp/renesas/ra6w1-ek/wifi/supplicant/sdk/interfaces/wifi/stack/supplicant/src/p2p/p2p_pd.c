					       ...
					       int join)
{
	...
	p2p_buf_add_capability(buf, (u8) (p2p->dev_capab &
...
out:
...
#ifdef  CONFIG_P2P_UNUSED_CMD
	p2p->cfg->set_ps_ctrl(p2p->cfg->cb_ctx);
#endif
		...
		if (p2p_send_action(p2p, (unsigned int) freq, sa, p2p->cfg->dev_addr,
				    p2p->cfg->dev_addr,
	...
	if (!p2p->cfg->p2ps_prov_complete) {
		...
		p2p->cfg->p2ps_prov_complete(p2p->cfg->cb_ctx, reject,
					     ...
					     msg.persistent_ssid_len,
					     0, 0, NULL, NULL, 0, (unsigned int) freq,
					     NULL, 0);
		...
		if (reject != P2P_SC_SUCCESS)
			p2p->cfg->p2ps_prov_complete(p2p->cfg->cb_ctx, reject,
						     ...
						     0, NULL, NULL, 0, (unsigned int) freq,
						     NULL, 0);
		else
			p2p->cfg->p2ps_prov_complete(p2p->cfg->cb_ctx,
						     *msg.status,
						     ...
						     0, NULL,
						     (const u8 *) &resp_fcap,
						     sizeof(resp_fcap), (unsigned int) freq,
						     NULL, 0);
		...
		p2p->cfg->p2ps_prov_complete(p2p->cfg->cb_ctx, *msg.status, sa,
					     ...
					     0, 0, NULL,
					     (const u8 *) &resp_fcap,
					     sizeof(resp_fcap), (unsigned int) freq, NULL, 0);
	} else if (msg.status) {
	...
	if ((status == P2P_SC_SUCCESS || status == P2P_SC_SUCCESS_DEFERRED) &&
		...
		if (p2p->cfg->p2ps_prov_complete) {
			...
			p2p->cfg->p2ps_prov_complete(
				...
				msg.feature_cap, msg.feature_cap_len, (unsigned int) freq,
			   ...
			   int join, int force_freq)
{
	...
	if (p2p_send_action(p2p, (unsigned int) freq, dev->info.p2p_device_addr,
		      ...
		      int user_initiated_pd)
{
	...
	p2p->user_initiated_pd = (u8) user_initiated_pd;
