...
#if 1 // TEST RA6Wx	//MODIFY_SUPPLICANT_FOR_FREERTOS
	ret = os_snprintf(buf + len, buflen - len, "inact_cnt=%d\n", sta->inact_cnt);
	if (os_snprintf_error(buflen - len, ret))
		return len;
	len += ret;
#endif /* 1 */
...
const char * timeout_next_str(int val)		//MODIFY_SUPPLICANT_FOR_FREERTOS
{
	switch (val) {
	case STA_NULLFUNC:
				      ...
				      char *buf, size_t buflen)
{
	int len, res, ret, i;
//	const char *keyid;
	u8 ieee802_11_ag = 0;	/* added FC9000  */
	...
	ret = os_snprintf(buf + len, buflen - len, "\naid=%d\ncapability=0x%x\n"
			  "listen_interval=%d",//supported_rates=",
...
#define RATE_MBPS(x)  ((x & 0x7f)/2)
	...
	for (i = 0; i < sta->supported_rates_len; i++) {
		if (RATE_MBPS(sta->supported_rates[i]) > 11) {
			ieee802_11_ag = 1;
			break;
		}
	}

	if (ieee802_11_ag == 1 && sta->no_ht_set == 1) {
		if (hapd->iface->conf->channel > 14) {
			ret = os_snprintf(buf + len, buflen - len, "802.11a\n");
		} else {
			ret = os_snprintf(buf + len, buflen - len, "802.11g\n");
		}
	} else if(ieee802_11_ag == 1 && sta->no_ht_set == 0) {
		ret = os_snprintf(buf + len, buflen - len, "802.11n\n");
	} else if(ieee802_11_ag == 0) {
		ret = os_snprintf(buf + len, buflen - len, "802.11b\n");
	} else {
		ret = os_snprintf(buf + len, buflen - len, "unkown\n");
	}

	if (ret < 0 || (size_t) ret >= buflen - len)
		return len;
	len += ret;
#endif /* added RRQ61xxx */

	ret = os_snprintf(buf + len, buflen - len, "timeout_next=%s\n",
			  timeout_next_str(sta->timeout_next));
	if (os_snprintf_error(buflen - len, ret))
		return len;
	len += ret;

#if 0 /* disable mib */		//MODIFY_SUPPLICANT_FOR_FREERTOS
...
#if 0 // by Shingu 20190621	//MODIFY_SUPPLICANT_FOR_FREERTOS
