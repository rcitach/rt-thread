...
#if 1   /* by Shingu 20160726 (P2P Config) */	//MODIFY_SUPPLICANT_FOR_FREERTOS
			 ...
			 struct p2p_device *dev, struct p2p_message *msg)
{
	 ...
	 */
	if (!p2p->cfg->get_pref_freq_list || 
		p2p->cfg->num_pref_chan ||
...
fail:
	...
	if (status == P2P_SC_SUCCESS) {
		...
		sprintf(p2p_accept_mac, MACSTR, MAC2STR(sa));	//MODIFY_SUPPLICANT_FOR_FREERTOS

		p2p->pending_action_state =
			P2P_PENDING_GO_NEG_RESPONSE_FAILURE;

#if 1   /* by Shingu 20161025 (PRINT) */
		ra6wx_notice_prt(">>> P2P Connection Request from " MACSTR "\n", MAC2STR(sa));
#endif  /* 1 */
	}
			     ...
			     const u8 *data, size_t len, int rx_freq)
{
	...
	if(os_memcmp(p2p->ssid, (msg.group_id+ETH_ALEN), msg.group_id_len-ETH_ALEN)&&(!(dev->flags & P2P_DEV_WAIT_GO_NEG_RESPONSE))){
		dev->flags |= P2P_DEV_WAIT_GO_NEG_RESPONSE;
		p2p->ssid_len = msg.group_id_len-ETH_ALEN;
		os_memcpy(p2p->ssid, msg.group_id + ETH_ALEN, p2p->ssid_len);
		p2p_dbg(p2p, "[%s](%d)(p2p_ssid:%s)",__func__, __LINE__, p2p->ssid);
	}
