...
int hostapd_flush(struct hostapd_data *hapd)
{
	if (hapd->driver == NULL || hapd->driver->flush == NULL)
		return 0;
	return hapd->driver->flush(hapd->drv_priv);		// i802_flush
}
		     ...
		     int center_segment0, int center_segment1)
{
	...
	if (hapd->driver->set_freq == NULL)
		return 0;
	return hapd->driver->set_freq(hapd->drv_priv, &data);	// i802_set_freq
}

#if 0 // FC9000 inline fucntion : ap_drv_ops.h	//MODIFY_SUPPLICANT_FOR_FREERTOS
			  ...
			  int no_encrypt)
{
	...
	return hapd_temp->driver->send_mlme(hapd->drv_priv, msg, len, noack, 0,
			      ...
			      const u16 *csa_offs, size_t csa_offs_len)
{
	...
	return hapd_temp->driver->send_mlme(hapd_temp->drv_priv, msg, len, noack , 0, 
				       csa_offs, csa_offs_len, 0, 0);
#else
	return hapd->driver->send_mlme(hapd->drv_priv, msg, len, noack, 0,
				       csa_offs, csa_offs_len, 0, 0);
#endif // CONFIG_OWE_TRANS
