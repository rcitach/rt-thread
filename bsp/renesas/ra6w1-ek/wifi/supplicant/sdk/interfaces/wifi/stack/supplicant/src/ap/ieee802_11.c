...
#if 0 //If you use PMKSA Caching, you will need to remove it.
...
#ifdef CONFIG_AP_WIFI_MODE // RRQ61000 Only
   if(hapd->iconf->wifi_mode == WIFI_MODE_G_ONLY ||
      hapd->iconf->wifi_mode == WIFI_MODE_GN ||
      hapd->iconf->wifi_mode == WIFI_MODE_AN ||
       hapd->iconf->wifi_mode == WIFI_MODE_A_ONLY) {
...
#ifdef  CONFIG_LAST_SEQ_CTRL
	sta->last_seq_ctrl = WLAN_INVALID_MGMT_SEQ;
#endif	// CONFIG_LAST_SEQ_CTRL
...
#ifdef  CONFIG_IEEE80211W
	if (mgmt->u.deauth.reason_code == WLAN_REASON_UNSPECIFIED)
		hostapd_drv_sta_deauth(hapd, sta->addr, WLAN_REASON_CLASS2_FRAME_FROM_NONAUTH_STA);
#endif  /* CONFIG_IEEE80211W */
		    ...
		    struct hostapd_frame_info *fi)
{
	...
	case WLAN_FC_STYPE_DISASSOC:
		...
		if (wifi_disconn_notify_cb != NULL) {
			wifi_disconn_notify_cb(mgmt->u.deauth.reason_code);
		}
		break;
...
#ifdef  CONFIG_P2P_UNUSED_CMD
	if (hapd->conf->p2p)
		hapd->p2p->cfg->set_ps_ctrl(hapd->p2p->cfg->cb_ctx);
#endif	
