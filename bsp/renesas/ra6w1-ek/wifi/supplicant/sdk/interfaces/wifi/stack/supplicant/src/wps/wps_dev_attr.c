...
int wps_build_vendor_ext_m1(struct wps_device_data *dev, struct wpabuf *msg)
{
	if (dev->vendor_ext_m1 != NULL) {
		...
		wpabuf_put_be16(msg, (u16) wpabuf_len(dev->vendor_ext_m1));
...
int wps_build_vendor_ext(struct wps_device_data *dev, struct wpabuf *msg)
{
	...
	for (i = 0; i < MAX_WPS_VENDOR_EXTENSIONS; i++) {
		...
		wpabuf_put_be16(msg, ATTR_VENDOR_EXT);
		wpabuf_put_be16(msg, (u16) wpabuf_len(dev->vendor_ext[i]));
...
int wps_build_application_ext(struct wps_device_data *dev, struct wpabuf *msg)
{
	...
	wpabuf_put_be16(msg, (u16) wpabuf_len(dev->application_ext));
