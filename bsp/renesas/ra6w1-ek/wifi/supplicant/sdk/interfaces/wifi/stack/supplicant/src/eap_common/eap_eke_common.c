		       ...
		       size_t id_p_len, u8 *key)
{
	...
	/* temp = prf(0+, password) */
	os_memset(zeros, 0, (size_t) sess->prf_len);
	if (eap_eke_prf(sess->prf, zeros, (size_t) sess->prf_len,
			password, password_len, NULL, 0, temp) < 0)
		return -1;
	wpa_hexdump_key(MSG_DEBUG, "EAP-EKE: temp = prf(0+, password)",
			temp, (size_t) sess->prf_len);
	...
	if (eap_eke_prfplus(sess->prf, temp, (size_t) sess->prf_len,
		   ...
		   u8 *ret_dhcomp)
{
	...
	wpa_hexdump(MSG_DEBUG, "EAP-EKE: IV for Encr(key, y)",
		    iv, AES_BLOCK_SIZE);
	os_memcpy(pub, dhpub, (size_t) dh_len);
	if (aes_128_cbc_encrypt(key, iv, pub, (size_t) dh_len) < 0)
		return -1;
	os_memcpy(ret_dhcomp, iv, AES_BLOCK_SIZE);
	os_memcpy(ret_dhcomp + AES_BLOCK_SIZE, pub, (size_t) dh_len);
	wpa_hexdump(MSG_DEBUG, "EAP-EKE: DHComponent = Encr(key, y)",
		    ret_dhcomp, (size_t) (AES_BLOCK_SIZE + dh_len));
			  ...
			  const u8 *dhpriv, const u8 *peer_dhcomp)
{
	...
	os_memset(zeros, 0, (size_t) sess->auth_len);
	if (eap_eke_prf(sess->prf, zeros, (size_t) sess->auth_len, modexp, dh->prime_len,
			NULL, 0, sess->shared_secret) < 0)
		return -1;
	wpa_hexdump_key(MSG_DEBUG, "EAP-EKE: SharedSecret",
			sess->shared_secret, (size_t) sess->auth_len);
			 ...
			 const u8 *id_p, size_t id_p_len)
{
	...
	if (eap_eke_prfplus(sess->prf, sess->shared_secret, (size_t) sess->prf_len,
		      ...
		      const u8 *nonce_p, const u8 *nonce_s)
{
	...
	label_len = os_strlen(label);
	data_len = label_len + id_s_len + id_p_len + 2U * (size_t) sess->nonce_len;
	data = os_malloc(data_len);
	if (data == NULL)
		return -1;
	pos = data;
	...
	pos += id_p_len;
	os_memcpy(pos, nonce_p, (size_t) sess->nonce_len);
	pos += sess->nonce_len;
	os_memcpy(pos, nonce_s, (size_t) sess->nonce_len);
	if (eap_eke_prfplus(sess->prf, sess->shared_secret, (size_t) sess->prf_len,
			    data, data_len, sess->ka, (size_t) sess->prf_len) < 0) {
		os_free(data);
		return -1;
	}
	os_free(data);

	wpa_hexdump_key(MSG_DEBUG, "EAP-EKE: Ka", sess->ka, (size_t) sess->prf_len);
		 ...
		 const struct wpabuf *msgs, u8 *auth)
{
	...
	wpa_hexdump_key(MSG_DEBUG, "EAP-EKE: Ka for Auth",
			sess->ka, (size_t) sess->auth_len);
	wpa_hexdump_buf(MSG_MSGDUMP, "EAP-EKE: Messages for Auth", msgs);
	return eap_eke_prf(sess->prf, sess->ka, (size_t) sess->auth_len,
