...
int p2p_random(char *buf, size_t len)
{
	...
	for (i = 0; i < len; i++) {
		val = (u8) buf[i];
		val %= (u8) (2 * letters + numbers);
		if (val < letters)
			buf[i] = (char) ('A' + val);
		else if (val < 2 * letters)
			buf[i] = (char) ('a' + (val - letters));
		else
			buf[i] = (char) ('0' + (val - 2 * letters));
	}
	...
	while (o < chan->reg_classes) {
		...
		while (c < op->channels) {
			int freq = p2p_channel_to_freq(op->reg_class,
						       op->channel[c]);
			if (freq > 0 && freq_range_list_includes(list, (unsigned int) freq)) {
				op->channels--;
			       ...
			       const struct p2p_channels *channels)
{
	...
	for (i = 0; p2p->cfg->pref_chan && i < p2p->cfg->num_pref_chan; i++) {
		...
		if (p2p_channels_includes_freq(tmpc, (unsigned int) freq))
		       ...
		       const struct p2p_channels *chan)
{
	...
	for (i = 0; i < chan->reg_classes; i++) {
		...
		c = &chan->reg_class[i];
		ret = os_snprintf(pos, (size_t) (end - pos), " %u:", c->reg_class);
		if (os_snprintf_error((size_t) (end - pos), ret))
			break;
		pos += ret;

		for (j = 0; j < c->channels; j++) {
			ret = os_snprintf(pos, (size_t) (end - pos), "%s%u",
					  j == 0 ? "" : ",",
					  c->channel[j]);
			if (os_snprintf_error((size_t) (end - pos), ret))
				break;
