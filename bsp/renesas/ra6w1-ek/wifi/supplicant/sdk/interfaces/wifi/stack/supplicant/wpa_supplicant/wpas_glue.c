...
void * gp_wifi_atcmd_ctrl = NULL;
unsigned int (* gp_wifi_atcmd_event_cb)(void * const p_ctrl, int index, unsigned char * p_in, unsigned int inlen) = NULL;
#endif
...
static int wpa_supplicant_add_pmkid(void *_wpa_s, void *network_ctx,
				    ...
				    int akmp)
{
#if 0  /* WPA by Shingu */	//MODIFY_SUPPLICANT_FOR_FREERTOS	
	struct wpa_supplicant *wpa_s = _wpa_s;
	struct wpa_ssid *ssid;
...
static void wpa_supplicant_set_anon_id(void *ctx, const u8 *id, size_t len)
{
	...
	if (wpa_s->conf->update_config) {
		res = wpa_config_write(wpa_s->confname, wpa_s->conf, wpa_s->ifname);
		if (res) {
			wpa_printf(MSG_DEBUG, "Failed to update config after "
				   "anonymous_id update");
		}
...
int wpa_supplicant_set_atcmd_event_callback(void * const p_ctrl,
                                            unsigned int (* p_callback)(void * const p_ctrl, int index,
                                                                        unsigned char * p_in, unsigned int inlen))
{
    #if (ATCMD_IF_SUPPORT == 1)
    gp_wifi_atcmd_ctrl = p_ctrl;

    gp_wifi_atcmd_event_cb = p_callback;
