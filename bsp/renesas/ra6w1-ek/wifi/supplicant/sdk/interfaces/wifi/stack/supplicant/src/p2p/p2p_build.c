...
void p2p_buf_update_ie_hdr(struct wpabuf *buf, u8 *len)
{
	...
	*len = (u8) ((u8 *) wpabuf_put(buf, 0) - len - 1);
}
				   ...
				   unsigned int size)
{
	...
	wpabuf_put_u8(buf, WLAN_EID_VENDOR_SPECIFIC);
	wpabuf_put_u8(buf, (u8) (4 + count * sizeof(u16)));
			      ...
			      struct p2p_channels *chan)
{
	...
	for (i = 0; i < chan->reg_classes; i++) {
		...
		wpabuf_put_u8(buf, (u8) c->channels);
		wpabuf_put_data(buf, c->channel, c->channels);
	}

	/* Update attribute length */
	WPA_PUT_LE16(len, (u16) ((u8 *) wpabuf_put(buf, 0) - len - 2));
	wpa_hexdump(MSG_DEBUG, "P2P: * Channel List",
		    len + 2, (size_t) ((u8 *) wpabuf_put(buf, 0) - len - 2));
}
			     ...
			     struct p2p_device *peer)
{
	...
	if (peer && peer->wps_method != WPS_NOT_READY) {
		...
		methods = (u16) (methods | (p2p->cfg->config_methods &
	...
	wpabuf_put_u8(buf, (u8) p2p->cfg->num_sec_dev_types);
			  ...
			  const u8 *ssid, size_t ssid_len)
{
	...
	wpabuf_put_le16(buf, (u16) (ETH_ALEN + ssid_len));
	wpabuf_put_data(buf, dev_addr, ETH_ALEN);
		     ...
		     struct p2p_noa_desc *desc1, struct p2p_noa_desc *desc2)
{
	...
	wpabuf_put_le16(buf, (u16) (2 + (desc1 ? 13 : 0) + (desc2 ? 13 : 0)));
	wpabuf_put_u8(buf, noa_index);
	wpabuf_put_u8(buf, (u8) ((opp_ps ? 0x80 : 0) | (ctwindow & 0x7f)));
				    ...
				    size_t *total_len, u8 *attr_len)
{
	...
	if (*ie_len) {
		...
		remaining = (size_t) (255 - ie_data_len);
	} else {
		/*
	...
	if (remaining < sizeof(u32) + sizeof(u16) + sizeof(u8)) {
		...
		holder[sizeof(u32) + sizeof(u16)] = (u8) svc_len;
		     ...
		     int all_attr)
{
	...
	if (all_attr && p2p->cfg->num_sec_dev_types) {
		...
		wpabuf_put_be16(buf, (u16) (WPS_DEV_TYPE_LEN *
	...
	for (i = 0; i < P2P_MAX_WPS_VENDOR_EXT; i++) {
		...
		wpabuf_put_be16(buf, (u16) wpabuf_len(p2p->wps_vendor_ext[i]));
