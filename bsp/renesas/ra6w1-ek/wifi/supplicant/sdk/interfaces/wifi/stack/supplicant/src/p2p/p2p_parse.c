			       ...
			       struct p2p_message *msg)
{
	...
	case P2P_ATTR_SERVICE_HASH:
		...
		msg->service_hash_count = (u8) (len / P2PS_HASH_LEN);
...
int p2p_parse_p2p_ie(const struct wpabuf *buf, struct p2p_message *msg)
{
	...
	while (pos < end) {
		...
		if (attr_len > end - pos) {
			...
			wpa_hexdump(MSG_MSGDUMP, "P2P: Data", pos, (size_t) (end - pos));
			return -1;
		}
			       ...
			       char *end)
{
	...
	for (i = 0; i < info.num_clients; i++) {
		...
		cli = &info.client[i];
		ret = os_snprintf(pos, (size_t) (end - pos), "p2p_group_client: "
		...
		ret = os_snprintf(pos, (size_t) (end - pos),
		...
		for (s = 0; s < cli->num_sec_dev_types; s++) {
			ret = os_snprintf(pos, (size_t) (end - pos), " dev_type=%s",
		...
		ret = os_snprintf(pos, (size_t) (end - pos), " dev_name='%s'\n", name);
		if (os_snprintf_error((size_t) (end - pos), ret))
			return pos - buf;
		pos += ret;
	}
...
int p2p_attr_text(struct wpabuf *data, char *buf, char *end)
{
	...
	ret = os_snprintf(pos, (size_t) (end - pos), "p2p_device_name=%s\n",
			  msg.device_name);
	if (os_snprintf_error((size_t) (end - pos), ret))
		return pos - buf;
	pos += ret;
	...
	if (msg.p2p_device_addr) {
		ret = os_snprintf(pos, (size_t) (end - pos), "p2p_device_addr=" MACSTR
				  "\n",
				  MAC2STR(msg.p2p_device_addr));
		if (os_snprintf_error((size_t) (end - pos), ret))
			return pos - buf;
	...
	ret = os_snprintf(pos, (size_t) (end - pos), "p2p_config_methods=0x%x\n",
			  msg.config_methods);
	if (os_snprintf_error((size_t) (end - pos), ret))
		return pos - buf;
	pos += ret;
