			    ...
			    struct wpabuf *msg, u8 next_payload)
{
	...
	WPA_PUT_BE16(t->transform_id, (u16) data->proposal.encr);
	...
	plen = (size_t) ((u8 *) wpabuf_put(msg, 0) - (u8 *) t);
	WPA_PUT_BE16(t->transform_length, (u16) plen);

	t = wpabuf_put(msg, sizeof(*t));
	t->type = 3;
	WPA_PUT_BE16(t->transform_length, sizeof(*t));
	t->transform_type = IKEV2_TRANSFORM_PRF;
	WPA_PUT_BE16(t->transform_id, (u16) data->proposal.prf);

	t = wpabuf_put(msg, sizeof(*t));
	t->type = 3;
	WPA_PUT_BE16(t->transform_length, sizeof(*t));
	t->transform_type = IKEV2_TRANSFORM_INTEG;
	WPA_PUT_BE16(t->transform_id, (u16) data->proposal.integ);

	t = wpabuf_put(msg, sizeof(*t));
	WPA_PUT_BE16(t->transform_length, sizeof(*t));
	t->transform_type = IKEV2_TRANSFORM_DH;
	WPA_PUT_BE16(t->transform_id, (u16) data->proposal.dh);

	plen = (size_t) ((u8 *) wpabuf_put(msg, 0) - (u8 *) p);
	WPA_PUT_BE16(p->proposal_length, (u16) plen);

	plen = (size_t) ((u8 *) wpabuf_put(msg, 0) - (u8 *) phdr);
	WPA_PUT_BE16(phdr->payload_length, (u16) plen);
			   ...
			   struct wpabuf *msg, u8 next_payload)
{
	...
	wpabuf_put_be16(msg, (u16) data->proposal.dh); /* DH Group # */
				    ...
				    struct wpabuf *msg, u8 next_payload)
{
	...
	case INVALID_KE_PAYLOAD:
		...
		wpabuf_put_be16(msg, (u16) data->proposal.dh);
