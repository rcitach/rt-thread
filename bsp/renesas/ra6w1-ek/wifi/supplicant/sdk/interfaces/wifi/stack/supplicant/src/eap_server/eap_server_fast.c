				      ...
				      u8 *master_secret)
{
	...
	if (aes_unwrap(data->pac_opaque_encr, sizeof(data->pac_opaque_encr),
		       (int) ((pac_opaque_len - 8) / 8), pac_opaque, buf) < 0) {
	...
	while (end - pos > 1) {
		...
		case PAC_OPAQUE_TYPE_LIFETIME:
			...
			lifetime = (os_time_t) WPA_GET_BE32(pos);
			break;
					    ...
					    struct eap_fast_data *data, u8 id)
{
	...
	wpabuf_put_u8(req, (u8) (EAP_TLS_FLAGS_START | data->fast_version));

	/* RFC 4851, 4.1.1. Authority ID Data */
	eap_fast_put_tlv(req, PAC_TYPE_A_ID, data->srv_id, (u16) data->srv_id_len);
	...
	struct eap_sm *sm, struct eap_fast_data *data)
{
	...
	binding->received_version = (u8) data->peer_version;
	 ...
	 */
	binding->nonce[sizeof(binding->nonce) - 1] &= (u8) ~0x01;
					  ...
					  struct eap_fast_data *data)
{
	...
	*pos++ = 4;
	WPA_PUT_BE32(pos, (u32) (now.sec + data->pac_key_lifetime));
	pos += 4;

	if (sm->identity) {
		*pos++ = PAC_OPAQUE_TYPE_IDENTITY;
		*pos++ = (u8) sm->identity_len;
		os_memcpy(pos, sm->identity, sm->identity_len);
		pos += sm->identity_len;
	}

	pac_len = (size_t) (pos - pac_buf);
	while (pac_len % 8) {
	...
	if (aes_wrap(data->pac_opaque_encr, sizeof(data->pac_opaque_encr),
		     (int) (pac_len / 8), pac_buf, pac_opaque) < 0) {
		os_free(pac_buf);
		os_free(pac_opaque);
		return NULL;
	}
	...
	/* PAC-Opaque */
	eap_fast_put_tlv(buf, PAC_TYPE_PAC_OPAQUE, pac_opaque, (u16) pac_len);
	os_free(pac_opaque);
	...
	wpabuf_put_be32(buf, (u32) (now.sec + data->pac_key_lifetime));

	/* A-ID (inside PAC-Info) */
	eap_fast_put_tlv(buf, PAC_TYPE_A_ID, data->srv_id, (u16) data->srv_id_len);
	...
	pac_info->len = host_to_be16((u16) (pos - (u8 *) (pac_info + 1)));
	pac_tlv->length = host_to_be16((u16) (pos - (u8 *) (pac_tlv + 1)));
