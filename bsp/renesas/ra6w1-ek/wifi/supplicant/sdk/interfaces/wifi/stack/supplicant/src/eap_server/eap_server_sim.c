			      ...
			      const u8 *nonce_s)
{
	...
	if (data->next_pseudonym) {
		...
		eap_sim_msg_add(msg, EAP_SIM_AT_NEXT_PSEUDONYM,
				(u16) os_strlen(data->next_pseudonym),
	...
	if (data->next_reauth_id) {
		...
		eap_sim_msg_add(msg, EAP_SIM_AT_NEXT_REAUTH_ID,
				(u16) os_strlen(data->next_reauth_id),
					       ...
					       u8 id)
{
	...
	eap_sim_msg_add(msg, EAP_SIM_AT_RAND, 0, (u8 *) data->rand,
			(size_t) (data->num_chal * GSM_RAND_LEN));
...
skip_id_update:
	...
	eap_sim_derive_mk(sm->identity, identity_len, attr->nonce_mt,
			  (u16) attr->selected_version, ver_list, sizeof(ver_list),
				      ...
				      struct eap_sim_attrs *attr)
{
	if (attr->mac == NULL ||
	    eap_sim_verify_mac(data->k_aut, respData, attr->mac,
			       (u8 *) data->sres,
			       (size_t) (data->num_chal * EAP_SIM_SRES_LEN))) {
...
static u8 * eap_sim_get_session_id(struct eap_sm *sm, void *priv, size_t *len)
{
	...
	if (!data->reauth)
		*len = (size_t) (1 + data->num_chal * GSM_RAND_LEN + EAP_SIM_NONCE_MT_LEN);
	else
	...
	id[0] = EAP_TYPE_SIM;
	if (!data->reauth) {
		os_memcpy(id + 1, data->rand, (size_t) (data->num_chal * GSM_RAND_LEN));
