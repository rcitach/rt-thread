...
u8 *global_root_ca = NULL;
u8 *global_client_cert = NULL;
u8 *global_private_key = NULL;
u8 *global_dh_param = NULL;
...
static int eap_tls_params_read_cert_blob(int module, int type, unsigned char **out, size_t *outlen)
{
	int err = 0;
	...
	/* Get actual length of certificate */
	err = RM_CERT_Read(module, type, &format, NULL, &buflen);
	if (err == RM_CERT_ERR_OK) {
		buf = os_calloc(buflen + 1, sizeof(unsigned char));
		if (!buf) {
			return -1;
		}

		/* Get certificate */
		err = RM_CERT_Read(module, type, &format, buf, &buflen);
		if (err == RM_CERT_ERR_OK) {
			*out = buf;
			*outlen = buflen;
			return 0;
		...
		err = -1;
	} else if (err == RM_CERT_ERR_EMPTY_CERTIFICATE) {
		err = 0;
	} else {
		err = -1;
	}
...
static int eap_tls_params_set_cert_blob(struct tls_connection_params *params)
{
    int err = 0;
    ...
    if (global_root_ca) {
        os_free((void *)global_root_ca);
        global_root_ca = NULL;
    }

    if (global_client_cert) {
        os_free((void *)global_client_cert);
        global_client_cert = NULL;
    }

    if (global_private_key) {
        os_free((void *)global_private_key);
        global_private_key = NULL;
    }

    if (global_dh_param) {
        os_free((void *)global_dh_param);
        global_dh_param = NULL;
    }
    ...
    err = eap_tls_params_read_cert_blob(RM_CERT_MODULE_WPA_ENTERPRISE, RM_CERT_TYPE_CA_CERT, &data, &data_len);
    if (err) {
        goto err;
    }

    params->ca_cert_blob = global_root_ca = data;
    params->ca_cert_blob_len = data_len;

    //read certificate
    data = NULL;
    data_len = 0;

    err = eap_tls_params_read_cert_blob(RM_CERT_MODULE_WPA_ENTERPRISE, RM_CERT_TYPE_CERT, &data, &data_len);
    if (err) {
        goto err;
    }

    params->client_cert_blob = global_client_cert = data;
    params->client_cert_blob_len = data_len;

    //read private key
    data = NULL;
    data_len = 0;

    err = eap_tls_params_read_cert_blob(RM_CERT_MODULE_WPA_ENTERPRISE, RM_CERT_TYPE_PRIVATE_KEY, &data, &data_len);
    if (err) {
        goto err;
    }

    params->private_key_blob = global_private_key = data;
    params->private_key_blob_len = data_len;

    //read private key
    data = NULL;
    data_len = 0;

    err = eap_tls_params_read_cert_blob(RM_CERT_MODULE_WPA_ENTERPRISE, RM_CERT_TYPE_DH_PARAMS, &data, &data_len);
    if (err) {
        goto err;
    }

    params->dh_blob = global_dh_param = data;
    params->dh_blob_len = data_len;
...
err:
    ...
    if (global_root_ca) {
        os_free(global_root_ca);
        global_root_ca = NULL;
    }

    if (global_client_cert) {
        os_free(global_client_cert);
        global_client_cert = NULL;
    }

    if (global_private_key) {
        os_free(global_private_key);
        global_private_key = NULL;
    }

    if (global_dh_param) {
        os_free(global_dh_param);
        global_dh_param = NULL;
    }

    params->ca_cert_blob = NULL;
    params->ca_cert_blob_len = 0;
    params->client_cert_blob = NULL;
    params->client_cert_blob_len = 0;
    params->private_key_blob = NULL;
				    ...
				    struct eap_peer_config *config, int phase2)
{
    ...
    eap_tls_params_set_cert_blob(params);
			  ...
			  struct eap_peer_config *config, u8 eap_type)
{
	...
	data->tls_out_limit = (size_t) config->fragment_size;
	if (data->phase2) {
				  ...
				  struct wpabuf **out_data)
{
	...
	*out_data = eap_tls_msg_alloc(eap_type, 1U + (size_t) length_included * 4U + len,
				      EAP_CODE_RESPONSE, id);
	if (*out_data == NULL)
		return -1;

	flags = wpabuf_put(*out_data, 1);
	*flags = (u8) peap_version;
	if (more_fragments)
				       ...
				       int peap_version)
{
	...
	wpabuf_put_u8(resp, (u8) peap_version); /* Flags */
	return resp;
}
			...
			char *buf, size_t buflen, int verbose)
{
	...
	ret = os_snprintf(buf + len, buflen - (size_t) len,
			  "eap_tls_version=%s\n"
			  "EAP TLS cipher=%s\n"
			  "tls_session_reused=%d\n",
			  version, name,
			  tls_connection_resumed(data->ssl_ctx, data->conn));
	if (os_snprintf_error(buflen - (size_t) len, ret))
		return len;
	len += ret;
	...
	*resp = eap_msg_alloc(EAP_VENDOR_IETF, EAP_TYPE_NAK, num_types,
	...
	for (i = 0; i < num_types; i++) {
		if (types[i].vendor == EAP_VENDOR_IETF &&
		    types[i].method < 256)
			wpabuf_put_u8(*resp, (u8) types[i].method);
	}
