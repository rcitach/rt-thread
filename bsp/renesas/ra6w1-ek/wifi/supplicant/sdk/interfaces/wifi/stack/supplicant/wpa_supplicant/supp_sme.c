						 ...
						 int *ret_use_pk)
{
	...
	if (wpa_s->sae_commit_override) {
		wpa_printf_dbg(MSG_DEBUG, "SAE: TESTING - commit override");
	...
	if (!password) {
		wpa_printf_dbg(MSG_DEBUG, "SAE: No password available");
		return NULL;
	}
	...
	if (sme_set_sae_group(wpa_s) < 0) {
		wpa_printf_dbg(MSG_DEBUG, "SAE: Failed to select group");
		return NULL;
	}
	...
	if (!use_pt &&
	    sae_prepare_commit(wpa_s->own_addr, bssid,
			       ...
			       &wpa_s->sme.sae) < 0) {
		wpa_printf_dbg(MSG_DEBUG, "SAE: Could not pick PWE");
		return NULL;
	}
				    ...
				    int start)
{
	...
	if (bss == NULL) {
		ra6wx_assoc_prt("[%s] No scan result available for "
				"the network\n", __func__);
	...
	ra6wx_assoc_prt("        [%s] freq=%d, bssid=[" MACSTR "], ssid=[%s], "
		"ssid_len=%d\n",
			__func__,
			params.freq,
	...
	if ((wpa_bss_get_vendor_ie(bss, WPA_IE_VENDOR_TYPE) ||
		...
		if (wpa_supplicant_set_suites(wpa_s, bss, ssid,
			...
			ra6wx_assoc_prt("[%s] Failed to set WPA "
	...
	wpa_s->sme.assoc_req_ie_len += wpas_supp_op_class_ie(
		wpa_s, ssid, bss,			//MODIFY_SUPPLICANT_FOR_FREERTOS - chg
	...
	if (params.auth_alg == WPA_AUTH_ALG_OPEN &&
		...
		if (!indic || indic[1] < 2) {
			wpa_printf_dbg(MSG_DEBUG, "SME: " MACSTR
...
no_fils:
	...
	if (wpa_drv_authenticate(wpa_s, &params)) {
		wpa_msg(wpa_s, MSG_INFO, "SME: Authentication request to the "
			"driver failed");
		printf(RED_COLOR " [%s:%d] Failed wpa_drv_authenticate \n" CLEAR_COLOR, __func__, __LINE__);
		wpas_connection_failed(wpa_s, bss->bssid);
		wpa_supplicant_mark_disassoc(wpa_s);
		wpabuf_free(resp);
		wpas_connect_work_done(wpa_s);
		...
		if (DEBUG_SUPP_TMP)
            printf(CYAN_COLOR " [%s:%d] Success wpa_drv_authenticate \n" CLEAR_COLOR, __func__, __LINE__);
	}
...
#if 0   // Not used	//MODIFY_SUPPLICANT_FOR_FREERTOS
...
#if 1 /* Station WPA3 SAE: Fixed the problem of not trying to reconnect. */
			wpa_dbg(wpa_s, MSG_USER_DEBUG, "[%s] SAE_SILENTLY_DISCARD", __func__);
			return -1;
#else
			return 0;
#endif /* Station WPA3 SAE: Fixed the problem of not trying to reconnect. */
		}
...
void ra6wx_sme_event_conn(struct wpa_supplicant *wpa_s, union wpa_event_data *data)
{
	...
	if (ssid == NULL) {
		printf("SME: Ignore authentication event when network is not selected \n");
		return;
	}

#if 0
	if (wpa_s->wpa_state != WPA_AUTHENTICATING) {
		printf("SME: Ignore authentication event when not in WPA_AUTHENTICATING state \n");
		return;
	}
#endif
#if 0 //cfg80211_connect
	if (os_memcmp(wpa_s->pending_bssid, data->conn_info.peer, ETH_ALEN) != 0) {
		printf("SME: Ignore authentication with unexpected peer " MACSTR "\n", MAC2STR(data->conn_info.peer));
		return;
	}
#endif
...
#if 0 //cfg80211_connect
	if (data->conn_info.status_code != WLAN_STATUS_SUCCESS) {
		printf(RED_COLOR " [%s] Failed status_code:%d peer " MACSTR "\n" CLEAR_COLOR,
												__func__, MAC2STR(data->conn_info.peer));
	} else {
		if (DEBUG_SUPP_TMP) {
			printf(" [%s] OK status_code:%d peer " MACSTR "\n",
						__func__, data->conn_info.status_code, MAC2STR(data->conn_info.peer));
		}
	}
#endif
	TX_FUNC_END("");
...
//MODIFY_SUPPLICANT_FOR_FREERTOS
void ra6wx_sme_event_auth(struct wpa_supplicant *wpa_s, union wpa_event_data *data)
{
	struct wpa_ssid *ssid = wpa_s->current_ssid;
...
//MODIFY_SUPPLICANT_FOR_FREERTOS
void ra6wx_sme_event_auth_timed_out(struct wpa_supplicant *wpa_s,
                  union wpa_event_data *data)
{
	ra6wx_assoc_prt("[%s] SME: Authentication timed out\n", __func__);
...
//MODIFY_SUPPLICANT_FOR_FREERTOS
void ra6wx_sme_event_assoc_timed_out(struct wpa_supplicant *wpa_s,
                   union wpa_event_data *data)
{
	ra6wx_assoc_prt("[%s] SME: Association timed out\n", __func__);
...
mscs_fail:
	...
	//MODIFY_SUPPLICANT_FOR_FREERTOS	
	ra6wx_assoc_prt("[%s] Trying to associate with " MACSTR
		" (SSID='%s' freq=%d MHz)\n",
		__func__, MAC2STR(params.bssid),
	...
	if (wpa_drv_associate(wpa_s, &params))
	{
		wpa_msg(wpa_s, MSG_INFO, "SME: Association request to the "
			"driver failed");
		printf(RED_COLOR " [%s:%d] Failed wpa_drv_associate \n" CLEAR_COLOR, __func__, __LINE__);	// CONN_TEMPORARY
		wpas_connection_failed(wpa_s, wpa_s->pending_bssid);
...
{
#ifdef CONFIG_IEEE80211R	//MODIFY_SUPPLICANT_FOR_FREERTOS
	os_free(wpa_s->sme.ft_ies);
	wpa_s->sme.ft_ies = NULL;
	wpa_s->sme.ft_ies_len = 0;
#endif /* CONFIG_IEEE80211R */
...
#define P2P_MAX_CHANNELS 50	//MODIFY_SUPPLICANT_FOR_FREERTOS
...
#ifdef CONFIG_IEEE80211W_SIGMA		//MODIFY_SUPPLICANT_FOR_FREERTOS
		wpa_s->disconnected = 1;
#endif /* CONFIG_IEEE80211W_SIGMA */
		return 1;
	}
