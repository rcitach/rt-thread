...
int wifi_conn_fail_reason_code = 0;
int wifi_disconn_reason_code = 0;
...
#define	sme_event_auth				ra6wx_sme_event_auth
#define	sme_event_auth_timed_out	ra6wx_sme_event_auth_timed_out	
...
extern int RM_PMGR_W_dpm_abnormal_wifi_conn_retry_cnt_get(void);
...
#endif	/*CONFIG_IEEE80211H || CONFIG_5G_SUPPORT*/
//////MODIFY_SUPPLICANT_FOR_FREERTOS - end add

extern int get_run_mode(void);
void wpa_scan_results_free(struct wpa_scan_results *res);	//MODIFY_SUPPLICANT_FOR_FREERTOS
...
#if 1 /* FC9000 Only ?? supplicant 2.6 */	//MODIFY_SUPPLICANT_FOR_FREERTOS
	struct wpa_bss *bss=NULL;
#endif
...
#if 0 //trinity_0150401	//MODIFY_SUPPLICANT_FOR_FREERTOS
#ifdef CONFIG_AP
					 ...
					 struct wpa_bss *bss, int debug_print)
{
	...
	memset(&ie, 0, sizeof(struct wpa_ie_data));	//MODIFY_SUPPLICANT_FOR_FREERTOS - add
				     ...
				     int only_first_ssid, int debug_print)
{
	...
	if (debug_print) {
		wpa_dbg(wpa_s, MSG_EXCESSIVE, "%d: " MACSTR
...
{	//MODIFY_SUPPLICANT_FOR_FREERTOS - chg
	...
	if (wpa_s->current_ssid) {
		...
		ra6wx_ev_prt("[%s] Scan results matching the currently selected network\n", __func__);
		for (i = 0; i < wpa_s->last_scan_res_used; i++) {
			...
			if (ssid != wpa_s->current_ssid)
				continue;
#if 0
			ra6wx_assoc_prt("[%s] %u" MACSTR " ssid='%s' (%d)\n",
				__func__,
				i, MAC2STR(bss->bssid),
	...
	if (only_first_ssid) {
		ra6wx_ev_prt("[%s] Try to find BSS matching pre-selected network id=%d\n",
			__func__, group->id);
	}
#ifdef CONFIG_PRIO_GROUP
	else {
		ra6wx_ev_prt("[%s] Selecting BSS from priority group %d\n",
	...
	for (i = 0; i < wpa_s->last_scan_res_used; i++) {
		...
		*selected_ssid = wpa_scan_res_match(wpa_s, i, bss, group, only_first_ssid, 1);
		...
		if (wpa_s->scan_for_connection) {
			ra6wx_notice_prt(">>> Selected BSS " MACSTR " ssid='%s' (%d)\n",
					MAC2STR(bss->bssid),
					wpa_ssid_txt(bss->ssid, bss->ssid_len), bss->level);
		}
		return bss;
...
#ifdef CONFIG_WPS
		//wpas_wps_pbc_overlap(wpa_s);	//MODIFY_SUPPLICANT_FOR_FREERTOS
...
wpa_supplicant_pick_new_network(struct wpa_supplicant *wpa_s)
{
	...
	for (ssid = wpa_s->conf->ssid; ssid; ssid = ssid->next)
	{
		if (wpas_network_disabled(wpa_s, ssid))
			continue;
		if (ssid->mode == WPAS_MODE_IBSS || ssid->mode == WPAS_MODE_AP)
			return ssid;
	}
#endif	// CONFIG_PRIO_GROUP
	return NULL;
...
#ifdef  CONFIG_SUPP27_SCAN	//MODIFY_SUPPLICANT_FOR_FREERTOS
...
#ifdef CONFIG_SCAN_RESULT_OPTIMIZE	//MODIFY_SUPPLICANT_FOR_FREERTOS
static int wpas_select_network_from_last_scan(struct wpa_supplicant *wpa_s,
...
#endif /* CONFIG_SCAN_RESULT_OPTIMIZE */
{
	...
	if (selected) {
			...
			if (wpa_s->conf->roam_state == ROAM_SCANNING) {
					...
					(wpa_s->wpa_state == WPA_COMPLETED)))
				{
					...
					if (wpa_s->manual_scan_use_id) {
						...
						ra6wx_scan_prt("[%s] Assigned scan id %u\n",
									__func__, wpa_s->manual_scan_id);
					}
				}
			}
#endif  /* CONFIG_SIMPLE_ROAMING */	//MODIFY_SUPPLICANT_FOR_FREERTOS
		...
		wpa_s->num_retries = 0;
		ra6wx_scan_prt("[%s] wpa_s->num_retries = %d\n", __func__, wpa_s->num_retries);
...
#ifdef  CONFIG_WPS
		if (wpas_wps_scan_pbc_overlap(wpa_s, selected, ssid)) {
			ra6wx_notice_prt(WPS_EVENT_OVERLAP "PBC session overlap\n");
#ifdef CONFIG_P2P
			if (wpa_s->p2p_group_interface == P2P_GROUP_INTERFACE_CLIENT ||
				wpa_s->p2p_in_provisioning) {
				return -1;
		...
		if (   wpa_s->reassociate
			...
			ra6wx_assoc_prt("[%s] Request association with " MACSTR "\n", __func__, MAC2STR(selected->bssid));
...
#endif /* CONFIG_SCAN_RESULT_OPTIMIZE */	//MODIFY_SUPPLICANT_FOR_FREERTOS
		...
		*ssid_p = ssid;
		...
		if (wpa_s->scan_for_connection || wpa_s->last_scan_req != MANUAL_SCAN_REQ) {
#if !defined ( __DISABLE_NO_SUTIABLE_NETWORK_MSG__ )
			ra6wx_notice_prt("!!! No selected network !!!\n");
#endif // !__DISABLE_NO_SUTIABLE_NETWORK_MSG__

			wpa_s->disconnect_reason = 0;

#if CFG_PMGR
#if !defined ( __DISABLE_DPM_ABNORM__ )
			if (RM_PMGR_W_dpm_abnormal_wifi_conn_retry_cnt_get() > 0) {
			...
			ra6wx_scan_prt("[%s] wpa_supplicant_cancel_scan()\n", __func__);
...
#ifdef CONFIG_SCAN_RESULT_OPTIMIZE		//MODIFY_SUPPLICANT_FOR_FREERTOS - add
static int wpas_connect_network_from_last_scan(struct wpa_supplicant *wpa_s,
					struct wpa_bss *selected, struct wpa_ssid *ssid)
{
	...
	ra6wx_scan_prt("[%s] Start connecting to the selected network. \n", __func__);
#ifdef CONFIG_FAST_RECONNECT_V2
	ra6wx_is_fast_reconnect = 0;
#endif
	if (wpa_supplicant_connect(wpa_s, selected, ssid) < 0) {
		ra6wx_scan_prt("[%s] Connect failed\n", __func__);
		return -1;
	}
...
#if 1 /* FC9000 Only */		//MODIFY_SUPPLICANT_FOR_FREERTOS
/* Supplicant 2.7 Code */
...
//static void wpa_supp_ev_scan_results(struct wpa_supplicant *wpa_s, union wpa_event_data *data)
static int wpa_supp_ev_scan_results(struct wpa_supplicant *wpa_s, union wpa_event_data *data) 
{
	...
	if (scan_res == NULL) {
		...
		ra6wx_scan_prt("              <%s> Failed to get scan results "
				"- try scanning again\n", __func__);
...
#if 1 //[temp-230516]build error
            || !wpa_s->radio->external_scan_req_interface
#else
...
#ifdef CONFIG_AP
	if (ap) {
		ra6wx_scan_prt("<%s> Ignore scan results in AP mode\n", __func__);
...
#if 0
	ra6wx_scan_prt("<%s> New scan results available (own=%u ext=%u)\n",
			__func__,
	...
	if (wpa_s->last_scan_req == MANUAL_SCAN_REQ &&
		...
		ra6wx_scan_prt("[%s] " WPA_EVENT_SCAN_RESULTS "id=%u\n",
				__func__, wpa_s->manual_scan_id);
	...
	if (!wpa_s->own_scan_running
		...
		ra6wx_scan_prt("              <%s> Do not use results from "
	...
	if (data->assoc_info.req_ies && data->assoc_info.resp_ies) {
		...
		if (ieee802_11_parse_elems(data->assoc_info.req_ies,
			...
			wpa_s->connection_vht = req_elems.vht_capabilities &&
				...
				(!data->assoc_info.freq 
#ifdef  CONFIG_BAND
				|| wpas_freq_to_band(data->assoc_info.freq) != BAND_2_4_GHZ
#endif	// CONFIG_BAND
				);
...
static void wpa_supplicant_event_conn(struct wpa_supplicant *wpa_s,
				       union wpa_event_data *data)
{
	u8 bssid[ETH_ALEN];
	...
	if (DEBUG_SUPP_TMP) printf(" [%s] Connected with " MACSTR "\n ", __func__, MAC2STR(bssid));
	...
	wpa_s->assoc_freq = data->conn_info.freq;
...
static void wpa_supplicant_atcmd_disassoc(void)
{
	if (!gp_wifi_atcmd_ctrl || !gp_wifi_atcmd_event_cb) {
		return ;
	}

	gp_wifi_atcmd_event_cb(gp_wifi_atcmd_ctrl, 1, NULL, 0);
}
#endif
					  ...
					  int locally_generated)
{
	...
	wpa_supplicant_atcmd_disassoc();
#endif

	rm_wifi_event_signal_disconnected(reason_code);
	...
	if (!is_zero_ether_addr(bssid) ||
	    wpa_s->wpa_state >= WPA_AUTHENTICATING) {
#if 1
		ra6wx_notice_prt(ANSI_COLOR_LIGHT_RED "[%s] " WPA_EVENT_DISCONNECTED "bssid=" MACSTR
				" reason=%d%s\n" ANSI_COLOR_DEFULT,
				__func__, MAC2STR(bssid), reason_code,
				locally_generated ? " locally_generated=1" : "");
#else
		wpa_msg(wpa_s, MSG_INFO, ANSI_COLOR_LIGHT_RED"[%s] "WPA_EVENT_DISCONNECTED "bssid=" MACSTR
			" reason=%d%s"ANSI_COLOR_DEFULT, __func__,
			MAC2STR(bssid), reason_code,
			locally_generated ? " locally_generated=1" : "");
#endif /* 1 */
		if (os_strcmp(wpa_s->ifname, STA_DEVICE_NAME) == 0)
		{
			eloop_register_timeout(0, 0, wpa_supplicant_reconfig_net, wpa_s, NULL);
		}
	}
}
...
#ifdef __WIFI_CONN_RETRY_STOP_AT_WK_CONN_FAIL__
...
#if CFG_PMGR
#if !defined ( __DISABLE_DPM_ABNORM__ )
#ifdef __DPM_ABN_SLEEP1_BY_WK_CONN_FAIL__
	...
	case EVENT_INTERFACE_ADDED:
		...
		if (!wpa_s->global->p2p &&
		    ...
		    wpas_p2p_add_p2pdev_interface(
			    wpa_s, wpa_s->global->params.conf_p2p_dev) == NULL) {
			wpa_printf(MSG_INFO,
			      ...
			      struct deauth_info *info)
{
	...
	if ((os_strcmp(wpa_s->ifname, P2P_DEVICE_NAME) == 0 && wpa_s->key_mgmt == 0) ||
	    wpa_s->key_mgmt == WPA_KEY_MGMT_WPS) {
		wpa_dbg(wpa_s, MSG_DEBUG,"Ignore deauth event in WPS mode\n");
		return;
	}
					...
					struct channel_list_changed *info)
{
	...
	if (get_run_mode() != WIFI_DEVICE_MODE_EXT_STATION &&
		...
		if (get_run_mode() >= WIFI_DEVICE_MODE_EXT_AP_STATION && wpa_s->conf->ssid->wifi_mode > WIFI_MODE_BGN ) {
			ra6wx_notice_prt("\nwifi_mode cannot be set in sys_mode(%d).\n", get_run_mode());
		} else if (wpa_s->conf->ssid->wifi_mode >= WIFI_MODE_BGN
			&& wpa_s->conf->ssid->wifi_mode < WIFI_MODE_MAX) {
			wifi_mode = wpa_s->conf->ssid->wifi_mode;
            /* TODO: currently 'wifi_mode' is not used by fc80211_get_hw_feature_data()
                     to check after 5GHz is enabled in HW */
		}
...
#else
	ra6wx_notice_prt("[%s] %s: Updating hw mode\n", __func__,  wpa_s->ifname);
...
#endif /* CONFIG_SME || CONFIG_IEEE80211W */
				       ...
				       struct dfs_event *radar)
{
	...
	{
        /*
            Note: this else part will not be entered because WPA_DRIVER_FLAGS_DFS_OFFLOAD 
            is not supported in rwnx driver.
        */
					...
					struct dfs_event *radar)
{
	...
	{
        /*
            Note: this is supposed to be invoked if radar is NOT detected during 'STA' mode
            In STA mode, running radar detection is not mandatory (AP mode will run radar 
            detect function), so EVENT_DFS_CAC_FINISHED will not be posted from lower layer.
        */
				       ...
				       struct dfs_event *radar)
{
	...
	{
        /*
            Note: this is supposed to be invoked if radar is detected during 'STA' mode
            In STA mode, running radar detection is not mandatory (AP mode will run radar 
            detect function), so EVENT_DFS_CAC_ABORTED will not be posted from lower layer.
        */
				    ...
				    union wpa_event_data *data)
{
	...
	if (
#ifdef CONFIG_AP_WIFI_MODE
	    wpa_s->current_ssid->dpm_opt_wifi_mode &&
#endif /* CONFIG_AP_WIFI_MODE */
		(data->assoc_reject.status_code == 0x12 || data->assoc_reject.status_code == 0x1b)) {
#ifdef CONFIG_AP_WIFI_MODE
		if (wpa_s->current_ssid->wifi_mode == WIFI_MODE_BG) {
			ra6wx_notice_prt(">>> Wi-Fi mode : b/g -> AUTO\n");
			wpa_s->current_ssid->wifi_mode = WIFI_MODE_AUTO_STA;
		} else if (wpa_s->current_ssid->wifi_mode == WIFI_MODE_G_ONLY) {
			ra6wx_notice_prt(">>> Wi-Fi mode : g-only -> g/n\n");
			wpa_s->current_ssid->wifi_mode = WIFI_MODE_GN;
		}
#endif /* CONFIG_AP_WIFI_MODE */
#ifdef CONFIG_AP
	...
	if (data->assoc_reject.bssid) {
		ra6wx_notice_prt(WPA_EVENT_ASSOC_REJECT
			...
			data->assoc_reject.reason_code != STA_CONNECT_FAIL_REASON_UNSPECIFIED ?
			" qca_driver_reason=" : "",
...
#if 0 /* by Shingu 20200513 */	//MODIFY_SUPPLICANT_FOR_FREERTOS
	if (wpa_s->drv_flags & WPA_DRIVER_FLAGS_SME) {
			  ...
			  union wpa_event_data *data)
{
...
#ifndef CONFIG_NO_STDOUT_DEBUG
	{
		...
		if (event == EVENT_RX_MGMT && wpa_s->current_ssid->mode == WPAS_MODE_AP) {	//MODIFY_SUPPLICANT_FOR_FREERTOS - add
			level = MSG_EXCESSIVE;
		}

#endif	// CONFIG_WPA_DBG
		wpa_dbg(wpa_s, level, ANSI_COLOR_GREEN "[%s] EVENT_%s (%d) received" ANSI_COLOR_DEFULT, __func__, event_to_string(event), event);
	}
...
#if 0 //cfg80211_connect
	case EVENT_CONN:
		if (DEBUG_SUPP_TMP) printf(" [%s:%d] event: EVENT_CONN status_code:%d \n", __func__, __LINE__, data->conn_info.status_code);
		wpa_s->auth_status_code = data->conn_info.status_code;
		wpa_s->assoc_status_code = WLAN_STATUS_SUCCESS;

		ra6wx_sme_event_conn(wpa_s, data);
		wpa_supplicant_event_conn(wpa_s, data);
		wpa_s->assoc_status_code = WLAN_STATUS_SUCCESS;
	...
	case EVENT_ASSOC_TIMED_OUT:
		...
		case EVENT_DHCP_NO_RESPONSE:
			if (wpa_s->wpa_state == WPA_COMPLETED && wpa_s->conf->ssid)
			{
				...
				{
					ra6wx_notice_prt(YELLOW_COLOR "WEP key might be incorrect. \n" CLEAR_COLOR);
					wpa_supplicant_deauthenticate(wpa_s, WLAN_REASON_UNSPECIFIED);
	            ...
	            else
	            {
	                ...
	                ra6wx_notice_prt(YELLOW_COLOR "IP not assigned\n" CLEAR_COLOR);
	                wpa_supplicant_deauthenticate(wpa_s, WLAN_REASON_UNSPECIFIED);
	            }
			}
			break;
	...
	case EVENT_CH_SWITCH:
		...
		//MODIFY_SUPPLICANT_FOR_FREERTOS
		ra6wx_info_prt("[%s] freq=%d ht_enabled=%d ch_offset=%d cf1=%d cf2=%d\n",
			       __func__,
			       data->ch_switch.freq,
...
#ifdef CONFIG_IEEE80211W	//MODIFY_SUPPLICANT_FOR_FREERTOS - del
		if (wpa_s->drv_flags & WPA_DRIVER_FLAGS_SME)
	...
	case EVENT_DFS_PRE_CAC_EXPIRED:
		if (data)
			wpas_ap_event_dfs_pre_cac_expired(wpa_s,
							&data->dfs_event);
		break;		
...
#endif /* CONFIG_SAE */
			wpa_dbg(wpa_s, MSG_EXCESSIVE, "AP: ignore received "
				"management frame(%d) in non-AP mode", stype);
			break;
#ifdef CONFIG_AP
		}

		//p2p go mode crash bugfix
...
void wpa_scan_results_free(struct wpa_scan_results *res)
{
	...
	ra6wx_scan_prt("[%s] Free %d scan results\n", __func__,  res->num);
...
void wpa_supplicant_copy_ie_for_current_bss(struct wpa_supplicant *wpa_s, struct wpa_bss *bss)
{
	...
	ra6wx_scan_prt("[%s] Copy IE information for current_bss\n", __func__);
	ra6wx_scan_prt("[%s] wpa_s->current_bss  = 0x%p\n", __func__, wpa_s->current_bss);
	...
	if(bss->ie_len) {
		...
		if(bss->ie) {
			ra6wx_scan_prt("[%s] Alloc  ie = 0x%x, len = %d\n", __func__,
						bss->ie, bss->ie_len);
	...
	if(bss->beacon_ie_len) {
		...
		if(bss->beacon_ie) {
			ra6wx_scan_prt("[%s] Alloc  beacon_ie = 0x%x, len = %d\n", __func__,
