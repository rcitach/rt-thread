			    ...
			    const struct wpabuf *msg, size_t *plen)
{
	...
	if (*pos == EAP_TYPE_EXPANDED) {
		...
		pos++;
		exp_vendor = (int) WPA_GET_BE24(pos);
		pos += 3;
			      ...
			      size_t payload_len, u8 code, u8 identifier)
{
	...
	hdr->length = host_to_be16((unsigned short) len);

	if (vendor == EAP_VENDOR_IETF) {
		wpabuf_put_u8(buf, type);
	} else {
		wpabuf_put_u8(buf, EAP_TYPE_EXPANDED);
		wpabuf_put_be24(buf, (u32) vendor);
...
void eap_update_len(struct wpabuf *msg)
{
	...
	if (wpabuf_len(msg) < sizeof(*hdr))
		return;
	hdr->length = host_to_be16((unsigned short) wpabuf_len(msg));
}
