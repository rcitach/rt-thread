				     ...
				     struct wpabuf *ie)
{
	...
	/* P2P Capability */
	dev_capab &= (u8) ~P2P_DEV_CAPAB_CLIENT_DISCOVERABILITY;
...
static void p2p_group_add_noa(struct wpabuf *ie, struct wpabuf *noa)
{
	...
	wpabuf_put_le16(ie, (u16) wpabuf_len(noa));
			    ...
			    int max_clients)
{
	...
	WPA_PUT_LE16(group_info + 1,
		     (u16) ((u8 *) wpabuf_put(buf, 0) - group_info - 3));
}
			  ...
			  const u8 *searching_dev, int rx_freq)
{
	...
	if (p2p->cfg->send_action(p2p->cfg->cb_ctx, (unsigned int) freq, m->addr,
...

{
	...
	*num = (unsigned int) p2p_channels_to_freqs(&intersect, common_freqs, *num);
