...
extern int ra6w1_regdb_is_valid_freq(char* country, int freq, unsigned int exclude_flags);
...
/* when item is added / deleted, sync with ssid_fields[] */
network_field_t network_fields[] = {
	...
	{ "imsi_identity",			0 },
#ifdef UNUSED_CODE_SUPP_CONFIG
	{ "machine_identity",			0 },
#endif /* UNUSED_CODE_SUPP_CONFIG */
	{ "password",					0 },
#ifdef UNUSED_CODE_SUPP_CONFIG
	{ "machine_password",					0 },
#endif /* UNUSED_CODE_SUPP_CONFIG */
	...
	{ "machine_ca_cert",					0 },
	{ "machine_ca_path",					0 },
	{ "machine_client_cert",					0 },
	{ "machine_private_key",					0 },
	{ "machine_private_key_passwd",					0 },
	{ "machine_dh_file",					0 },
	{ "machine_subject_match",					0 },
	{ "machine_check_cert_subject",					0 },
	{ "machine_altsubject_match",					0 },
	{ "machine_domain_suffix_match",					0 },
	{ "machine_domain_match",					0 },
	{ "machine_pin",					0 },
	{ "machine_engine_id",					0 },
	{ "machine_key_id",					0 },
	{ "machine_cert_id",					0 },
	{ "machine_ca_cert_id",					0 },
	{ "machine_engine",					0 },
	{ "machine_ocsp",					0 },	
	{ "eapol_flags",				0 },
	{ "sim_num",					0 },
				...
				int line, const char *value)
{
	...
	if (tmp == NULL) {
		wpa_printf(MSG_ERROR, "[%s] Line %d: failed to parse %s '%s'.",
			   __func__, line, data->name,
	...
	if (data->param3 && res_len < (size_t) data->param3) {
		wpa_printf(MSG_ERROR, "[%s] Line %d: too short %s (len=%lu "
			   "min_len=%ld)", __func__, line, data->name,
	...
	if (data->param4 && res_len > (size_t) data->param4) {
		wpa_printf(MSG_ERROR, "[%s] Line %d: too long %s (len=%lu "
			   "max_len=%ld)", __func__, line, data->name,
				...
				int line, const char *value)
{
	...
	if (*end) {
		wpa_printf(MSG_ERROR, "[%s] Line %d: %s ==> invalid number \"%s\"",
			   __func__, line, data->name, value);
		return -1;
	}
	...
	*dst = val;
	...
	if (data->param3 && *dst < (long) data->param3) {
		wpa_printf(MSG_ERROR, "[%s] Line %d: too small %s (value=%d "
			   "min_value=%ld)", __func__, line, data->name, *dst,
			   (long) data->param3);
	...
	if (data->param4 && *dst > (long) data->param4) {
		wpa_printf(MSG_ERROR, "[%s] Line %d: too large %s (value=%d "
			   "max_value=%ld)", __func__, line, data->name, *dst,
			   (long) data->param4);
...
#ifdef CONFIG_AP_POWER		//MODIFY_SUPPLICANT_FOR_FREERTOS - add
...
{ //MODIFY_SUPPLICANT_FOR_FREERTOS-chg
	 ...
	 *	     : RA6W1 need special character [ ' ] in cli command line distinguisher.
	 */
				   ...
				   struct wpa_ssid *ssid)
{
	...
	if (ssid->passphrase)
		return wpa_config_write_string_ascii(
			...
			os_strlen(ssid->passphrase)); // max 63

	if (ssid->psk_set)
		return wpa_config_write_string_hex(ssid->psk, PMK_LEN); // %02x * 32 == 64
				     ...
				     const char *value)
{
	...
	if(val == WPA_CIPHER_TKIP &&
		(ssid->wifi_mode == WIFI_MODE_N_ONLY || 
		 ssid->wifi_mode == WIFI_MODE_N_ONLY_5G)) {
		ra6w1_cli_prt("[%s] Not allowed	TKIP in N only mode\n",
				__func__);
		return -1;
	}
#endif /* CONFIG_AP_WIFI_MODE */
				     ...
				     const char *value)
{
	...
	if(ssid->mode == WPAS_MODE_AP) {
		ra6w1_cli_prt("[%s] Not allowed WEP in N only mode\n",
				__func__);
		return -1;
	}
#endif /* CONFIG_AP_WIFI_MODE */
...
#ifdef CONFIG_AP_WIFI_MODE	//[[ trinity_0160711 -- Issue #100
	if(ssid->mode == WPAS_MODE_AP) {
		ra6w1_cli_prt("[%s] Not allowed WEP in AP mode\n",
				__func__);
		return NULL;
	}
#endif /* CONFIG_AP_WIFI_MODE */
 ...
 * functions.
 */
/* when item is added / deleted, sync with network_fields[] */
...
#if 0 // wpa_supplicant - Original code
	{ INT(dtim_period) },
	{ INT(beacon_int) },
#else // Tunning value for RRQ61x
 ...
 * @dbg_info: 1 (shows debug info) / 0 (do not show debug info)
 * Returns: channel or frequency value, <0 if error, ==0 if country code valid in query mode
 */
int chk_channel_by_country(char *country, int set_channel, int mode, int *rec_channel, int dbg_info)
{
#if 0	/* by Shingu 20161024 (Concurrent) */
	int ret_channel = -1;
#endif	/* 0 */

    /* 1 (incorrect country code), 2 (invalid freq), 3 (not valid for the country) */
    int ch_validity = 0;

    if (set_channel == -1) { // country code query mode
        wpa_printf(MSG_DEBUG, "[%s:%d] country query mode \n", __func__, __LINE__);
        if (ra6w1_regdb_get_cty_idx_from_cc(country) != -1) {
            wpa_printf(MSG_DEBUG, "[%s:%d] country code itself is ok \n", __func__, __LINE__);
            return 0;
        } else { // not in country code list
            ch_validity = 1;
            goto CTY_VALIDITY_CHK_ERROR;
        }
    }
	...
	if (set_channel == 0) {
        if (ra6w1_regdb_get_cty_idx_from_cc(country) != -1) {
            wpa_printf(MSG_INFO, "[%s] %s is valid, set_channel(%d) \n", __func__, country, set_channel);
            return set_channel;
        } else {
            if (dbg_info) {
        		ra6w1_regdb_print_country();
        		ra6wx_err_prt("Incorrect Country Code (%s)\n", country);
            }
            return -1;
        }
	}
	...
	if (mode) {
        ...
        wpa_printf(MSG_DEBUG, "[%s] set_channel (%d) \n", __func__, set_channel);
#else
		if (set_channel == 2484)
			set_channel = 14;
    ...
    // ... set_channel is always 'channel' regardless of mode at this moment.
    wpa_printf(MSG_DEBUG, "[%s] Non-query mode \n", __func__);
    int freq_tmp = ra6w1_regdb_ch_to_freq(set_channel);
    if (freq_tmp != -1) {
        wpa_printf(MSG_DEBUG, "[%s] freq_tmp (%d)\n", __func__, freq_tmp);
        if (ra6w1_regdb_is_valid_freq(country, freq_tmp, 0)) {
            wpa_printf(MSG_DEBUG, "[%s] %d is valid in %s \n", __func__, freq_tmp, country);        
            if (mode) {
                return freq_tmp;
            } else { // should return as channel
                return set_channel;
            }
        } else { // input freq not valid in the country
            ch_validity = 3;
            ra6wx_notice_prt("[%s:%d] input freq is not valid for this country\n", __func__, __LINE__);
            if (rec_channel) { 
                /* in case "recommanded freq" is explicitly requested, based on input freq 
                   (on which band is deduced), get one based on min channel policy (the 
                   minimum channel among available ones)
                */
                int min_ch = 0;
                if (IS_FREQ_2P4GHZ(freq_tmp)) {
                    ra6w1_regdb_get_ch_range_by_country_n_band(country, 2, &min_ch, NULL, NULL, 0);
                    min_ch = mode?(i3ed11_ch_to_freq(min_ch, 0 /*IEEE80211_BAND_2GHZ*/)):(min_ch);
                } else {
                    ra6w1_regdb_get_ch_range_by_country_n_band(country, 5, &min_ch, NULL, NULL, 0);
                    if (min_ch == -1) { 
                        /* in case this country does not support 5GHz at all, 
                         recommend channel 1 in 2.4GHz */
                        min_ch = mode?(i3ed11_ch_to_freq(1, 0 /*IEEE80211_BAND_2GHZ*/)):1;
                    } else {
                        min_ch = mode?(i3ed11_ch_to_freq(min_ch, 1 /*IEEE80211_BAND_5GHZ*/)):(min_ch);
                    }
                }
                
                *rec_channel = min_ch; // set min channel supported from the country
                
                wpa_printf(MSG_DEBUG, "[%s] recommending channel %d ... \n", __func__, *rec_channel);
            }
        }       
    } else {
        if (dbg_info) {
            ra6wx_err_prt("Invalid freq !!!\n");
        }
        ch_validity = 2;
    }

	if (ch_validity == 1 || ch_validity == 2) {
CTY_VALIDITY_CHK_ERROR:        
        if (ch_validity == 1) {
            if (dbg_info) {
		        ra6wx_err_prt("Incorrect Country Code !!! \n");
            }
        } else if (ch_validity == 2) {
            if (dbg_info) {
                ra6wx_err_prt("Invalid frequency !!! \n");
            }
        }
        if (dbg_info) {
		    ra6w1_regdb_print_country();
...
set_default :
	...
	ra6wx_info_prt("Set Default %s = [%d] \n",
		      mode ? "Frequency":"Channel", ret_channel);
	return ret_channel;
#else
    if (dbg_info) {
    	ra6wx_notice_prt("[%s:%d] Current country code \"%s\" does not support ch %d.\n", 
            __func__, __LINE__, country, set_channel);
    }

	return NOTINCLUDED_CHANNEL;
#endif	/* 0 */
}
//MODIFY_SUPPLICANT_FOR_FREERTOS-add end
		...
		char *ifname, int index)
{
    ...
    if (index != 0 && index != 1) {
        if (index == FIXED_NETWORK_ID_NONE && 
            os_strcmp(ifname, STA_DEVICE_NAME) == 0) {
            index = FIXED_NETWORK_ID_STA;
        } else {
            ra6wx_err_prt("[%s] Unknown ID %d [STA:0 / AP,"
#ifdef	CONFIG_P2P
                ",P2P"
#endif	/* CONFIG_P2P */
#ifdef CONFIG_MESH
    ...
    id = index; // staion : 0, softap, p2p, mesh : 1 
#else
	if ((index >= FIXED_NETWORK_ID_STA)
		...
		if (os_strcmp(ifname, STA_DEVICE_NAME) == 0) {
		  ...
		  || index == FIXED_NETWORK_ID_MESH_POINT) {
			...
			ra6wx_err_prt("[%s] Unknown ID %d [STA:0 / AP:1"
#ifdef	CONFIG_P2P
				" / P2P:2"
#endif	/* CONFIG_P2P */
#ifdef CONFIG_MESH
...
#else
        if (ssid->id == id) {
            wpa_printf(MSG_INFO, "network_id(%d) already exists.", id);
            return NULL;
        }
#endif
		prev = ssid;
...
void wpa_config_set_global_defaults(struct wpa_config *config)
{
	...
	if ((ra6w1_tls_version != 0x301)
		&& (ra6w1_tls_version != 0x302)
		&& (ra6w1_tls_version != 0x303)) {
#if defined(MBEDTLS_SSL_PROTO_TLS1)
		ra6w1_tls_version = 0x301;
...
int wpa_config_set(struct wpa_ssid *ssid, const char *var, const char *value, int line)
{
	...
	if (ssid->key_mgmt & WPA_KEY_MGMT_SAE) { /* SAE */
		...
		if (ssid->key_mgmt & WPA_KEY_MGMT_PSK) { /* SAE & WPA2-PSK */
			if (os_strcmp(var, "sae_password") == 0) {
				if (os_strlen(value) > (63+2) || os_strlen(value) < (8+2)) {
					return -1;
				}
			}
		} else {
			if (os_strcmp(var, "sae_password") == 0) {
				if (os_strlen(value) > (128+2)) {
					return -1;
				}
			}
		}
	...
	for (i = 0; i < NUM_SSID_FIELDS; i++) {
		...
		if (field->parser(field, ssid, __LINE__, value) < 0) 		// wpa_config_parse_str
		{
			ra6wx_err_prt("[%s] Failed to parse %s => '%s'\n", __func__, var, value);
			ret = -1;
		}
		...
		if (os_strncmp("wep_key", var, 7) == 0) {
			for (int idx = 0; idx < 3; idx++) {
				...
				if (field->parser(field, ssid, 0, value) < 0)
				{
	...
	if (i == NUM_SSID_FIELDS) {
		ra6wx_err_prt("[%s] Unknown network field '%s'\n", __func__, var);
		ret = -1;
	}
...
void wpa_config_update_psk(struct wpa_ssid *ssid)
{
    ...
    struct os_time now;
    int time;
    //ra6wx_notice_prt("[%s] start\n", __func__);
    //ra6wx_notice_prt("ssid->passphrase=%s\n", ssid->passphrase);
    //ra6wx_notice_prt("ssid->sae_password=%s\n", ssid->sae_password);
    //ra6wx_notice_prt("ssid->psk=%s\n",  wpa_config_write_string_hex(ssid->psk, PMK_LEN));

    if (fast_connection_sleep_flag == pdTRUE) {
        time = portCONVERT_TICKS_2_MS(xTaskGetTickCount());
...
#ifdef PSK_RAW_KEY_TIME_PRINT 
	if (fast_connection_sleep_flag == pdTRUE) {
		time = portCONVERT_TICKS_2_MS(xTaskGetTickCount()) - time;
		ra6wx_notice_prt("[%s] pbkdf2_sha1: %d msec\n", __func__, time);
	}

	//ra6wx_notice_prt("[%s] end\n", __func__);
	//ra6wx_notice_prt("ssid->passphrase=%s\n", ssid->passphrase);
	//ra6wx_notice_prt("ssid->sae_password=%s\n", ssid->sae_password);
...
#undef ecw2cw
...
#ifdef CONFIG_MESH	//MODIFY_SUPPLICANT_FOR_FREERTOS
	...
	else if (get_run_mode() == WIFI_DEVICE_MODE_EXT_MESH_PORTAL)
	...
	config->update_config = 1;	//MODIFY_SUPPLICANT_FOR_FREERTOS
					...
					const char *pos)
{
	...
	wpa_printf(MSG_DEBUG, "%s = 0x%x", data->name,
		   WPA_GET_BE32((u8 *) dst));
#else
	ra6wx_p2p_prt("[%s] What do I do for this function ???\n", __func__);
	...
	struct wpa_config *config, int line, const char *pos)
{
	...
	return 0;
#else
	ra6wx_p2p_prt("[%s] Have to include CONIFIG_WPS feature...\n", __func__);
	return -1;
#endif	// CONFIG_WPS
}
...
//#define OFFSET(v) ((void *) &((struct wpa_ssid *) 0)->v)	//MODIFY_SUPPLICANT_FOR_FREERTOS - chg
