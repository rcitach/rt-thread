				      ...
				      u8 dialog_token, int freq)
{
	...
	if (p2p_send_action(p2p, (unsigned int) freq, dst, p2p->cfg->dev_addr, dst,
			    ...
			    const u8 *data, size_t len, int rx_freq)
{
	...
	p2p->cfg->sd_request(p2p->cfg->cb_ctx, freq, sa, dialog_token,
			     update_indic, pos, (size_t) (end - pos));
		     ...
		     u8 dialog_token, const struct wpabuf *resp_tlvs)
{
	...
	p2p->pending_action_state = P2P_NO_PENDING_ACTION;
	if (p2p_send_action(p2p, (unsigned int) freq, dst, p2p->cfg->dev_addr,
			    p2p->cfg->dev_addr,
			     ...
			     const u8 *data, size_t len, int rx_freq)
{
	...
	resp = p2p_build_gas_comeback_resp(dialog_token, WLAN_STATUS_SUCCESS,
					   ...
					   p2p->sd_frag_id, (u8) more,
					   (u16) wpabuf_len(p2p->sd_resp));
	if (resp == NULL)
		return;
	...
	p2p->pending_action_state = P2P_NO_PENDING_ACTION;
	if (p2p_send_action(p2p, (unsigned int) rx_freq, sa, p2p->cfg->dev_addr,
			    p2p->cfg->dev_addr,
			    wpabuf_head(resp), wpabuf_len(resp), wait_time) < 0)
		p2p_dbg(p2p, "Failed to send Action frame");
...
skip_nqp_header:
	if (wpabuf_resize(&p2p->sd_rx_resp, (size_t) (end - pos)) < 0)
		return;
	wpabuf_put_data(p2p->sd_rx_resp, pos, (size_t) (end - pos));
