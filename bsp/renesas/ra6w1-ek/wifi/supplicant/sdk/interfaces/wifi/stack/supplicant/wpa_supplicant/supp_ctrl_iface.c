...
//////MODIFY_SUPPLICANT_FOR_FREERTOS - start

#ifdef CONFIG_SCAN_RESULT_OPTIMIZE
#define	REPLY_SIZE_LONG		CLI_SCAN_RSP_BUF_SIZE
#else
#define	REPLY_SIZE_LONG		1024
#endif
#define	REPLY_SIZE_MIDDLE	1024
...
#else
extern ra6wx_cli_rx_ev_data_t   *cli_rx_ev_data;
...
extern u8 ra6wx_is_fast_reconnect;
#endif
...
#ifdef CONFIG_SAE
const int support_sae_groups[MAX_SAE_GROUPS] = { 19, 20, 21, 0, }; //{ 19, 20, 21, 25, 26, 0 };
const int support_sae_groups_size = sizeof(support_sae_groups);
#endif // CONFIG_SAE
...
//////MODIFY_SUPPLICANT_FOR_FREERTOS - end
...
static int wpas_ctrl_set_band(struct wpa_supplicant *wpa_s, char *bands)
{
	...
	wpa_s->conf->setband_mask = setband_mask;
#if CFG_PMGR
	if (RM_PMGR_W_dpm_is_enabled() == pdTRUE)
		dpm_supp_conn_info->setband_mask = (unsigned char)setband_mask;
#endif /* CFG_PMGR */
...
{	//MODIFY_SUPPLICANT_FOR_FREERTOS
	...
	if (wpa_config_set(ssid, name, value, 0) < 0) {
		ra6wx_err_prt("  [%s] Failed to set network variable '%s'\n",
				__func__, name);
...
static int set_country_for_sta(struct wpa_supplicant *wpa_s, char *cmd)
{
	...
	if (chk_channel_by_country(cmd, -1, 0, NULL, 1) < 0) {
		return -1;
    }

#if CFG_PMGR
    if ((get_sys_mode() == WIFI_DEVICE_MODE_EXT_STATION) && 
        (RM_WIFI_dpm_supp_is_connected() == pdPASS) && 
        (wpa_s->setband_mask == WPA_SETBAND_5G)) {
#else
	if ((get_sys_mode() == WIFI_DEVICE_MODE_EXT_STATION) && 
        (rm_wifi_is_wpa_state(WPA_COMPLETED, 0) == FSP_SUCCESS) && 
        (wpa_s->setband_mask == WPA_SETBAND_5G)) {
#endif /* CFG_PMGR */
          unsigned int ch_bitmap_5g = 0;
          ra6w1_regdb_get_ch_range_by_country_n_band(cmd, 5, NULL, NULL, 
                                 &ch_bitmap_5g, 0);          
          if (ch_bitmap_5g == 0) { // this country dose not support 5GHz channels
               wpa_printf(MSG_DEBUG, 
     "[%s:%d] The new country does not support 5GHz, disconnecting.. reconfigure STA \n", 
                __func__, __LINE__);
               if (wpas_request_disconnection(wpa_s)) {
                   return -1; 
               }
          }
    }

    if (wpa_s->conf->country[0] != cmd[0] || wpa_s->conf->country[1] != cmd[1]) {
	...
	if (wpa_s->current_bss &&
	    ...
	    wpa_printf(MSG_DEBUG,
    "[%s:%d] new country's range does not cover the current freq\n", __func__, __LINE__);
...
{
	struct wpa_ssid *ssid_loc = NULL;
	int freq = 0;

	ssid_loc = wpa_config_get_network(wpa_s->conf, 1);

	if (ssid_loc == NULL) {
		if (chk_channel_by_country(cmd, -1 /* just chk country code's validity */, 0, NULL, 1) == 0) {
    		ap_ctrl_iface_set_country(wpa_s, cmd);
    		os_memcpy(wpa_s->conf->country, cmd, sizeof(wpa_s->conf->country));
        }
	} else {
		struct wpa_ssid *ssid = wpa_config_get_network(wpa_s->conf, 1);
		int rec_frequency, rec_channel, freq_tmp=0, band;
		char rec_freq[5], rec_chan[4];
        memset(rec_freq, 0x0, 5);
        memset(rec_chan, 0x0, 4);

        band = (wpa_s->ap_iface->conf->channel >= 36)?1:0;
		...
		if (wpa_s->setband_mask == WPA_SETBAND_5G) {
			int res;
			int min_ch;
			if (wpa_s->ap_iface->conf->channel < 36) {
				res = ra6w1_regdb_get_ch_range_by_country_n_band(wpa_s->ap_iface->conf->country, 5, &min_ch, NULL, NULL, 0);
				if (res == pdTRUE) {
					printf("[%s] channel=%d min_ch=%d\n", __func__, wpa_s->ap_iface->conf->channel, min_ch);
					wpa_s->ap_iface->conf->channel = min_ch;
				}
			}
		}

        freq_tmp = i3ed11_ch_to_freq(wpa_s->ap_iface->conf->channel, band);
		freq = chk_channel_by_country(cmd, freq_tmp, 1, &rec_frequency, 1);
		if (freq == -1) {
			return -1;
		} else if (freq == -2) {
		    /* NOTINCLUDED_CHANNEL, select recommended freq ... */
            rec_channel = i3ed11_freq_to_ch(rec_frequency);

			ra6wx_notice_prt("Set default channel (%d)\n", rec_channel);
			...
			ap_ctrl_iface_ap(wpa_s, rec_chan, NULL, 0);
			wpa_s->ap_iface->conf->channel = rec_channel;
            if ((rec_frequency >= 2412 && rec_frequency <= 2484) &&
                (wpa_s->setband_mask & WPA_SETBAND_5G)) {
                /* In case the current SoftAP profile is 5GHz band, and the new country
                   user wants to set to does not support 5GHz band */
                wpas_ctrl_set_band(wpa_s, "2G");              
                wpa_s->ap_iface->bss[0]->iface->conf->hw_mode = HOSTAPD_MODE_IEEE80211G;
                wpa_supplicant_ctrl_iface_update_network(wpa_s, ssid, "wifi_mode", "0");
            }
            
			wpa_supplicant_ctrl_iface_update_network(wpa_s, ssid, "frequency", rec_freq);
		} else {
            if (wpa_s->conf->country[0] != cmd[0] || wpa_s->conf->country[1] != cmd[1]) {
                ap_ctrl_iface_set_country(wpa_s, cmd);
                os_memcpy(wpa_s->conf->country, cmd, sizeof(wpa_s->conf->country));
...
/* country code update is done only for AP mode 
   <-- this is now done at wpa_config_process_global()
...
static int wpa_supplicant_ctrl_iface_set_country(
					 ...
					 char *cmd)
{
	...
	*value++ = '\0';
    ...
    wpa_printf(MSG_DEBUG, "CTRL_IFACE SET '%s'='%s'\n", cmd, value);
	if (os_strcasecmp(cmd, "EAPOL::heldPeriod") == 0) {
        ...
        if (os_strstr(value, "6G")) {
            wpa_printf(MSG_DEBUG, "6GHz not supported! ");
            ret = -1;
        } else
#endif /* !__SUPPORT_SETBAND_6GHZ__ */
#if !defined (__SUPPORT_SETBAND_5GHZ__)
        if (os_strstr(value, "AUTO") || os_strstr(value, "5G")) {
            wpa_printf(MSG_DEBUG, "AUTO or 5GHz not supported! ");
            ret = -1;
        } else
    ...
    } else if (os_strcasecmp(cmd, "disassoc_imminent_rssi_threshold") == 0) {
        wpa_s->conf->disassoc_imminent_rssi_threshold = atoi(value);
        ...
        /* reply_len = */
        int result;
        result = wpa_supplicant_ctrl_iface_set_country(select_sta0(wpa_s), cmd + 8);
        if (result == 0) {
            os_strlwr(cmd);
            goto WPAS_DEF_SET_HANDLER;
        } else {
            return -1;
        }
	} else {
WPAS_DEF_SET_HANDLER:
		value[-1] = '=';
		ret = wpa_config_process_global(wpa_s->conf, cmd, -1);
		if (ret == 0) {
            /* for example, if country changes, regardless of STA or SoftAP mode,
            now country update ind is sent to MAC layer. */
			wpa_supplicant_update_config(wpa_s);
        } else if (ret == 1 && os_strncasecmp(cmd, "country", 7) == 0) {
            wpa_supplicant_update_config(wpa_s);
            ret = 0;
        } else if (ret == 1) {
			ret = 0;
					 ...
					 char *cmd, char *buf, size_t buflen)
{
	...
	if (os_strcasecmp(cmd, "max_command_len") == 0) {
...
#endif /* CONFIG_TESTING_OPTIONS */
	} else if (os_strcmp(cmd, "setband") == 0) {
        char setband_str[9] = {0,};
        int i = 0, offset = 0;
        if (wpa_s->setband_mask & WPA_SETBAND_2G) {
            offset += sprintf(setband_str+offset, "2G");
            i++;
            wpa_printf(MSG_DEBUG, "[%s:%d] 2G, i(%d), offset(%d) \n", __func__, __LINE__,
                    i, offset);
        }
        if (wpa_s->setband_mask & WPA_SETBAND_5G) {
            offset += sprintf(setband_str+offset, "%s5G", i?",":"");
            i++;
            wpa_printf(MSG_DEBUG, "[%s:%d] 5G, i(%d), offset(%d) \n", __func__, __LINE__,
                    i, offset);
        }
        if (wpa_s->setband_mask & WPA_SETBAND_6G) {
            offset += sprintf(setband_str+offset, "%s6G", i?",":"");
            i++;
            wpa_printf(MSG_DEBUG, "[%s:%d] 6G, i(%d), offset(%d) \n", __func__, __LINE__,
                    i, offset);
        }

        wpa_printf(MSG_DEBUG, "[%s:%d] i(%d), offset(%d) \n", __func__, __LINE__,
                    i, offset);
	    if (i == 0) {
            sprintf(setband_str+offset, "AUTO");
        }

		strcpy(buf, setband_str);
        res = strlen(buf);
        wpa_printf(MSG_DEBUG, "[%s:%d] i(%d), offset(%d), res(%d), buf(%s) \n", __func__, __LINE__,
                    i, offset, res, buf);
	} else {
					     ...
					     size_t buflen)
{
	...
	if (os_strcmp(cmd, "any") == 0)
        ...
        if (global_pin) {
            ...
            wpa_printf(MSG_DEBUG, "No generated PIN\n");
            return -1;
        }
...
#if 0	//MODIFY_SUPPLICANT_FOR_FREERTOS - del
	...
	*param++ = '\0';
	...
	if (get_run_mode() == WIFI_DEVICE_MODE_EXT_P2P_STATION &&
		...
		if (sta_wpa_s->wpa_state == WPA_COMPLETED) {
			ra6wx_err_prt("Using frequency is fixed. (%d MHz)\n", sta_wpa_s->assoc_freq);
			return -1;
		}
	}
#endif // CONFIG_P2P_CONCURRENT
	...
	if (os_strcmp(cmd, "oper_channel") == 0) {
		...
		ra6wx_notice_prt(">>> Own operating channel: %d\n", channel);

		if ((wpa_s->conf->p2p_oper_channel >= 0) && (wpa_s->conf->p2p_oper_channel <= 13)) {
			...
			return p2p_set_oper_channel(wpa_s->global->p2p, 115, channel, 1);
		}

oper_error:
		ra6wx_err_prt("Usage: net.cli p2p_set oper_channel "
			      "[Channel 1,6,11,36,40,44,48 or 0(auto)]\n");
		return -1;
	}
	...
	if (os_strcmp(cmd, "listen_channel") == 0) {
...
#ifdef CONFIG_P2P
 #ifdef CONFIG_P2P_CONCURRENT
		if (!wpa_s->global->p2p || get_run_mode() == WIFI_DEVICE_MODE_EXT_P2P_GO)
			return 0;
 #endif
#endif
		...
		ra6wx_notice_prt(">>> Own listen channel: %d\n", channel);

		return p2p_set_listen_channel(wpa_s->global->p2p, 81, channel, 0);
listen_error:
		ra6wx_err_prt("Usage: net.cli p2p_set listen_channel "
...
find_timeout_error:
		ra6wx_err_prt("Usage: net.cli p2p_set find_timeout "
			      "[Timeout(1~86400 sec.)]\n");
		return -1;
	}
...
go_intent_error:
		ra6wx_err_prt("Usage: net.cli p2p_set go_intent "
			      "[GO Intent(0~15)]\n");
		return -1;
	}

	if (os_strcmp(cmd, "ssid_postfix") == 0) {
		if (strlen(param) > SSID_MAX_LEN - 9) {
			ra6wx_err_prt("ssid_postfix max length: %d\n", SSID_MAX_LEN - 9);
			return -1;
		}

		if (!(wpa_s->conf->p2p_ssid_postfix))
		{
			wpa_s->conf->p2p_ssid_postfix = os_malloc((SSID_MAX_LEN-9)+1);
			memset(wpa_s->conf->p2p_ssid_postfix, 0, (SSID_MAX_LEN-9)+1);
		}
	...
	if (!wpa_s->global->p2p) {
		ra6wx_err_prt("Unsupported on non-P2P mode.\n");
		return -1;
	}
#endif	/* 1 */
	...
	if (os_strcmp(cmd, "noa") == 0) {
		...
		if (count == 0 && duration > 0)
			goto noa_error;
		ra6wx_iface_prt("[%s] P2P_SET GO NoA: count=%d duration=%d "
...
#endif
noa_error:
		ra6wx_err_prt("Usage: net.cli p2p_set noa [Count/Type(0~255)] "
...
opp_ps_error:
		ra6wx_err_prt("Usage: net.cli p2p_set opp_ps "
			      "[CTWindow(0~127)]\n");
	}
	...
	ra6wx_iface_prt("[%s] Unknown P2P_SET field value '%s'\n",
						__func__, cmd);

p2p_set_error:
	ra6wx_err_prt("Supported P2P_SET Params:\n"
		     "oper_channel\n"
		     "listen_channel\n"
		     "find_timeout\n"
		     "go_intent\n"
...
static int p2p_ctrl_get(struct wpa_supplicant *wpa_s, char *buf, size_t buflen)
{
	...
	sprintf(buf_common, "oper_channel=%d\n"
			    "listen_channel=%d\n"
			    "find_timeout=%d\n"
			    "go_intent=%d\n"
			    "ssid_postfix=%s\n",
...
static void p2p_ctrl_send_event(int cmd, char *str, int len)
{
	...
	status = xQueueSend(wifi_monitor_event_q, &wifi_monitor_msg, OS_QUEUE_NO_WAIT);
#else
	status = xQueueSend(wifi_monitor_event_q, wifi_monitor_msg, OS_QUEUE_NO_WAIT);
#endif
	if (status != pdTRUE) {
		ra6wx_err_prt("[%s] tx_queue_send error !!! (%d)\n",
			     __func__, status);
	...
	if (status != ERR_OK) {
		ra6wx_iface_prt("[%s] event set error !!! (%d)\n", __func__,
			       status);
		os_free(wifi_monitor_msg);
		return;
	}
...
static void p2p_ctrl_read(struct wpa_supplicant *wpa_s, int entry)
{
	...
	if (!wpa_s->global->p2p) {
		ra6w1_cli_prt("No P2P Interface\n");
		return;
	}
	...
	if (entry == 0) {
		...
		if (wpa_s->current_ssid && wpa_s->current_ssid->passphrase) {
			...
			ra6w1_cli_prt("No P2P GO Interface\n");
			return;
		}
		...
		if (global_pin) {
			...
			ra6w1_cli_prt("No Generated PIN\n");
		}
			    ...
			    char *buf, size_t buflen)
{
	...
	if (dev == NULL || (dev->flags & P2P_DEV_PROBE_REQ_ONLY)) {
		ra6wx_p2p_prt("[%s] Cannot connect to unknown P2P Device "
			MACSTR "\n",
...
/* caller should free freqs[] once done in using it. */
int * freq_range_to_channel_list_by_mode(struct wpa_supplicant *wpa_s, 
                                                     char *val, enum hostapd_hw_mode mode)
{
	...
	for (i = 0; i < wpa_s->hw.num_modes; i++) {
        ...
        // mode (11g/11a/... is matched)
        
		for (j = 0; j < hw_mode->num_channels; j++) {
			unsigned int freq;

			if (hw_mode->channels[j].flag & HOSTAPD_CHAN_DISABLED)
				continue;

			freq = hw_mode->channels[j].freq;
			if (!freq_range_list_includes(&ranges, freq))
				continue;
...
write_status:
...
#ifdef CONFIG_P2P
 #ifdef CONFIG_P2P_CONCURRENT
        if (get_run_mode() == WIFI_DEVICE_MODE_EXT_P2P || get_run_mode() == WIFI_DEVICE_MODE_EXT_P2P_GO) {
            int p2p_freq = 0;

            if (wpa_s->global->p2p->cfg->op_channel >= 36) {
                p2p_freq = i3ed11_ch_to_freq(wpa_s->global->p2p->cfg->op_channel, 1 /* IEEE80211_BAND_5GHZ */);
            } else {
                p2p_freq = i3ed11_ch_to_freq(wpa_s->global->p2p->cfg->op_channel, 0 /* IEEE80211_BAND_2GHZ */);
            }
 ...
 #ifdef CONFIG_P2P_CONCURRENT
            if (get_run_mode() == WIFI_DEVICE_MODE_EXT_P2P || get_run_mode() == WIFI_DEVICE_MODE_EXT_P2P_GO) {
                ret = os_snprintf(pos, end - pos, "p2p_oper_channel=%u\n", wpa_s->global->p2p->cfg->op_channel);
            }
			else 
 #endif
#endif
...
#if 0 // Debug
		printf("[%s] wpa_s->connection_set=%d ht=%d vht=%d he=%d\n", __func__,
...
#define SAE_GROUPS(x) (!wpa_s->conf->sae_groups[0] ? support_sae_groups[x] : wpa_s->conf->sae_groups[x])
			   ...
			   char *reply, int reply_size, int *reply_len)
{
	...
	if (params == NULL)
	{
		...
		*reply_len = (int) strlen(tmp_sae_groups);
	...
	else
	{
		...
		for (int idx = 0; idx < (int) strlen(params); idx++) {
			if (!(isdigit((int)params[idx]) != 0 || isblank(params[idx]) != 0)) { /* check : num, space or tab */
				goto Fail;
			}
		}
		...
		for (int i = 0; t_sae_groups[i]; i++)
		{
			wpa_s->conf->sae_groups[i] = (int)t_sae_groups[i];
		}		

		strcpy(reply, "OK");		
		*reply_len = (int) strlen(reply);
	}
...
#if 1 // Profile 1 (Softap, p2p, mesh)
        ra6wx_err_prt("[%s] Unknown ID %d [ STA : 0 / AP,P2P"
#ifdef CONFIG_MESH
                    ",MESH"
#endif /* CONFIG_MESH */
                    ":1]\n", __func__, index);
#else
		ra6w1_cli_prt("[%s] Unknown ID %d [ STA : 0 / AP : 1 / P2P : 2"
 #ifdef CONFIG_MESH
                    " / MESH : 3"
 #endif /* CONFIG_MESH */
                    "]\n", __func__, index);
#endif /* Profile 1 (Softap, p2p, mesh) */
		return -1;
	}
...
{ //MODIFY_SUPPLICANT_FOR_FREERTOS
	...
	if (os_strcmp(cmd, "all") == 0) {
		...
		if (wpa_s->current_ssid) {
			...
			if (wpa_s->current_ssid->mode == WPAS_MODE_AP) {
				...
				vTaskDelay(portCONVERT_MS_2_TICKS(200)); /* Waiting time for packet transmission completion(200 ms) */
#endif /* 0 */
#ifdef CONFIG_AP
				ap_ctrl_iface_ap(wpa_s, "stop", NULL, 0);
#endif /* CONFIG_AP */
	...
	if (ssid == NULL) {
		ra6wx_iface_prt("[%s] Could not find network id=%d\n",
					__func__, id);
		return -1;
	}
	...
	if (wpa_config_remove_network(wpa_s->conf, id) < 0) {
		ra6wx_iface_prt("[%s] Not able to remove the network id=%d\n",
					__func__, id);
		return -1;
	}
...
{	//MODIFY_SUPPLICANT_FOR_FREERTOS
	...
	*value++ = '\0';
	...
	ra6wx_ascii_dump("Value", (u8 *) value, os_strlen(value));

	ssid = wpa_config_get_network(wpa_s->conf, id);
	if (ssid == NULL) {
		ra6wx_iface_prt("[%s] Could not find network id=%d", __func__, id);
		return -1;
	}

    if (id == 1 && (ssid->mode == WPAS_MODE_P2P_GO || ssid->mode == WPAS_MODE_P2P_GROUP_FORMATION)) {
	...
	if (os_strcasecmp(name, "channel") == 0) {
		...
		if (set_freq != 0) {
            if (set_freq >= 36) {
   			    set_freq = i3ed11_ch_to_freq(set_freq, 1 /* IEEE80211_BAND_5GHZ */);
            } else {
			    set_freq = i3ed11_ch_to_freq(set_freq, 0 /* IEEE80211_BAND_2GHZ */);
            }
		}
		...
		if (get_sys_mode() >= WIFI_DEVICE_MODE_EXT_AP_STATION) {
			ra6wx_notice_prt("\nwifi_mode cannot be set in sys_mode(%d).\n", get_run_mode());
			return -1;
		} else if (ssid->mode == WPAS_MODE_AP) {
			if (wpa_s->setband_mask == WPA_SETBAND_5G) {
				if((WIFI_MODE_AN > atoi(value)) || (WIFI_MODE_N_ONLY_5G < atoi(value))) {
					return -1;
				}
			} else if (wpa_s->setband_mask == WPA_SETBAND_2G) {
				if((WIFI_MODE_BGN > atoi(value)) || (WIFI_MODE_B_ONLY < atoi(value))) {
					return -1;
				}
			}
		}
		...
		if ((ssid->mode == WPAS_MODE_AP)
			&& (   atoi(value) == WIFI_MODE_BGN
				|| atoi(value) == WIFI_MODE_GN
				|| atoi(value) == WIFI_MODE_N_ONLY
                || atoi(value) == WIFI_MODE_AN
                || atoi(value) == WIFI_MODE_N_ONLY_5G)
			&& ssid->pairwise_cipher == WPA_CIPHER_TKIP) {
				ra6w1_cli_prt("[%s] Not allowed to set to a mode containing N "
			     		"mode in WEP or TKIP mode\n",
			     		__func__);
				return -1;
		}
#if 1   /* by Shingu 20170104 (JP Channel) */
		if (atoi(value) != WIFI_MODE_B_ONLY) {
			char *freq_value;
			...
			if (atoi(freq_value) == 2484) {
				ra6w1_cli_prt("[%s] Not allowed to set B only "
			     		"mode on Channel 14\n", __func__);
				return -1;
			}
		}
		...
		if (ssid->mode == WPAS_MODE_AP
		&& os_strcasecmp(value, "TKIP") == 0) {
		    if (ssid->wifi_mode <= WIFI_MODE_B_ONLY) {
		        ssid->wifi_mode = WIFI_MODE_BG;
            } else {
                ssid->wifi_mode = WIFI_MODE_A_ONLY;
            }
		}
	...
	if (os_strncasecmp(name, "eap", 3) == 0 &&
		...
		if (atoi(value + 4) == 0 || atoi(value + 4) == 1)
			ra6w1_peap_version = atoi(value + 4);
		else
			return -1;
		strcpy(value, "PEAP");
	}
	...
	struct wpa_supplicant *wpa_s, char *cmd, char *buf, size_t buflen)
{
	...
	*name++ = '\0';
	...
	if (id == 0 && (os_strcasecmp(name, "channel") == 0 || 
                    os_strcasecmp(name, "frequency") == 0)) {
		return -1;
	}
	...
	if (os_strcasecmp(name, "channel") == 0) {
		...
		if (value == NULL) {
			ra6wx_iface_prt("[%s] Failed to get network variable "
				       "'%s'\n", __func__, name);
			return -1;
		}
...
static int wpa_supplicant_ctrl_iface_set_log(struct wpa_supplicant *wpa_s, char *cmd)
{
	unsigned long value;
	char *module = NULL;
...
static int wpa_supplicant_ctrl_iface_get_log(
	struct wpa_supplicant *wpa_s, char *buf, size_t buflen)
{
	size_t res = 0;
	char *buf_t = buf;
...
{ 	//MODIFY_SUPPLICANT_FOR_FREERTOS
	...
	ra6w1_nvram_prt("[%s] SAVE_CONFIG - %spdate configuration%s\n", __func__, ret < 0 ? "Failed to u":"u",  ret < 0 ? "":" updated");

	//ra6w1_nvram_refresh(); /* Save env */

#if CFG_PMGR
	//RM_PMGR_W_rtm_static_set(RTM_STATIC_KEY_DPM_WAKEUP_FLAG, 0, 0); // for wifi recofnig
#endif /* CFG_PMGR */
					  ...
					  char *addr)
{
	...
	struct wpa_bss *bss;
	struct wpa_ssid *ssid = wpa_s->current_ssid;
#if 0   /* by Bright : Merge 2.7 */	//MODIFY_SUPPLICANT_FOR_FREERTOS - del
					  ...
					  char *cmd, char *buf, size_t buflen)
{
	...
	if (RM_PMGR_W_dpm_is_enabled() == pdTRUE) {
		ra6wx_notice_prt("Not supported ROAM - DPM enabled... \n");
	...
	ra6wx_scan_prt("CTRL_IFACE ROAM " MACSTR "\n", MAC2STR(bssid));

	if (!ssid) {
		ra6wx_err_prt("[%s] ROAM: No network configuration known "
			     "for the target AP \n", __func__);
		return -1;
	}
	bss = wpa_bss_get(wpa_s, bssid, ssid->ssid, ssid->ssid_len);
	if (!bss) {
		ra6wx_err_prt("[%s] ROAM: Target AP not found "
			     "from BSS table \n", __func__);
		return -1;
	}
	wpa_s->conf->roam_state = ROAM_ENABLE;
...
static int wpa_supplicant_ctrl_iface_roam_threshold(struct wpa_supplicant *wpa_s,
					      char *cmd)
{
...
int p2p_ctrl_find(struct wpa_supplicant *wpa_s, char *cmd)
{
    ...
    if (!timeout) {                                    // if timeout is empty(null),
        if (wpa_s->conf->p2p_find_timeout) {           // and if p2p_find_timeout exists, 
            timeout = wpa_s->conf->p2p_find_timeout;   // set timeout value to p2p_find_timeout
        }
    }

	return _wpas_p2p_find(wpa_s, timeout, type, _dev_type != NULL, _dev_type,
			    ...
			    char *buf, size_t buflen)
{
	...
	new_pin = _wpas_p2p_connect(wpa_s, addr, pin, wps_method,
...
#if 0
static int _p2p_ctrl_invite_persistent(struct wpa_supplicant *wpa_s, char *cmd)
{
	char *pos;
	int id;
	struct wpa_ssid *ssid;
...
#if 1
	return _wpas_p2p_group_add(wpa_s, persistent, freq, freq2, ht40, vht,
...
static int wpa_supp_ctrl_iface_set_peap_ver(char *pos)
{
	...
	if (val == 0 || val == 1)
		ra6w1_peap_version = val;
	else {
		ra6wx_err_prt("Invalid Command! (0 or 1)\n");
		return -1;
	}
...
//	wps_registrar_flush(wpa_s->wps->registrar);
...
#else
	ra6wx_iface_prt("[%s] Have to cancel timer function ...\n", __func__);
#endif	/* 0 */
	...
	wpa_s->reassoc_try = 0;
	wpa_s->conf->scan_cur_freq = 0;
#endif /* CONFIG_RECONNECT_OPTIMIZE */

    wpa_s->setband_mask = WPA_SETBAND_DEF;
	...
	if (!dl_list_empty(&wpa_s->bss)) {
		ra6wx_notice_prt("BSS table not empty after flush: %u entries, current_bss=%p bssid="
...
#endif  /* 0 */	//MODIFY_SUPPLICANT_FOR_FREERTOS
			   ...
			   char *reply, int reply_size, int *reply_len)
{
	...
	if (params) {
		...
		pos = os_strstr(params, "band=");
		if (pos) {
            int band_val = atoi((char*)(pos + 5));
            if (band_val == 2 && (wpa_s->setband_mask & WPA_SETBAND_2G || wpa_s->setband_mask == WPA_SETBAND_AUTO)) {
                wpa_s->scan_band = 2;
            } else if (band_val == 5 && (wpa_s->setband_mask & WPA_SETBAND_5G || wpa_s->setband_mask == WPA_SETBAND_AUTO)) {
#if defined (CONFIG_5G_SUPPORT)
                wpa_s->scan_band = 5;
#else
                *reply_len = -1;
                wpa_printf(MSG_ERROR, "[%s] band not supported %s", __func__, pos+5);
                goto done;                
#endif /* CONFIG_5G_SUPPORT */
            } else {
                *reply_len = -1;
                wpa_printf(MSG_ERROR, "[%s] invalid band entered %s, setband_mask=%d", __func__, pos+5, wpa_s->setband_mask);
                goto done;
            }
            wpa_printf(MSG_DEBUG, "[%s] band entered %s", __func__, pos+5);
		}
...
#if 0 //Add support for scan in case of connected is not complete 
					      ...
					      char *buf, size_t buflen)
{
	...
	wpa_s->conf->model_name		= alloc_strdup(CHIPSET_NAME);

	if (wpa_s->conf->model_number)
	{
		os_free(wpa_s->conf->model_number);
	}
	wpa_s->conf->model_number	= alloc_strdup(CHIPSET_NAME);

	if (wpa_s->conf->serial_number)
	{
		os_free(wpa_s->conf->serial_number);
	}
	wpa_s->conf->serial_number	= alloc_strdup("12345678");

	if (wpa_s->conf->device_name)
	{
		os_free(wpa_s->conf->device_name);
	}
	wpa_s->conf->device_name	= alloc_strdup("RA6Wx Series");
	...
	/*
		...
		device_type=1-0060F204-1
	*/

	wpa_s->conf->device_type[0] = 0x00;
	/* If the value is not 0x06, the Windows OS will require a WPS PIN when attempting to connect. */
	wpa_s->conf->device_type[1] = 0x06;
...
static int wpa_supplicant_ctrl_iface_get_country(
	struct wpa_supplicant *wpa_s, char *buf, size_t buflen)
{
	char *pos, *end;
	int ret;
					...
					char *addr)
{
	...
	if ((wpa_s->conf->acl_num >= NUM_MAX_ACL) ||
		...
		ra6wx_err_prt("ACL list is fulled. (max : %d) \n", NUM_MAX_ACL);
		return -1;
	}
	...
	if (exist) {
		ra6wx_err_prt("There already is...\n");
		return -1;
	}
...
/*	ACCEPT_UNLESS_DENIED = 0,
	...
	if (os_strcasecmp(value, "deny") == 0) {
		...
		if (i >= NUM_MAX_ACL) {
			ra6wx_err_prt("[%s] Invalid command!! (or Not found "
				     "'%s') \n", __func__, value + 7);
			return -1;
		}
	} else {
		ra6wx_notice_prt("acl <allow|deny|none|clear|add [mac_addr]"
	...
	struct wpa_supplicant *wpa_s, char *pos)
{
	...
	if (pos[0] < '0' || pos[0] > '9') {
		ra6w1_cli_prt("Wrong input argument value\n");
		return -1;
	}
	...
	struct wpa_supplicant *wpa_s, char *pos)
{
	...
	//ra6wx_info_prt("greenfield=%d int_val=%d pos=%s\n",
					 ...
					 char *buf, size_t *resp_len)
{
    ...
    if (is_supplicant_done() == 0) {
        ...
        reply_len = 27;
        os_memcpy(reply, "supplicant not initialized.", reply_len);
        return reply;
    } else {
    	if ((os_strcmp(buf, "STATUS") == 0) || (os_strcmp(buf, "AP-STATUS") == 0)) {
	...
	if (os_strcmp(buf, "PING") == 0) {
		...
		if (wpa_supplicant_ctrl_iface_set_log(wpa_s, buf + 7)) {
			reply_len = -1;
		}
	} else if (os_strncmp(buf, "GET_LOG", 7) == 0) {
		reply_len = wpa_supplicant_ctrl_iface_get_log(wpa_s, reply, reply_size);
#endif /* CONFIG_LOG_MASK */
	} else if (os_strncmp(buf, "COUNTRY ", 8) == 0) {
		extern int ra6w1_regdb_get_cty_idx_from_cc(char* cc);
        extern void ra6w1_regdb_print_country(void);
        
        if (strcmp((buf + 8), "?") == 0 || strcmp((buf + 8), "help") == 0) {     
            ra6w1_regdb_print_country();
            reply_len = 0;
            goto CTRL_IFACE_PROCESS_END;

        } else if (ra6w1_regdb_get_cty_idx_from_cc(buf + 8) == -1) {
            os_memcpy(reply, "Incorrect param!!\n", 17);
            reply_len = 17;
            goto CTRL_IFACE_PROCESS_END;
            
        } else {
            goto CTRL_IFACE_PROCESS_SET;
        }
	} else if (os_strcmp(buf, "COUNTRY") == 0) {
		reply_len = wpa_supplicant_ctrl_iface_get_country(
			select_sta0(wpa_s), reply, reply_size);

	} else if (os_strcmp(buf, "IFNAME") == 0) {
		reply_len = os_strlen(wpa_s->ifname);
		os_memcpy(reply, wpa_s->ifname, reply_len);
#ifdef UNUSED_CODE_RELOG
	} else if (os_strncmp(buf, "RELOG", 5) == 0) {
		if (wpa_debug_reopen_file() < 0)
			reply_len = -1;
#endif	// UNUSED_CODE_RELOG
	    ...
	    char * cmd_buf = NULL;
CTRL_IFACE_PROCESS_SET:
        if (os_strncasecmp(buf, "COUNTRY ", 8) == 0) {
            cmd_buf = buf;
        } else  {
            cmd_buf = buf + 4;
        }

        if (wpa_supplicant_ctrl_iface_set(select_sta0(wpa_s), cmd_buf))
            reply_len = -1;
#ifdef UNUSED_CODE_DELETE
        ...
        reply_len = wpa_supplicant_ctrl_iface_get(select_sta0(wpa_s), buf + 4,
                        reply, reply_size);
        ...
        if (select_sta0(wpa_s)->wpa_state == WPA_INTERFACE_DISABLED) {
            reply_len = -1;
        }
        else {
            wpas_request_connection(select_sta0(wpa_s));
        }
    } else if (os_strcmp(buf, "REATTACH") == 0) {
        if (select_sta0(wpa_s)->wpa_state == WPA_INTERFACE_DISABLED ||
            !select_sta0(wpa_s)->current_ssid) {
            reply_len = -1;
        }
        else {
            wpa_s->reattach = 1;
        ...
        else if (select_sta0(wpa_s)->disconnected) {
      ...
      /* Soft-AP */
...
#ifdef CONFIG_CONCURRENT
		if (get_run_mode() == WIFI_DEVICE_MODE_EXT_AP_STATION) {
			ra6wx_err_prt("Not supported in concurrent mode.\n");
			reply_len = -1;
		} else 
#endif // CONFIG_CONCURRENT
		{
		...
		if (wpas_p2p_group_remove(wpa_s, buf + 17))
			reply_len = -1;
	} else if (os_strcmp(buf, "P2P_GROUP_REMOVE") == 0) {
		...
		reply_len = p2p_ctrl_peer(wpa_s, buf + 9, reply,
					      reply_size);
	} else if (os_strncmp(buf, "P2P_ACCEPT", 11) == 0) {
	...
	} else if (os_strncmp(buf, WPA_CTRL_RSP, os_strlen(WPA_CTRL_RSP)) == 0)
	{
        ...
        if (wpa_supplicant_reload_configuration(select_sta0(wpa_s))) {
            reply_len = -1;
        }
        ...
        if (wpas_request_disconnection(select_sta0(wpa_s))) {
            reply_len = -1;
        }
	} else if (os_strcmp(buf, "SCAN") == 0) {
		wpas_ctrl_scan(select_sta0(wpa_s), NULL, reply, reply_size, &reply_len);
	} else if (os_strncmp(buf, "SCAN ", 5) == 0) {
		wpas_ctrl_scan(select_sta0(wpa_s), buf + 5, reply, reply_size, &reply_len);
		...
		reply_len = wpa_supplicant_ctrl_iface_add_network(wpa_s, buf + 12, reply, reply_size);
        ...
        if (wpa_supplicant_ctrl_iface_roam(select_sta0(wpa_s), buf + 5)) {
            reply_len = -1;
        }
...
static int wpa_supplicant_global_iface_add(struct wpa_global *global, char *cmd)
{
	...
	if (create_iface) {
		...
		wpa_printf(MSG_DEBUG, "CTRL_IFACE interface '%s' created with MAC addr: "
...
#else
	struct wpa_driver_ops *drv = wpa_drivers;
	iface = drv->get_interfaces(global->drv_priv[i]);
#endif
...
void wpa_supplicant_global_ctrl_iface_recv(struct wpa_global *global, char *buf)
{
	char	*reply = NULL, *reply_buf = NULL;
	size_t	reply_len = 0;
	UINT	status;
#ifndef CONFIG_SCAN_REPLY_OPTIMIZE
	ra6wx_cli_rx_ev_data_t *cli_rx_ev_data_p;
	ra6w1_cli_rsp_buf_t *cli_rsp_buf_p = (ra6w1_cli_rsp_buf_t *)&cli_rsp_buf;
#endif /* CONFIG_SCAN_REPLY_OPTIMIZE */

    TX_FUNC_START("");
	/* CLI command Handler */
	reply_buf = wpa_supplicant_ctrl_iface_process(global->ifaces, buf, &reply_len);
	reply = reply_buf;
	...
	if (reply) {
...
#else
		memset((VOID *)cli_rx_ev_data, 0, sizeof(ra6wx_cli_rx_ev_data_t));
		memset(cli_rsp_buf_p, 0, sizeof(ra6w1_cli_rsp_buf_t));

		cli_rx_ev_data_p = (ra6wx_cli_rx_ev_data_t *)cli_rx_ev_data;
		...
		temp_pack[0] = RA6WX_SP_CLI_RX_FLAG;
#ifndef CONFIG_SCAN_REPLY_OPTIMIZE
		...
		if (status != pdTRUE) {
			ra6wx_iface_prt("[%s] msg send error !!! (%d)\n",
						__func__, status);
#ifdef CONFIG_SCAN_REPLY_OPTIMIZE
		...
		// printf(YELLOW_COLOR " [%s] Send Queue(TO_CLI_QUEUE) RA6WX_SP_CLI_RX_FLAG \n" CLEAR_COLOR, __func__);
	}
    TX_FUNC_END(" ");
}
