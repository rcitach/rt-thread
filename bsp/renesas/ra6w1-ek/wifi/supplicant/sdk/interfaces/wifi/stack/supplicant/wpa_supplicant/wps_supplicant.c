...
static u8 is_wps_in_use = 0;	//MODIFY_SUPPLICANT_FOR_FREERTOS
...
#endif	// CONFIG_SCAN_RESULT_OPTIMIZE
	{
#ifdef PROBE_REQ_WITH_SSID_FOR_ASSOC	//MODIFY_SUPPLICANT_FOR_FREERTOS - add
					  ...
					  struct wps_event_fail *fail)
{
	if (fail->error_indication > 0 &&
		...
		wpa_msg(wpa_s, MSG_INFO,
			WPS_EVENT_FAIL "msg=%d config_error=%d reason=%d",
...
static void wpa_supplicant_wps_event_success(struct wpa_supplicant *wpa_s)
{
	...
	wifi_conn_fail_reason_code = WLAN_STATUS_SUCCESS;
#ifdef CONFIG_NOTIFY
...
static struct wpa_ssid * wpas_wps_add_network(struct wpa_supplicant *wpa_s,
					      ...
					      const u8 *bssid, int network_id)
{
	struct wpa_ssid *ssid;

	//MODIFY_SUPPLICANT_FOR_FREERTOS
	ssid = wpa_config_add_network(wpa_s->conf, wpa_s->ifname, network_id);
	if (ssid == NULL)
		return NULL;
#ifdef CONFIG_NOTIFY
	wpas_notify_network_added(wpa_s, ssid);
		       ...
		       int p2p_group, int multi_ap_backhaul_sta)
{
	...
	if(p2p_group == 1)
		network_id = FIXED_NETWORK_ID_P2P;

	ssid = wpas_wps_add_network(wpa_s, 0, NULL, bssid, network_id);
	if (ssid == NULL)
		return -1;
	ssid->temporary = 1;
#ifdef CONFIG_P2P
				 ...
				 const u8 *ssid_val, size_t ssid_len, int freq)
{
	...
	if(p2p_group == 1)
		network_id = FIXED_NETWORK_ID_P2P;
	ssid = wpas_wps_add_network(wpa_s, 0, dev_addr, bssid, network_id);
	if (ssid == NULL) {
	...
	if (p2p_group && wpa_s->go_params && wpa_s->go_params->ssid_len) {
		...
		if (ssid->ssid) {
			...
			os_memcpy(ssid->ssid, wpa_s->go_params->ssid,
				  ssid->ssid_len);
			os_memcpy(&ssid->ssid[P2P_WILDCARD_SSID_LEN + 2],
	                  wpa_s->conf->p2p_ssid_postfix,
	                  strlen(wpa_s->conf->p2p_ssid_postfix));
	        ssid->ssid_len = P2P_WILDCARD_SSID_LEN + 2 + strlen(wpa_s->conf->p2p_ssid_postfix);
			if (wpa_s->go_params->freq > 56160) {
				/* P2P in 60 GHz uses PBSS */
				ssid->pbss = 1;
			}
		       ...
		       const char *pin, struct wps_new_ap_settings *settings)
{
	...
	if (!pin)
		return -1;
	wpas_clear_wps(wpa_s);
	ssid = wpas_wps_add_network(wpa_s, 1, NULL, bssid, network_id);
	if (ssid == NULL)
		return -1;
	ssid->temporary = 1;
	pos = val;
					...
					struct wpa_scan_res *res)
{
	...
	if (uuid)
		os_memcpy(ap->uuid, uuid, WPS_UUID_LEN);
	//MODIFY_SUPPLICANT_FOR_FREERTOS - chg
	ra6wx_wps_prt("     [%s] WPS: AP " MACSTR " type %d added\n\n", __func__,
