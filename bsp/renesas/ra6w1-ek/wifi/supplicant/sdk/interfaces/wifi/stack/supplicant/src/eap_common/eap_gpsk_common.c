				       ...
				       u8 *pk, size_t pk_len)
{
	...
	if (data == NULL)
		return -1;
	pos = data;
	WPA_PUT_BE16(pos, (u16) psk_len);
	pos += 2;
	os_memcpy(pos, psk, psk_len);
	pos += psk_len;
	WPA_PUT_BE32(pos, EAP_GPSK_VENDOR_IETF); /* CSuite/Vendor = IETF */
	pos += 4;
	WPA_PUT_BE16(pos, (u16) csuite_specifier); /* CSuite/Specifier */
	pos += 2;
			 ...
			 u8 *pk, size_t *pk_len)
{
	...
	pos += id_server_len;
	wpa_hexdump(MSG_DEBUG, "EAP-GPSK: Seed", seed, (size_t) (pos - seed));

	switch (specifier) {
	case EAP_GPSK_CIPHER_AES:
		ret = eap_gpsk_derive_keys_aes(psk, psk_len, seed, (size_t) (pos - seed),
					       msk, emsk, sk, sk_len,
					       pk, pk_len);
		break;
#ifdef EAP_GPSK_SHA256
			       ...
			       u8 method_type, u8 *sid, size_t *sid_len)
{
	...
	ret = eap_gpsk_derive_mid_helper((u32) specifier,
					 kdf_out, sizeof(kdf_out),
					 psk, seed, (size_t) (pos - seed),
					 method_type);
