		       ...
		       int num_chal, const u8 *kc, u8 *mk)
{
	...
	len[0] = identity_len;
	addr[1] = kc;
	len[1] = (size_t) (num_chal * EAP_SIM_KC_LEN);
	addr[2] = nonce_mt;
		       ...
		       struct eap_sim_attrs *attr, int aka, int encr)
{
	...
	while (pos < end) {
		...
		case EAP_SIM_AT_VERSION_LIST:
			...
			list_len = (size_t) (apos[0] * 256 + apos[1]);
				   ...
				   const u8 *extra, size_t extra_len)
{
	...
	eap->length = host_to_be16((unsigned short) wpabuf_len(msg->buf));
			  ...
			  const u8 *data, size_t len)
{
	...
	attr_len += pad_len;
	if (wpabuf_resize(&msg->buf, (size_t) attr_len))
		return NULL;
	start = wpabuf_put(msg->buf, 0);
	wpabuf_put_u8(msg->buf, attr);
	wpabuf_put_u8(msg->buf, (u8) (attr_len / 4));
	wpabuf_put_data(msg->buf, data, len);
	if (pad_len)
		os_memset(wpabuf_put(msg->buf, (size_t) pad_len), 0, (size_t) pad_len);
	return start;
}
...
u8 * eap_sim_msg_add_mac(struct eap_sim_msg *msg, u8 attr)
{
	...
	if (pos)
		msg->mac = (size_t) ((pos - wpabuf_head_u8(msg->buf)) + 4);
	return pos;
}
			       ...
			       u8 attr_encr)
{
	...
	if (pos == NULL)
		return -1;
	msg->iv = (size_t) ((pos - wpabuf_head_u8(msg->buf)) + 4);
	...
	msg->encr = (size_t) (pos - wpabuf_head_u8(msg->buf));
...
int eap_sim_msg_add_encr_end(struct eap_sim_msg *msg, u8 *k_encr, int attr_pad)
{
	...
	if (encr_len % 16) {
		...
		pos = eap_sim_msg_add(msg, (u8) attr_pad, 0, NULL, (size_t) (pad_len - 4));
		if (pos == NULL)
			return -1;
		os_memset(pos + 4, 0, (size_t) (pad_len - 4));
		encr_len += (size_t) pad_len;
	}
	wpa_printf(MSG_DEBUG, "   (AT_ENCR_DATA data len %lu)",
		   (unsigned long) encr_len);
	wpabuf_mhead_u8(msg->buf)[msg->encr + 1] = (u8) (encr_len / 4 + 1);
