...
static void pmksa_cache_set_expiration(struct rsn_pmksa_cache *pmksa)
{
	...
	eloop_register_timeout((unsigned int) (sec + 1), 0, pmksa_cache_expire, pmksa, NULL);
	...
	if (entry && !wpa_key_mgmt_sae(entry->akmp)) {
		...
		if (sec < 0)
			sec = 0;
		eloop_register_timeout((unsigned int) sec, 0, pmksa_cache_reauth, pmksa,
				       NULL);
	}
}
		...
		const u8 *cache_id)
{
	...
	os_get_reltime(&now);
	entry->expiration = now.sec + (os_time_t) pmksa->sm->dot11RSNAConfigPMKLifetime;
	entry->reauth_time = now.sec + (os_time_t) (pmksa->sm->dot11RSNAConfigPMKLifetime *
		      ...
		      struct rsn_pmksa_cache_entry *entry)
{
	...
	wpa_sm_add_pmkid(pmksa->sm, entry->network_ctx, entry->aa, entry->pmkid,
			 ...
			 (u8) pmksa->sm->dot11RSNAConfigPMKReauthThreshold,
			 entry->akmp);
...
int pmksa_cache_list(struct rsn_pmksa_cache *pmksa, char *buf, size_t len)
{
	...
	os_get_reltime(&now);
	ret = os_snprintf(pos, (size_t) (buf + len - pos),
			  "Index / AA / PMKID / expiration (in seconds) / "
			  "opportunistic%s\n",
			  cache_id_used ? " / FILS Cache Identifier" : "");
	if (os_snprintf_error((size_t) (buf + len - pos), ret))
		return pos - buf;
	pos += ret;
	i = 0;
	entry = pmksa->pmksa;
	while (entry) {
		i++;
		ret = os_snprintf(pos, (size_t) (buf + len - pos), "%d " MACSTR " ",
				  i, MAC2STR(entry->aa));
		if (os_snprintf_error((size_t) (buf + len - pos), ret))
			return pos - buf;
		pos += ret;
		pos += wpa_snprintf_hex(pos, (size_t) (buf + len - pos), entry->pmkid,
					PMKID_LEN);
		ret = os_snprintf(pos, (size_t) (buf + len - pos), " %d %d",
		...
		pos += ret;
		if (entry->fils_cache_id_set) {
			ret = os_snprintf(pos, (size_t) (buf + len - pos), " %02x%02x",
...
void pmksa_cache_reconfig(struct rsn_pmksa_cache *pmksa)
{
	...
	for (entry = pmksa->pmksa; entry; entry = entry->next) {
		...
		life_time = (u32) (entry->expiration - now.sec);
		reauth_threshold = (u8) ((long unsigned int) (entry->reauth_time - now.sec) * 100U /
			life_time);
		if (!reauth_threshold)
			continue;
