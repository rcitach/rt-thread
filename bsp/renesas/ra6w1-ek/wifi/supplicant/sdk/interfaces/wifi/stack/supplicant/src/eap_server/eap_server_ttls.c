						...
						u32 avp_code, int mandatory)
{
	...
	wpabuf_free(resp);
	wpabuf_put(avp, (size_t) (pos - (u8 *) wpabuf_head(avp)));
	return avp;
}
					    ...
					    struct eap_ttls_data *data, u8 id)
{
	...
	wpabuf_put_u8(req, (u8) (EAP_TLS_FLAGS_START | data->ttls_version));
	...
	struct eap_sm *sm, struct eap_ttls_data *data)
{
	...
	if (data->mschapv2_resp_ok) {
		...
		*pos++ = data->mschapv2_ident;
		ret = os_snprintf((char *) pos, (size_t) (end - pos), "S=");
		if (!os_snprintf_error((size_t) (end - pos), ret))
			pos += ret;
		pos += wpa_snprintf_hex_uppercase(
			(char *) pos, (size_t) (end - pos), data->mschapv2_auth_response,
	...
	wpabuf_set(&msgbuf, req, (size_t) (pos - req));
