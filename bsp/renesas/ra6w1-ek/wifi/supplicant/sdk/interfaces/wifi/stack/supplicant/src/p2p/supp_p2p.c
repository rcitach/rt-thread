...
void p2p_set_state(struct p2p_data *p2p, int new_state)
{
	...
	p2p->state = (enum p2p_state)new_state;
}
...
static void p2p_listen_in_find(struct p2p_data *p2p, int dev_disc)
{
...
#else
	tu = ((r % 3) + 1) * 100; // 100, 200, or 300 ms
	...
	/* If static period (DEFAULT_P2P_LISTEN_TIME) is necessary, open here. */
	if (p2p->cfg->start_listen(p2p->cfg->cb_ctx, freq,
		DEFAULT_P2P_LISTEN_TIME, ies) < 0) {
		ra6wx_p2p_prt("[%s] Failed to start listen mode\n", __func__);
	} else {
		p2p->in_listen = 1;
		if (DEFAULT_P2P_LISTEN_TIME >= 1000)
			p2p_set_timeout(p2p, DEFAULT_P2P_LISTEN_TIME / 1000, 0);
		else
			p2p_set_timeout(p2p, 0, DEFAULT_P2P_LISTEN_TIME * 1000);
	}
...
#if 1   /* by Shingu 20160630 (P2P Optimize) */
/**
 * p2p_remove_device - Fetch a peer entry
 * @p2p: P2P module context from p2p_init()
 * @addr: P2P Device Address of the peer
 * @all: All P2P Device is removed if all=1
 */
void p2p_remove_device(struct p2p_data *p2p, const u8 *addr, int all)
{
	struct p2p_device *dev, *prev;
	dl_list_for_each_safe(dev, prev, &p2p->devices, struct p2p_device, list) {
		if (all || os_memcmp(dev->info.p2p_device_addr, addr, ETH_ALEN) == 0) {
			dl_list_del(&dev->list);
			p2p_device_free(p2p, dev);
		}
	}
		   ...
		   size_t ies_len, int scan_res)
{
	...
	int wfd_changed;
#if 0   /* by Shingu : duplicated RA6WX P2P_peer_result */	//MODIFY_SUPPLICANT_FOR_FREERTOS
...
static void p2p_search(struct p2p_data *p2p)
{
	...
	res = p2p->cfg->p2p_scan(p2p->cfg->cb_ctx, P2P_SCAN_SOCIAL, freq,
#ifdef  CONFIG_P2P_UNUSED_CMD
	...
	if (res < 0) {
		ra6wx_p2p_prt("[%s] Scan request failed\n", __func__);
		p2p_continue_find(p2p);
	} else {
		ra6wx_p2p_prt("[%s] Running p2p_scan\n", __func__);
	     ...
	     enum p2p_discovery_type type)
{
	...
	ra6wx_p2p_prt("[%s] Starting find (type=%d)\n", __func__, type);
	os_get_reltime(&p2p->find_start);

	if (p2p->state == P2P_SEARCH) {
		ra6wx_err_prt("P2P_FIND is now running\n");
		return -1;
	}
	...
	p2p->p2p_find_timeout = timeout;

	if (timeout) {
		p2p->p2p_find_timeout = timeout;
		eloop_register_timeout(timeout, 0, p2p_find_timeout, p2p,
					    NULL);
	} else {
		p2p->p2p_find_timeout = DEFAULT_P2P_FIND_TIMEOUT;
		eloop_register_timeout(DEFAULT_P2P_FIND_TIMEOUT, 0,
	...
	if (res == 0) {
		...
		ra6wx_p2p_prt("[%s] Failed to start p2p_scan - another p2p_scan "
			"was already running\n", __func__);
		...
		ra6wx_p2p_prt("[%s] Failed to start p2p_scan\n", __func__);
		p2p_set_state(p2p, P2P_IDLE);
...
int _p2p_find(struct p2p_data *p2p, unsigned int timeout,
	     enum p2p_discovery_type type,
	     ...
	     u8 seek_count, const char **seek, int freq, bool include_6ghz)
{
	...
	if (p2p->state == P2P_SEARCH) {
		p2p_dbg(p2p, "P2P_FIND is now running");
		return -1;
	}
...
#else
		p2p_dbg(p2p, "Received P2P Action - p2p_process_presence_req ~~~~~\n");
#endif
		break;
...
#else
		p2p_dbg(p2p, "Received P2P Action - p2p_process_presence_resp ~~~~~\n");
#endif
		break;
...
#else
		p2p_dbg(p2p, "in_listen=%d when received P2PS Probe Request at %u MHz; own Listen channel %u, pending listen freq %u MHz",
			p2p->in_listen, rx_freq, p2p->cfg->channel, p2p->pending_listen_freq);
#endif
...
#else
			p2p_dbg(p2p, "Not in Listen state (in_listen=%d) - ignore Probe Request", p2p->in_listen);
#endif	// CONFIG_P2P_OPTION
			p2p_parse_free(&msg);
			return P2P_PREQ_NOT_LISTEN;
		}
...
#if 0   /* by Shingu 20160324 (CANCEL_ROC) */
...
#if 0   /* by Shingu (Wi-Fi Direct) */	//MODIFY_SUPPLICANT_FOR_FREERTOS
...
//MODIFY_SUPPLICANT_FOR_FREERTOS
struct p2p_data * wpa_supplicant_p2p_init(const struct p2p_config *cfg)
{
	struct p2p_data *p2p;
...
static void p2p_go_neg_req_cb(struct p2p_data *p2p, int success)
{
	...
	if (!success &&
		...
		p2p->cfg->send_action_done(p2p->cfg->cb_ctx);
		//MODIFY_SUPPLICANT_FOR_FREERTOS
		p2p_send_dev_disc_req(p2p, dev, 0);
		return;
	}
...
#if 0   /* by Shingu 20170209 (TTA Certificate) */	//MODIFY_SUPPLICANT_FOR_FREERTOS
	if (noa) {
