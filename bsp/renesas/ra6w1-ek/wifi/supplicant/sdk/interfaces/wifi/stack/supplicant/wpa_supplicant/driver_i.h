				       ...
				       struct wpa_driver_auth_params *params)
{
	if (wpa_s->driver->authenticate)
		return wpa_s->driver->authenticate(wpa_s->drv_priv, params);	// driver_fc80211_authenticate
	return -1;
}
				    ...
				    struct wpa_driver_associate_params *params)
{
	if (wpa_s->driver->associate) {
		return wpa_s->driver->associate(wpa_s->drv_priv, params);		// wpa_driver_fc80211_associate
	}
	return -1;
}
			       ...
			       struct wpa_driver_scan_params *params)
{
	...
	if (wpa_s->driver->scan2)
		return wpa_s->driver->scan2(wpa_s->drv_priv, params);		// driver_fc80211_scan2
	return -1;
}
...
static inline int wpa_drv_get_bssid(struct wpa_supplicant *wpa_s, u8 *bssid)
{
	if (wpa_s->driver->get_bssid) {
		return wpa_s->driver->get_bssid(wpa_s->drv_priv, bssid);	// wpa_driver_fc80211_get_bssid
	}
	return -1;
}
				  ...
				  enum key_flag key_flag)
{
	...
	if (wpa_s->driver->set_key) {
		return wpa_s->driver->set_key(wpa_s->drv_priv, &params);		// driver_fc80211_set_key wpa_driver_fc80211_set_key
	}
	return -1;
}
				 ...
				 struct wpa_driver_ap_params *params)
{
	if (wpa_s->driver->set_ap)
		return wpa_s->driver->set_ap(wpa_s->drv_priv, params);	// wpa_driver_fc80211_set_ap
	return -1;
}
				 ...
				 u8 *if_addr, const char *bridge)
{
	if (wpa_s->driver->if_add)
		return wpa_s->driver->if_add(wpa_s->drv_priv, type, ifname,		// wpa_driver_fc80211_if_add
...
static inline int wpa_drv_if_remove(struct wpa_supplicant *wpa_s,
				    ...
				    const char *ifname)
{
	if (wpa_s->driver->if_remove)		// driver_fc80211_if_remove
...
static inline int wpa_drv_shared_freq(struct wpa_supplicant *wpa_s)	// MODIFY_SUPPLICANT_FOR_FREERTOS - add
{
...
static inline int wpa_drv_sta_power_save(struct wpa_supplicant *wpa_s,
						int pwrsave_mode)
{
	if (!wpa_s->driver->sta_power_save)
		return -1;
	return wpa_s->driver->sta_power_save(wpa_s->drv_priv, pwrsave_mode);
}
