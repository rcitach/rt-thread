...
static void * eap_sim_init(struct eap_sm *sm)
{
	...
	if (config && config->phase1) {
		...
		if (pos) {
			data->min_num_chal = (size_t) atoi(pos + 17);
...
static int eap_sim_ext_sim_req(struct eap_sm *sm, struct eap_sim_data *data)
{
	...
	pos += os_snprintf(pos, (size_t) (end - pos), "GSM-AUTH");
	for (i = 0; i < data->num_chal; i++) {
		pos += os_snprintf(pos, (size_t) (end - pos), ":");
		pos += wpa_snprintf_hex(pos, (size_t) (end - pos), data->rand[i],
					GSM_RAND_LEN);
	}
					      ...
					      enum eap_sim_id_req id_req)
{
	...
	if (!data->reauth) {
		...
		eap_sim_msg_add(msg, EAP_SIM_AT_SELECTED_VERSION,
				(u16) data->selected_version, NULL, 0);
	}
						 ...
						 struct eap_sim_attrs *attr)
{
	...
	eap_sim_derive_mk(identity, identity_len, data->nonce_mt,
			  (u16) data->selected_version, data->ver_list,
			  data->ver_list_len, (int) data->num_chal,
	...
	const struct wpabuf *reqData, struct eap_sim_attrs *attr)
{
	...
	return eap_sim_response_notification(data, id, (u16) attr->notification);
}
	...
	const struct wpabuf *reqData, struct eap_sim_attrs *attr)
{
	...
	eap_sim_derive_keys_reauth((u16) data->counter,
