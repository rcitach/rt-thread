...
int p2p_send_dev_disc_req(struct p2p_data *p2p, struct p2p_device *dev, int injector)
{
	struct p2p_device *go;
	struct wpabuf *req;
	...
	if (injector) {
		struct p2p_device *go_tmp = os_malloc(sizeof (struct p2p_device));
		...
		os_memcpy(dev->info.p2p_device_addr, dev->member_in_go_iface, ETH_ALEN);
	...
	if (p2p_send_action(p2p, (unsigned int) dev->oper_freq, go->info.p2p_device_addr,
	...
	if (injector)
		os_memcpy(dev->info.p2p_device_addr, go->info.p2p_device_addr, ETH_ALEN);
				   ...
				   const u8 *addr, int freq, u8 status)
{
	...
	if (p2p_send_action(p2p, (unsigned int) freq, addr, p2p->cfg->dev_addr,
			    p2p->cfg->dev_addr,
			     ...
			     const u8 *data, size_t len, int rx_freq)
{
	...
	p2p->pending_client_disc_freq = (unsigned int) rx_freq;
	tu = 100;
	if (p2p->cfg->start_listen(p2p->cfg->cb_ctx, (unsigned int) rx_freq, (unsigned int) (1024 * tu / 1000),
		    ies) < 0) {
