...
extern void dpm_get_capabilities_info(unsigned short *cap);
extern void dpm_get_center_freq(unsigned short *freq);
...
int wpa_debug_level = MSG_WARNING; // MSG_EXCESSIVE, MSG_MSGDUMP, MSG_DEBUG, MSG_USER_DEBUG, MSG_INFO, MSG_WARNING, MSG_ERROR
...
#endif  /* UNUSED_CODE_DELETE */	//MODIFY_SUPPLICANT_FOR_FREERTOS
				    ...
				    struct wpa_ssid *ssid)
{
...
#else
		ra6wx_info_prt("[%s] WPA: Invalid mode %d (not "
	...
	if (!ssid->psk_set) {
		...
		wpa_msg(wpa_s, MSG_INFO, "WPA: No PSK configured for "
			"WPA-None");
#else
		ra6wx_info_prt("[%s] WPA: No PSK configured for WPA-None\n", __func__);
#endif	//  __CONFIG_DBG_LOG_MSG__
		return -1;
	}
...
#else
		ra6wx_info_prt("[%s] WPA: Invalid group cipher %d for WPA-None\n",
...
static void wpa_supplicant_timeout(void *eloop_ctx, void *timeout_ctx)
{
	...
	wpa_msg(wpa_s, MSG_INFO, "Authentication with " MACSTR " timed out.",
		MAC2STR(bssid));
#else
	ra6wx_info_prt("[%s] Auth with " MACSTR " timed out.\n", __func__, MAC2STR(bssid));
#endif  //__CONFIG_DBG_LOG_MSG__

	//wpa_blacklist_add(wpa_s, bssid);	//MODIFY_SUPPLICANT_FOR_FREERTOS
...
#if 1 // For RRQ61x
    // see caller function
    // wpas_event_dfs_cac_finished(), wpas_event_dfs_cac_aborted()

    RA6W1_UNUSED_ARG(wpa_s);
    RA6W1_UNUSED_ARG(sec_diff);
#else
	int new_sec = wpa_s->last_auth_timeout_sec + sec_diff;

	if (eloop_is_timeout_registered(wpa_supplicant_timeout, wpa_s, NULL)) {
		ra6wx_debug_prt("[%s] Authentication timeout restart: %d sec\n", __func__, new_sec);
		eloop_cancel_timeout(wpa_supplicant_timeout, wpa_s, NULL);
...
#ifdef CONFIG_IEEE80211W
extern void sme_deinit(struct wpa_supplicant *wpa_s);
#endif /* CONFIG_IEEE80211W */
...
static void wpa_supplicant_cleanup(struct wpa_supplicant *wpa_s)
{
	...
	wpa_bssid_ignore_clear(wpa_s);		//MODIFY_SUPPLICANT_FOR_FREERTOS
	...
	wpa_s->ric_ies = NULL;
#endif	// defined ( CONFIG_RIC_ELEMENT )
...
void wpa_supplicant_reconfig_net(void *eloop_ctx, void *timeout_ctx)
{
	...
	if (os_strcmp(wpa_s->ifname, SOFTAP_DEVICE_NAME) == 0) {
		ra6wx_notice_prt("[%s] Not Support(%s) !!!\n", __func__, wpa_s->ifname);
		return;
	}
	...
	if (os_strcmp(wpa_s->ifname, STA_DEVICE_NAME) == 0) {
		/* Station */
		ra6wx_assoc_prt(">>> [%s] INF_NET_STATION\n", __func__);
		...
		/* Station */
		ra6wx_assoc_prt(">>> [%s] MESH_POINT\n", __func__);
		os_memcpy(wifi_monitor_msg->data, "4", 1);
	}
#endif /* CONFIG_MESH */
	else
	{
		ra6wx_notice_prt("[%s] Unknown mode(%s) !!!\n", __func__, wpa_s->ifname);
	...
	if (status != pdTRUE) {
		ra6wx_err_prt("[%s] queue send error(%s) !!! (%d)\n",
	...
	if ((status & WIFI_EVENT_SEND_MSG_TO_APP) == pdFALSE) {
		ra6wx_err_prt("[%s] event set error(%s) !!! (%d)\n", __func__, wpa_s->ifname,  status);
		vPortFree(wifi_monitor_msg);
		return;
	}
...
extern u8 fast_reconnect_scan;		//MODIFY_SUPPLICANT_FOR_FREERTOS
#endif

#if (ATCMD_IF_SUPPORT == 1)
static void wpa_supplicant_atcmd_conn_resp(void * ctx)
{
	char check_dhcp[SSID_MAX_LEN + 1] = "1";

	if (ctx) {
		/* check_dhcp = "1:<ssid>" */
		sprintf(check_dhcp + 1, ":%s", (char *) ctx);
	}

	if (!gp_wifi_atcmd_ctrl || !gp_wifi_atcmd_event_cb) {
		return;
	}

	gp_wifi_atcmd_event_cb(gp_wifi_atcmd_ctrl, 0, (unsigned char *)check_dhcp, strlen(check_dhcp));
}
#endif

static void wpa_supplicant_wifi_event_signal_connected(struct wpa_supplicant *wpa_s)
{
	struct wpa_ssid *ssid = wpa_s->current_ssid;
	unsigned int key_mgmt = wpa_sm_get_key_mgmt(wpa_s->wpa);
	unsigned int proto = wpa_sm_get_proto(wpa_s->wpa);
	unsigned int pairwise_cipher = wpa_sm_get_pairwise_cipher(wpa_s->wpa);

	rm_wifi_event_signal_connected(ssid->ssid, ssid->ssid_len, wpa_s->bssid,
				       rm_wifi_helper_security_type_get(key_mgmt, proto, pairwise_cipher),
			      ...
			      enum wpa_states state)
{
	...
	if (state == WPA_COMPLETED) {
		...
		wpa_s->normal_scans = 0;
#ifdef  CONFIG_RECONNECT_OPTIMIZE /* by Shingu 20170717 (Scan Optimization) */	//MODIFY_SUPPLICANT_FOR_FREERTOS
		wpa_s->reassoc_try = 0;
...
#if CFG_PMGR
		if(RM_PMGR_W_dpm_is_wakeup() && wpa_s->assoc_freq == 0) {
			wpa_s->assoc_freq = wpa_s->reassoc_freq;
			//ra6wx_notice_prt(GRN_COL "\nAssoc freq %d for Dpm info saving\n", wpa_s->assoc_freq);
		}
#endif /* CFG_PMGR */
#endif /* CONFIG_RECONNECT_OPTIMIZE */
		...
		if (do_connect_prt) {
			...
			if (wpa_s->current_ssid->mode == WPAS_MODE_P2P_GO) {
				...
				if ((wpa_s->conf->scan_cur_freq != 0 || fast_connection_sleep_flag == pdTRUE)
					&& (unsigned int) wpa_s->conf->scan_cur_freq != wpa_s->assoc_freq)
				{
					ra6wx_notice_prt("Fast_Connection ASSOC_CH=%d --> %d\n", 
							i3ed11_freq_to_ch(wpa_s->conf->scan_cur_freq), i3ed11_freq_to_ch(wpa_s->assoc_freq));
				}
...
#if (ATCMD_IF_SUPPORT == 1)
				/* Notify Wi-Fi connection status on AT-CMD UART */
				wpa_supplicant_atcmd_conn_resp(wpa_s->current_ssid->ssid);
#endif
				wpa_supplicant_wifi_event_signal_connected(wpa_s);
				 ...
				 */
				if (get_run_mode() == WIFI_DEVICE_MODE_EXT_MESH_PORTAL) {
					extern int fc80211_mesh_switch_channel(int , int);
					
					ra6wx_notice_prt("[%s] MESH Frequency Change --> assoc: %d , cur_bss: %d cur_ssid: %d \n",
...
#ifdef CONFIG_SIMPLE_ROAMING	//MODIFY_SUPPLICANT_FOR_FREERTOS
...
*/
		if (state != old_state) {
			...
			if (RM_PMGR_W_dpm_is_enabled() == pdTRUE) {
				if (is_zero_ether_addr(dpm_supp_conn_info->bssid))
				{
					eloop_register_timeout(0, 0, wpa_supplicant_reconfig_net, wpa_s, NULL); /* 0 ms */
				}
			} 
			else 
#endif /* CFG_PMGR */
			if (os_strcmp(wpa_s->ifname, SOFTAP_DEVICE_NAME) != 0) {
#ifdef CONFIG_MESH_PORTAL
				if (!(get_run_mode() == WIFI_DEVICE_MODE_EXT_MESH_PORTAL && os_strcmp(wpa_s->ifname, MESH_POINT_DEVICE_NAME) == 0)
...
#if CFG_PMGR
				wpa_supp_dpm_backup_conn_info(wpa_s);	//MODIFY_SUPPLICANT_FOR_FREERTOS
#endif /* CFG_PMGR */
			}
#if 0	/* by Bright : Merge 2.7 */		//MODIFY_SUPPLICANT_FOR_FREERTOS
			wpas_notify_auth_changed(wpa_s);
...
#if CFG_PMGR
	RM_WIFI_dpm_supp_conn_state_set(wpa_s->wpa_state);		//MODIFY_SUPPLICANT_FOR_FREERTOS
#endif /* CFG_PMGR */
...
void wpa_supplicant_terminate_proc(struct wpa_global *global)
{
	...
	if (pending)
		return;
	//eloop_terminate();	//MODIFY_SUPPLICANT_FOR_FREERTOS
}
...
#if CFG_PMGR
	if (wpa_s->wpa_state != WPA_INTERFACE_DISABLED && RM_PMGR_W_dpm_is_wakeup() == 0) {
...
int wpa_supplicant_reload_configuration(struct wpa_supplicant *wpa_s)
{
...
#if 1   /* by Shingu 20160928 (WPA_CLI Optimize) */	//MODIFY_SUPPLICANT_FOR_FREERTOS
	if (os_strcmp(wpa_s->ifname, STA_DEVICE_NAME) != 0)
		return -1;
#endif  /* 1 */
	...
	if (RM_PMGR_W_dpm_is_wakeup() == pdFALSE) {
		...
		if (conf == NULL) {
			ra6wx_err_prt("[%s] Failed to parse the config "
				"NVRAM - exiting\n", __func__);
			return -1;
		}
#if CFG_PMGR
	} else {
		ra6wx_err_prt("[%s] conf is not set - exiting\n", __func__);
		return -1;
	}
#endif /* CFG_PMGR */
	...
	if (wpa_s->confanother &&
	    !wpa_config_read(wpa_s->confanother, (char *)conf)) {
...
#if 0	/* FC9000 */	//MODIFY_SUPPLICANT_FOR_FREERTOS
	if (reconf_ctrl)
...
#if CFG_PMGR
	if (RM_PMGR_W_dpm_is_wakeup() == pdFALSE) {	//MODIFY_SUPPLICANT_FOR_FREERTOS
#endif /* CFG_PMGR */
		wpa_config_free(wpa_s->conf);
		wpa_s->conf = conf;
#if CFG_PMGR
					 ...
					 struct wpa_ie_data *ie)
{
...
#else
			ra6wx_err_prt("[%s] Failed to parse WPA IE from assoc info\n", __func__);
#endif  /* __CONFIG_DBG_LOG_MSG__ */
		}
		return -1;
	}
	...
	wpa_dbg(wpa_s, MSG_DEBUG, "WPA: Using WPA IE from AssocReq to set "
		"cipher suites");
#else
	ra6wx_assoc_prt("[%s] Using WPA IE from AssocReq to set cipher suites\n", __func__);
#endif  /* __CONFIG_DBG_LOG_MSG__ */
...
#else
		ra6wx_err_prt("[%s] WPA: Driver used disabled group "
			"cipher 0x%x (mask 0x%x) - reject\n",
			__func__,
			ie->group_cipher, ssid->group_cipher);
#endif  /* __CONFIG_DBG_LOG_MSG__ */
...
#else
		ra6wx_err_prt("[%s] WPA: Driver used disabled pairwise "
			"cipher 0x%x (mask 0x%x) - reject\n",
			__func__,
			ie->pairwise_cipher, ssid->pairwise_cipher);
#endif  /* __CONFIG_DBG_LOG_MSG__ */
...
#else
		ra6wx_err_prt("[%s] WPA: Driver used disabled key "
			"management 0x%x (mask 0x%x) - reject\n",
	...
	if (!(ie->capabilities & WPA_CAPABILITY_MFPC) &&
		...
		wpa_msg(wpa_s, MSG_INFO, "WPA: Driver associated with an AP "
			...
			"reject");
#else
		ra6wx_info_prt("[%s] WPA: Driver assoc with an AP "
...
#endif	// defined CONFIG_OWE || defined CONFIG_HS20
...
#else
		ra6wx_wpa_prt("[%s] WPA: Failed to select WPA/RSN\n", __func__);
#endif	//  __CONFIG_DBG_LOG_MSG__

#if 1   /* by Bright : Merge 2.7 */	//MODIFY_SUPPLICANT_FOR_FREERTOS
		wpa_dbg(wpa_s, MSG_DEBUG,
...
#else
			"");
#endif // CONFIG_OWE || defined CONFIG_HS20
		if (bss_rsn) {
...
#else
	ra6wx_wpa_prt("[%s] WPA: Selected cipher suites: group %d "
		"pairwise %d key_mgmt %d proto %d\n",
		__func__,
		ie.group_cipher, ie.pairwise_cipher, ie.key_mgmt, proto);
#endif	// __CONFIG_DBG_LOG_MSG__
			      ...
			      struct wpa_bss *bss, struct wpa_ssid *ssid)
{
...
#ifdef  CONFIG_CONCURRENT /* by Shingu 20160901 (Concurrent) */		//MODIFY_SUPPLICANT_FOR_FREERTOS
	if (get_run_mode() == WIFI_DEVICE_MODE_EXT_AP_STATION) {
		...
		//[TVR-1688]Fixed an exception caused by an AP not running on a Concurrent AP-STA.
			if (ap_ctrl_iface_ap(ap_wpa_s, cmd, NULL, 0) < 0)
				return; 					
		...
		if (chan != 1 && chan != 6 && chan != 11) {
			if (p2p_wpa_s->global->p2p) {
				ra6wx_notice_prt("STA channel(%d) is not social "
			...
			if (p2p_wpa_s->conf->p2p_oper_channel != chan ||
				...
				ra6wx_notice_prt("Setting P2P channels to %d\n", chan);
				p2p_wpa_s->conf->p2p_oper_channel = chan;
				p2p_wpa_s->conf->p2p_listen_channel = chan;
			}

			if (!p2p_wpa_s->global->p2p) {
				ra6wx_notice_prt("Now P2P interface is " "reactivated.\n");
				wpas_p2p_init(p2p_wpa_s->global, p2p_wpa_s);
			}

			p2p_set_oper_channel(p2p_wpa_s->global->p2p, 81, chan, 1);
			p2p_set_listen_channel(p2p_wpa_s->global->p2p, 81, chan, 0);
		}
	...
	if ((wpa_s->drv_flags & WPA_DRIVER_FLAGS_SME) &&
		...
		// Authentication does not request connect
		sme_authenticate(wpa_s, bss, ssid);

		// Assoc makes a connection request.
		sme_associate(wpa_s, ssid->mode, bss->bssid, FC80211_AUTHTYPE_OPEN_SYSTEM);

		return;
	}
#ifndef CONFIG_ASSOC_CB
	else{
		struct wpa_radio_work work;
		struct wpa_connect_work cwork;

		memset(&work, 0x00, sizeof(struct wpa_radio_work));
		memset(&cwork, 0x00, sizeof(struct wpa_connect_work));
		...
		work.ctx = &cwork;
		work.wpa_s = wpa_s;
		//sme_associate(wpa_s, ssid->mode, bss->bssid, wpa_s->auth_alg);
		wpas_start_assoc_cb(&work, 1);
	}
#endif /* CONFIG_ASSOC_CB */
...
#else
		ra6wx_wpa_prt("[%s] Reject wpa_supplicant_associate() call since connect_work exist\n", __func__);
#endif	// __CONFIG_DBG_LOG_MSG__
		return;
	}
#endif	// CONFIG_ASSOC_CB
...
#else
		ra6wx_wpa_prt("[%s] Reject wpa_supplicant_associate() call since pending work exist\n", __func__);
#endif	// __CONFIG_DBG_LOG_MSG__
		return;
	}
			  ...
			  struct hostapd_freq_params *freq)
{
	...
	for (j = 0; j < wpa_s->last_scan_res_used; j++) {
		...
		if (ssid->ssid_len == bss->ssid_len &&
			...
			//MODIFY_SUPPLICANT_FOR_FREERTOS
			ra6wx_debug_prt("[%s] IBSS already found in scan results, adjust control freq: %d\n",
																			__func__, bss->freq);
			freq->freq = bss->freq;
			obss_scan = 0;
			break;
	...
	if (wpa_s->group_cipher == WPA_CIPHER_WEP40 ||
	    ...
	    wpa_s->pairwise_cipher == WPA_CIPHER_TKIP) {
		//MODIFY_SUPPLICANT_FOR_FREERTOS
		ra6wx_debug_prt("[%s] IBSS: WEP/TKIP detected, do not try to enable HT\n", __func__);
		return;
	}
	...
	enum wpa_drv_update_connect_params_mask *mask)
{
...
#endif /* CONFIG_PMKSA */
#if defined(CONFIG_FILS) || defined(CONFIG_PMKSA)
		const u8 *cache_id = NULL;
#endif /* CONFIG_FILS || CONFIG_PMKSA */
...
#ifdef  CONFIG_P2P_UNUSED_CMD
	//wpa_s->cross_connect_disallowed = 0;//MODIFY_SUPPLICANT_FOR_FREERTOS	
	if (bss) {
		struct wpabuf *p2p;
	...
	if (!wpa_s->disable_mbo_oce && mbo_ie && rwnx_sku_id_read_otp() != TIN_SKU_WIFI4_B24) {
		int len;
...
static void wpas_start_assoc_cb(struct wpa_radio_work *work, int deinit)
{
	...
	if (wpa_s->p2pdev && wpa_s->p2pdev->set_sta_uapsd)
...
#ifdef CONFIG_SCAN_RESULT_OPTIMIZE // trinity_0160905, Issue #185	//MODIFY_SUPPLICANT_FOR_FREERTOS
...
struct wpa_ssid * wpa_supplicant_add_network(struct wpa_supplicant *wpa_s)
{
	...
	/* FC9000 Only */		//MODIFY_SUPPLICANT_FOR_FREERTOS
...
#endif	// 0   /* by Bright : Merge 2.7 */	//MODIFY_SUPPLICANT_FOR_FREERTOS
...
#ifdef CONFIG_FAST_CONNECTION		//MODIFY_SUPPLICANT_FOR_FREERTOS
	wpa_s->num_retries = 0;
...
{
#if 0   /* by Bright */		//MODIFY_SUPPLICANT_FOR_FREERTOS
	int old_ap_scan;
#endif	// 0
...
#ifdef  CONFIG_CONCURRENT /* by Shingu 20160905 (Concurrent) */		//MODIFY_SUPPLICANT_FOR_FREERTOS
	if (get_run_mode() == WIFI_DEVICE_MODE_EXT_AP_STATION
#ifdef CONFIG_P2P_CONCURRENT
...
static int select_driver(struct wpa_supplicant *wpa_s, int i)
{
	...
	/* FC9000 */	//MODIFY_SUPPLICANT_FOR_FREERTOS
	if (wpa_drivers->global_init && global->drv_priv[i] == NULL) {
		global->drv_priv[i] = wpa_drivers->global_init();

		if (global->drv_priv[i] == NULL) {
			wpa_printf(MSG_ERROR, "Failed to initialize driver " "'%s'", __func__, wpa_drivers->name);
			return -1;
		}
	}
...
#else
		ra6wx_err_prt("[%s] No driver interfaces build into "
			"wpa_supplicant\n", __func__);
#endif  /* __CONFIG_DBG_LOG_MSG__ */
		return -1;
	}
...
int wpa_supplicant_update_mac_addr(struct wpa_supplicant *wpa_s)
{
	...
	if (wpa_s->l2 && l2_packet_get_own_addr(wpa_s->l2, wpa_s->own_addr)) {
		ra6wx_fatal_prt("        [%s] Failed to get own L2 address\n",
						__func__);
		return -1;
	}
	...
	if (RM_PMGR_W_dpm_is_wakeup() == pdFALSE) {
		...
		if (   (os_strcmp(wpa_s->ifname, STA_DEVICE_NAME) == 0)
            && (  get_run_mode() == WIFI_DEVICE_MODE_EXT_AP
				...
				ra6wx_iface_prt(">>> MAC address (%s) : " MACSTR "\n", wpa_s->ifname, MAC2STR(wpa_s->own_addr));
		} else {
				ra6wx_notice_prt(">>> MAC address (%s) : " MACSTR "\n",
				wpa_s->ifname, MAC2STR(wpa_s->own_addr));
		}
#if CFG_PMGR
	}
...
struct wpa_supplicant * wpas_add_meshpoint_interface(struct wpa_supplicant *wpa_s)
{
	...
	if (wpas_mesh_add_interface(wpa_s, ifname, sizeof(ifname)) < 0) {
		ra6wx_err_prt(">>> RA6WX Supplicant Failed to add "
	...
	if (!softap_wpa_s) {
		ra6wx_fatal_prt("[%s] Failed to add SoftAP Device interface\n",
				__func__);
		return NULL;
	}
	...
	for(ssid_p = softap_wpa_s->conf->ssid ; ssid_p ; ssid_p = ssid_p->next)
	{
		if (ssid_p->ssid == NULL || ssid_p->ssid_len == 0) {
			ra6wx_ap_prt("[%s] No SSID configured for AP mode\n", __func__);
			continue;
		} else if (ssid_p->mode != WPAS_MODE_AP) {
			ra6wx_ap_prt("[%s] Not AP mode\n", __func__);
			continue;
		}
		break;
	}
	...
	if (ssid_p && wpa_supplicant_create_ap(softap_wpa_s, ssid_p) < 0) {
		ra6wx_fatal_prt("[%s] Failed to Create AP\n", __func__);
...
struct wpa_supplicant * wpas_add_softap_interface(struct wpa_supplicant *wpa_s)
{
	...
	os_snprintf(ifname, sizeof(ifname), SOFTAP_DEVICE_NAME);
	ra6wx_notice_prt(">>> Add SoftAP Inteface (%s) ...\n", ifname);
...
static int wpa_supplicant_init_iface(struct wpa_supplicant *wpa_s,  struct wpa_interface *iface)
{
...
#if 1 /* supplicant 2.7 */
	ra6wx_debug_prt(" [%s] Initializing interface '%s' conf '%s' driver "
	...
	if ((   get_run_mode() == WIFI_DEVICE_MODE_EXT_AP
		...
		ra6wx_debug_prt(" [%s] Configuration file '%s' -> '%s' \n",
			   __func__, iface->confname, wpa_s->confname);
	...
	if (iface->ifname == NULL) {
		ra6wx_fatal_prt("    [%s] Interface name is required\n", __func__);
		return -1;
	}
	...
	if ((get_run_mode() == WIFI_DEVICE_MODE_EXT_AP ||
		...
		/* RA6WX needs to read the NVRAM before anything else. */
...
#else
		ra6wx_err_prt("[%s] Invalid WPA parameter value for "
			"dot11RSNAConfigPMKLifetime\n", __func__);
#endif	/* __CONFIG_DBG_LOG_MSG__ */
		return -1;
	}
	...
	if (get_run_mode() != WIFI_DEVICE_MODE_EXT_STATION &&
		...
		if (wpa_s->conf->ssid->wifi_mode >= WIFI_MODE_BGN
			&& wpa_s->conf->ssid->wifi_mode < WIFI_MODE_MAX) {
	...
	wpa_s->setband_mask = wpa_s->conf->setband_mask;
...
#else
			ra6wx_err_prt("[%s] FST: Cannot attach iface %s to group %s\n",
	...
	if ((get_run_mode() == WIFI_DEVICE_MODE_EXT_P2P ||
		...
		if (wpas_p2p_init(wpa_s->global, wpa_s) < 0) {
			ra6wx_err_prt("    [%s] Error wpas_p2p_init()\n",
						__func__);
			return -1;
		}
	}
...
#else
		ra6wx_err_prt("[%s] Failed to init P2P\n", __func__);
#endif	/* __CONFIG_DBG_LOG_MSG__ */
		return -1;
	}
#endif  // FC9000 Only
...
#ifdef CONFIG_EAP_PROXY
	{
		...
		if (wpa_s->mnc_len > 0) {
			...
			wpa_printf_dbg(MSG_DEBUG, "eap_proxy: IMSI %s (MNC length %d)",
			   	wpa_s->imsi, wpa_s->mnc_len);
#else
			ra6wx_debug_prt(" [%s] eap_proxy: IMSI %s (MNC length %d) \n",
			   	__func__, wpa_s->imsi, wpa_s->mnc_len);
#endif
		} else {
#if 0
			wpa_printf_dbg(MSG_DEBUG, "eap_proxy: IMSI not available");
#else
			ra6wx_debug_prt(" [%s] eap_proxy: IMSI not available \n", __func__);
#endif
		}
	}
...
#ifdef CONFIG_P2P
	if ((wpa_s == wpa_s->parent) && (get_run_mode() == WIFI_DEVICE_MODE_EXT_P2P || get_run_mode() == WIFI_DEVICE_MODE_EXT_P2P_GO))
		wpas_p2p_disconnect(wpa_s);
	if (wpa_s == wpa_s->global->p2p_init_wpa_s && wpa_s->global->p2p) {
		ra6wx_p2p_prt("[%s] P2P: Disable P2P since removing "
...
#else
		ra6wx_info_prt("[%s] " WPA_EVENT_TERMINATING"\n", __func__);
#endif	/* __CONFIG_DBG_LOG_MSG__ */
...
static int wpa_supplicant_match_existing(struct wpa_global *global)
{
	...
	ifp = if_nameindex();
	if (!ifp) {
		wpa_printf(MSG_ERROR, "if_nameindex");
		return -1;
	}
						 ...
						 struct wpa_supplicant *parent)
{
	...
	if (wpa_supplicant_init_iface(new_wpa_s, &new_iface)) {
		ra6wx_iface_prt("[%s] Failed to add interface %s\n",
			   	__func__, iface->ifname);
	...
	if (RM_PMGR_W_dpm_is_wakeup() == pdFALSE) {
		...
		if ((os_strcmp(new_wpa_s->ifname, STA_DEVICE_NAME) == 0) &&
			...
			ra6wx_iface_prt(">>> %s interface add OK\n", new_wpa_s->ifname);
		}
#if 0
		else {
			ra6wx_notice_prt(">>> %s interface add OK\n", new_wpa_s->ifname);
		}
#endif /* 0 */
...
#ifdef  UNUSED_CODE_DELETE /* by Bright : Merge 2.7 */
...
#endif	// UNUSED_CODE_DELETE /* by Bright : Merge 2.7 */
...
struct wpa_global * wpa_supplicant_init(struct wpa_params *params)
{
	...
	if (RM_PMGR_W_dpm_is_wakeup() == pdFALSE) {
#endif /* CFG_PMGR */
		ra6wx_notice_prt(">>> Renesas "CHIPSET_NAME" wpa_supplicant " VER_NUM RELEASE_VER_STR"\n");
#if CFG_PMGR
	}
#endif /* CFG_PMGR */
...
#if 0 // For Renesas RRQ61x series
	global->ctrl_iface = wpa_supplicant_global_ctrl_iface_init(global);
#endif /* 0 */

#if 1 // For Renesas RRQ61x series
	global->drv_count = 1;
...
bool is_dpm_fast_reconnect(void)
{
	...
	if (RM_PMGR_W_dpm_is_wakeup()) {
		if( ra6wx_is_fast_reconnect && (fast_reconnect_tries == 1  || fast_reconnect_tries == 2 )) return 1;
	}
#endif /* CFG_PMGR */
	return 0;
}
...
UINT	dpm_supplicant_done_flag = 0;
void dpm_supplicant_done(void)
{
   dpm_supplicant_done_flag = 1;
}

UINT is_dpm_supplicant_done(void)
{
	return dpm_supplicant_done_flag;
}

unsigned int wait_supplicant_done(unsigned int timeout);

/**
 * wait_supplicant_done - wait for supplicant initialization
 * (until supplicant_done_flag becomes 1).
 *
 * @timeout - timeout in 10 ms units.
 * Returns 0 if timeout occured, non-zero otherwise.
 */
unsigned int wait_supplicant_done(unsigned int timeout)
{
    unsigned int cnt = 1;
...
unsigned int wait_supplicant_deinit(unsigned int timeout);

/**
 * wait_supplicant_deinit - wait for supplicant deinitialization
 * (until supplicant_done_flag becomes 0).
 *
 * @timeout - timeout in 10 ms units.
 * Returns 0 if timeout occured, non-zero otherwise.
 */
...
int wpa_supplicant_run(struct wpa_global *global, struct wpa_supplicant *wpa_s)
{
	TX_FUNC_START("  ");
#if 0 // for debug
		printf("[%s] supp_run: ssid_id=%d, net_s(%d)\n", 
					__func__, wpa_s->conf->ssid->id, wifi_netif_status(0));
#endif

#if CFG_PMGR
	if (RM_PMGR_W_dpm_is_wakeup() == pdTRUE && dpm_supp_conn_info->id == wpa_s->conf->ssid->id) {
		wpa_supp_dpm_restore_conn_info(wpa_s);
		wpa_supp_dpm_restore_key_info(wpa_s);	// Delete on wakeup upon request from mac
	}
	...
	if (RM_PMGR_W_dpm_is_wakeup() == pdTRUE) {
		...
		dpm_get_center_freq(&freq);
		dpm_get_capabilities_info(&cap);

		//ra6wx_notice_prt("[%s] Fill the current bss +++\n", __func__);
...
void wpa_supplicant_update_config(struct wpa_supplicant *wpa_s)
{
	if ((wpa_s->conf->changed_parameters & CFG_CHANGED_COUNTRY) &&
    	...
    	free_hw_features(wpa_s);
    	wpa_s->hw.modes = fc80211_get_hw_feature_data(0, &wpa_s->hw.num_modes, NULL, 0, 0, 0, NULL);
	}
...
void wpas_connection_failed(struct wpa_supplicant *wpa_s, const u8 *bssid)
{
	...
	if (wpa_s->conf->ssid->key_mgmt == WPA_KEY_MGMT_NONE
		...
		ra6wx_assoc_prt("[%s]Changed WEP_KEY_INDEX=%d\n", __func__, wpa_s->conf->ssid->wep_tx_keyidx);
	}
#endif /* AUTO_WEPKEY_INDEX */
#ifdef CONFIG_RECONNECT_OPTIMIZE
#if CFG_PMGR
...
#else
		ra6wx_debug_prt( " [%s] Continuous association failures - consider temporary network disabling \n", __func__);
#endif
...
#if (ATCMD_IF_SUPPORT == 1)
		/* Connect trial failure */
		if (gp_wifi_atcmd_ctrl || gp_wifi_atcmd_event_cb) {
			gp_wifi_atcmd_event_cb(gp_wifi_atcmd_ctrl, 11, NULL, 0);
		}
#endif
...
#ifdef CONFIG_FAST_RECONNECT_V2
#if CFG_PMGR
	if (fast_reconnect && RM_PMGR_W_dpm_is_wakeup()/*&& !wpas_network_disabled(wpa_s, fast_reconnect_ssid) &&
		!wpas_temp_disabled(wpa_s, fast_reconnect_ssid)*/
		&& fast_reconnect_tries < 2) {
			fast_reconnect_tries++;
			ra6wx_notice_prt("[%s] Try to reconnect to the same BSS without Scan(tries=%d)\n", __func__, fast_reconnect_tries);
			wpa_s->reassociate = 1;
			ra6wx_is_fast_reconnect = 1;
...
#endif /* CFG_PMGR */
	{
		ra6wx_notice_prt("[%s] Not a DPM wakeup. Request scan. \n", __func__);
...
#else
		os_free(eap->cert.private_key_passwd);
#endif  /* CONFIG_SUPP27_STR_CLR_FREE */
...
void wpas_auth_failed(struct wpa_supplicant *wpa_s, char *reason)
{
	...
	ssid->auth_failures++;	//MODIFY_SUPPLICANT_FOR_FREERTOS
 ...
 */
int wpas_request_disconnection(struct wpa_supplicant *wpa_s)
{
    if (os_strcmp(wpa_s->ifname, STA_DEVICE_NAME) != 0) {
        return -1;
    }
...
//MODIFY_SUPPLICANT_FOR_FREERTOS - add
void dump_freq_array(struct wpa_supplicant *wpa_s, const char *title,
			int *freq_array, unsigned int len)
{
	unsigned int i;

	ra6wx_scan_prt("[%s] Shared frequencies (len=%u): %s\n", __func__, len, title);

	for (i = 0; i < len; i++)
		ra6wx_scan_prt("\tfreq[%u]: %d\n", i, freq_array[i]);
}
...
// note : can be invoked in P2P mode.
			   ...
			   int *freq_array, unsigned int len)
{
	...
	ra6wx_scan_prt("[%s] Determining shared radio frequencies (max len %u)\n",
					__func__, len);
...
*/
int wpa_supp_ifname_to_ifindex(const char *ifname)	//MODIFY_SUPPLICANT_FOR_FREERTOS
{
					...
					unsigned int type)
{
	...
	if (wpa_s->pno) {
		...
		if (wpa_s->sched_scanning && (type & MAC_ADDR_RAND_SCHED_SCAN)) {
		wpas_scan_restart_sched_scan(wpa_s);
	}
#endif	// defined ( CONFIG_SCHED_SCAN )
...
static int wpa_supp_dpm_backup_conn_info(struct wpa_supplicant *wpa_s)
{
	...
	if (ssid == NULL) {
		ra6wx_dpm_prt("[%s] Failed to parse network profile\n",
				__func__);
		return -1;
	}

	conf = (struct wpa_config *)wpa_s->conf;
	if (conf == NULL) {
		ra6wx_dpm_prt("[%s] Failed to parse network config\n",
				__func__);
		return -1;
	}
	...
	if ((ssid->mode != WPAS_MODE_INFRA)
		...
		ra6wx_dpm_prt("[%s] Not supported this mode. \n", __func__);
		return 0;
	}

	os_memset(dpm_supp_conn_info, 0, sizeof(dpm_supp_conn_info_t));
	os_memset(dpm_supp_conn_ext_info, 0, sizeof(dpm_supp_conn_ext_info_t));
	os_memset(dpm_supp_conn_info->bssid, 0, ETH_ALEN);
	os_memset(dpm_supp_conn_info->ssid, 0, DPM_MAX_SSID_LEN);
	os_memset(dpm_supp_conn_info->psk, 0, DPM_PSK_LEN);
	os_memset(dpm_supp_conn_ext_info->passphrase, 0, wificonfigMAX_PASSPHRASE_LEN + 1);

	dpm_supp_conn_info->setband_mask = (unsigned char)wpa_s->setband_mask;
...
#ifdef CONFIG_IEEE80211W
	dpm_supp_conn_ext_info->ieee80211w = ssid->ieee80211w;
#endif // CONFIG_IEEE80211W
	if (ssid->passphrase)
		strcpy(dpm_supp_conn_ext_info->passphrase, ssid->passphrase);

#ifdef CONFIG_SAE
	if (ssid->sae_password) {
		strcpy(dpm_supp_conn_ext_info->sae_password, ssid->sae_password);
	}

	if (conf->sae_groups) {
		for (int idx = 0 ; idx < MAX_SAE_GROUPS ; idx++) {
			dpm_supp_conn_ext_info->sae_groups[idx] = conf->sae_groups[idx];
			if (conf->sae_groups[idx] == 0) {
				break;
			}
		}
	}
...
#ifdef IEEE8021X_EAPOL
	if ((ssid->eap.identity_len > 0) && (ssid->eap.identity_len <= sizeof(dpm_supp_conn_ext_info->identity))) {
		dpm_supp_conn_ext_info->identity_len = ssid->eap.identity_len;
		os_memcpy(dpm_supp_conn_ext_info->identity, ssid->eap.identity, ssid->eap.identity_len);
	}

	if ((ssid->eap.password_len > 0) && (ssid->eap.password_len <= sizeof(dpm_supp_conn_ext_info->password))) {
		dpm_supp_conn_ext_info->password_len = ssid->eap.password_len;
		os_memcpy(dpm_supp_conn_ext_info->password, ssid->eap.password, ssid->eap.password_len);
	}
#endif // IEEE8021X_EAPOL

	ra6wx_dpm_prt("[%s] Finish to backup basic WiFi information\n",
				__func__);
	return 1;
}
...
dpm_clr_retry :
	...
	ra6wx_dpm_prt("[%s] Clear RTM Connected info...\n", __func__);
...
void wpa_supp_dpm_restore_conn_info(struct wpa_supplicant *wpa_s)
{
	...
	wpa_state = RM_WIFI_dpm_supp_state_get();
	...
	ra6wx_dpm_prt("[DPM] Done restore the Connection Information\n");
