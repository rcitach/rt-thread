...
SM_STATE(SUPP_PAE, CONNECTING)
{
	...
	if (send_start) {
		 ...
		 */
		if (sm->conf.wps && !((unsigned int) sm->conf.wps & EAPOL_PEER_IS_WPS20_AP)) {
...
SM_STATE(SUPP_PAE, S_FORCE_AUTH)
{
	...
	eapol_sm_set_port_authorized(sm);
	sm->sPortMode = SUPP_ForceAuthorized;
}
...
SM_STATE(SUPP_PAE, S_FORCE_UNAUTH)
{
	...
	eapol_sm_set_port_unauthorized(sm);
	sm->sPortMode = SUPP_ForceUnauthorized;
	eapol_sm_txLogoff(sm);
}
...
SM_STEP(SUPP_PAE)
{
	...
	else if (((sm->portControl == SUPP_Auto) &&
		  (sm->sPortMode != sm->portControl)) ||
		 sm->initialize || !sm->portEnabled)
		SM_ENTER_GLOBAL(SUPP_PAE, DISCONNECTED);
	else if ((sm->portControl == SUPP_ForceAuthorized) &&
		 (sm->sPortMode != sm->portControl) &&
		 !(sm->initialize || !sm->portEnabled))
		SM_ENTER_GLOBAL(SUPP_PAE, S_FORCE_AUTH);
	else if ((sm->portControl == SUPP_ForceUnauthorized) &&
		 (sm->sPortMode != sm->portControl) &&
		 !(sm->initialize || !sm->portEnabled))
...
static void eapol_sm_processKey(struct eapol_sm *sm)
{
	...
	hmac_md5(keydata.sign_key, (size_t) sign_key_len,
	...
	if (key_len == rx_key_length) {
		...
		os_memcpy(ekey + IEEE8021X_KEY_IV_LEN, keydata.encr_key,
			  (size_t) encr_key_len);
		os_memcpy(datakey, key + 1, (size_t) key_len);
		rc4_skip(ekey, (size_t) (IEEE8021X_KEY_IV_LEN + encr_key_len), 0,
			 datakey, (size_t) key_len);
		...
		os_memcpy(datakey, keydata.encr_key, (size_t) key_len);
	...
	if (sm->ctx->set_wep_key &&
		...
		if ((sm->unicast_key_received ||
		     !((unsigned int) sm->conf.required_keys & EAPOL_REQUIRE_KEY_UNICAST)) &&
		    (sm->broadcast_key_received ||
		     !((unsigned int) sm->conf.required_keys & EAPOL_REQUIRE_KEY_BROADCAST)))
		{
...
static void eapol_sm_set_port_authorized(struct eapol_sm *sm)
{
	...
	cb = sm->suppPortStatus != SUPP_Authorized || sm->force_authorized_update;
	sm->force_authorized_update = false;
	sm->suppPortStatus = SUPP_Authorized;
...
static void eapol_sm_set_port_unauthorized(struct eapol_sm *sm)
{
	...
	cb = sm->suppPortStatus != SUPP_Unauthorized || sm->force_authorized_update;
	sm->force_authorized_update = false;
	sm->suppPortStatus = SUPP_Unauthorized;
	if (cb && sm->ctx->port_cb)
...
static const char * eapol_port_control(PortControl ctrl)
{
	...
	case SUPP_Auto:
		return "Auto";
	case SUPP_ForceUnauthorized:
		return "ForceUnauthorized";
	case SUPP_ForceAuthorized:
			...
			int startPeriod, int maxStart)
{
	if (sm == NULL)
		return;
	if (heldPeriod >= 0)
		sm->heldPeriod = (unsigned int) heldPeriod;
	if (authPeriod >= 0)
		sm->authPeriod = (unsigned int) authPeriod;
	if (startPeriod >= 0)
		sm->startPeriod = (unsigned int) startPeriod;
	if (maxStart >= 0)
		sm->maxStart = (unsigned int) maxStart;
}
...
#endif /* CONFIG_EAP_PROXY */
	len += eap_sm_get_status(sm->eap, buf + len, buflen - (size_t) len, verbose);
		      ...
		      size_t len)
{
	...
	case IEEE802_1X_TYPE_EAPOL_KEY:
		...
		os_free(sm->last_rx_key);
		sm->last_rx_key = os_malloc((size_t) data_len);
		if (sm->last_rx_key) {
			wpa_printf(MSG_DEBUG, "EAPOL: Received EAPOL-Key "
				   "frame");
			os_memcpy(sm->last_rx_key, buf, (size_t) data_len);
			sm->last_rx_key_len = (size_t) data_len;
			sm->rxKey = true;
			eapol_sm_step(sm);
		}
		break;
