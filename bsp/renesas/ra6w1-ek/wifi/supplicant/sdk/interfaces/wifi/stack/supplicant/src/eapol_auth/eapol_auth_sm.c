			       ...
			       const char *fmt, ...)
{
	...
	maxlen = (int) (os_strlen(fmt) + 100);
	format = os_malloc((size_t) maxlen);
	if (!format)
		return;

	va_start(ap, fmt);
	vsnprintf(format, (size_t) maxlen, fmt, ap);
	va_end(ap);
...
static void eapol_auth_tx_req(struct eapol_state_machine *sm)
{
	...
	if ((unsigned int) sm->flags & EAPOL_SM_WAIT_START) {
...
SM_STATE(AUTH_PAE, DISCONNECTED)
{
	...
	if (!from_initialize) {
		sm->eapol->cb.finished(sm->eapol->conf.ctx, sm->sta, 0,
				       (int) ((unsigned int) sm->flags & EAPOL_SM_PREAUTH),
				       sm->remediation);
	}
}
...
SM_STATE(AUTH_PAE, HELD)
{
	...
	sm->authPortStatus = Unauthorized;
	setPortUnauthorized();
	sm->quietWhile = (int) sm->quietPeriod;
	sm->eapolLogoff = false;
	...
	sm->eapol->cb.finished(sm->eapol->conf.ctx, sm->sta, 0,
			       (int) ((unsigned int) sm->flags & EAPOL_SM_PREAUTH), sm->remediation);
}
...
SM_STATE(AUTH_PAE, AUTHENTICATED)
{
	...
	setPortAuthorized();
	sm->reAuthCount = 0;
	if ((unsigned int) sm->flags & EAPOL_SM_PREAUTH)
		extra = " (pre-authentication)";
	else if ((unsigned int) sm->flags & EAPOL_SM_FROM_PMKSA_CACHE)
		extra = " (PMKSA cache)";
	eapol_auth_vlogger(sm->eapol, sm->addr, EAPOL_LOGGER_INFO,
...
SM_STATE(BE_AUTH, RESPONSE)
{
	...
	sm->eapolEap = false;
	sm->eap_if->eapNoReq = false;
	sm->aWhile = (int) sm->serverTimeout;
...
SM_STATE(REAUTH_TIMER, INITIALIZE)
{
	...
	sm->reAuthWhen = (int) sm->reAuthPeriod;
}
...
SM_STEP(AUTH_KEY_TX)
{
	...
	case AUTH_KEY_TX_NO_KEY_TRANSMIT:
		if (sm->keyTxEnabled && sm->eap_if->eapKeyAvailable &&
		    sm->keyRun && !((unsigned int) sm->flags & EAPOL_SM_USES_WPA))
		 ...
		 const char *identity, const char *radius_cui)
{
	...
	sm->reAuthPeriod = (unsigned int) eapol->conf.eap_reauth_period;
...
static struct eapol_callbacks eapol_cb =				//MODIFY_SUPPLICANT_FOR_FREERTOS 
{
...
void eapol_callback_init(void)
{
	eapol_cb.get_eap_user = eapol_sm_get_eap_user;
	eapol_cb.get_eap_req_id_text = eapol_sm_get_eap_req_id_text;
}
			...
			const char *value)
{
	...
	if (os_strcasecmp(param, "quietPeriod") == 0) {
		sm->quietPeriod = (unsigned int) atoi(value);
		return 0;
	}

	if (os_strcasecmp(param, "serverTimeout") == 0) {
		sm->serverTimeout = (unsigned int) atoi(value);
		return 0;
	}

	if (os_strcasecmp(param, "reAuthPeriod") == 0) {
		sm->reAuthPeriod = (unsigned int) atoi(value);
		return 0;
	}
