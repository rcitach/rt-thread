				...
				size_t *out_len, const char *table, int add_pad)
{
	...
	if ((unsigned int) add_pad & BASE64_LF)
	...
	while (end - in >= 3) {
		...
		*pos++ = table[in[2] & 0x3f];
		in += 3;
		line_len += 4;
		if (((unsigned int) add_pad & BASE64_LF) && line_len >= 72) {
			*pos++ = '\n';
			line_len = 0;
		}
	}
		...
		*pos++ = table[(in[0] >> 2) & 0x3f];
		if (end - in == 1) {
			*pos++ = table[((in[0] & 0x03) << 4) & 0x3f];
			if ((unsigned int) add_pad & BASE64_PAD)
				*pos++ = '=';
		} else {
	...
	if (((unsigned int) add_pad & BASE64_LF) && line_len)
		*pos++ = '\n';
