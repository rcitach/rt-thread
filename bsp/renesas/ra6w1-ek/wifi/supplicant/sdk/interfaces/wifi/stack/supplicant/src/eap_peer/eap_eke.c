					  ...
					  size_t payload_len)
{
	...
	wpa_hexdump_ascii(MSG_DEBUG, "EAP-EKE: Server Identity",
			  pos, (size_t) (end - pos));
	os_free(data->serverid);
	data->serverid = os_memdup(pos, (size_t) (end - pos));
	if (data->serverid == NULL) {
		return eap_eke_build_fail(data, ret, id,
					  EAP_EKE_FAIL_PRIVATE_INTERNAL_ERROR);
	}
	data->serverid_len = (size_t) (end - pos);
					      ...
					      size_t payload_len)
{
	...
	resp = eap_eke_build_msg(data, id,
				 (size_t) (data->sess.dhcomp_len + data->sess.pnonce_len),
				 EAP_EKE_COMMIT);
	if (resp == NULL) {
		forced_memzero(key, sizeof(key));
		return eap_eke_build_fail(data, ret, id,
	...
	rpos = wpabuf_put(resp, (size_t) data->sess.dhcomp_len);
	...
	wpa_hexdump(MSG_DEBUG, "EAP-EKE: DHComponent_P",
		    rpos, (size_t) data->sess.dhcomp_len);

	if (random_get_bytes(data->nonce_p, (size_t) data->sess.nonce_len)) {
		wpabuf_free(resp);
		return eap_eke_build_fail(data, ret, id,
					  EAP_EKE_FAIL_PRIVATE_INTERNAL_ERROR);
	}
	...
	if (eap_eke_prot(&data->sess, data->nonce_p, (size_t) data->sess.nonce_len,
					       ...
					       size_t payload_len)
{
	...
	if (eap_eke_decrypt_prot(&data->sess, pos, (size_t) data->sess.pnonce_ps_len,
	...
	wpa_hexdump_key(MSG_DEBUG, "EAP-EKE: Received Nonce_P | Nonce_S",
			nonces, (size_t) (2 * data->sess.nonce_len));
	if (os_memcmp(data->nonce_p, nonces, (size_t) data->sess.nonce_len) != 0) {
	...
	wpa_hexdump(MSG_DEBUG, "EAP-EKE: Auth_S", auth_s, (size_t) data->sess.prf_len);
	...
	resp = eap_eke_build_msg(data, id,
				 (size_t) (data->sess.pnonce_len + data->sess.prf_len),
				 EAP_EKE_CONFIRM);
	if (resp == NULL) {
		return eap_eke_build_fail(data, ret, id,
					  EAP_EKE_FAIL_PRIVATE_INTERNAL_ERROR);
	...
	prot_len = wpabuf_tailroom(resp);
	if (eap_eke_prot(&data->sess, data->nonce_s, (size_t) data->sess.nonce_len,
			 wpabuf_put(resp, 0), &prot_len) < 0) {
		wpabuf_free(resp);
		return eap_eke_build_fail(data, ret, id,
					  EAP_EKE_FAIL_PRIVATE_INTERNAL_ERROR);
	...
	auth = wpabuf_put(resp, (size_t) data->sess.prf_len);
	if (eap_eke_auth(&data->sess, "EAP-EKE peer", data->msgs, auth) < 0) {
		wpabuf_free(resp);
		return eap_eke_build_fail(data, ret, id,
					  EAP_EKE_FAIL_PRIVATE_INTERNAL_ERROR);
	}
	wpa_hexdump(MSG_DEBUG, "EAP-EKE: Auth_P", auth, (size_t) data->sess.prf_len);
				       ...
				       const struct wpabuf *reqData)
{
	...
	wpa_hexdump(MSG_DEBUG, "EAP-EKE: Received Data", pos, (size_t) (end - pos));
	...
	switch (eke_exch) {
	case EAP_EKE_ID:
		resp = eap_eke_process_id(data, ret, reqData, pos, (size_t) (end - pos));
		break;
	case EAP_EKE_COMMIT:
