					    ...
					    struct eap_peap_data *data, u8 id)
{
	...
	wpabuf_put_u8(req, (u8) (EAP_TLS_FLAGS_START | data->peap_version));
						 ...
						 u8 id)
{
	...
	if (data->peap_version == 0 && data->tlv_request == TLV_REQ_SUCCESS &&
		...
		wpabuf_put_u8(buf, (u8) data->recv_version); /* RecvVersion */
						  ...
						  u8 id, int success)
{
	...
	hdr->identifier = id;
	hdr->length = host_to_be16((unsigned short) req_len);
					...
					struct wpabuf *in_data)
{
	...
	while (left >= 4) {
		...
		case EAP_TLV_RESULT_TLV:
			result_tlv = pos;
			result_tlv_len = (size_t) tlv_len;
			break;
		case EAP_TLV_CRYPTO_BINDING_TLV:
			crypto_tlv = pos;
			crypto_tlv_len = (size_t) tlv_len;
			break;
		default:
