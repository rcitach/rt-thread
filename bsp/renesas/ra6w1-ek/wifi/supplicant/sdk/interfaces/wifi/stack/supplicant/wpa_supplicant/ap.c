				 ...
				 struct hostapd_config *conf)
{
	...
	if (mode->he_capab[wpas_mode_to_ieee80211_mode(ssid->mode)].he_supported 
		&& ssid->he
		)
		conf->ieee80211ax = 1;
			      ...
			      struct hostapd_config *conf)
{
	...
	if (ssid->acs == 1 || ssid->frequency == 0) {
        if (ssid->wifi_mode <= WIFI_MODE_B_ONLY) {
    		if (ssid->wifi_mode == WIFI_MODE_B_ONLY) {
                ...
                if (ssid->wifi_mode == WIFI_MODE_G_ONLY ||
...
#endif /* CONFIG_ACS */
    {
		...
		if (conf->hw_mode == HOSTAPD_MODE_IEEE80211A) { // 5Ghz
			if (ssid->wifi_mode < WIFI_MODE_AN) {
				ra6wx_notice_prt(ANSI_COLOR_LIGHT_YELLOW "\nChange: wifi_mode(%d-->%d) for WLAN1.\n\n" ANSI_COLOR_DEFULT,
								ssid->wifi_mode, WIFI_MODE_AN);
				ssid->wifi_mode = WIFI_MODE_AN;
			}
		} else if (conf->hw_mode == HOSTAPD_MODE_IEEE80211B || conf->hw_mode == HOSTAPD_MODE_IEEE80211G) { // 2.4Ghz
			if (ssid->wifi_mode >= WIFI_MODE_AN) {
				if (conf->hw_mode == HOSTAPD_MODE_IEEE80211G) {
					ra6wx_notice_prt(ANSI_COLOR_LIGHT_YELLOW "\nChange: wifi_mode(%d-->%d) for WLAN1.\n\n" ANSI_COLOR_DEFULT,
								ssid->wifi_mode, WIFI_MODE_GN);
					ssid->wifi_mode = WIFI_MODE_GN;
				} else {
					ra6wx_notice_prt(ANSI_COLOR_LIGHT_YELLOW "\nChange: wifi_mode(%d-->%d) for WLAN1.\n\n" ANSI_COLOR_DEFULT,
								ssid->wifi_mode, WIFI_MODE_BGN);
					ssid->wifi_mode = WIFI_MODE_BGN;
				}
			}
		}
        ...
        if (wpa_s->setband_mask == WPA_SETBAND_2G && ssid->wifi_mode <= WIFI_MODE_B_ONLY) { // 11bgn 2.4Ghz
    		if (conf->wifi_mode == WIFI_MODE_BG ||
    			conf->wifi_mode == WIFI_MODE_G_ONLY ||
    			conf->wifi_mode == WIFI_MODE_B_ONLY) {
    			ssid->disable_ht = 1; // 11bg
    		} else {
    			ssid->disable_ht = 0; // 11n
    		}
        ...
        } else if(wpa_s->setband_mask == WPA_SETBAND_5G && ssid->wifi_mode >= WIFI_MODE_AN) { // 11an 5Ghz
            if (ssid->wifi_mode == WIFI_MODE_A_ONLY) {
                ssid->disable_ht = 1; // 11a
            } else {
                ssid->disable_ht = 0; // 11n
            }
            conf->hw_mode = HOSTAPD_MODE_IEEE80211A;
			...
			// 11BGN
			conf->wifi_mode = ssid->wifi_mode = WIFI_MODE_BGN;
			conf->hw_mode = HOSTAPD_MODE_IEEE80211G;
			ssid->disable_ht = 0; // 11n
		} else if (wpa_s->setband_mask == WPA_SETBAND_5G) {
			// 11AN
			conf->wifi_mode = ssid->wifi_mode = WIFI_MODE_AN;
			conf->hw_mode = HOSTAPD_MODE_IEEE80211A;
			ssid->disable_ht = 0; // 11n
		}
...
#ifdef CONFIG_AP_WIFI_MODE
		if(ssid->wifi_mode == WIFI_MODE_N_ONLY || 
				  ...
				  struct hostapd_config *conf)
{
    ...
    if (ssid->acs || ssid->frequency == 0) {
		wpa_printf(MSG_DEBUG, "Use automatic channel selection acs:%d freq:%d ", ssid->acs, ssid->frequency);
	...
	if (ieee80211_is_dfs(ssid->frequency, wpa_s->hw.modes,
        ...
        wpa_printf(MSG_DEBUG,"[%s:%d] OPS channel is a DFS channel \n", __func__, __LINE__);
		conf->ieee80211h = 1;
		conf->ieee80211d = 1;
	...
	bss->ssid.ssid_set = 1;
	 ...
	 if(ssid->mode == WPAS_MODE_P2P_GO ||
	     ssid->mode == WPAS_MODE_P2P_GROUP_FORMATION)
	 {
		wpa_s->conf->p2p_ssid = ssid->ssid;
		wpa_s->conf->p2p_ssid_len= ssid->ssid_len;
	 }
#endif
	...
	if (ssid->key_mgmt == WPA_KEY_MGMT_OWE) {
		...
		if(wpa_s->conf->owe_transition_mode == 1){	
			...
			memset(bss->owe_transition_ifname, 0, sizeof(bss->iface));
	...
	if (wpa_key_mgmt_wpa_psk(ssid->key_mgmt)||(ssid->key_mgmt == WPA_KEY_MGMT_OWE))
		bss->wpa = ssid->proto;
...
{
#if 0	//MODIFY_SUPPLICANT_FOR_FREERTOS - chg
...
static void wpas_ap_configured_cb(void *ctx)
{
	...
	if ((wpa_s->conf->ssid->mode == WPAS_MODE_AP) &&
	    (wpa_s->conf->ssid->frequency == 0))
		supplicant_done();

#if (ATCMD_IF_SUPPORT == 1)
	extern void atcmd_print_initdone_softap_mode(void);
	atcmd_print_initdone_softap_mode();
#endif
}
			     ...
			     struct wpa_ssid *ssid)
{
    ...
    if (wpa_s->setband_mask == WPA_SETBAND_5G) {
        int res;
        int min_ch;
        if (i3ed11_freq_to_ch(ssid->frequency) <  36) {
            res = ra6w1_regdb_get_ch_range_by_country_n_band(wpa_s->conf->country, 5, &min_ch, NULL, NULL, 0);
            if (res == pdTRUE) {
                ssid->frequency = i3ed11_ch_to_freq(min_ch, 1);
...
#ifdef CONFIG_ACS
	if (ssid->frequency == 0) {
		ra6wx_notice_prt(">>> AP Operating Channel: AUTO\n");
	}
	else
#endif /* CONFIG_ACS */
	{
        if (ra6w1_regdb_is_valid_freq(wpa_s->conf->country, ssid->frequency, 0)) {
			ra6wx_notice_prt(">>> AP Operating Channel: %d(%d)\n",
			i3ed11_freq_to_ch(ssid->frequency), ssid->frequency);
        } else {
#ifdef CONFIG_ACS
            ra6wx_notice_prt(">>> AP ch Err: %d(%d) CCODE: %s, Change to AUTO ch\n",
            i3ed11_freq_to_ch(ssid->frequency), ssid->frequency, wpa_s->conf->country);
#else
            ra6wx_notice_prt(">>> AP ch Err: %d(%d) CCODE: %s, Change to default ch\n",
            i3ed11_freq_to_ch(ssid->frequency), ssid->frequency, wpa_s->conf->country);
#endif /* CONFIG_ACS */
            ssid->frequency = 0;
        }
...
#endif	// CONFIG_P2P_UNUSED_CMD
	if (params.p2p && (wpa_s->drv_flags & WPA_DRIVER_FLAGS_AP_UAPSD))
	...
	if (ieee80211_is_dfs(params.freq.freq, wpa_s->hw.modes,
			     wpa_s->hw.num_modes)) {
        wpa_printf(MSG_DEBUG, " set channel after CAC \n");
	...
	//hapd_iface->smps_modes = wpa_s->drv_smps_modes;		 // MODIFY_RENESAS_ONLY ???
...
#if defined(CONFIG_P2P) && defined(CONFIG_ACS) //MODIFY_SUPPLICANT_FOR_FREERTOS
		...
		char *country)
{
	...
	hapd = wpa_s->ap_iface->bss[0];
    conf = hapd->iconf;
	os_memset(hapd->iconf->country, 0, sizeof(conf->country));
...
char * ap_ctrl_iface_get_country(struct wpa_supplicant *wpa_s)
{
	...
	if (wpa_s->ap_iface == NULL)
		return NULL;
	conf = wpa_s->ap_iface->conf;

    if (conf->country[0]) {
        temp_country[0] = conf->country[0];
        temp_country[1] = conf->country[1];
    } else {
        return NULL;
    }
...
extern struct wpa_supplicant *select_sta0(struct wpa_supplicant *wpa_s);
...
int ap_ctrl_iface_ap(struct wpa_supplicant *wpa_s, char *cmd, char *buf, size_t buflen)
{
	...
	if (os_strcasecmp(cmd, "START") == 0) {
		if (wpa_s->ap_iface) {
			ra6wx_err_prt("AP iface is already running.\n");
			return -1;
		}
        ...
        if (wpa_s->ap_iface->conf->channel >= 36) {
            set_freq = i3ed11_ch_to_freq(wpa_s->ap_iface->conf->channel, FC80211_BAND_5GHZ);
        } else {
            set_freq = i3ed11_ch_to_freq(wpa_s->ap_iface->conf->channel, FC80211_BAND_2GHZ);
        }
#endif
    } else if (os_strcasecmp(cmd, "HELP") == 0) {
	    ra6wx_notice_prt("Usage: wpa_cli ap <start|stop|restart>\n");
	    ra6wx_notice_prt("                   start   - Start AP\n");
	    ra6wx_notice_prt("                   stop    - Stop AP\n");
	    ra6wx_notice_prt("                   restart - Restart AP with previous working channel\n");
		goto end;
	} else {
        int param_len = strlen(cmd);
        int freq_val = (int)atoi(cmd);

        ra6wx_ap_prt("[%s:%d] cmd=%s, param_len=%d, freq_val=%d \n", __func__, __LINE__,
                              cmd,    param_len,    freq_val);
        // Check freq_value
        if (freq_val == 0) {
            goto wrong_arg;
        }

        if (param_len == 4) { // frequency val
            set_freq = freq_val;
        } else if (param_len >= 1 && param_len <= 3) { // channel num
            set_freq = i3ed11_ch_to_freq(freq_val, (freq_val >= 36)?FC80211_BAND_5GHZ:FC80211_BAND_2GHZ);     
        } else {
wrong_arg :
            ra6wx_err_prt("Wrong argument !!!\n");
			return -1;
        }

        ra6wx_ap_prt("[%s:%d] wpa_s->conf->country=%s, set_freq=%d \n", __func__, __LINE__, 
                              wpa_s->conf->country, set_freq);

        if (chk_channel_by_country(wpa_s->conf->country, set_freq, 1, NULL, 1) < 0) {
            return -1;
        }

        if (set_freq != 0 && set_freq == ssid_p->frequency) {
            ra6wx_err_prt("Same Frequency\n");
            return -1;
        }
	}
	...
	if (   get_run_mode() == WIFI_DEVICE_MODE_EXT_AP_STATION
		...
		if (sta_wpa_s->current_bss) {
			if (set_freq != sta_wpa_s->current_bss->freq) {
				ra6wx_err_prt("Input ch is different from STA ch.\n");
				return -1;
			} 
		}
#ifdef CONFIG_CONCURRENT
		else if (get_run_mode() == WIFI_DEVICE_MODE_EXT_AP_STATION
				&& sta_wpa_s->wpa_state >= WPA_SCANNING
				&& sta_wpa_s->wpa_state < WPA_COMPLETED) { // connecting
			ra6wx_err_prt("Can't change channel (station is connecting)\n");
			return -1;
		}
#endif // CONFIG_CONCURRENT

		if (set_freq == 0) {
			ra6wx_err_prt("Not support the ACS in concurrent mode.\n");
			return -1;
		}
	}
#endif	/* CONFIG_CONCURRENT */
	...
	#define MAX_RETRY_CREATE_AP 5
	for (int retry = 0; retry < MAX_RETRY_CREATE_AP; retry++) {
		int old_acs = 0;
		...
		if (retry > 0) {
			ra6wx_notice_prt("[%s] Create AP: OK\n", __func__);
		}
		break;
	}
	...
	{
		ra6wx_fatal_prt("[%s] Buffer size is small\n", __func__);
	}
	return 0; // reply_len
}
...
const char * ap_get_state(struct wpa_supplicant *wpa_s)
{
	...
	return hostapd_get_state(wpa_s->ap_iface->bss[0]);
}
...
//#ifdef CONFIG_CTRL_IFACE	//MODIFY_SUPPLICANT_FOR_FREERTOS
...
int ap_ctrl_iface_sta_disassociate(struct wpa_supplicant *wpa_s,
				   const char *txtaddr)
{
	u8 mac_addr[ETH_ALEN]; //MODIFY_SUPPLICANT_FOR_FREERTOS - add
	...
	{
		ra6wx_err_prt("[%s] Invalid disassociate MAC Address '%s'\n", __func__, txtaddr);
		return -1;
	}
...
int ap_ctrl_iface_isolate(struct wpa_supplicant *wpa_s, const int isolate)
{
	if (wpa_s->ap_iface == NULL)
		return -1;
	return hostapd_drv_set_ap_isolate(wpa_s->ap_iface->bss[0], isolate);
}
#endif /* CONFIG_AP_ISOLATION */
...
#endif /* CONFIG_MESH */
			{
				for (int i = 0; wpa_s->conf->sae_groups[i]; i++) {
		...
		if (power > 0) {
			for (i = 0; i < NUM_SUPPORTS_COUNTRY; i++) {
				...
				if ((power < min_power) || (power > max_pwr)) {
					ra6wx_err_prt("Country Code [%s] ap_power [%d mW ~ %d mW]\n",
...
#if 0
ret_error:
	ra6wx_err_prt("ap_power [\"auto\" / power(mW)]\n");
	return -1;
#endif	/* 0 */
}
...
int ap_ctrl_iface_get_ap_power(struct wpa_supplicant *wpa_s, char *ap_pwr)
{
	...
	{
		ra6w1_cli_prt("AP power is = %d dbm\n", *get_power);		
		ret = *get_power;
	}
...
int ap_ctrl_iface_wmm_params(struct wpa_supplicant *wpa_s, char *pos)
{
	...
	if ((ecwmin == -1 || ecwmax == -1) || ecwmin >= ecwmax) {
		ra6wx_err_prt("Invalid CW Parameter! "
			     "(1, 3, 7, 15, 31, 63, "
			     "127, 255, 511, 1023)\n");
		goto invalid_params;
	}
...
invalid_cmd:
	ra6wx_warn_prt("Invalid Cmd : cli wmm_params "
...
int ap_ctrl_iface_ap_max_inactivity(struct wpa_supplicant *wpa_s, char *pos)
{
	...
	if (rm_wifi_get_int_val_from_str(pos, &val, POL_1) != 0) {
		goto err_rtn;
	}
...
err_rtn :
	...
	if (wpa_s->ap_iface != NULL) {
		...
		/*
		 * NOTE: It's only for AP interface is not starting.
		 * The configured ap_max_inactivity will be changed to default value(hostapd_config_defaults_bss()),
		 * while AP is starting.  It's because there is no operation to read configuration file.
		 * In order to apply it, struct wpa_ssid->ap_max_inactivity should be setup.
		 */
		struct wpa_ssid *ssid_p = NULL;
		...
		if (ssid_p == NULL) {
			ra6wx_ap_prt("[%s] Not found SSID configured for AP mode\n", __func__);
			return 0;
		}

		ssid_p->ap_max_inactivity = val;
	}
					      ...
					      struct dfs_event *radar)
{
	...
	hostapd_dfs_pre_cac_expired(iface, radar->freq, radar->ht_enabled,
