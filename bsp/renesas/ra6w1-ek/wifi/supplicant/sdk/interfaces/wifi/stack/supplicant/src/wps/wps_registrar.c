					     ...
					     struct wpabuf *msg)
{
	...
	if (reg->sel_reg_dev_password_id_override >= 0)
		id = (u16) reg->sel_reg_dev_password_id_override;
					    ...
					    struct wpabuf *msg)
{
	...
	methods &= (u16) ~WPS_CONFIG_PUSHBUTTON;
	methods &= (u16) ~(WPS_CONFIG_VIRT_PUSHBUTTON |
		     WPS_CONFIG_PHY_PUSHBUTTON);
	if (reg->pbc)
		wps_set_pushbutton(&methods, reg->wps->config_methods);
	if (reg->sel_reg_config_methods_override >= 0)
		methods = (u16) reg->sel_reg_config_methods_override;
					  ...
					  struct wpabuf *msg)
{
	 ...
	 * external Registrars.
	 */
	methods = (u16) (reg->wps->config_methods & ~WPS_CONFIG_PUSHBUTTON);
	methods &= (u16) ~(WPS_CONFIG_VIRT_PUSHBUTTON |
		     WPS_CONFIG_PHY_PUSHBUTTON);
...
static void wps_registrar_invalidate_unused(struct wps_registrar *reg)
{
	...
	dl_list_for_each(pin, &reg->pins, struct wps_uuid_pin, list) {
		if (pin->wildcard_uuid == 1 && !((u32) pin->flags & PIN_LOCKED)) {
			  ...
			  int timeout)
{
	...
	if (timeout) {
		p->flags = (int) ((u32) p->flags | PIN_EXPIRES);
...
int wps_registrar_unlock_pin(struct wps_registrar *reg, const u8 *uuid)
{
	...
	dl_list_for_each(pin, &reg->pins, struct wps_uuid_pin, list) {
		if (os_memcmp(pin->uuid, uuid, WPS_UUID_LEN) == 0) {
			...
			pin->flags = (int) ((u32) pin->flags & ~PIN_LOCKED);
			return 0;
		}
	}
			       ...
			       const struct wps_credential *cred)
{
	...
	wpabuf_put_be16(msg, (u16) cred->ssid_len);
				      ...
				      const struct wps_credential *cred)
{
	...
	wpabuf_put_be16(msg, (u16) cred->key_len);
			      ...
			      const struct wps_credential *cred)
{
	...
	wpabuf_put_be16(msg, (u16) wpabuf_len(wbuf));
...
use_provided:
	...
	wpabuf_put_be16(msg, ATTR_CRED);
	wpabuf_put_be16(msg, (u16) wpabuf_len(cred));
	wpabuf_put_buf(msg, cred);
	wpabuf_clear_free(cred);
...
static struct wpabuf * wps_build_ap_cred(struct wps_data *wps)
{
	...
	wpabuf_put_be16(msg, ATTR_CRED);
	wpabuf_put_be16(msg, (u16) wpabuf_len(plain));
...
static struct wpabuf * wps_build_m2(struct wps_data *wps)
{
	...
	if (wps_build_version(msg) ||
	    ...
	    wps_build_rf_bands(&wps->wps->dev, msg,
			       (u8) wps->wps->rf_band_cb(wps->wps->cb_ctx)) ||
					      ...
					      u16 dev_pw_id)
{
	...
	if (reg->selected_registrar) {
		...
		methods = reg->wps->config_methods & (u16) ~WPS_CONFIG_PUSHBUTTON;
		methods &= (u16) ~(WPS_CONFIG_VIRT_PUSHBUTTON |
			     WPS_CONFIG_PHY_PUSHBUTTON);
		if (reg->pbc) {
