...
int wps_build_public_key(struct wps_data *wps, struct wpabuf *msg)
{
	...
	wpabuf_put_be16(msg, (u16) wpabuf_len(pubkey));
		      ...
		      u8 multi_ap_subelem)
{
	...
	if (wpabuf_tailroom(msg) <
	    (size_t) (7U + 3U + (req_to_enroll ? 3U : 0) +
	    (auth_macs ? 2U + auth_macs_count * ETH_ALEN : 0)))
		return -1;
	...
	if (auth_macs && auth_macs_count) {
		...
		wpabuf_put_u8(msg, (u8) (auth_macs_count * ETH_ALEN));
	...
	WPA_PUT_BE16(len, (u16) ((u8 *) wpabuf_put(msg, 0) - len - 2));
...
int wps_build_auth_type_flags(struct wps_data *wps, struct wpabuf *msg)
{
	...
	auth_types &= (u16) ~WPS_AUTH_WPA;
	auth_types &= (u16) ~WPS_AUTH_WPA2;
	auth_types &= (u16) ~WPS_AUTH_SHARED;
			    ...
			    struct wpabuf *plain)
{
	...
	os_memset(wpabuf_put(plain, pad_len), (int) pad_len, pad_len);

	wpabuf_put_be16(msg, ATTR_ENCR_SETTINGS);
	wpabuf_put_be16(msg, (u16) (block_size + wpabuf_len(plain)));
...
struct wpabuf * wps_ie_encapsulate(struct wpabuf *data)
{
	...
	while (end > pos) {
		size_t frag_len = (size_t) (end - pos);
		if (frag_len > 251)
			frag_len = 251;
		wpabuf_put_u8(ie, WLAN_EID_VENDOR_SPECIFIC);
		wpabuf_put_u8(ie, (u8) (4 + frag_len));
