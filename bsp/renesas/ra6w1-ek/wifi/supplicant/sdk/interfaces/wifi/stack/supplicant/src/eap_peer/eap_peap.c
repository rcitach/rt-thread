...
int ra6w1_peap_version = EAP_PEAP_VERSION;
...
static void * eap_peap_init(struct eap_sm *sm)
{
    ...
    data->peap_version = ra6w1_peap_version;
				     ...
				     struct wpabuf *buf)
{
	...
	wpabuf_put_u8(buf, (u8) data->peap_version); /* Version */
	wpabuf_put_u8(buf, (u8) data->peap_version); /* RecvVersion */
			   ...
			   int force_failure)
{
	...
	if (result_tlv) {
		...
		*resp = eap_tlv_build_result(sm, data, crypto_tlv != NULL,
					     eap_get_id(req), (u16) resp_status);
	}
...
continue_req:
	...
	if (data->peap_version == 0 && !skip_change) {
		...
		nhdr->length = host_to_be16((unsigned short) (sizeof(struct eap_hdr) +
					    wpabuf_len(in_decrypted)));
