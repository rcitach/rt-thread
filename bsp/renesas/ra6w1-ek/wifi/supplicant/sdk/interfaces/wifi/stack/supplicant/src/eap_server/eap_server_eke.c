					    ...
					    struct eap_eke_data *data, u8 id)
{
	...
	msg = eap_eke_build_msg(data, id, (size_t) data->sess.dhcomp_len,
				EAP_EKE_COMMIT);
	if (msg == NULL) {
		eap_eke_fail(data, EAP_EKE_FAIL_PRIVATE_INTERNAL_ERROR);
		return eap_eke_build_failure(data, id);
	...
	if (eap_eke_dhcomp(&data->sess, data->key, pub,
			   wpabuf_put(msg, (size_t) data->sess.dhcomp_len))
	    < 0) {
					     ...
					     struct eap_eke_data *data, u8 id)
{
	...
	plen = (size_t) (data->sess.pnonce_ps_len + data->sess.prf_len);
	...
	if (random_get_bytes(data->nonce_s, (size_t) data->sess.nonce_len)) {
		wpabuf_free(msg);
		eap_eke_fail(data, EAP_EKE_FAIL_PRIVATE_INTERNAL_ERROR);
		return eap_eke_build_failure(data, id);
	}
	wpa_hexdump_key(MSG_DEBUG, "EAP-EKE: Nonce_S",
			data->nonce_s, (size_t) data->sess.nonce_len);

	os_memcpy(nonces, data->nonce_p, (size_t) data->sess.nonce_len);
	os_memcpy(nonces + data->sess.nonce_len, data->nonce_s,
		  (size_t) data->sess.nonce_len);
	prot_len = wpabuf_tailroom(msg);
	if (eap_eke_prot(&data->sess, nonces, (size_t) (2 * data->sess.nonce_len),
	...
	auth = wpabuf_put(msg, (size_t) data->sess.prf_len);
	if (eap_eke_auth(&data->sess, "EAP-EKE server", data->msgs, auth) < 0) {
		wpabuf_free(msg);
		eap_eke_fail(data, EAP_EKE_FAIL_PRIVATE_INTERNAL_ERROR);
		return eap_eke_build_failure(data, id);
	}
	wpa_hexdump(MSG_DEBUG, "EAP-EKE: Auth_S", auth, (size_t) data->sess.prf_len);
				     ...
				     const u8 *payload, size_t payloadlen)
{
	...
	os_free(data->peerid);
	data->peerid = os_memdup(pos, (size_t) (end - pos));
	if (data->peerid == NULL) {
		wpa_printf(MSG_INFO, "EAP-EKE: Failed to allocate memory for peerid");
		eap_eke_fail(data, EAP_EKE_FAIL_PRIVATE_INTERNAL_ERROR);
		return;
	}
	data->peerid_len = (size_t) (end - pos);
				   ...
				   const u8 *payload, size_t payloadlen)
{
	...
	wpa_hexdump(MSG_DEBUG, "EAP-EKE: DHComponent_P",
		    pos, (size_t) data->sess.dhcomp_len);
	dhcomp = pos;
	pos += data->sess.dhcomp_len;
	wpa_hexdump(MSG_DEBUG, "EAP-EKE: PNonce_P", pos, (size_t) data->sess.pnonce_len);
	pnonce = pos;
	pos += data->sess.pnonce_len;
	wpa_hexdump(MSG_DEBUG, "EAP-EKE: CBValue", pos, (size_t) (end - pos));
	...
	if (eap_eke_decrypt_prot(&data->sess, pnonce, (size_t) data->sess.pnonce_len,
	...
	wpa_hexdump_key(MSG_DEBUG, "EAP-EKE: Nonce_P",
			data->nonce_p, (size_t) data->sess.nonce_len);
				    ...
				    const u8 *payload, size_t payloadlen)
{
	...
	if (eap_eke_decrypt_prot(&data->sess, payload, (size_t) data->sess.pnonce_len,
				 nonce, &decrypt_len) < 0) {
	...
	wpa_hexdump_key(MSG_DEBUG, "EAP-EKE: Received Nonce_S",
			nonce, (size_t) data->sess.nonce_len);
	if (os_memcmp(nonce, data->nonce_s, (size_t) data->sess.nonce_len) != 0) {
	...
	wpa_hexdump(MSG_DEBUG, "EAP-EKE: Auth_P", auth_p, (size_t) data->sess.prf_len);
	if (os_memcmp_const(auth_p, payload + data->sess.pnonce_len,
			    (size_t) data->sess.prf_len) != 0) {
			     ...
			     struct wpabuf *respData)
{
	...
	wpa_hexdump(MSG_DEBUG, "EAP-EKE: Received payload", pos, (size_t) (end - pos));

	switch (eke_exch) {
	case EAP_EKE_ID:
		eap_eke_process_identity(sm, data, respData, pos, (size_t) (end - pos));
		break;
	case EAP_EKE_COMMIT:
		eap_eke_process_commit(sm, data, respData, pos, (size_t) (end - pos));
		break;
	case EAP_EKE_CONFIRM:
		eap_eke_process_confirm(sm, data, respData, pos, (size_t) (end - pos));
		break;
	case EAP_EKE_FAILURE:
		eap_eke_process_failure(sm, data, respData, pos, (size_t) (end - pos));
		break;
	}
}
...
static u8 * eap_eke_get_session_id(struct eap_sm *sm, void *priv, size_t *len)
{
	...
	sid_len = (size_t) (1 + 2 * data->sess.nonce_len);
	sid = os_malloc(sid_len);
	if (sid == NULL)
		return NULL;
	sid[0] = EAP_TYPE_EKE;
	os_memcpy(sid + 1, data->nonce_p, (size_t) data->sess.nonce_len);
