...
#define ieee80211_ht_operation def_ieee80211_ht_operation   	// Avoiding collisions with rwnx_driver
#define ieee80211_vht_operation def_ieee80211_vht_operation		// Avoiding collisions with rwnx_driver
					    ...
					    int show_errors)
{
	...
	case OUI_MICROSOFT:
		...
		case WMM_OUI_TYPE:
			...
			case WMM_OUI_SUBTYPE_PARAMETER_ELEMENT:
				 ...
				 */
				elems->wmm = pos;
				elems->wmm_len = (u8) elen;
				break;
			case WMM_OUI_SUBTYPE_TSPEC_ELEMENT:
				elems->wmm_tspec = pos;
				elems->wmm_tspec_len = (u8) elen;
				break;
			default:
	...
	case OUI_WFA:
		...
		case P2P_OUI_TYPE:
			...
			elems->p2p = pos;
			elems->p2p_len = (u8) elen;
			break;
		case WFD_OUI_TYPE:
			/* Wi-Fi Alliance - WFD IE */
			elems->wfd = pos;
			elems->wfd_len = (u8) elen;
			break;
		case HS20_INDICATION_OUI_TYPE:
			/* Hotspot 2.0 */
			elems->hs20 = pos;
			elems->hs20_len = (u8) elen;
			break;
		case HS20_OSEN_OUI_TYPE:
			/* Hotspot 2.0 OSEN */
			elems->osen = pos;
			elems->osen_len = (u8) elen;
			break;
		case MBO_OUI_TYPE:
			/* MBO-OCE */
			elems->mbo = pos;
			elems->mbo_len = (u8) elen;
			break;
		case HS20_ROAMING_CONS_SEL_OUI_TYPE:
			/* Hotspot 2.0 Roaming Consortium Selection */
			elems->roaming_cons_sel = pos;
			elems->roaming_cons_sel_len = (u8) elen;
			break;
		case MULTI_AP_OUI_TYPE:
			elems->multi_ap = pos;
			elems->multi_ap_len = (u8) elen;
			break;
		case OWE_OUI_TYPE:
	...
	case OUI_BROADCOM:
		...
		case VENDOR_HT_CAPAB_OUI_TYPE:
			...
			elems->vendor_ht_cap_len = (u8) elen;
			break;
	...
	case OUI_QCA:
		...
		case QCA_VENDOR_ELEM_P2P_PREF_CHAN_LIST:
			...
			elems->pref_freq_list_len = (u8) elen;
			break;
		default:
				      ...
				      int show_errors)
{
	...
	case WLAN_EID_EXT_FILS_REQ_PARAMS:
		...
		elems->fils_req_params_len = (u8) elen;
		break;
	case WLAN_EID_EXT_FILS_KEY_CONFIRM:
		elems->fils_key_confirm = pos;
		elems->fils_key_confirm_len = (u8) elen;
		break;
...
const u8 * get_hdr_bssid(const struct _ieee80211_hdr *hdr, size_t len)
{
	u16 fc, type, stype;
...
enum hostapd_hw_mode ieee80211_freq_to_chan(int freq, u8 *channel)
{
	...
	return ieee80211_freq_to_channel_ext((unsigned int) freq, 0, CHANWIDTH_USE_HT,
					     &op_class, channel);
}
						   ...
						   u8 *op_class, u8 *channel)
{
	...
	if (freq > 5950 && freq <= 7115) {
		...
		*channel = (u8) ((freq - 5950) / 5);
		return HOSTAPD_MODE_IEEE80211A;
	}

	if (freq == 5935) {
		*op_class = 136;
		*channel = (u8) ((freq - 5925) / 5);
		return HOSTAPD_MODE_IEEE80211A;
	}
#endif	// CONFIG_5G_SUPPORT
...
unsigned int is_6ghz_freq(int freq)
{
	...
	if (center_idx_to_bw_6ghz((u8) ((freq - 5950) / 5)) < 0)
		return false;
...
//MODIFY_SUPPLICANT_FOR_FREERTOS
unsigned int is_6ghz_op_class(u8 op_class)
{
...
//MODIFY_SUPPLICANT_FOR_FREERTOS
unsigned int is_6ghz_psc_frequency(int freq)
{
	int i;
				    ...
				    size_t nei_rep_len)
{
	...
	while (pos) {
		...
		*nei_pos++ = (u8) atoi(pos); /* Operating Class */
		pos = os_strchr(pos, ',');
		if (pos == NULL) {
		...
		*nei_pos++ = (u8) atoi(pos); /* Channel Number */
		pos = os_strchr(pos, ',');
		if (pos == NULL) {
		...
		*nei_pos++ = (u8) atoi(pos); /* PHY Type */
		...
		nei_start[1] = (u8) (nei_pos - nei_start - 2);
	}
...
//MODIFY_SUPPLICANT_FOR_FREERTOS
unsigned int ieee802_11_rsnx_capab_len(const u8 *rsnxe, size_t rsnxe_len,
			       unsigned int capab)
{
	const u8 *end;
	size_t flen, i;
...
//MODIFY_SUPPLICANT_FOR_FREERTOS
unsigned int ieee802_11_rsnx_capab(const u8 *rsnxe, unsigned int capab)
{
