...
extern int ra6w1_regdb_get_act_ch_info(enum ra6w1_band_group_all band, int ch, int req_flag);
...
acs_survey_interference_factor(struct freq_survey *survey, s8 min_nf)
{
	...
	factor = pow(10, survey->nf / 5.0L) +
		((fabs(total) < epsilon) ? 0 : (busy / total)) *
			 ...
			 long double *ideal_factor)
{
	...
	wpa_printf(MSG_DEBUG, "ACS: mode(%d) num_channels(%d) ", mode->mode, mode->num_channels);
	for (i = 0; i < mode->num_channels; i++) {
		double total_weight;
		...
		if (!chan_pri_allowed(chan)) {
			wpa_printf(MSG_DEBUG, "ACS: continue not allowed pri (flag:0x%x, allowed_bw:0x%x)", chan->flag, chan->allowed_bw);
			//continue;	// SKIP	TEMPORARY
		}

		if (!is_in_chanlist(iface, chan)) {
			wpa_printf(MSG_DEBUG, "ACS: continue not chan list ");
			continue;
		}

		if (!is_in_freqlist(iface, chan)) {
			wpa_printf(MSG_DEBUG, "ACS: continue not freq list ");
			continue;
		}

		if (chan->max_tx_power < iface->conf->min_tx_power) {
			wpa_printf(MSG_DEBUG, "ACS: continue not tx power ");
			continue;
		}
...
bw_selected:
	...
	wpa_printf(MSG_DEBUG, "ACS: num_hw_features :%d ", iface->num_hw_features);
	for (i = 0; i < iface->num_hw_features; i++) {
		mode = &iface->hw_features[i];
		if (!hostapd_hw_skip_mode(iface, mode)) {
			...
			wpa_printf(MSG_DEBUG, "ACS: skip (%d mode->mode:%d) ", i, mode->mode);
		}
	}
					...
					int *freq)
{
	struct hostapd_channel_data *chan;
	int i, pwr_val;
    enum ra6w1_band_group_all band;
	...
	for (i = 0; i < mode->num_channels; i++) {
        ...
        if (mode->mode < HOSTAPD_MODE_IEEE80211A) {
            band = RRQ61X_BND_GRP_ALL_2G_OFDM;
        } else {
            band = RRQ61X_BND_GRP_ALL_5G_OFDM;
        }
        
        pwr_val = ra6w1_regdb_get_act_ch_info(band, chan->chan, 0);
        if (pwr_val >= -3 && pwr_val <= -1) {
            continue;
        }

		*freq++ = chan->freq;
        wpa_printf(MSG_DEBUG, "[%s:%d] channel = %d added\n", __func__, __LINE__, chan->chan);
	}
...
static int acs_request_scan(struct hostapd_iface *iface)
{
    ...
    wpa_printf(MSG_DEBUG, "[%s:%d] num_channels = %d\n", __func__, __LINE__, num_channels);
	...
	for (i = 0; i < iface->num_hw_features; i++) {
		...
		if (!hostapd_hw_skip_mode(iface, mode)) {
            ...
            wpa_printf(MSG_DEBUG, "[%s:%d] hw_mode = %d skipped \n", __func__, __LINE__, mode->mode);
        }
	}
...
#ifdef CONFIG_AP
    /* Temporary - after 5GHz gets stablized, this code can be removed 
       after verification */
