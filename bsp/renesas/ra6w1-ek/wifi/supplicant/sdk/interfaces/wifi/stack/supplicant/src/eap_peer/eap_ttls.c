			      ...
			      struct ttls_parse_avp *parse)
{
	...
	avp_flags = (u8) ((avp_length >> 24) & 0xff);
	avp_length &= 0xffffff;
			       ...
			       struct ttls_parse_avp *parse)
{
	...
	while (left > 0) {
		...
		pos += (u8) ((size_t) avp_length + pad);
		if (left < (size_t) avp_length + pad)
			left = 0;
		else
			left -= ((size_t) avp_length + pad);
	}
			       ...
			       size_t buflen, int verbose)
{
	...
	case EAP_TTLS_PHASE2_EAP:
		ret = os_snprintf(buf + len, buflen - (size_t) len, "EAP-%s\n",
				  data->phase2_method ?
				  data->phase2_method->name : "?");
		break;
	case EAP_TTLS_PHASE2_MSCHAPV2:
		ret = os_snprintf(buf + len, buflen - (size_t) len, "MSCHAPV2\n");
		break;
	case EAP_TTLS_PHASE2_MSCHAP:
		ret = os_snprintf(buf + len, buflen - (size_t) len, "MSCHAP\n");
		break;
	case EAP_TTLS_PHASE2_PAP:
		ret = os_snprintf(buf + len, buflen - (size_t) len, "PAP\n");
		break;
	case EAP_TTLS_PHASE2_CHAP:
		ret = os_snprintf(buf + len, buflen - (size_t) len, "CHAP\n");
		break;
	default:
		ret = 0;
		break;
