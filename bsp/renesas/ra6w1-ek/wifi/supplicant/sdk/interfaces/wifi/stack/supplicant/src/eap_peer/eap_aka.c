...
static int eap_aka_ext_sim_req(struct eap_sm *sm, struct eap_aka_data *data)
{
	...
	pos += os_snprintf(pos, (size_t) (end - pos), "UMTS-AUTH");
	pos += os_snprintf(pos, (size_t) (end - pos), ":");
	pos += wpa_snprintf_hex(pos, (size_t) (end - pos), data->rand, EAP_AKA_RAND_LEN);
	pos += os_snprintf(pos, (size_t) (end - pos), ":");
	wpa_snprintf_hex(pos, (size_t) (end - pos), data->autn, EAP_AKA_AUTN_LEN);
					    ...
					    int err)
{
	...
	eap_sim_msg_add(msg, EAP_SIM_AT_CLIENT_ERROR_CODE, (u16) err, NULL, 0);
						 ...
						 enum eap_sim_id_req id_req)
{
	...
	if (identity) {
		...
		eap_sim_msg_add(msg, EAP_SIM_AT_IDENTITY, (u16) identity_len,
				identity, identity_len);
	}
						  ...
						  u8 id)
{
	...
	eap_sim_msg_add(msg, EAP_SIM_AT_RES, (u16) (data->res_len * 8),
					       ...
					       const u8 *nonce_s)
{
	...
	eap_sim_msg_add(msg, EAP_SIM_AT_COUNTER, (u16) counter, NULL, 0);
						     ...
						     u8 id, u16 notification)
{
	...
	if (k_aut && data->reauth) {
		...
		eap_sim_msg_add(msg, EAP_SIM_AT_COUNTER, (u16) data->counter,
				NULL, 0);
	...
	const struct wpabuf *reqData, struct eap_sim_attrs *attr)
{
	...
	return eap_aka_response_notification(data, id, (u16) attr->notification);
}
	...
	const struct wpabuf *reqData, struct eap_sim_attrs *attr)
{
	...
	if (eattr.counter < 0 || (size_t) eattr.counter <= data->counter) {
		...
		data->counter_too_small = (unsigned int) eattr.counter;
	...
	data->counter = (unsigned int) eattr.counter;
	...
	if (data->eap_method == EAP_TYPE_AKA_PRIME) {
		eap_aka_prime_derive_keys_reauth(data->k_re, (u16) data->counter,
						 data->reauth_id,
						 data->reauth_id_len,
						 data->nonce_s,
						 data->msk, data->emsk);
	} else {
		eap_sim_derive_keys_reauth((u16) data->counter, data->reauth_id,
					   data->reauth_id_len,
