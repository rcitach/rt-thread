...
static void wpabuf_overflow(const struct wpabuf *buf, size_t len)
{
	...
	wpa_printf(MSG_ERROR, "wpabuf %p (size=%lu used=%lu) overflow len=%lu",
		   ...
		   (unsigned long) len);
#if 0   /* by Bright : For WPA3 : Don't need control-interface on FC9000's supplicant */ //MODIFY_SUPPLICANT_FOR_FREERTOS
	wpa_trace_show("wpabuf overflow");
#endif	// 0
#if 0 //[[ trinity_0160928_BEGIN	//MODIFY_SUPPLICANT_FOR_FREERTOS
	abort();
#endif	// 0
}
...
void wpabuf_printf(struct wpabuf *buf, char *fmt, ...)
{
	...
	if (res < 0 || (size_t) res >= buf->size - buf->used)
		wpabuf_overflow(buf, (size_t) res);
	buf->used = buf->used + (size_t) res;
}
