		    ...
		    const char *reason)
{
	...
	wpa_s->num_bss--;
	wpa_dbg(wpa_s, MSG_EXCESSIVE, "BSS: Remove id %u BSSID " MACSTR
				    ...
				    struct os_reltime *fetch_time)
{
	...
	if (wpa_s->num_bss + 1 > wpa_s->conf->bss_max_count) {
		ra6wx_scan_prt("<%s> Fail to store bss info because of over bss max count (%d). Discard BSSID=%02x:%02x:%02x:%02x:%02x:%02x SSID=%s\n",
...
#ifdef UPDATE_REQUIRED_SSID_ACTIVATED_IN_SCAN_RESULTS
	else if (wpa_s->num_bss + 1 == wpa_s->conf->bss_max_count
#ifdef CONFIG_AP
		 && wpa_s->ap_iface == NULL && wpa_s->conf->ssid != NULL
#endif /* CONFIG_AP */
		...
		if (wpa_s->conf->ssid->ssid_len > 0) {
			...
			if (strncmp(wpa_ssid_plain_txt(wpa_s->conf->ssid->ssid, wpa_s->conf->ssid->ssid_len), (const char*)ssid, wpa_s->conf->ssid->ssid_len) == 0
				...
				ra6wx_scan_prt("<%s> Current SSID Update (BSSID=%02x:%02x:%02x:%02x:%02x:%02x SSID=%s)\n",
					__func__,
					res->bssid[0], res->bssid[1], res->bssid[2], res->bssid[3], res->bssid[4],res->bssid[5],
					wpa_ssid_plain_txt(ssid, ssid_len));
			} else {
	...
	wpa_s->num_bss++;
	wpa_dbg(wpa_s, MSG_EXCESSIVE, "BSS: Add new id %u BSSID " MACSTR
		" SSID '%s' freq %d",
...
{
	const u8 *old_ie = NULL;
	const u8 *new_ie = NULL;
...
#if 1 // FC9000 Only ???	//MODIFY_SUPPLICANT_FOR_FREERTOS - add
...
#endif	//MODIFY_SUPPLICANT_FOR_FREERTOS - add
	       ...
	       struct wpa_scan_res *res, struct os_reltime *fetch_time)
{
...
#else	// CONFIG_SCAN_RESULT_OPTIMIZE
	{
#ifndef CONFIG_SCAN_UMAC_HEAP_ALLOC
		if (bss == wpa_s->current_bss) {
			ra6wx_scan_prt("[%s] %02x:%02x:%02x:%02x:%02x:%02x - Update current bss 0x%x, current ie_len = %d. beacon_ie_len = %d\n",
					__func__,
					bss->bssid[0],
					bss->bssid[1],
					bss->bssid[2],
			...
			if (res->beacon_ie_len)  {
				...
				ra6wx_scan_prt("[%s] New beacon_ie_len = %d\n",__func__, bss->beacon_ie_len);
...
#else	// CONFIG_SCAN_UMAC_HEAP_ALLOC
		{
				...
				ieee80211_freq_to_chan(res->freq, &ch);
                ra6wx_scan_prt(ANSI_COLOR_LIGHT_YELLOW"[%s] "  // function
                        "%8d "              // signal level
                        "%7d "              // beacon interval
                        "       0x%04x "    // capability
                        "%9d "              //  signal quality
...
#if 0
#ifdef CONFIG_REUSED_UMAC_BSS_LIST	//MODIFY_SUPPLICANT_FOR_FREERTOS - add
	rdev_bss_lock();
#endif
#endif
	if (bss == NULL) {
			...
			int new_scan)
{
	...
	dl_list_for_each_safe(bss, n, &wpa_s->bss, struct wpa_bss, list) {
		if (wpa_bss_in_use(wpa_s, bss)) {
			...
			if ((portCONVERT_TICKS_2_MS(xTaskGetTickCount()) - (bss->last_update.sec * 1000 + bss->last_update.usec / 1000)) > 1000) {
				...
				new_bss_signal = get_sta_signal_poll();

				if (new_bss_signal < 0 && new_bss_signal == -WPA_INVALID_NOISE) {
					ULONG now = xTaskGetTickCount();
					bss->last_update.sec = (portCONVERT_TICKS_2_MS(now)) / 1000;
					bss->last_update.usec = ((now - portCONVERT_MS_2_TICKS(bss->last_update.sec*1000)) * 1000);
...
#endif /* 1 */	//MODIFY_SUPPLICANT_FOR_FREERTOS - add
...
#ifdef CONFIG_IMMEDIATE_SCAN		//MODIFY_SUPPLICANT_FOR_FREERTOS - add
...
void wpa_bss_flush(struct wpa_supplicant *wpa_s)
{
	...
	dl_list_for_each_safe(bss, n, &wpa_s->bss, struct wpa_bss, list) {
		if (wpa_bss_in_use(wpa_s, bss))
		{
			...
			ra6wx_scan_prt(ANSI_COLOR_GREEN "[%s]  Do not remove bss in use(0x%p) %2u BSSID " MACSTR
				" SSID '%s'\n" ANSI_COLOR_DEFULT,
