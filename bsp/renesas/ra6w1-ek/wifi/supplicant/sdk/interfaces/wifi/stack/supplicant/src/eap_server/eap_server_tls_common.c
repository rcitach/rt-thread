			    ...
			    int verify_peer, int eap_type)
{
	...
	data->tls_out_limit = (size_t) (sm->cfg->fragment_size > 0 ?
			       ...
			       size_t context_len, size_t len)
{
    ...
    if (tls_connection_export_key(sm->cfg->ssl_ctx, data->conn, label, out, len)) {
		os_free(out);
		return NULL;
	}
...
struct wpabuf * eap_server_tls_build_ack(u8 id, int eap_type, int version)
{
	...
	wpabuf_put_u8(req, (u8) version); /* Flags */
	return req;
}
				     ...
				     const u8 **pos, size_t *left)
{
	...
	if (data->tls_in &&
	    eap_server_tls_process_cont(data, *pos, (size_t) (end - *pos)) < 0)
		return -1;

	if (flags & EAP_TLS_FLAGS_MORE_FRAGMENTS) {
		if (eap_server_tls_process_fragment(data, flags, tls_msg_len,
						    *pos, (size_t) (end - *pos)) < 0)
			return -1;
	...
	if (data->tls_in == NULL) {
		...
		wpabuf_set(&data->tmpbuf, *pos, (size_t) (end - *pos));
