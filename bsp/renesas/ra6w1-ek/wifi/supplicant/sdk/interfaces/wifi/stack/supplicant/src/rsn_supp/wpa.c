...
#undef   DEBUG_PRINT_WPA_TK_KEY

#if defined DEBUG_PRINT_WPA_TK_KEY || defined ENABLE_PRINT_TK_KEY_FOR_WIRESHARK_DBG
#include "common_utils.h"
#endif // DEBUG_PRINT_WPA_TK_KEY || ENABLE_PRINT_TK_KEY_FOR_WIRESHARK_DBG
...
#if CFG_PMGR
static void wpa_supp_dpm_backup_key_info(enum backup_key type, enum wpa_alg alg,
			int key_idx, int set_tx, const u8 *seq, int seq_len,
			struct wpa_sm *sm, enum key_flag key_flag);
#endif /* CFG_PMGR */
...
{
	int ret = -1;
#ifdef  CONFIG_SUPP27_MIC_LEN
	size_t mic_len = wpa_mic_len(sm->key_mgmt, sm->pmk_len);
#endif	// CONFIG_SUPP27_MIC_LEN
	...
	wpa_printf(MSG_DEBUG, "WPA: Send EAPOL-Key frame to " MACSTR
			...
			(int) mic_len, 
#else
			wpa_mic_len(sm->key_mgmt, sm->pmk_len),
#endif	// CONFIG_SUPP27_MIC_LEN
			sm->key_mgmt);
			       ...
			       struct wpa_ptk *ptk)
{
	...
	ret = wpa_eapol_key_send(sm, ptk, ver, dst, ETH_P_EAPOL, rbuf, rlen, key_mic);
	TX_FUNC_END("");
...
#if 1   /* by Shingu 20170713 (4HS ALL FAIL) */	//MODIFY_SUPPLICANT_FOR_FREERTOS - add
	sm->cnt_hs_3rd = 0;
#endif  /* 1 */
...
#if CFG_PMGR
	wpa_supp_dpm_backup_key_info(KEY_PTK, alg, 0, 1, key_rsc, rsclen, sm, KEY_FLAG_PAIRWISE | key_flag); // Backup for PTK //MODIFY_SUPPLICANT_FOR_FREERTOS
#endif /* CFG_PMGR */
...
#if CFG_PMGR
	wpa_supp_dpm_backup_key_info(KEY_GTK, gd->alg, gd->keyidx, gd->tx, key_rsc, gd->key_rsc_len, sm, 0);   // Backup for PTK //MODIFY_SUPPLICANT_FOR_FREERTOS
#endif /* CFG_PMGR */
				       ...
				       int key_info)
{
	...
	if (DEBUG_SUPP_TMP)
        printf("RSN: received GTK in pairwise handshake");
...
//If scan result optimized, impossible to retrive beacon ie again, because it is already freed.
				      ...
				      int ver, u16 key_info)
{
	...
	ret = wpa_eapol_key_send(sm, &sm->ptk, ver, sm->bssid, ETH_P_EAPOL,
				  rbuf, rlen, key_mic);
		     ...
		     unsigned int value)
{
	...
	case WPA_PARAM_PROTO:
		sm->proto = value;
#if CFG_PMGR
		if (RM_PMGR_W_rtm_exist() == pdTRUE) {		// MODIFY_SUPPLICANT_FOR_FREERTOS - add
			dpm_supp_key_info->proto = value;
		}
#endif /* CFG_PMGR */
		break;
...
#if CFG_PMGR
		if (RM_PMGR_W_rtm_exist() == pdTRUE) {		// MODIFY_SUPPLICANT_FOR_FREERTOS - add
			dpm_supp_key_info->pairwise_cipher = value;
		}
#endif /* CFG_PMGR */
		break;
...
#if CFG_PMGR
		if (RM_PMGR_W_rtm_exist() == pdTRUE) {		// MODIFY_SUPPLICANT_FOR_FREERTOS - add
			dpm_supp_key_info->group_cipher = value;
		}
#endif /* CFG_PMGR */
		break;
...
#if CFG_PMGR
		if (RM_PMGR_W_rtm_exist() == pdTRUE) {		// MODIFY_SUPPLICANT_FOR_FREERTOS - add
			dpm_supp_key_info->key_mgmt = value;
		}
#endif /* CFG_PMGR */
		break;
...
int wpa_sm_set_bssid(struct wpa_sm *sm, const u8 *bssid)	//MODIFY_SUPPLICANT_FOR_FREERTOS - add
{
	if (sm == NULL)
		return -1;
...
unsigned int wpa_sm_get_proto(struct wpa_sm *sm)
{
	return sm->proto;
}
...
int fils_process_assoc_resp(struct wpa_sm *sm, const u8 *resp, size_t len)
{
	...
	mgmt = (const struct _ieee80211_mgmt *) resp;
...
static void wpa_supp_dpm_backup_key_info(enum backup_key type, enum wpa_alg alg,
			...
			struct wpa_sm *sm, enum key_flag key_flag)
{
#ifdef DEBUG_PRINT_WPA_TK_KEY
	ra6w1_prt(ANSI_COLOR_GREEN "\n[%s]\n", __func__);
#endif // DEBUG_PRINT_WPA_TK_KEY
	...
	if (type == KEY_PTK) {
		...
		ra6w1_prt("<PTK>\n");
#endif // DEBUG_PRINT_WPA_TK_KEY
		os_memset(&dpm_supp_key_info->ptk, 0, sizeof(cipher_ptk_t));
		dpm_supp_key_info->ptk.wpa_alg = alg;
		dpm_supp_key_info->ptk.key_idx = key_idx;
		dpm_supp_key_info->ptk.set_tx = set_tx;
		dpm_supp_key_info->ptk.seq_len = seq_len;
		if (key_flag) {
			dpm_supp_key_info->key_flag = key_flag;
		}

		dpm_supp_key_info->ptk.ptk_kck_len = (unsigned char)sm->ptk.kck_len;
		dpm_supp_key_info->ptk.ptk_kek_len = (unsigned char)sm->ptk.kek_len;
		dpm_supp_key_info->ptk.ptk_tk_len  = (unsigned char)sm->ptk.tk_len;
		dpm_supp_key_info->pmk_len = (unsigned char)sm->pmk_len;

		if (seq != NULL) {
			memcpy(dpm_supp_key_info->ptk.seq, seq, seq_len);
#ifdef DEBUG_PRINT_WPA_TK_KEY
			ra6w1_prt("\tseq(%02d)=", (int)seq_len);
		...
		memcpy(dpm_supp_key_info->ptk.ptk_kck, sm->ptk.kck, sm->ptk.kck_len);
#ifdef DEBUG_PRINT_WPA_TK_KEY
		ra6w1_prt("\tptk_kck(%02d)=", (int)sm->ptk.kck_len);
		for (int i = 0; i < (int)sm->ptk.kck_len; i++)
			ra6w1_prt("%02x", sm->ptk.kck[i]);	
		ra6w1_prt("\n");
#endif // DEBUG_PRINT_WPA_TK_KEY
			
		memcpy(dpm_supp_key_info->ptk.ptk_kek, sm->ptk.kek,  sm->ptk.kek_len);
#ifdef DEBUG_PRINT_WPA_TK_KEY
		ra6w1_prt("\tptk_kek(%02d)=", (int)sm->ptk.kek_len);
		for (int i = 0; i < (int)sm->ptk.kek_len; i++)
			ra6w1_prt("%02x", sm->ptk.kek[i]);
		ra6w1_prt("\n");
#endif // DEBUG_PRINT_WPA_TK_KEY

		memcpy(dpm_supp_key_info->ptk.ptk_tk, sm->ptk.tk,  sm->ptk.tk_len);
		
#ifdef DEBUG_PRINT_WPA_TK_KEY
		ra6w1_prt("\tptk_tk (%02d)=", (int)sm->ptk.tk_len);
		for (int i = 0; i < (int)sm->ptk.tk_len; i++)
			ra6w1_prt("%02x", sm->ptk.tk[i]);
		ra6w1_prt("\n");
#elif defined ENABLE_PRINT_TK_KEY_FOR_WIRESHARK_DBG
            ra6w1_prt(ANSI_COLOR_LIGHT_GREEN "ptk \"tk\",\"");
            for (int i = 0; i < (int)sm->ptk.tk_len; i++)
                ra6w1_prt("%02x", sm->ptk.tk[i]);
            ra6w1_prt("\"\n" ANSI_COLOR_DEFULT);
#endif // DEBUG_PRINT_WPA_TK_KEY
...
#ifdef DEBUG_PRINT_WPA_TK_KEY
		ra6w1_prt(ANSI_BCOLOR_DEFULT);
#endif // DEBUG_PRINT_WPA_TK_KEY
	} else if (type == KEY_GTK) {	
		/* === GTK === */
#ifdef DEBUG_PRINT_WPA_TK_KEY
		ra6w1_prt(ANSI_COLOR_GREEN "<GTK>\n");
#endif // DEBUG_PRINT_WPA_TK_KEY
		os_memset(&dpm_supp_key_info->gtk, 0, sizeof(cipher_gtk_t));
		dpm_supp_key_info->gtk.wpa_alg = alg;
		dpm_supp_key_info->gtk.key_idx = key_idx;
		dpm_supp_key_info->gtk.set_tx = set_tx;
		dpm_supp_key_info->gtk.seq_len = seq_len;
		dpm_supp_key_info->gtk.gtk_len = sm->gtk.gtk_len;

		if (seq != NULL) {
			memcpy(dpm_supp_key_info->gtk.seq, seq, seq_len);
#ifdef DEBUG_PRINT_WPA_TK_KEY
			ra6w1_prt("\tseq(%02d)=", (int)seq_len);
			for (int i = 0; i < seq_len; i++)
				ra6w1_prt("%02x", seq[i]);	
		...
		memcpy(dpm_supp_key_info->gtk.gtk, sm->gtk.gtk, sm->gtk.gtk_len);
#ifdef DEBUG_PRINT_WPA_TK_KEY
		ra6w1_prt(ANSI_COLOR_GREEN "\tgtk(%02d)=", (int)sm->gtk.gtk_len);
		for (int i = 0; (unsigned int)i < sm->gtk.gtk_len; i++)
			ra6w1_prt("%02x", sm->gtk.gtk[i]);
		ra6w1_prt("\n");
#elif defined ENABLE_PRINT_TK_KEY_FOR_WIRESHARK_DBG
		ra6w1_prt(ANSI_COLOR_LIGHT_GREEN "gtk \"tk\",\"");
		for (int i = 0; (unsigned int)i < sm->gtk.gtk_len; i++)
		    ra6w1_prt("%02x", sm->gtk.gtk[i]);
		ra6w1_prt("\"\n" ANSI_COLOR_DEFULT);
#endif // DEBUG_PRINT_WPA_TK_KEY
		...
		ra6wx_dpm_prt("	  <%s> Failed backup key info!!\n", __func__);
	}
}

int wpa_supp_dpm_restore_key_info(struct wpa_supplicant *wpa_s)
{
	struct wpa_sm *sm = wpa_s->wpa;
	int err = 0;
	u8 gtk_buf[32];

 	if (dpm_supp_key_info->wep_key_len)
 	{
#ifdef CONFIG_WEP
		//wpa_set_wep_keys(wpa_s, wpa_s->current_ssid);
#endif	// CONFIG_WEP
 	}
 	else if ((dpm_supp_key_info->ptk.wpa_alg == WPA_ALG_TKIP || dpm_supp_key_info->ptk.wpa_alg == WPA_ALG_CCMP)
 		   || (dpm_supp_key_info->gtk.wpa_alg == WPA_ALG_TKIP || dpm_supp_key_info->gtk.wpa_alg == WPA_ALG_CCMP))
 	{
		ra6wx_dpm_prt("<%s> Security : start set \n", __func__);
		memcpy(sm->ptk.kck, dpm_supp_key_info->ptk.ptk_kck, (size_t)dpm_supp_key_info->ptk.ptk_kck_len);
		memcpy(sm->ptk.kek, dpm_supp_key_info->ptk.ptk_kek, (size_t)dpm_supp_key_info->ptk.ptk_kek_len);
		sm->ptk.kck_len = (size_t)dpm_supp_key_info->ptk.ptk_kck_len;
		sm->ptk.kek_len = (size_t)dpm_supp_key_info->ptk.ptk_kek_len;

#ifdef WPA2_PATCH_201701
		os_memcpy(sm->gtk.gtk, dpm_supp_key_info->gtk.gtk, (size_t)dpm_supp_key_info->gtk.gtk_len);
		sm->gtk.gtk_len = (size_t)dpm_supp_key_info->gtk.gtk_len;

		sm->ptk.installed = 1;
#endif /* WPA2_PATCH_201701 */

		sm->pmk_len = dpm_supp_key_info->pmk_len;

		if (sm->group_cipher == WPA_CIPHER_TKIP) {
			/* Swap Tx/Rx keys for Michael MIC */
			os_memcpy(gtk_buf, dpm_supp_key_info->gtk.gtk, 16);
			os_memcpy(gtk_buf + 16, dpm_supp_key_info->gtk.gtk + 24, 8);
			os_memcpy(gtk_buf + 24, dpm_supp_key_info->gtk.gtk + 16, 8);
		}

		/* Key setting already done before the DPM */
		sm->ptk_set = 1;
        sm->msg_3_of_4_ok = 1; // To update GTK.
		ra6wx_dpm_prt("<%s> Security : end set \n", __func__);
 	}
 	else
 	{
		ra6wx_dpm_prt("<%s> Security : Open mode\n", __func__);
		return 0;
 	}

	if (err > 0) {
 		ra6wx_err_prt("<%s> Unsupported Auth Algorithm\n", __func__);
		return -1;
 	}

#ifdef DEBUG_PRINT_WPA_TK_KEY
	if (dpm_supp_key_info->wep_key_len == 0) {
		ra6w1_prt(ANSI_COLOR_YELLOW "[%s]\n"
				"\twpa_alg=%d\n"
				"\tkey_idx=%d\n"
				"\tset_tx =%d\n",
				__func__,
				dpm_supp_key_info->ptk.wpa_alg,
				dpm_supp_key_info->ptk.key_idx,
				dpm_supp_key_info->ptk.set_tx);

		ra6w1_prt("<PTK>\n");

		ra6w1_prt("\tseq(%02d)=", (int)dpm_supp_key_info->ptk.seq_len);
		for (int i = 0; i < dpm_supp_key_info->ptk.seq_len; i++)
			ra6w1_prt("%02x", dpm_supp_key_info->ptk.seq[i]);
		ra6w1_prt("\n");

		ra6w1_prt("\tptk_kck(%02d)=", (int)dpm_supp_key_info->ptk.ptk_kck_len);
		for (int i = 0; i < dpm_supp_key_info->ptk.ptk_kck_len; i++)
			ra6w1_prt("%02x", dpm_supp_key_info->ptk.ptk_kck[i]);	
		ra6w1_prt("\n");
		
		ra6w1_prt("\tptk_kek(%02d)=", (int)dpm_supp_key_info->ptk.ptk_kek_len);
		for (int i = 0; i < dpm_supp_key_info->ptk.ptk_kek_len; i++)
			ra6w1_prt("%02x", dpm_supp_key_info->ptk.ptk_kek[i]);	
		ra6w1_prt("\n");

		ra6w1_prt("\tptk_tk (%02d)=", (int)dpm_supp_key_info->ptk.ptk_tk_len);
		for (int i = 0; i < dpm_supp_key_info->ptk.ptk_tk_len; i++)
			ra6w1_prt("%02x", dpm_supp_key_info->ptk.ptk_tk[i]);
		ra6w1_prt("\n");
		...
		ra6w1_prt("\n<GTK>\n"
				...
				"\tkey_idx=%d\n"
				"\tset_tx =%d\n",
				dpm_supp_key_info->gtk.wpa_alg,
				dpm_supp_key_info->gtk.key_idx,
				dpm_supp_key_info->gtk.set_tx);

		ra6w1_prt("\tseq(%02d)=", (int)dpm_supp_key_info->gtk.seq_len);
		for (int i = 0; i < dpm_supp_key_info->gtk.seq_len; i++)
			ra6w1_prt("%02x", dpm_supp_key_info->gtk.seq[i]);
		ra6w1_prt("\n");
		
		ra6w1_prt("\tgtk(%02d)=", (int)dpm_supp_key_info->gtk.gtk_len);
		for (int i = 0; i < dpm_supp_key_info->gtk.gtk_len; i++)
			ra6w1_prt("%02x", dpm_supp_key_info->gtk.gtk[i]);
		ra6w1_prt("\n");
		ra6w1_prt(ANSI_COLOR_DEFULT);
		}
#endif // DEBUG_PRINT_WPA_TK_KEY
	return 0;
}
