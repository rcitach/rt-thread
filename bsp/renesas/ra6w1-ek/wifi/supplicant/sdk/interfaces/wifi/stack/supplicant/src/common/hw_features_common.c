			    ...
			    struct he_capabilities *he_cap)
{
	...
	if (is_6ghz_freq(freq)) {
		...
		if (center_idx_to_bw_6ghz((u8) channel) < 0) {
			wpa_printf(MSG_ERROR,
		...
		if (!center_segment0) {
			...
			int freq1, freq2 = 0;
			int bw = center_idx_to_bw_6ghz((u8) center_segment0);
			...
			if (center_segment1) {
				if (center_idx_to_bw_6ghz((u8) center_segment1) != 2 ||
				    bw != 2) {
					wpa_printf(MSG_ERROR,
		      ...
		      int disabled)
{
	...
	if (disabled)
		htcaps->ht_capabilities_info &= (u16) ~msk;
	else
