...
#if 1   /* by Shingu 20160322 (SIGMA) */	//MODIFY_SUPPLICANT_FOR_FREERTOS
int global_pin;
#endif  /* 1 */
	     ...
	     const char *label, u8 *res, size_t res_len)
{
	...
	iter = (int) ((res_len + SHA256_MAC_LEN - 1) / SHA256_MAC_LEN);
	opos = res;
	left = res_len;

	for (i = 1; i <= iter; i++) {
		WPA_PUT_BE32(i_buf, (u32) i);
...
int wps_dev_type_str2bin(const char *str, u8 dev_type[WPS_DEV_TYPE_LEN])
{
	...
	/* <categ>-<OUI>-<subcateg> */
	WPA_PUT_BE16(dev_type, (u16) atoi(str));
	pos = os_strchr(str, '-');
	if (pos == NULL)
		return -1;
	pos++;
	...
	if (pos == NULL)
		return -1;
	pos++;
	WPA_PUT_BE16(&dev_type[6], (u16) atoi(pos));
...
void uuid_gen_mac_addr(const u8 *mac_addr, u8 *uuid)
{
	...
	uuid[6] = (u8) ((5 << 4) | (uuid[6] & 0x0f));

	/* Variant specified in RFC 4122 */
	uuid[8] = (u8) (0x80 | (uuid[8] & 0x3f));
}
