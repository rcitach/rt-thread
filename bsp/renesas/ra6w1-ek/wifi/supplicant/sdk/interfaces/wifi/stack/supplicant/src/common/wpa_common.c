		      ...
		      const u8 *buf, size_t len, u8 *mic)
{
	...
	case WPA_KEY_INFO_TYPE_HMAC_MD5_RC4:
		wpa_printf_dbg(MSG_DEBUG, "WPA: EAPOL-Key MIC using HMAC-MD5");
		return hmac_md5(key, key_len, buf, len, mic);
#endif /* CONFIG_FIPS */
	case WPA_KEY_INFO_TYPE_HMAC_SHA1_AES:
		wpa_printf_dbg(MSG_DEBUG, "WPA: EAPOL-Key MIC using HMAC-SHA1");
		if (hmac_sha1(key, key_len, buf, len, hash))
			return -1;
		os_memcpy(mic, hash, MD5_MAC_LEN);
		break;
	case WPA_KEY_INFO_TYPE_AES_128_CMAC:
		wpa_printf_dbg(MSG_DEBUG, "WPA: EAPOL-Key MIC using AES-CMAC");
...
#define MAX_Z_LEN 66 /* with NIST P-521 */
	...
	ptk->tk_len = (size_t) wpa_cipher_key_len(cipher);
	ptk->kdk_len = kdk_len;
	if (ptk->tk_len == 0) {
		wpa_printf(MSG_ERROR,
	...
	if (wpa_key_mgmt_sha384(akmp)) {
		...
		wpa_printf_dbg(MSG_DEBUG, "WPA: PTK derivation using PRF(SHA384)");
		...
		wpa_printf_dbg(MSG_DEBUG, "WPA: PTK derivation using PRF(SHA256)");
		...
		wpa_printf_dbg(MSG_DEBUG, "WPA: PTK derivation using PRF(SHA512)");
		...
		wpa_printf_dbg(MSG_DEBUG, "WPA: PTK derivation using PRF(SHA1)");
	...
	wpa_printf_dbg(MSG_DEBUG, "WPA: PTK derivation - A1=" MACSTR " A2=" MACSTR,
		     ...
		     size_t dh_ss_len, u8 *pmk, size_t *pmk_len)
{
	...
	wpa_printf_dbg(MSG_DEBUG, "FILS: rMSK to PMK derivation");
		    ...
		    u8 *fils_ft, size_t *fils_ft_len, size_t kdk_len)
{
	...
	if (wpa_key_mgmt_sha384(akmp)) {
		wpa_printf_dbg(MSG_DEBUG, "FILS: PTK derivation using PRF(SHA384)");
		if (sha384_prf(pmk, pmk_len, label, data, data_len,
			       tmp, key_data_len) < 0)
			goto err;
	} else {
		wpa_printf_dbg(MSG_DEBUG, "FILS: PTK derivation using PRF(SHA256)");
		if (sha256_prf(pmk, pmk_len, label, data, data_len,
			       tmp, key_data_len) < 0)
			goto err;
	}

	wpa_printf_dbg(MSG_DEBUG, "FILS: PTK derivation - SPA=" MACSTR
		     ...
		     size_t *key_auth_len)
{
	...
	wpa_printf_dbg(MSG_DEBUG, "FILS: Key-Auth derivation: STA-MAC=" MACSTR
			     ...
			     struct wpa_ft_ies *parse, int use_sha384)
{
	...
	while (end - pos >= 2) {
		...
		len = *pos++;
		if (len > end - pos) {
			wpa_printf_dbg(MSG_DEBUG, "FT: Truncated subelement");
			break;
		}
		...
		case FTIE_SUBELEM_BIGTK:
			...
			break;
		default:
			wpa_printf_dbg(MSG_DEBUG, "FT: Unknown subelem id %u", id);
			break;
		}
		     ...
		     struct wpa_ft_ies *parse, int use_sha384)
{
	...
	while (end - pos >= 2) {
		...
		case WLAN_EID_RSN:
			...
			ret = wpa_parse_wpa_ie_rsn(parse->rsn - 2,
						   ...
						   &data);
			if (ret < 0) {
				wpa_printf_dbg(MSG_DEBUG, "FT: Failed to parse "
					   "RSN IE: %d", ret);
				return -1;
			}
	...
	if (parse->rsnxe)
		prot_ie_count--;
	if (prot_ie_count < 0) {
		wpa_printf_dbg(MSG_DEBUG, "FT: Some required IEs not included in "
			   "the protected IE count");
		return -1;
	}

	if (prot_ie_count == 0 && parse->ric) {
		wpa_printf_dbg(MSG_DEBUG, "FT: RIC IE(s) in the frame, but not "
	...
	if (prot_ie_count) {
		wpa_printf_dbg(MSG_DEBUG, "FT: %d protected IEs missing from "
		    ...
		    size_t kdk_len)
{
	...
	if (pasn_use_sha384(akmp, cipher)) {
		wpa_printf_dbg(MSG_DEBUG, "PASN: PTK derivation using SHA384");

		if (sha384_prf(pmk, pmk_len, label, data, data_len, tmp,
			       ptk_len) < 0)
			goto err;
	} else {
		wpa_printf_dbg(MSG_DEBUG, "PASN: PTK derivation using SHA256");
	     ...
	     const u8 *frame, size_t frame_len, u8 *mic)
{
	...
	if (pasn_use_sha384(akmp, cipher)) {
		wpa_printf_dbg(MSG_DEBUG, "PASN: MIC using HMAC-SHA384");
		...
		wpa_printf_dbg(MSG_DEBUG, "PASN: MIC using HMAC-SHA256");
...
{
	if (pasn_use_sha384(akmp, cipher)) {
		wpa_printf_dbg(MSG_DEBUG, "PASN: Frame hash using SHA-384");
		return sha384_vector(1, &data, &len, hash);
	} else {
		wpa_printf_dbg(MSG_DEBUG, "PASN: Frame hash using SHA-256");
			 ...
			 struct wpa_ie_data *data)
{
	...
	if (rsn_ie_len < sizeof(struct rsn_ie_hdr)) {
		wpa_printf_dbg(MSG_DEBUG, "%s: ie len too short %lu",
	...
	if (rsn_ie_len >= 6 && rsn_ie[1] >= 4 &&
		...
		if (hdr->elem_id != WLAN_EID_RSN ||
			...
			wpa_printf_dbg(MSG_DEBUG, "%s: malformed ie or unknown version",
				   __func__);
			return -2;
		}

		pos = (const u8 *) (hdr + 1);
		left = (int) (rsn_ie_len - sizeof(*hdr));
	}
	...
	if (left >= RSN_SELECTOR_LEN) {
		...
		wpa_printf_dbg(MSG_DEBUG, "%s: ie length mismatch, %u too much",
			   __func__, left);
		return -3;
	}
	...
	if (left >= 2) {
		...
		if (count == 0 || count > left / RSN_SELECTOR_LEN) {
			wpa_printf_dbg(MSG_DEBUG, "%s: ie count botch (pairwise), "
		...
		if ((unsigned int) data->pairwise_cipher & WPA_CIPHER_AES_128_CMAC) {
			wpa_printf_dbg(MSG_DEBUG, "%s: AES-128-CMAC used as "
				   "pairwise cipher", __func__);
			return -1;
		}
	} else if (left == 1) {
		wpa_printf_dbg(MSG_DEBUG, "%s: ie too short (for key mgmt)",
			   __func__);
		return -5;
	}
	...
	if (left >= 2) {
		...
		left -= 2;
		if (count == 0 || count > left / RSN_SELECTOR_LEN) {
			wpa_printf_dbg(MSG_DEBUG, "%s: ie count botch (key mgmt), "
				   "count %u left %u", __func__, count, left);
			return -6;
		}
		for (i = 0; i < count; i++) {
		...
		wpa_printf_dbg(MSG_DEBUG, "%s: ie too short (for capabilities)",
			   __func__);
		return -7;
	}
	...
	if (left >= 2) {
		...
		if (num_pmkid > (unsigned int) left / PMKID_LEN) {
			wpa_printf_dbg(MSG_DEBUG, "%s: PMKID underflow "
				   "(num_pmkid=%u left=%d)",
			...
			left -= (int) (data->num_pmkid * PMKID_LEN);
		}
	}
			 ...
			 struct wpa_ie_data *data)
{
	...
	pos = (const u8 *) (hdr + 1);
	left = (int) (wpa_ie_len - sizeof(*hdr));
		      ...
		      int use_sha384)
{
	...
	if (ssid_len > SSID_MAX_LEN || r0kh_id_len > FT_R0KH_ID_MAX_LEN)
		return -1;
	wpa_printf_dbg(MSG_DEBUG, "FT: Derive PMK-R0 using KDF-%s",
	...
	wpa_printf_dbg(MSG_DEBUG, "FT: S0KH-ID: " MACSTR, MAC2STR(s0kh_id));
	pos = buf;
	*pos++ = ssid_len;
		      ...
		      u8 *pmk_r1, u8 *pmk_r1_name)
{
	...
	wpa_printf_dbg(MSG_DEBUG, "FT: Derive PMK-R1 using KDF-%s",
	...
	wpa_printf_dbg(MSG_DEBUG, "FT: S1KH-ID: " MACSTR, MAC2STR(s1kh_id));
	pos = buf;
		      ...
		      size_t kdk_len)
{
	 ...
	 *                  BSSID || STA-ADDR)
	 */
	wpa_printf_dbg(MSG_DEBUG, "FT: Derive PTK using KDF-%s",
		   use_sha384 ? "SHA384" : "SHA256");
	...
	wpa_printf_dbg(MSG_DEBUG, "FT: BSSID=" MACSTR " STA-ADDR=" MACSTR,
	       ...
	       u8 *pmkid, int akmp)
{
	...
	if (0) {
		...
		wpa_printf_dbg(MSG_DEBUG, "RSN: Derive PMKID using HMAC-SHA-384");
		hmac_sha384_vector(pmk, pmk_len, 3, addr, len, hash);
#endif /* CONFIG_FILS || CONFIG_SHA384 */
	} else if (wpa_key_mgmt_sha256(akmp)) {
		wpa_printf_dbg(MSG_DEBUG, "RSN: Derive PMKID using HMAC-SHA-256");
		hmac_sha256_vector(pmk, pmk_len, 3, addr, len, hash);
	} else {
		wpa_printf_dbg(MSG_DEBUG, "RSN: Derive PMKID using HMAC-SHA-1");
...
#ifdef CONFIG_IEEE80211W
const char * pmf_mode_txt(int mode)
{
	switch (mode) {
...
{
	if ((unsigned int) akm & WPA_KEY_MGMT_FT_IEEE8021X_SHA384)
		return RSN_AUTH_KEY_MGMT_FT_802_1X_SHA384;
	if ((unsigned int) akm & WPA_KEY_MGMT_FT_IEEE8021X)
		return RSN_AUTH_KEY_MGMT_FT_802_1X;
	if ((unsigned int) akm & WPA_KEY_MGMT_FT_PSK)
		return RSN_AUTH_KEY_MGMT_FT_PSK;
	if ((unsigned int) akm & WPA_KEY_MGMT_IEEE8021X_SHA256)
		return RSN_AUTH_KEY_MGMT_802_1X_SHA256;
	if ((unsigned int) akm & WPA_KEY_MGMT_IEEE8021X)
		return RSN_AUTH_KEY_MGMT_UNSPEC_802_1X;
	if ((unsigned int) akm & WPA_KEY_MGMT_PSK_SHA256)
		return RSN_AUTH_KEY_MGMT_PSK_SHA256;
	if ((unsigned int) akm & WPA_KEY_MGMT_PSK)
		return RSN_AUTH_KEY_MGMT_PSK_OVER_802_1X;
	if ((unsigned int) akm & WPA_KEY_MGMT_CCKM)
		return RSN_AUTH_KEY_MGMT_CCKM;
	if ((unsigned int) akm & WPA_KEY_MGMT_OSEN)
		return RSN_AUTH_KEY_MGMT_OSEN;
	if ((unsigned int) akm & WPA_KEY_MGMT_IEEE8021X_SUITE_B)
		return RSN_AUTH_KEY_MGMT_802_1X_SUITE_B;
	if ((unsigned int) akm & WPA_KEY_MGMT_IEEE8021X_SUITE_B_192)
		return RSN_AUTH_KEY_MGMT_802_1X_SUITE_B_192;
	if ((unsigned int) akm & WPA_KEY_MGMT_FILS_SHA256)
		return RSN_AUTH_KEY_MGMT_FILS_SHA256;
	if ((unsigned int) akm & WPA_KEY_MGMT_FILS_SHA384)
		return RSN_AUTH_KEY_MGMT_FILS_SHA384;
	if ((unsigned int) akm & WPA_KEY_MGMT_FT_FILS_SHA256)
		return RSN_AUTH_KEY_MGMT_FT_FILS_SHA256;
	if ((unsigned int) akm & WPA_KEY_MGMT_FT_FILS_SHA384)
		return RSN_AUTH_KEY_MGMT_FT_FILS_SHA384;
	if ((unsigned int) akm & WPA_KEY_MGMT_SAE)
		return RSN_AUTH_KEY_MGMT_SAE;
	if ((unsigned int) akm & WPA_KEY_MGMT_FT_SAE)
		return RSN_AUTH_KEY_MGMT_FT_SAE;
	if ((unsigned int) akm & WPA_KEY_MGMT_OWE)
		return RSN_AUTH_KEY_MGMT_OWE;
	if ((unsigned int) akm & WPA_KEY_MGMT_DPP)
...
int wpa_insert_pmkid(u8 *ies, size_t *ies_len, const u8 *pmkid)
{
	...
	if (rpos == rend) {
		...
		os_memmove(rpos + 2, rpos, (size_t) (end - rpos));
		*rpos++ = 0;
		*rpos++ = 0;
		added += 2;
		start[1] += 2;
	...
	if (rpos == rend) {
		...
		os_memmove(rpos + 2 + PMKID_LEN, rpos, (size_t) (end + added - rpos));
		WPA_PUT_LE16(rpos, 1);
		rpos += 2;
		...
		if (num_pmkid != 0) {
			...
			os_memmove(rpos + 2, after, (size_t) (end - after));
			start[1] -= (u8) (num_pmkid * PMKID_LEN);
			added -= num_pmkid * PMKID_LEN;
		}
		WPA_PUT_LE16(rpos, 1);
		rpos += 2;
		os_memmove(rpos + PMKID_LEN, rpos, (size_t) (end + added - rpos));
		os_memcpy(rpos, pmkid, PMKID_LEN);
		added += PMKID_LEN;
		start[1] += PMKID_LEN;
	}
...
{
	if ((unsigned int) cipher & WPA_CIPHER_CCMP_256)
		return RSN_CIPHER_SUITE_CCMP_256;
	if ((unsigned int) cipher & WPA_CIPHER_GCMP_256)
		return RSN_CIPHER_SUITE_GCMP_256;
	if ((unsigned int) cipher & WPA_CIPHER_CCMP)
		return (proto == WPA_PROTO_RSN ?
			RSN_CIPHER_SUITE_CCMP : WPA_CIPHER_SUITE_CCMP);
	if ((unsigned int) cipher & WPA_CIPHER_GCMP)
		return RSN_CIPHER_SUITE_GCMP;
	if ((unsigned int) cipher & WPA_CIPHER_TKIP)
		return (proto == WPA_PROTO_RSN ?
			RSN_CIPHER_SUITE_TKIP : WPA_CIPHER_SUITE_TKIP);
	if ((unsigned int) cipher & WPA_CIPHER_NONE)
		return (proto == WPA_PROTO_RSN ?
			RSN_CIPHER_SUITE_NONE : WPA_CIPHER_SUITE_NONE);
	if ((unsigned int) cipher & WPA_CIPHER_GTK_NOT_USED)
		return RSN_CIPHER_SUITE_NO_GROUP_ADDRESSED;
	if ((unsigned int) cipher & WPA_CIPHER_AES_128_CMAC)
		return RSN_CIPHER_SUITE_AES_128_CMAC;
	if ((unsigned int) cipher & WPA_CIPHER_BIP_GMAC_128)
		return RSN_CIPHER_SUITE_BIP_GMAC_128;
	if ((unsigned int) cipher & WPA_CIPHER_BIP_GMAC_256)
		return RSN_CIPHER_SUITE_BIP_GMAC_256;
	if ((unsigned int) cipher & WPA_CIPHER_BIP_CMAC_256)
...
int rsn_cipher_put_suites(u8 *start, int ciphers)
{
	...
	if ((unsigned int) ciphers & WPA_CIPHER_CCMP_256) {
		RSN_SELECTOR_PUT(pos, RSN_CIPHER_SUITE_CCMP_256);
		pos += RSN_SELECTOR_LEN;
	}
	if ((unsigned int) ciphers & WPA_CIPHER_GCMP_256) {
		RSN_SELECTOR_PUT(pos, RSN_CIPHER_SUITE_GCMP_256);
		pos += RSN_SELECTOR_LEN;
	}
	if ((unsigned int) ciphers & WPA_CIPHER_CCMP) {
		RSN_SELECTOR_PUT(pos, RSN_CIPHER_SUITE_CCMP);
		pos += RSN_SELECTOR_LEN;
	}
	if ((unsigned int) ciphers & WPA_CIPHER_GCMP) {
		RSN_SELECTOR_PUT(pos, RSN_CIPHER_SUITE_GCMP);
		pos += RSN_SELECTOR_LEN;
	}
	if ((unsigned int) ciphers & WPA_CIPHER_TKIP) {
		RSN_SELECTOR_PUT(pos, RSN_CIPHER_SUITE_TKIP);
		pos += RSN_SELECTOR_LEN;
	}
	if ((unsigned int) ciphers & WPA_CIPHER_NONE) {
...
int wpa_pick_pairwise_cipher(int ciphers, int none_allowed)
{
	...
	if ((unsigned int) ciphers & WPA_CIPHER_TKIP)
		return WPA_CIPHER_TKIP;
	if (none_allowed && ((unsigned int) ciphers & WPA_CIPHER_NONE))
		return WPA_CIPHER_NONE;
	return -1;
}
...
int wpa_pick_group_cipher(int ciphers)
{
	...
	if ((unsigned int) ciphers & WPA_CIPHER_GCMP)
		return WPA_CIPHER_GCMP;
	if ((unsigned int) ciphers & WPA_CIPHER_GTK_NOT_USED)
		return WPA_CIPHER_GTK_NOT_USED;
	if ((unsigned int) ciphers & WPA_CIPHER_TKIP)
		return WPA_CIPHER_TKIP;
	return -1;
	...
	while (*start != '\0') {
		...
		*end = '\0';
		if (os_strcmp(start, "CCMP-256") == 0)
			val = (int) ((unsigned int) val | WPA_CIPHER_CCMP_256);
		else if (os_strcmp(start, "GCMP-256") == 0)
			val = (int) ((unsigned int) val | WPA_CIPHER_GCMP_256);
		else if (os_strcmp(start, "CCMP") == 0)
			val = (int) ((unsigned int) val | WPA_CIPHER_CCMP);
		else if (os_strcmp(start, "GCMP") == 0)
			val = (int) ((unsigned int) val | WPA_CIPHER_GCMP);
#ifndef CONFIG_NO_TKIP
		else if (os_strcmp(start, "TKIP") == 0)
			val = (int) ((unsigned int) val | WPA_CIPHER_TKIP);
#endif /* CONFIG_NO_TKIP */
#ifdef CONFIG_WEP
		else if (os_strcmp(start, "WEP104") == 0)
			val = (int) ((unsigned int) val | WPA_CIPHER_WEP104);
		else if (os_strcmp(start, "WEP40") == 0)
			val = (int) ((unsigned int) val | WPA_CIPHER_WEP40);
#endif /* CONFIG_WEP */
		else if (os_strcmp(start, "NONE") == 0)
			val = (int) ((unsigned int) val | WPA_CIPHER_NONE);
		else if (os_strcmp(start, "GTK_NOT_USED") == 0)
			val = (int) ((unsigned int) val | WPA_CIPHER_GTK_NOT_USED);
		else if (os_strcmp(start, "AES-128-CMAC") == 0)
			val = (int) ((unsigned int) val | WPA_CIPHER_AES_128_CMAC);
		else if (os_strcmp(start, "BIP-GMAC-128") == 0)
			val = (int) ((unsigned int) val | WPA_CIPHER_BIP_GMAC_128);
		else if (os_strcmp(start, "BIP-GMAC-256") == 0)
			val = (int) ((unsigned int) val | WPA_CIPHER_BIP_GMAC_256);
		else if (os_strcmp(start, "BIP-CMAC-256") == 0)
			val = (int) ((unsigned int) val | WPA_CIPHER_BIP_CMAC_256);
		else {
			os_free(buf);
			return -1;
		}
...
int wpa_write_ciphers(char *start, char *end, int ciphers, const char *delim)
{
	...
	if ((unsigned int) ciphers & WPA_CIPHER_CCMP_256) {
		ret = os_snprintf(pos, (size_t) (end - pos), "%sCCMP-256",
				  pos == start ? "" : delim);
	...
	if ((unsigned int) ciphers & WPA_CIPHER_GCMP_256) {
		ret = os_snprintf(pos, (size_t) (end - pos), "%sGCMP-256",
				  pos == start ? "" : delim);
		if (os_snprintf_error((size_t) (end - pos), ret))
			return -1;
		pos += ret;
	...
	if ((unsigned int) ciphers & WPA_CIPHER_CCMP) {
		ret = os_snprintf(pos, (size_t) (end - pos), "%sCCMP",
				  pos == start ? "" : delim);
		if (os_snprintf_error((size_t) (end - pos), ret))
			return -1;
		pos += ret;
	...
	if ((unsigned int) ciphers & WPA_CIPHER_GCMP) {
		ret = os_snprintf(pos, (size_t) (end - pos), "%sGCMP",
				  pos == start ? "" : delim);
		if (os_snprintf_error((size_t) (end - pos), ret))
			return -1;
		pos += ret;
	...
	if ((unsigned int) ciphers & WPA_CIPHER_TKIP) {
		ret = os_snprintf(pos, (size_t) (end - pos), "%sTKIP",
				  pos == start ? "" : delim);
		if (os_snprintf_error((size_t) (end - pos), ret))
			return -1;
		pos += ret;
	}
	if ((unsigned int) ciphers & WPA_CIPHER_AES_128_CMAC) {
		ret = os_snprintf(pos, (size_t) (end - pos), "%sAES-128-CMAC",
				  pos == start ? "" : delim);
		if (os_snprintf_error((size_t) (end - pos), ret))
			return -1;
		pos += ret;
	}
	if ((unsigned int) ciphers & WPA_CIPHER_BIP_GMAC_128) {
		ret = os_snprintf(pos, (size_t) (end - pos), "%sBIP-GMAC-128",
				  pos == start ? "" : delim);
		if (os_snprintf_error((size_t) (end - pos), ret))
			return -1;
		pos += ret;
	}
	if ((unsigned int) ciphers & WPA_CIPHER_BIP_GMAC_256) {
		ret = os_snprintf(pos, (size_t) (end - pos), "%sBIP-GMAC-256",
				  pos == start ? "" : delim);
		if (os_snprintf_error((size_t) (end - pos), ret))
			return -1;
		pos += ret;
	}
	if ((unsigned int) ciphers & WPA_CIPHER_BIP_CMAC_256) {
		ret = os_snprintf(pos, (size_t) (end - pos), "%sBIP-CMAC-256",
				  pos == start ? "" : delim);
		if (os_snprintf_error((size_t) (end - pos), ret))
			return -1;
		pos += ret;
	...
	if ((unsigned int) ciphers & WPA_CIPHER_NONE) {
		ret = os_snprintf(pos, (size_t) (end - pos), "%sNONE",
				  pos == start ? "" : delim);
		if (os_snprintf_error((size_t) (end - pos), ret))
			return -1;
		pos += ret;
...
int wpa_select_ap_group_cipher(int wpa, int wpa_pairwise, int rsn_pairwise)
{
	...
	if ((unsigned int) pairwise & WPA_CIPHER_TKIP)
		return WPA_CIPHER_TKIP;
	if (((unsigned int) pairwise & (WPA_CIPHER_CCMP | WPA_CIPHER_GCMP)) == WPA_CIPHER_GCMP)
		return WPA_CIPHER_GCMP;
	if (((unsigned int) pairwise & (WPA_CIPHER_GCMP_256 | WPA_CIPHER_CCMP |
			 WPA_CIPHER_GCMP)) == WPA_CIPHER_GCMP_256)
		return WPA_CIPHER_GCMP_256;
	if (((unsigned int) pairwise & (WPA_CIPHER_CCMP_256 | WPA_CIPHER_CCMP |
...
{
	struct _ieee80211_mgmt *auth;

	wpa_printf_dbg(MSG_DEBUG, "PASN: Add authentication header. trans_seq=%u",
		   trans_seq);

	auth = wpabuf_put(buf, offsetof(struct _ieee80211_mgmt,
					u.auth.variable));
...
int wpa_pasn_add_rsne(struct wpabuf *buf, const u8 *pmkid, int akmp, int cipher)
{
	...
	wpa_printf_dbg(MSG_DEBUG, "PASN: Add RSNE");
	...
	if (pmkid) {
		wpa_printf_dbg(MSG_DEBUG, "PASN: Adding PMKID");
			       ...
			       const struct wpabuf *comeback, int after)
{
	...
	wpa_printf_dbg(MSG_DEBUG, "PASN: Add PASN Parameters element");
	...
	if (comeback) {
		wpa_printf_dbg(MSG_DEBUG, "PASN: Adding comeback data");
			      ...
			      struct wpabuf *wrapped_data_buf)
{
	...
	wpa_printf_dbg(MSG_DEBUG, "PASN: Add wrapped data");
...
int wpa_pasn_validate_rsne(const struct wpa_ie_data *data)
{
	...
	if ((data->capabilities & capab) != capab) {
		wpa_printf_dbg(MSG_DEBUG, "PASN: Invalid RSNE capabilities");
		return -1;
	}

	if (!data->has_group || data->group_cipher != WPA_CIPHER_GTK_NOT_USED) {
		wpa_printf_dbg(MSG_DEBUG, "PASN: Invalid group data cipher");
		return -1;
	}

	if (!data->has_pairwise || !data->pairwise_cipher ||
	    (data->pairwise_cipher & (data->pairwise_cipher - 1))) {
		wpa_printf_dbg(MSG_DEBUG, "PASN: No valid pairwise suite");
		return -1;
	}
	...
	if (data->mgmt_group_cipher != WPA_CIPHER_GTK_NOT_USED) {
		wpa_printf_dbg(MSG_DEBUG, "PASN: Invalid group mgmt cipher");
		return -1;
	}

	if (data->num_pmkid > 1) {
		wpa_printf_dbg(MSG_DEBUG, "PASN: Invalid number of PMKIDs");
		return -1;
	}
				...
				struct wpa_pasn_params_data *pasn_params)
{
	...
	if (!pasn_params) {
		wpa_printf_dbg(MSG_DEBUG, "PASN: Invalid params");
		return -1;
	}
	...
	case WPA_PASN_WRAPPED_DATA_FT:
		break;
	default:
		wpa_printf_dbg(MSG_DEBUG, "PASN: Invalid wrapped data format");
		return -1;
	}
