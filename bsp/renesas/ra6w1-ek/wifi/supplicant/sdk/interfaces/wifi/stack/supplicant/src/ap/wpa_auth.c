...
#if defined ENABLE_PRINT_TK_KEY_FOR_WIRESHARK_DBG
            if (sm->PTK.tk != NULL) {
                ra6w1_prt(ANSI_COLOR_LIGHT_GREEN  "ptk \"tk\",\"");
                for (int i = 0; i < klen; i++) {
                	ra6w1_prt("%02x", sm->PTK.tk[i]);
				}
                ra6w1_prt("\" peer_addr=" MACSTR ANSI_COLOR_DEFULT"\n" , MAC2STR(sm->addr));
            }
#endif // ENABLE_PRINT_TK_KEY_FOR_WIRESHARK_DBG
        }
...
#if defined ENABLE_PRINT_TK_KEY_FOR_WIRESHARK_DBG
    if (group->GTK[group->GN - 1] != NULL) {
        ra6w1_prt(ANSI_COLOR_LIGHT_GREEN "gtk \"tk\",\"");
        for (int i = 0; i < group->GTK_len; i++) {
            ra6w1_prt("%02x", group->GTK[group->GN - 1][i]);
		}
        ra6w1_prt("\"\n" ANSI_COLOR_DEFULT);
    }
#endif // ENABLE_PRINT_TK_KEY_FOR_WIRESHARK_DBG
