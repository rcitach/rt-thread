/* Serialize WPA archive timeouts with driver/EAPOL events, never in timer ISR. */
...
int __wrap_eloop_register_timeout(unsigned int secs, unsigned int usecs,
                                  eloop_timeout_handler handler, void *ctx, void *user)
{
    ...
    rt_list_insert_before(&timeouts, &t->node);
    rearm();
...
int __wrap_eloop_cancel_timeout(eloop_timeout_handler handler, void *ctx, void *user)
{
    rt_list_t *node, *next;
    int count = 0;
    if (!event_queue)
...
int __wrap_eloop_cancel_timeout_one(eloop_timeout_handler handler, void *ctx,
...
int __wrap_eloop_is_timeout_registered(eloop_timeout_handler handler, void *ctx, void *user)
{
    rt_list_t *node;
    int found = 0;
    rt_mutex_take(&timer_lock, RT_WAITING_FOREVER);
...
int __wrap_eloop_deplete_timeout(unsigned int secs, unsigned int usecs,
                                eloop_timeout_handler handler, void *ctx, void *user)
{
    return change_timeout(secs, usecs, handler, ctx, user, 0);
}

int __wrap_eloop_replenish_timeout(unsigned int secs, unsigned int usecs,
                                  eloop_timeout_handler handler, void *ctx, void *user)
{
