/* RA6W1 vendor core with an optional RT-Thread WLAN front end. */
#include <rtthread.h>
#include <dev_wlan.h>
...
#define WIFI_TX_HEADROOM   PBUF_LINK_ENCAPSULATION_HLEN
...
struct wifi_scan_entry
{
    ...
    /* Points into the same allocation, after the entry array. */
    const u8 *rsn;
    size_t rsn_len;
};
...
static void security_state(void *ctx, enum wpa_states state)
{
    ...
    if (state != WPA_COMPLETED || wifi.state != WIFI_JOINING)
        return;
    /* Only authorize firmware here. Link management belongs to WLAN, not the
...
/* Return a usable SSID and its optional RSN IE, preferring probe responses. */
static const u8 *scan_result_ies(const struct wpa_scan_res *r, size_t *ssid_len,
...
static struct wifi_scan_entry *scan_cache_create(const struct wpa_scan_results *results,
                                                size_t *entry_count)
{
    struct wifi_scan_entry *cache;
    const u8 *ssid, *rsn;
    u8 *rsn_data;
    size_t i, count = 0, capacity = 0, rsn_bytes = 0, ssid_len, rsn_len;

    *entry_count = 0;
    if (!results || !results->res)
        return RT_NULL;

    /* Count only reportable entries. Both passes run before the UMAC results
     * are released or another scan can start. Each IE is at most 257 bytes,
     * and at most WIFI_SCAN_LIMIT entries are retained. */
    for (i = 0; i < results->num && capacity < WIFI_SCAN_LIMIT; i++)
    {
        if (!scan_result_ies(results->res[i], &ssid_len, &rsn, &rsn_len))
            continue;
        capacity++;
        rsn_bytes += rsn_len;
    }
    ...
    /* A single owned snapshot avoids per-AP allocations and fixed 257-byte
     * RSN slots. Existing scan cleanup still releases everything in one free. */
    cache = rt_calloc(1, capacity * sizeof(*cache) + rsn_bytes);
    if (!cache)
    {
        LOG_E("cannot allocate scan results");
        return RT_NULL;
    }
    rsn_data = (u8 *)(cache + capacity);
    for (i = 0; i < results->num && count < capacity; i++)
    {
        const struct wpa_scan_res *r = results->res[i];
        struct wifi_scan_entry *entry = &cache[count];
        struct wpa_ie_data parsed;
        ssid = scan_result_ies(r, &ssid_len, &rsn, &rsn_len);
        if (!ssid)
            continue;
        ...
        if (rsn)
        {
            memcpy(rsn_data, rsn, rsn_len);
            entry->rsn = rsn_data;
            entry->rsn_len = rsn_len;
            rsn_data += rsn_len;
            /* Host security supports WPA2-PSK/CCMP; reject SAE-only,
             * mandatory-PMF, TKIP-group and enterprise networks. Optional
             * PMF remains compatible with this station. */
...
static void scan_complete(rt_bool_t aborted)
{
    ...
    cache = scan_cache_create(results, &count);
    if (results)
    {
...
static rt_err_t wlan_hw_init(struct rt_wlan_device *device)
{
    ...
    result = rt_thread_startup(wifi.worker);
    if (result != RT_EOK)
    {
        LOG_E("driver event worker startup failed: %d", result);
        rt_thread_delete(wifi.worker);
        wifi.worker = RT_NULL;
        wifi_wpa_timers_deinit();
        wifi_vendor_event_queue_deinit();
        wifi.queue = RT_NULL;
    ...
    supplicant_done();
    result = RT_EOK;
    LOG_I("%s ready, WLAN protocol, powersave=%d", RT_WLAN_DEVICE_STA_NAME,
          wifi.powersave);
out:
...
static void wifi_control_init(void)
{
    if (!wifi.control_initialized)
    {
        rt_mutex_init(&wifi.control, "wifictl", RT_IPC_FLAG_PRIO);
        wifi.control_initialized = RT_TRUE;
    }
}

static rt_err_t wifi_ready_check(void)
{
    if (wifi.ready)
        return RT_EOK;
    return wifi.init_result != RT_EOK ? wifi.init_result : -RT_ERROR;
}

static rt_err_t wlan_init(struct rt_wlan_device *device)
{
    (void)device;
    /* All hardware, queues, timers and worker threads are created by the
     * INIT_APP_EXPORT entry before WLAN management calls this operation. */
    return wifi_ready_check();
}

static rt_err_t wlan_mode(struct rt_wlan_device *device, rt_wlan_mode_t mode)
{
    rt_err_t result = RT_EOK;
    rt_bool_t detach_protocol = RT_FALSE;
    ...
    if (mode == RT_WLAN_NONE)
    {
        wlan = is_ap ? &wifi.ap_device : &wifi.device;
        if (!is_ap)
        {
        ...
        detach_protocol = RT_TRUE;
    }
    /* Protocol callbacks can enter WLAN management. Detach after releasing
     * the operation callback to avoid a management lock inversion. */
    if ((result == RT_EOK) && detach_protocol && wlan->prot != RT_NULL)
...
static int wlan_get_info(struct rt_wlan_device *device, struct rt_wlan_info *info)
{
    ...
    if (wifi.state == WIFI_LINKED)
    {
        ...
        if (!result)
        {
            ...
            /* rate_info.legacy is reported in 100 kbit/s units. Prefer the
             * RX rate because this is the direction used by udp_rx; fall
             * back to TX when the firmware does not report RX bitrate. */
            if ((station.filled & BIT_ULL(FC80211_STA_INFO_RX_BITRATE)) &&
                station.rxrate.legacy)
                info->datarate = (rt_uint32_t)station.rxrate.legacy * 100000U;
            else if ((station.filled & BIT_ULL(FC80211_STA_INFO_TX_BITRATE)) &&
                     station.txrate.legacy)
                info->datarate = (rt_uint32_t)station.txrate.legacy * 100000U;
        }
        else
            result = -RT_ERROR;
    }
...
/* librwnx_drv forward_packet_to_lwip schedules this callback with
 * tcpip_try_callback: we are already on tcpip_thread, under its core lock
 * when enabled. Use the WLAN direct input entry to avoid a second mailbox
 * round trip. Other RX contexts must use rt_wlan_dev_transfer_prot instead.
...
static int wlan_send(struct rt_wlan_device *device, void *buffer, int length)
{
    ...
    copy = p->next || p->len != p->tot_len || p->ref != 1 ||
           ...
           (uintptr_t)p->payload < (uintptr_t)p + LWIP_MEM_ALIGN_SIZE(sizeof(*p)) +
                                   WIFI_TX_HEADROOM;
    tx = p;
    if (copy)
    {
...
static const struct rt_wlan_dev_ops wlan_ops __attribute__((unused)) =
{
    .wlan_init = wlan_init,
    .wlan_mode = wlan_mode,
    .wlan_scan = wlan_scan,
...
static int wifi_wlan_register(void)
{
    ...
    result = wlan_hw_init(RT_NULL);
    if (result != RT_EOK)
    {
        LOG_E("WLAN driver initialization failed: %d", result);
        return result;
    }
