...
static struct
{
    ...
    u8 pmk[32];
    u8 ap_pmk[32];
    /* Fixed header, SSID, supported rates and DS parameter precede TIM. */
    u8 ap_beacon_head[36 + 2 + 32 + 2 + 8 + 3];
...
static void notify(rt_wlan_dev_event_t event)
{
    ...
    /* Notifications generated by a synchronous WLAN operation (for
     * example SoftAP start/stop) do not have a vendor message to wake the
     * event worker.  Post a lightweight wake item so the management layer's
     * completion event is dispatched promptly.  Timer wakeups use the same
     * zeroed message format. */
    if (wifi.queue)
        (void)rt_mq_send(wifi.queue, wake, sizeof(wake));
}
                             ...
                             struct cfg80211_ap_settings *settings)
{
    ...
    /* This beacon advertises legacy rates, without HT capabilities or WMM. */
    settings->chandef.width = FC80211_CHAN_WIDTH_20_NOHT;
    settings->chandef.center_freq1 = channel->center_freq;
    settings->chandef.chan_type = FC80211_CHAN_NO_HT;

    memset(wifi.ap_beacon_head, 0, sizeof(wifi.ap_beacon_head));
    wifi.ap_beacon_head[0] = 0x80;
    memset(wifi.ap_beacon_head + 4, 0xff, 6);
    ...
    /* The driver inserts TIM between head and tail, as in the SDK hostapd
     * beacon builder. Do not put SSID/rates/channel after that insertion. */
    offset = 36;
    offset = append_ie(wifi.ap_beacon_head, offset, sizeof(wifi.ap_beacon_head),
                       0, info->ssid.val, info->hidden ? 0 : info->ssid.len);
    if (!offset)
        return -RT_ENOMEM;
    offset = append_ie(wifi.ap_beacon_head, offset, sizeof(wifi.ap_beacon_head),
    ...
    if (!offset)
        return -RT_ENOMEM;
    settings->beacon.head_len = offset;

    /* ERP information follows TIM for the advertised 2.4 GHz OFDM rates. */
    {
        const u8 erp = 0;
        offset = append_ie(wifi.ap_beacon_tail, 0, sizeof(wifi.ap_beacon_tail),
                           42, &erp, sizeof(erp));
    }
    if (!offset)
        return -RT_ENOMEM;
...
void rrq16x_lwip_mbox_rx(struct pbuf *p)
{
    ...
    p->if_idx = ifindex;
    /* The vendor callback is already serialized by tcpip_thread. Bypass the
     * WLAN protocol wrapper's tcpip_input() mailbox hop for this pbuf. */
    if (rt_wlan_lwip_direct_input(device, p) == RT_EOK)
        return;
    level = rt_hw_interrupt_disable();
