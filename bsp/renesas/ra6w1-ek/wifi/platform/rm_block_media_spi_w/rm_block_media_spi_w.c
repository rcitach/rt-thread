...
#include "rm_block_media_spi_w_cfg.h"

/***********************************************************************************************************************
 * Macro definitions
 **********************************************************************************************************************/

/* "SPI" in ASCII, used to identify general RM_BLOCK_MEDIA_SPI_Wcontrol block */
#define RM_BLOCK_MEDIA_SPI_W_OPEN    (0x51535049U)
...
{
    .open        = RM_BLOCK_MEDIA_SPI_W_Open,
    .mediaInit   = RM_BLOCK_MEDIA_SPI_W_MediaInit,
    .read        = RM_BLOCK_MEDIA_SPI_W_Read,
    .write       = RM_BLOCK_MEDIA_SPI_W_Write,
    .erase       = RM_BLOCK_MEDIA_SPI_W_Erase,
    .callbackSet = RM_BLOCK_MEDIA_SPI_W_CallbackSet,
    .statusGet   = RM_BLOCK_MEDIA_SPI_W_StatusGet,
    .infoGet     = RM_BLOCK_MEDIA_SPI_W_InfoGet,
    .close       = RM_BLOCK_MEDIA_SPI_W_Close,
};
...
static fsp_err_t rm_block_media_spi_w_erasecore(rm_block_media_spi_w_instance_ctrl_t * p_instance_ctrl,
                                            uint8_t *                     rom_address,
                                            uint32_t const                block_size);

static fsp_err_t rm_block_media_spi_w_program(rm_block_media_spi_w_instance_ctrl_t * p_instance_ctrl,
                                            uint8_t                            * p_device_address,
                                            uint8_t const                      * p_buffer,
                                            uint32_t const                       num_blocks);

static fsp_err_t rm_block_media_spi_w_rom_read(rm_block_media_spi_w_instance_ctrl_t * p_instance_ctrl,
                                             uint8_t * const                      p_dest,
                                             uint32_t const                       start_block,
                                             uint32_t const                       num_blocks);

static void rm_block_media_call_callback(rm_block_media_spi_w_instance_ctrl_t * p_instance_ctrl,
                                         rm_block_media_event_t               event);

/*******************************************************************************************************************//**
 * @addtogroup RM_BLOCK_MEDIA_SPI_W
 * @{
 **********************************************************************************************************************/
 ...
 **********************************************************************************************************************/
fsp_err_t RM_BLOCK_MEDIA_SPI_W_Open (rm_block_media_ctrl_t * const p_ctrl, rm_block_media_cfg_t const * const p_cfg)
{
    rm_block_media_spi_w_instance_ctrl_t * p_instance_ctrl = (rm_block_media_spi_w_instance_ctrl_t *) p_ctrl;

#if RM_BLOCK_MEDIA_SPI_W_CFG_PARAM_CHECKING_ENABLE
    FSP_ASSERT(NULL != p_cfg);
    FSP_ASSERT(NULL != p_cfg->p_extend);
    FSP_ASSERT(NULL != ((rm_block_media_spi_w_extended_cfg_t *) p_cfg->p_extend)->p_spi);
    FSP_ASSERT(NULL != p_instance_ctrl);
    FSP_ERROR_RETURN(RM_BLOCK_MEDIA_SPI_W_OPEN != p_instance_ctrl->open, FSP_ERR_ALREADY_OPEN);
#endif

    rm_block_media_spi_w_extended_cfg_t * p_extended_cfg = (rm_block_media_spi_w_extended_cfg_t *) p_cfg->p_extend;
    ...
    /* Module is now open */
    p_instance_ctrl->open = RM_BLOCK_MEDIA_SPI_W_OPEN;
 ...
 **********************************************************************************************************************/
fsp_err_t RM_BLOCK_MEDIA_SPI_W_InfoGet (rm_block_media_ctrl_t * const p_ctrl, rm_block_media_info_t * const p_info)
{
    rm_block_media_spi_w_instance_ctrl_t * p_instance_ctrl = (rm_block_media_spi_w_instance_ctrl_t *) p_ctrl;
    ...
    FSP_ERROR_RETURN(RM_BLOCK_MEDIA_SPI_W_OPEN == p_instance_ctrl->open, FSP_ERR_NOT_OPEN);
    FSP_ERROR_RETURN(p_instance_ctrl->initialized, FSP_ERR_NOT_INITIALIZED);
#endif

    rm_block_media_spi_w_extended_cfg_t * p_extended_cfg;
    p_extended_cfg = (rm_block_media_spi_w_extended_cfg_t *) p_instance_ctrl->p_cfg->p_extend;
 ...
 * @retval     FSP_ERR_NOT_OPEN          Module is not open.
 **********************************************************************************************************************/
fsp_err_t RM_BLOCK_MEDIA_SPI_W_MediaInit (rm_block_media_ctrl_t * const p_ctrl)
{
    rm_block_media_spi_w_instance_ctrl_t * p_instance_ctrl = (rm_block_media_spi_w_instance_ctrl_t *) p_ctrl;
#if RM_BLOCK_MEDIA_SPI_W_CFG_PARAM_CHECKING_ENABLE
    FSP_ASSERT(NULL != p_instance_ctrl);
...
/******************************************************************************************************************//**
 * Reads a number of blocks from spi flash memory. By default, this is a blocking function. Non-blocking operation
 ...
 **********************************************************************************************************************/
fsp_err_t RM_BLOCK_MEDIA_SPI_W_Read (rm_block_media_ctrl_t * const p_ctrl,
                                   uint8_t * const               p_dest,
                                   uint32_t const                start_block,
                                   uint32_t const                num_blocks)
{
    ...
    fsp_err_t err = rm_block_media_spi_w_rom_read(p_instance_ctrl, p_dest, start_block, num_blocks);
    FSP_ERROR_RETURN(FSP_SUCCESS == err, err);
 ...
 **********************************************************************************************************************/
fsp_err_t RM_BLOCK_MEDIA_SPI_W_StatusGet (rm_block_media_ctrl_t * const p_ctrl, rm_block_media_status_t * const p_status)
{
    rm_block_media_spi_w_instance_ctrl_t * p_instance_ctrl = (rm_block_media_spi_w_instance_ctrl_t *) p_ctrl;
...
/******************************************************************************************************************//**
 * Writes provided data to a number of blocks of spi flash memory. By default, this is a blocking function.
 ...
 **********************************************************************************************************************/
fsp_err_t RM_BLOCK_MEDIA_SPI_W_Write (rm_block_media_ctrl_t * const p_ctrl,
                                    ...
                                    uint32_t const                num_blocks)
{
    ...
    if(p_extended_cfg->deny_implicit_erase == false)
    {
        /* I think that the erase operation, which is executed every time a program operation is executed,
        * is not good in terms of a flash endurance.
        */
        err = RM_BLOCK_MEDIA_SPI_W_Erase(p_instance_ctrl, start_block, num_blocks);
        FSP_ERROR_RETURN(FSP_SUCCESS == err, err);
    }
    ...
    err = rm_block_media_spi_w_program(p_instance_ctrl, (uint8_t *) rom_address, p_src, num_blocks);
    FSP_ERROR_RETURN(FSP_SUCCESS == err, err);
 ...
 *
 * @retval  FSP_ERR_UNSUPPORTED         API not supported by RM_BLOCK_MEDIA_SPI_W.
 **********************************************************************************************************************/
fsp_err_t RM_BLOCK_MEDIA_SPI_W_CallbackSet (rm_block_media_ctrl_t * const p_ctrl,
                                          void (                      * p_callback)(
 ...
 **********************************************************************************************************************/
fsp_err_t RM_BLOCK_MEDIA_SPI_W_Close (rm_block_media_ctrl_t * const p_ctrl)
{
    rm_block_media_spi_w_instance_ctrl_t * p_instance_ctrl = (rm_block_media_spi_w_instance_ctrl_t *) p_ctrl;
...
/*******************************************************************************************************************//**
 * This function erases blocks of the SPI device. By default, this is a blocking function. Non-blocking operation
 * may be achieved by yielding control within the optional callback function.
 *
 ...
 *             * @ref spi_flash_api_t::statusGet
 **********************************************************************************************************************/
fsp_err_t RM_BLOCK_MEDIA_SPI_W_Erase (rm_block_media_ctrl_t * const p_ctrl,
                                    uint32_t const                start_block,
                                    uint32_t const                num_blocks)
{
    rm_block_media_spi_w_extended_cfg_t * p_extended_cfg;
    ...
    {
        err = rm_block_media_spi_w_erasecore(p_instance_ctrl, rom_address, block_size);
...
/*******************************************************************************************************************//**
 * This is the subroutine for the RM_BLOCK_MEDIA_SPI_W_Erase API.
 *
 ...
 *                                      * @ref spi_flash_api_t::statusGet
 **********************************************************************************************************************/
BSP_PLACE_CODE_IN_RAM static fsp_err_t rm_block_media_spi_w_erasecore (rm_block_media_spi_w_instance_ctrl_t * p_instance_ctrl,
                                    uint8_t *                     rom_address,
                                    uint32_t const                block_size)
{
    rm_block_media_spi_w_extended_cfg_t * p_extended_cfg;
...
/*******************************************************************************************************************//**
 * This is the subroutine for the RM_BLOCK_MEDIA_SPI_W_Write API.
 *
 * @retval  FSP_SUCCESS             No parameter error found
 * @return                          See @ref RENESAS_ERROR_CODES or HAL driver for other possible return codes or causes.
 *                                  This function calls
 ...
 *                                      * @ref spi_flash_api_t::statusGet
 **********************************************************************************************************************/
BSP_PLACE_CODE_IN_RAM static fsp_err_t rm_block_media_spi_w_program (rm_block_media_spi_w_instance_ctrl_t * p_instance_ctrl,
                                             uint8_t                            * p_device_address,
                                             uint8_t const                      * p_buffer,
                                             uint32_t const                       num_blocks)
{
...
/*******************************************************************************************************************//**
 * This is the subroutine for the RM_BLOCK_MEDIA_SPI_W_READ API to read data from banks using ROM area.
 *
 * @retval  FSP_SUCCESS             No parameter error found
 **********************************************************************************************************************/
static fsp_err_t rm_block_media_spi_w_rom_read (rm_block_media_spi_w_instance_ctrl_t * p_instance_ctrl,
...
/*******************************************************************************************************************//**
 * @} (end addtogroup RM_BLOCK_MEDIA_SPI_W)
 **********************************************************************************************************************/
