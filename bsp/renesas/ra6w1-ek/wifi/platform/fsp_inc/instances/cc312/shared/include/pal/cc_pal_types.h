 ...
 * are one byte. */
#define CALC_FULL_BYTES(numBits)                        ((numBits) / CC_BITS_IN_BYTE + \
                                                         (((numBits) & (CC_BITS_IN_BYTE - 1)) > 0))

/*! This macro calculates the number of full 32-bit words from bits, where
 * 31 bits are one word. */
#define CALC_FULL_32BIT_WORDS(numBits)                  ((numBits) / CC_BITS_IN_32BIT_WORD + \
                                                         (((numBits) & (CC_BITS_IN_32BIT_WORD - 1)) > 0))

/*! This macro calculates the number of full 32-bit words from bytes, where
 * three bytes are one word. */
#define CALC_32BIT_WORDS_FROM_BYTES(sizeBytes)          ((sizeBytes) / CC_32BIT_WORD_SIZE + \
                                                         (((sizeBytes) & (CC_32BIT_WORD_SIZE - 1)) > 0))
