...
/*! Temporary buffer size definition.*/
 #define CC_RSA_TMP_BUFF_SIZE                            (CC_RSA_OAEP_ENCODE_MAX_MASKDB_SIZE +                   \
                                                          CC_RSA_OAEP_ENCODE_MAX_SEEDMASK_SIZE +                 \
                                                          CC_PKA_RSA_HASH_CTX_SIZE_IN_WORDS * sizeof(uint32_t) + \
                                                          sizeof(CCHashResultBuf_t))
...
typedef struct CCRsaKgFipsContext_t
{
    ...
    uint8_t decBuff[((CC_RSA_MIN_VALID_KEY_SIZE_VALUE_IN_BITS / CC_BITS_IN_BYTE) - 2 *
                     (CC_HASH_SHA1_DIGEST_SIZE_IN_BYTES) - 2)];
...
typedef struct CCRsaFipsKatContext_t
{
    ...
    union
    {
        struct                         // used for RsaEnc and RsaDec
        {
            ...
            uint8_t decBuff[((CC_RSA_FIPS_MODULUS_SIZE_BITS / CC_BITS_IN_BYTE) - 2 *
                             (CC_HASH_SHA1_DIGEST_SIZE_IN_BYTES) - 2)];
        } userOaepData;
