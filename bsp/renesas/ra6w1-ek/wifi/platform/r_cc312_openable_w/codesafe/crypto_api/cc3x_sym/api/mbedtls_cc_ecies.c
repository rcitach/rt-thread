...

{
    ...
    /* write mpi to buf */
    if ((error =
             mbedtls_ecp_point_write_binary(pGrp, pPubKey, MBEDTLS_ECP_PF_UNCOMPRESSED, &keylen, pBuf, bufSize)) != 0)
    {
    ...
    if ((error =
             CC_EcpkiPublKeyBuildAndCheck(pDomain, pBuf, keylen, ECpublKeyFullCheck, pUserPublKey,
                                          ...
                                          void * p_rng)
{
    ...
    if ((error =
             ecies_convert_mbed_to_cc_public_key(pGrp, pDomain, pRecipUzPublKey, pCcReciptUzPublKey, pTempBuff)) != 0)
    {
    ...
    else
    {
        ...
        if ((error =
                 ecies_convert_mbed_to_cc_public_key(pGrp, pDomain, pExtEphUzPublicKey, pEphUzPublKey, pTempBuff)) != 0)
        {
