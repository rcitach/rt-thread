...
void PkiDbgPrintReg(const char * label, const uint32_t reg);

void PkiDbgPrintUint32BuffAsNum(const char * label, const uint32_t * pBuf, uint32_t sizeWords);

void PkiDbgPrintUint8BuffAsNum(const char * label, const uint8_t * pBuf, uint32_t sizeBytes);
void PkiDbgPrintUint8Buff(const char * label, const uint8_t * pBuf, uint32_t sizeBytes);
 ...
 *  then set value RES_DISCARD                                   */
  #define PKI_DBG_PRINT_REGS(label, reg1, reg2, reg3, reg4) \
    do {PKA_PRINTF("%s", label);                            \
        ...
        if (reg4 < 32) {PkiDbgPrintReg(#reg4 "=", reg4);}   \
    } while (0)
