                         ...
                         uint32_t blockSize)
{
	...
	if (outputDataAddrType == DLLI_ADDR){
		CC_HAL_WRITE_REGISTER(CC_REG_OFFSET(HOST_RGF, DST_LLI_WORD0) ,CRYPTO_RRQ61X_REMAPPER(outputDataAddr));
		CC_HAL_WRITE_REGISTER(CC_REG_OFFSET(HOST_RGF, DST_LLI_WORD1) ,blockSize);
	} else {
		CC_HAL_WRITE_REGISTER(CC_REG_OFFSET(HOST_RGF, SRAM_DEST_ADDR) ,CRYPTO_RRQ61X_REMAPPER(outputDataAddr));
	...
	if (inputDataAddrType == DLLI_ADDR){
		CC_HAL_WRITE_REGISTER(CC_REG_OFFSET(HOST_RGF, SRC_LLI_WORD0) ,CRYPTO_RRQ61X_REMAPPER(inputDataAddr));
		CC_HAL_WRITE_REGISTER(CC_REG_OFFSET(HOST_RGF, SRC_LLI_WORD1) ,blockSize);
	} else {
		CC_HAL_WRITE_REGISTER(CC_REG_OFFSET(HOST_RGF, SRAM_SRC_ADDR) ,CRYPTO_RRQ61X_REMAPPER(inputDataAddr));
