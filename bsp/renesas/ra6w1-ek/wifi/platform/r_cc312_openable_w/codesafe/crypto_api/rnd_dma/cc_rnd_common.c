...
static CCError_t PkaDeriveMPI(uint8_t  *rndVect_ptr,
			     uint8_t  *maxVect_ptr,
			     size_t     maxVectSizeBits,
			     size_t     rndSizeInBytes)
{
        uint8_t rndp = regTemps[26];
        uint8_t rndt = regTemps[27];
        uint8_t maxv = regTemps[28];

        uint32_t pkaReqRegs = PKA_MAX_COUNT_OF_PHYS_MEM_REGS;
        uint32_t rshiftbit /*, stat*/; //error: unused variable 'stat' [-Werror=unused-variable]
        CCError_t err = CC_OK;

        err = PkaInitAndMutexLock(maxVectSizeBits, &pkaReqRegs);
        if (err != CC_OK) {
                return err;
        }

        if( (rndSizeInBytes * 8) > maxVectSizeBits ){
                rshiftbit = ( rndSizeInBytes * 8 ) - maxVectSizeBits;
        }else{
                rshiftbit = 0;
        }

        /* Set modulus sizes to L0, L1 and order sizes in L2, L3 */
        CC_HAL_WRITE_REGISTER(CC_REG_OFFSET (CRY_KERNEL, PKA_L0), maxVectSizeBits);

        PkaCopyDataIntoPkaReg(rndp, 1, (uint32_t *)rndVect_ptr, CALC_32BIT_WORDS_FROM_BYTES(rndSizeInBytes));
        PkaCopyDataIntoPkaReg(maxv, 1, (uint32_t *)maxVect_ptr, CALC_32BIT_WORDS_FROM_BYTES(rndSizeInBytes));

        if( rshiftbit > 0 ){
                PKA_SHR_FILL0(LEN_ID_N_PKA_REG_BITS, rndt, rndp, (rshiftbit-1));
        }

        /*  Output data from PKA registers  */
        if( rshiftbit > 0 ){
                PkaCopyDataFromPkaReg((uint32_t *)rndVect_ptr, CALC_32BIT_WORDS_FROM_BYTES(rndSizeInBytes), rndt);
        }
...
CEXPORT_C CCError_t CC_RndGenerateVectorInRangeECP(
                                                    CCRndContext_t *rndContext_ptr, 
                                                    size_t   rndSizeInBits, 
                                                    uint8_t  *maxVect_ptr, 
                                                    uint8_t  *rndVect_ptr )
{
	...
	if (rndSizeInBits > 0x7FFFF)
        ...
        while (k < 0xFFFF) {
                ...
                PkaDeriveMPI(rndVect_ptr, maxVect_ptr, maxVectSizeBits, rndSizeInBytes);
                ...
                if (checkingSizeBytes == CC_AES_BLOCK_SIZE_IN_BYTES) {
                        ...
                        while (1) {
                                ...
                                *  repeat the same Additional Data            */
                                ...
                                /* swap from big endian to little*/
                                CC_CommonReverseMemcpy((rndVect_ptr + maxVectSizeBytes), (rndVect_ptr + maxVectSizeBytes), CC_AES_BLOCK_SIZE_IN_BYTES);
...
} /* End of CC_RndGenerateVectorInRangeECP function */
