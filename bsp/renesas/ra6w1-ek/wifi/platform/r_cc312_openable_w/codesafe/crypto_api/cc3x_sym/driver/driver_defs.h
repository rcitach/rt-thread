...
#define CC_2_DRIVER_DIRECTION(ssiDirection)       ((ssiDirection ==                                  \
                                                    CC_AES_ENCRYPT) ? (CRYPTO_DIRECTION_ENCRYPT) : ( \
...
#define MBEDTLS_2_DRIVER_DIRECTION(mbedtlsDir)    ((mbedtlsDir == \
                                                    1) ? (CRYPTO_DIRECTION_ENCRYPT) : (CRYPTO_DIRECTION_DECRYPT))
...
/* Verifies if one of the keys: Kpicv, Kceicv, HBK0, Kcp or Kce is in use */
#define CC_IS_OTP_KEY_IN_USE(owner, keyType, rc, isKeyInUse)                                        \
    do {                                                                                            \
        ...
        if (rc == CC_OK) {                                                                          \
            notInUse = BITFIELD_GET(otpVal,                                                         \
                                    CC_OTP_ ## owner ## _FLAG_ ## keyType ## _NOT_IN_USE_BIT_SHIFT, \
 ...
 */
drvError_t SetDataBuffersInfo(const uint8_t * pDataIn,
                              size_t          dataInSize,
                              CCBuffInfo_t  * pInputBuffInfo,
                              const uint8_t * pDataOut,
                              size_t          dataOutSize,
                              CCBuffInfo_t  * pOutputBuffInfo);
