...
\note The input data is placed as BE bytes arrays into 32-bit word buffers
   ...
   @param[in] mHash - SHA256 hash of the message (32 bytes).
   @param[in] pEncodedMsg - Pointer to PSS encoded message (EM). Size = modulus size.
   @param[out] pVerifyStat - Pointer to validation status value, equalled to: 1 - if valid, 0 - if not valid.

   @return CC_OK on success,
   @return A non-zero value from secureboot_error.h on failure.
*/

CCError_t RSA_PSS_Decode(unsigned long hwBaseAddress,  
