...
/* ROM ==> RT */
 ...
 #define _CCSbImageLoadAndVerify(preHashflashRead_func, \
                                 preHashUserContext,    \
                                 hwBaseAddress,         \
                                 isLoadFromFlash,       \
                                 isVerifyImage,         \
                                 ...
                                 AESIv,                 \
                                 pSwRecSignedData,      \
                                 pSwRecNoneSignedData,  \
                                 workspace_ptr,         \
                                 workspaceSize)         \
    SBRT_ImageLoadAndVerify(preHashflashRead_func,      \
                            preHashUserContext,         \
                            hwBaseAddress,              \
                            isLoadFromFlash,            \
                            isVerifyImage,              \
 ...
 #define BsvCryptoImageUpdate(hwBaseAddress, \
                              mode,          \
                              keyType,       \
                              pCtrStateBuf,  \
                              pDataIn,       \
...
/* rsa_hwdefs => pka_hw_defs */
 ...
 #define RSA_PKA_GetRegAddress(VirtReg, VirtualHwBaseAddr) \
    (*((volatile uint32_t *) (DX_BASE_CC + CC_REG_OFFSET(CRY_KERNEL, MEMORY_MAP0) + 4 * (VirtReg))))
