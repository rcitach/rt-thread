...
#define SUPPORT_PROD_HW_OTP
#ifdef  SUPPORT_PROD_HW_OTP
...
#define  CC_PROD_OTP_READ(otpData, otpWordOffset)				\
	do { 									\
            otpData = CRYPTO_OTP_READ((uint32_t)((otpWordOffset)*CC_32BIT_WORD_SIZE));\
	}while(0)

#define CC_PROD_OTP_WRITE(otpData, otpWordOffset)				\
	do { 									\
                CRYPTO_OTP_WRITE((uint32_t)((otpWordOffset)*CC_32BIT_WORD_SIZE), otpData);\
	}while(0)

#else   //SUPPORT_PROD_HW_OTP
