...
static void ssl_tls13_hkdf_encode_label (size_t                desired_length,
                                         ...
                                         size_t                buf_len)
{
    ...
    ssl_tls13_hkdf_encode_label(buf_len, label, label_len, ctx, ctx_len, hkdf_label, &hkdf_label_len);
    ...
    status = psa_key_derivation_input_bytes(&operation, PSA_KEY_DERIVATION_INPUT_SECRET, secret, secret_len);
    ...
    status = psa_key_derivation_input_bytes(&operation, PSA_KEY_DERIVATION_INPUT_INFO, hkdf_label, hkdf_label_len);
...
static int ssl_tls13_make_traffic_key (psa_algorithm_t       hash_alg,
                                       const unsigned char * secret,
                                       size_t                secret_len,
                                       unsigned char       * key,
                                       size_t                key_len,
                                     ...
                                     size_t                dstbuf_len)
{
    ...
    if (ctx_hashed == MBEDTLS_SSL_TLS1_3_CONTEXT_UNHASHED)
    {
        ...
        status = psa_hash_compute(hash_alg, ctx, ctx_len, hashed_context, PSA_HASH_LENGTH(hash_alg), &ctx_len);
        if (status != PSA_SUCCESS)
        {
                                     ...
                                     unsigned char       * secret_new)
{
    ...
    status = psa_key_derivation_input_bytes(&operation, PSA_KEY_DERIVATION_INPUT_SALT, tmp_secret, hlen);
    ...
    status = psa_key_derivation_input_bytes(&operation, PSA_KEY_DERIVATION_INPUT_SECRET, l_input, l_input_len);
                                                ...
                                                mbedtls_ssl_tls13_handshake_secrets * derived)
{
     ...
     */
    ret =
        mbedtls_ssl_tls13_derive_secret(hash_alg,
                                        handshake_secret,
                                        hash_len,
...
static int ssl_tls13_key_schedule_stage_application (mbedtls_ssl_context * ssl)
{
    ...
    MBEDTLS_SSL_DEBUG_BUF(4, "Master secret", handshake->tls13_master_secrets.app, PSA_HASH_LENGTH(hash_alg));
                                             ...
                                             int                   from)
{
    ...
    psa_algorithm_t hash_alg = mbedtls_md_psa_alg_from_type((mbedtls_md_type_t) ssl->handshake->ciphersuite_info->mac);
    size_t const    hash_len = PSA_HASH_LENGTH(hash_alg);
    ...
    ret = mbedtls_ssl_get_handshake_transcript(ssl, md_type, transcript, sizeof(transcript), &transcript_len);
    if (ret != 0)
    {
                                          ...
                                          mbedtls_ssl_context       * ssl /* DEBUG ONLY */)
{
 ...
 #if CFG_PMGR
    memcpy(&transform->keyblk.tlsv13, traffic_keys, sizeof(mbedtls_ssl_key_set));
 #endif                                /* CFG_PMGR */
 ...
 #if !defined(MBEDTLS_USE_PSA_CRYPTO)
    if ((ret =
             mbedtls_cipher_setkey(&transform->cipher_ctx_enc, key_enc,
    ...
    if ((ret =
             mbedtls_cipher_setkey(&transform->cipher_ctx_dec, key_dec,
     ...
     */
    if ((status =
             mbedtls_ssl_cipher_to_psa((mbedtls_cipher_type_t) ciphersuite_info->cipher, transform->taglen, &alg,
    ...
    if (alg != MBEDTLS_SSL_NULL_CIPHER)
    {
        ...
        if ((status =
                 psa_import_key(&attributes, key_enc, PSA_BITS_TO_BYTES(key_bits),
        ...
        if ((status =
                 psa_import_key(&attributes, key_dec, PSA_BITS_TO_BYTES(key_bits),
 ...
 */
MBEDTLS_CHECK_RETURN_CRITICAL
static int ssl_tls13_generate_early_key (mbedtls_ssl_context * ssl, mbedtls_ssl_key_set * traffic_keys)
{
    int                             ret = MBEDTLS_ERR_ERROR_CORRUPTION_DETECTED;
    ...
    MBEDTLS_SSL_DEBUG_BUF(4, "Client early traffic secret", tls13_early_secrets.client_early_traffic_secret, hash_len);
    ...
    MBEDTLS_SSL_DEBUG_BUF(4, "client early write_key", traffic_keys->client_write_key, traffic_keys->key_len);

    MBEDTLS_SSL_DEBUG_BUF(4, "client early write_iv", traffic_keys->client_write_iv, traffic_keys->iv_len);
...
int mbedtls_ssl_tls13_compute_early_transform (mbedtls_ssl_context * ssl)
{
    ...
    ret = mbedtls_ssl_tls13_populate_transform(transform_earlydata,
                                               ssl->conf->endpoint,
...
int mbedtls_ssl_tls13_key_schedule_stage_early (mbedtls_ssl_context * ssl)
{
    ...
    ret = mbedtls_ssl_tls13_evolve_secret(hash_alg, NULL, psk, psk_len, handshake->tls13_master_secrets.early);
 #if defined(MBEDTLS_USE_PSA_CRYPTO) && \
 ...
 */
MBEDTLS_CHECK_RETURN_CRITICAL
static int ssl_tls13_generate_handshake_keys (mbedtls_ssl_context * ssl, mbedtls_ssl_key_set * traffic_keys)
{
    int               ret = MBEDTLS_ERR_ERROR_CORRUPTION_DETECTED;
    mbedtls_md_type_t md_type;
    psa_algorithm_t   hash_alg;
    ...
    MBEDTLS_SSL_DEBUG_BUF(4, "client_handshake write_key", traffic_keys->client_write_key, traffic_keys->key_len);

    MBEDTLS_SSL_DEBUG_BUF(4, "server_handshake write_key", traffic_keys->server_write_key, traffic_keys->key_len);

    MBEDTLS_SSL_DEBUG_BUF(4, "client_handshake write_iv", traffic_keys->client_write_iv, traffic_keys->iv_len);

    MBEDTLS_SSL_DEBUG_BUF(4, "server_handshake write_iv", traffic_keys->server_write_iv, traffic_keys->iv_len);
...
static int ssl_tls13_key_schedule_stage_handshake (mbedtls_ssl_context * ssl)
{
    ...
    MBEDTLS_SSL_DEBUG_BUF(4, "Handshake secret", handshake->tls13_master_secrets.handshake, PSA_HASH_LENGTH(hash_alg));
 ...
 */
MBEDTLS_CHECK_RETURN_CRITICAL
static int ssl_tls13_generate_application_keys (mbedtls_ssl_context * ssl, mbedtls_ssl_key_set * traffic_keys)
{
    int ret = MBEDTLS_ERR_ERROR_CORRUPTION_DETECTED;
    mbedtls_ssl_handshake_params * handshake = ssl->handshake;
    ...
    MBEDTLS_SSL_DEBUG_BUF(4, "client application_write_key:", traffic_keys->client_write_key, key_len);
    MBEDTLS_SSL_DEBUG_BUF(4, "server application write key", traffic_keys->server_write_key, key_len);
...
int mbedtls_ssl_tls13_compute_resumption_master_secret (mbedtls_ssl_context * ssl)
{
    ...
    mbedtls_platform_zeroize(&handshake->tls13_master_secrets, sizeof(handshake->tls13_master_secrets));
...
cleanup:
 ...
 #if defined(MBEDTLS_SSL_TLS1_3_KEY_EXCHANGE_MODE_SOME_PSK_ENABLED)
int mbedtls_ssl_tls13_export_handshake_psk (mbedtls_ssl_context * ssl, unsigned char ** psk, size_t * psk_len)
{
  #if defined(MBEDTLS_USE_PSA_CRYPTO)
    psa_key_attributes_t key_attributes = PSA_KEY_ATTRIBUTES_INIT;
    psa_status_t         status         = PSA_ERROR_CORRUPTION_DETECTED;
...
int mbedtls_ssl_tls13_exporter (const psa_algorithm_t hash_alg,
                                ...
                                const unsigned char * context_value,
