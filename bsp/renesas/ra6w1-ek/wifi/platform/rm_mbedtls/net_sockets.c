...
#include "common.h"
 ...
 #include <string.h>
  ...
  /* RT-Thread does not enable POSIX I/O aliases by default. */
  #ifndef read
...
int mbedtls_net_connect (mbedtls_net_context * ctx, const char * host, const char * port, int proto)
{
    ...
    for (cur = addr_list; cur != NULL; cur = cur->ai_next)
    {
        ...
        {
            ctx->fd = (int) socket_dpm(ctx->name, cur->ai_family, cur->ai_socktype, cur->ai_protocol);
        }
        else
        {
...
int mbedtls_net_set_name (mbedtls_net_context * ctx, char * in, size_t inlen, int force)
{
 #if (LWIP_IPV4 || LWIP_IPV6) && CFG_PMGR
    ...
    if (!force && (strlen(ctx->name) > 0))
    {
        return MBEDTLS_ERR_NET_ALREADY_EXIST;
    }

    if (inlen > sizeof(ctx->name) - 1)
    {
        return MBEDTLS_ERR_NET_BAD_INPUT_DATA;
    }

    strncpy(ctx->name, in, inlen);

    return 0;
 #else
    FSP_PARAMETER_NOT_USED(ctx);
    FSP_PARAMETER_NOT_USED(in);
    FSP_PARAMETER_NOT_USED(inlen);
    FSP_PARAMETER_NOT_USED(force);

    return MBEDTLS_ERR_NET_NOT_SUPPORTED;
 #endif                                // (LWIP_IPV4 || LWIP_IPV6) && CFG_PMGR
}

/*
 * Initiate a TCP connection with host:port, local:port and the given protocol
 */
int mbedtls_net_connect_with_bind (mbedtls_net_context * ctx,
                                   const char          * host,
                                   const char          * port,
                                   int                   proto,
                                   const char          * bind_ip,
                                   ...
                                   unsigned int          timeout)
{
    ...
    if (required_bind)
    {
        ...
        if (getaddrinfo(bind_ip, bind_port, &hints, &addr_list) != 0)
        {
            return MBEDTLS_ERR_NET_UNKNOWN_HOST;
        }
    ...
    for (cur = addr_list; cur != NULL; cur = cur->ai_next)
    {
        ...
        if (timeout > 0)
        {
            ...
            FD_ZERO(&write_fds);
            FD_SET((unsigned int) ctx->fd, &write_fds);

            tv.tv_sec  = timeout / 1000;
            tv.tv_usec = (suseconds_t) ((timeout % 1000) * 1000);

            if (select(ctx->fd + 1, NULL, &write_fds, NULL, &tv) == 1)
            {
                int       err     = 0;
                socklen_t err_len = sizeof(err);

                getsockopt(ctx->fd, SOL_SOCKET, SO_ERROR, &err, &err_len);
                if (err == 0)
                {
                    ret = 0;
                    break;
 ...
 #else
    FSP_PARAMETER_NOT_USED(ctx);
    FSP_PARAMETER_NOT_USED(host);
    FSP_PARAMETER_NOT_USED(port);
    FSP_PARAMETER_NOT_USED(proto);
    FSP_PARAMETER_NOT_USED(bind_ip);
    FSP_PARAMETER_NOT_USED(bind_port);
