                                   ...
                                   size_t pms_offset)
{
    ...
    if (offset + len_bytes > ssl->out_content_len) {
    ...
    if (!mbedtls_pk_can_do(peer_pk, MBEDTLS_PK_RSA)) {
        ...
        if ((ret = mbedtls_pk_encrypt(peer_pk,
                                  ...
                                  ssl->out_content_len - offset - len_bytes,
...
#if defined(MBEDTLS_SSL_VARIABLE_CONTENT_LENGTH)
        nsigned char *end = ssl->out_msg + ssl->out_content_len;
#else
...
#if defined(MBEDTLS_SSL_VARIABLE_CONTENT_LENGTH)
        unsigned char *end = ssl->out_msg + ssl->out_content_len;
#else
        unsigned char *end = ssl->out_msg + MBEDTLS_SSL_OUT_CONTENT_LEN;
#endif
        size_t own_pubkey_max_len = (size_t) (end - own_pubkey);
    ...
    if (mbedtls_ssl_ciphersuite_uses_psk(ciphersuite_info)) {
...
#if defined(MBEDTLS_SSL_VARIABLE_CONTENT_LENGTH)
        if (header_len + 2 + content_len > ssl->out_content_len) {
            MBEDTLS_SSL_DEBUG_MSG(1,
                                  ("psk identity too long or SSL buffer too short"));
            return MBEDTLS_ERR_SSL_BUFFER_TOO_SMALL;
        }
        ...
        if (ciphersuite_info->key_exchange == MBEDTLS_KEY_EXCHANGE_ECDHE_PSK) {
            ...
            ret = mbedtls_ecdh_make_public(&ssl->handshake->ecdh_ctx,
                                           ...
                                           ssl->out_content_len - header_len,
                                           ssl->conf->f_rng, ssl->conf->p_rng);
#else
            ret = mbedtls_ecdh_make_public(&ssl->handshake->ecdh_ctx,
                                           &content_len,
