...
static const int ciphersuite_rrq_hw_preference[] =
{
#if defined(MBEDTLS_SSL_PROTO_TLS1_3)
    /* TLS 1.3 ciphersuites */
    MBEDTLS_TLS1_3_AES_128_GCM_SHA256,
...
const int *mbedtls_ssl_list_rrq_ciphersuites( void )
{
    ...
    if (supported_init == 0) {
        ...
        for(p = ciphersuite_rrq_preference, q = supported_ciphersuites;
            *p != 0 && q < supported_ciphersuites + MAX_CIPHERSUITES - 1;
            p++) {
            const mbedtls_ssl_ciphersuite_t *cs_info;
            if( ( cs_info = mbedtls_ssl_ciphersuite_from_id( *p ) ) != NULL &&
...
const int *mbedtls_ssl_list_rrq_hw_ciphersuites( void )
{
    ...
    if(supported_init == 0) {
        ...
        for(p = ciphersuite_rrq_hw_preference, q = supported_ciphersuites;
             *p != 0 && q < supported_ciphersuites + MAX_CIPHERSUITES - 1;
             p++) {
            const mbedtls_ssl_ciphersuite_t *cs_info;
            if ((cs_info = mbedtls_ssl_ciphersuite_from_id(*p)) != NULL &&
