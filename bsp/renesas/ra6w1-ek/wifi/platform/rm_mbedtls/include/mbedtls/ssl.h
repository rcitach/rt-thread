 ...
 * NONE must be zero so that memset()ing structure to zero works */
 ...
 #define MBEDTLS_SSL_PRESET_RRQ                                    3
 #define MBEDTLS_SSL_PRESET_RRQ_HW                                 4

// To be obsoleted.
 #define MBEDTLS_SSL_PRESET_RA6WX                                  MBEDTLS_SSL_PRESET_RRQ
 #define MBEDTLS_SSL_PRESET_RA6WX_ONLY_HW                          MBEDTLS_SSL_PRESET_RRQ_HW
...
struct mbedtls_ssl_config
{
    ...
    unsigned char force_sync_iv;
    unsigned int  force_sync_iv_gap;
};
...
struct mbedtls_ssl_context
{
    ...
    unsigned char           use_pac;
    mbedtls_ssl_eap_pac_t * MBEDTLS_PRIVATE(f_eap_pac);
    void                  * MBEDTLS_PRIVATE(p_eap_pac);

    unsigned char prev_in_iv[8];       /*!< only for AES_CCM & AES_GCM(ivlen: 12 - 4) */

 #if defined(MBEDTLS_SSL_VARIABLE_CONTENT_LENGTH)
    size_t MBEDTLS_PRIVATE(in_content_len);
    size_t MBEDTLS_PRIVATE(out_content_len);
 #endif                                /* MBEDTLS_SSL_VARIABLE_CONTENT_LENGTH */
};
 ...
 */
static inline int mbedtls_ssl_session_get_ticket_creation_time (mbedtls_ssl_session * session,
 ...
 */
static inline
void mbedtls_ssl_conf_dn_hints (mbedtls_ssl_config * conf, const mbedtls_x509_crt * crt)
{
...
/**
 * \brief          Set up buffer length of incoming/outgoing content
 *
 * \note           Added by Dialog
 *
 * \param conf     SSL configuration context
 * \param in       Length of incoming content
 * \param out      Length of outgoing content
 */
...
void mbedtls_ssl_set_sync_iv(mbedtls_ssl_config * conf, unsigned int enabled, unsigned int gap);
