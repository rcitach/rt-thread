                              ...
                              size_t *olen)
{
    ...
    size_t ext_len;
#if defined(MBEDTLS_SSL_VARIABLE_CONTENT_LENGTH)
    const unsigned char *end = ssl->out_msg + ssl->out_content_len;
#else
...
#if defined(MBEDTLS_SSL_ALPN)
#if defined(MBEDTLS_SSL_VARIABLE_CONTENT_LENGTH)
    unsigned char *end = buf + ssl->out_content_len - 4;
#else
...
static int ssl_write_certificate_request(mbedtls_ssl_context *ssl)
{
    ...
    unsigned char *buf, *p;
#if defined(MBEDTLS_SSL_VARIABLE_CONTENT_LENGTH)
    const unsigned char * const end = ssl->out_msg + ssl->out_content_len;
#else
...
#if defined(MBEDTLS_SSL_VARIABLE_CONTENT_LENGTH)
    size_t sig_max_len = ( ssl->out_buf + ssl->out_content_len 
                           - sig_start );
#else
    ...
    if (ciphersuite_info->key_exchange == MBEDTLS_KEY_EXCHANGE_ECJPAKE) {
...
#if defined(MBEDTLS_SSL_VARIABLE_CONTENT_LENGTH)
        unsigned char *end_p = ssl->out_msg + ssl->out_content_len -
                               ssl->out_msglen;
#else
        ...
        *out_p = MBEDTLS_ECP_TLS_NAMED_CURVE;
        ...
        ret = mbedtls_ecjpake_write_round_two(
            ...
            ssl->out_content_len - ssl->out_msglen, &len,
            ssl->conf->f_rng, ssl->conf->p_rng);
#else
        ret = mbedtls_ecjpake_write_round_two(
            &ssl->handshake->ecjpake_ctx,
...
curve_matching_done:
...
#if defined(MBEDTLS_SSL_VARIABLE_CONTENT_LENGTH)
        size_t own_pubkey_max_len = (size_t) (ssl->out_content_len
        ...
        *p = (uint8_t) len;
        ...
        if ((ret = mbedtls_ecdh_make_params(
                 ...
                 ssl->out_msg + ssl->out_msglen,
                 ssl->out_content_len - ssl->out_msglen,
                 ssl->conf->f_rng, ssl->conf->p_rng)) != 0) {
...
static int ssl_write_new_session_ticket(mbedtls_ssl_context *ssl)
{
    ...
    if ((ret = ssl->conf->f_ticket_write(ssl->conf->p_ticket,
                                         ...
                                         ssl->out_msg + 10,
                                         ssl->out_msg + ssl->out_content_len,
                                         &tlen, &lifetime)) != 0) {
