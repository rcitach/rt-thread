...
#elif defined(MBEDTLS_SSL_VARIABLE_CONTENT_LENGTH)
    in_buf_len = MBEDTLS_SSL_IN_BUFFER_LEN - MBEDTLS_SSL_IN_CONTENT_LEN + conf->in_content_len;
    ssl->in_buf_len = in_buf_len;
    ssl->in_content_len = ssl->conf->in_content_len;
#endif
...
#elif defined(MBEDTLS_SSL_VARIABLE_CONTENT_LENGTH)
    out_buf_len = MBEDTLS_SSL_OUT_BUFFER_LEN - MBEDTLS_SSL_OUT_CONTENT_LEN + conf->out_content_len;
    ssl->out_buf_len = out_buf_len;
    ssl->out_content_len = ssl->conf->out_content_len;
#endif
...
#if defined(MBEDTLS_SSL_VARIABLE_BUFFER_LENGTH) || defined(MBEDTLS_SSL_VARIABLE_CONTENT_LENGTH)
    ssl->in_buf_len = 0;
    ssl->out_buf_len = 0;
#endif

#if defined(MBEDTLS_SSL_VARIABLE_CONTENT_LENGTH)
    ssl->in_content_len = 0;
    ssl->out_content_len = 0;
#endif
    ssl->in_buf = NULL;
    ssl->out_buf = NULL;
                                     ...
                                     size_t psk_identity_len)
{
    ...
    if (psk_identity               == NULL ||
        ...
        psk_identity_len > conf->out_content_len)
    {
        return MBEDTLS_ERR_SSL_BAD_INPUT_DATA;
    }
#else
...
{
#if defined(MBEDTLS_SSL_VARIABLE_CONTENT_LENGTH)
    const size_t max_len = ssl->out_content_len;
#else
...
{
#if defined(MBEDTLS_SSL_VARIABLE_CONTENT_LENGTH)
    size_t max_len = ssl->in_content_len;
#else
...
{
#if defined(MBEDTLS_SSL_VARIABLE_CONTENT_LENGTH)
    size_t max_len = ssl->out_content_len;
#else
                                ...
                                int endpoint, int transport, int preset)
{
    ...
    switch (preset) {
        ...
        /*
         * RRQ Suite only using HW engine
         */
        case MBEDTLS_SSL_PRESET_RRQ_HW:
            conf->ciphersuite_list = mbedtls_ssl_list_rrq_hw_ciphersuites();
    ...
    conf->in_content_len = MBEDTLS_SSL_IN_CONTENT_LEN;
    conf->out_content_len = MBEDTLS_SSL_OUT_CONTENT_LEN;
...
#if defined(MBEDTLS_SSL_EAP_TLS)
    //Precalculate master for EAP-PAC
    if( ssl->use_pac && ( ssl->handshake->resume != 0 ) )
    {
        if( ssl->f_eap_pac != NULL )
        {
            ssl->f_eap_pac( ssl->p_eap_pac,
                            ssl->handshake->randbytes,
                            ssl->session_negotiate->master,
                            sizeof(ssl->session_negotiate->master) );
        }
    }
#endif /* MBEDTLS_SSL_EAP_TLS */
...
int mbedtls_ssl_write_certificate(mbedtls_ssl_context *ssl)
{
    ...
    while (crt != NULL) {
        n = crt->raw.len;
#if defined(MBEDTLS_SSL_VARIABLE_CONTENT_LENGTH)
        if (n > ssl->out_content_len - 3 - i) {
            MBEDTLS_SSL_DEBUG_MSG(1, ("certificate too large, %" MBEDTLS_PRINTF_SIZET
                                      " > %" MBEDTLS_PRINTF_SIZET,
                                      i + 3 + n, (size_t) ssl->out_content_len));
            return MBEDTLS_ERR_SSL_BUFFER_TOO_SMALL;
        }
#else
...
#if CFG_PMGR
    memcpy(transform->keyblk.tlsv12, keyblk, sizeof(keyblk));
#endif /* CFG_PMGR */
...
//Added by Renesas.
int mbedtls_ssl_conf_content_len( mbedtls_ssl_config *conf, size_t in, size_t out )
{
    //Not supported for DTLS yet.
    if (conf->transport == MBEDTLS_SSL_TRANSPORT_DATAGRAM)
    {
        return ( MBEDTLS_ERR_SSL_FEATURE_UNAVAILABLE );
    }
    ...
    if( in )
    {
        conf->in_content_len = in;
    }

    if( out )
    {
        conf->out_content_len = out;
    }
...
//Deprecated
int mbedtls_ssl_conf_content_heap( mbedtls_ssl_config *conf, unsigned int enabled )
{
    ((void) conf);
    ((void) enabled);
...
int mbedtls_ssl_set_pac_info( mbedtls_ssl_context *ssl,
                              mbedtls_ssl_eap_pac_t *f_eap_pac,
                              void *p_eap_pac,
                              const unsigned char *pac,
                              size_t pac_len )
{
    if( ssl == NULL || ssl->session_negotiate == NULL )
        return( MBEDTLS_ERR_SSL_BAD_INPUT_DATA );
    ...
    ssl->session_negotiate->ticket_lifetime = 0;
    ssl->use_pac = 0;
    ssl->f_eap_pac = NULL;
    ssl->p_eap_pac = NULL;
    ...
    if( pac_len )
    {
        ssl->session_negotiate->ticket = mbedtls_calloc( 1, pac_len );
        if( ssl->session_negotiate->ticket == NULL )
            return( MBEDTLS_ERR_SSL_ALLOC_FAILED );

        memcpy( ssl->session_negotiate->ticket, pac, pac_len );
        ssl->session_negotiate->ticket_len = pac_len;

        ssl->use_pac = 1;

        ssl->f_eap_pac = f_eap_pac;
        ssl->p_eap_pac = p_eap_pac;
    }

    return 0;
}

void mbedtls_ssl_set_sync_iv( mbedtls_ssl_config *conf,
                              unsigned int enabled, unsigned int gap )
{
    if( enabled )
    {
        conf->force_sync_iv = 1;
        conf->force_sync_iv_gap = gap;
    }
    else
    {
        conf->force_sync_iv = 0;
        conf->force_sync_iv_gap = 0;
    }

    return ;
}

int mbedtls_ssl_is_decrypting( mbedtls_ssl_context *ssl )
{
    size_t hdr_len;

    if( ssl && ssl->state == MBEDTLS_SSL_HANDSHAKE_OVER ){
        hdr_len = mbedtls_ssl_in_hdr_len( ssl );

        if( ssl->in_left >= hdr_len &&
            ssl->in_left < ( hdr_len + ssl->in_msglen ) )
        {
            return 1;
        }
    }
