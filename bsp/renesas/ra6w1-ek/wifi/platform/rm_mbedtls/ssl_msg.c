                                        ...
                                        int force_flush)
{
     ...
     */
#if defined(MBEDTLS_SSL_VARIABLE_CONTENT_LENGTH)
    if( ssl->out_msglen > ssl->out_content_len )
    {
        MBEDTLS_SSL_DEBUG_MSG(1, ("Record too large: "
                                    "size %" MBEDTLS_PRINTF_SIZET
                                    ", maximum %" MBEDTLS_PRINTF_SIZET,
                                    ssl->out_msglen,
                                    (size_t) ssl->out_content_len));
        return MBEDTLS_ERR_SSL_INTERNAL_ERROR;
    }
#else
    ...
    if (ssl->out_msgtype == MBEDTLS_SSL_MSG_HANDSHAKE) {
        ...
        if (ssl->conf->transport == MBEDTLS_SSL_TRANSPORT_DATAGRAM) {
            ...
            if (ssl->out_content_len - ssl->out_msglen < 8)
            {
                MBEDTLS_SSL_DEBUG_MSG(1, ("DTLS handshake message too large: "
                                          "size %" MBEDTLS_PRINTF_SIZET ", maximum %"
                                          MBEDTLS_PRINTF_SIZET,
                                          hs_len,
                                          (size_t) ( ssl->out_content_len - 12 )));
                return MBEDTLS_ERR_SSL_BAD_INPUT_DATA;
            }
#else
            /* Make room for the additional DTLS fields */
...
static int ssl_handle_possible_reconnect(mbedtls_ssl_context *ssl)
{
    ...
    ret = mbedtls_ssl_check_dtls_clihlo_cookie(
        ...
        ssl->in_buf, ssl->in_left,
         ssl->out_buf, ssl->out_content_len, &len);
#else
    ret = mbedtls_ssl_check_dtls_clihlo_cookie(
        ssl,
        ssl->cli_id, ssl->cli_id_len,
                                      ...
                                      mbedtls_record *rec)
{
     ...
     * configured maximum. */
#if defined(MBEDTLS_SSL_VARIABLE_CONTENT_LENGTH)
    if(rec->data_len > ssl->in_content_len) {
...
#if defined(MBEDTLS_SSL_VARIABLE_CONTENT_LENGTH)
        if (msg_len + 12 > ssl->in_content_len) {
            MBEDTLS_SSL_DEBUG_MSG(1, ("should never happen"));
            return MBEDTLS_ERR_SSL_INTERNAL_ERROR;
        }
#else
