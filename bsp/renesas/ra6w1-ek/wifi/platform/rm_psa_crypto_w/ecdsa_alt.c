...
static int ecdsa_wrst_sign (mbedtls_ecp_group * grp,
                            mbedtls_mpi * r,
                            mbedtls_mpi * s,
                            const mbedtls_mpi * d,
                            ...
                            void * p_rng)
{
    ...
    temp_wbuf =
        (uint8_t *) mbedtls_calloc(1, signature_size + sizeof(CCEcdsaSignUserContext_t) + sizeof(CCEcpkiUserPrivKey_t));
    if (temp_wbuf == NULL)
    {
    ...
    // mbedtls_platform_zeroize( temp_wbuf, signature_size + sizeof( CCEcdsaSignUserContext_t ) + sizeof( CCEcpkiUserPrivKey_t ) );

    key_size = mbedtls_mpi_size(d);

    hash_mode = message_size_to_hash_mode(blen);

    temp_context = (CCEcdsaSignUserContext_t *) &(temp_wbuf[0]);
    priv_key     = (CCEcpkiUserPrivKey_t *) &(temp_wbuf[sizeof(CCEcdsaSignUserContext_t)]);
    pSignature   = (uint8_t *) &(temp_wbuf[sizeof(CCEcdsaSignUserContext_t) + sizeof(CCEcpkiUserPrivKey_t)]);
    ...
    status = CC_EcpkiPrivKeyBuild(pDomain, temp_buf, mbedtls_mpi_size(d), priv_key);
    if (status != CC_OK)
    {
    ...
    mbedtls_platform_zeroize(temp_wbuf,
                             signature_size + sizeof(CCEcdsaSignUserContext_t) + sizeof(CCEcpkiUserPrivKey_t));
    mbedtls_free(temp_wbuf);
                              ...
                              const mbedtls_mpi       * s)
{
    ...
    ret = mbedtls_ecp_point_write_binary(grp, Q, MBEDTLS_ECP_PF_UNCOMPRESSED, &temp_size, pTemp_buf, sizeof(pTemp_buf));
    ...
    pPub_key =
        (CCEcpkiUserPublKey_t *) mbedtls_calloc(1,
                                                sizeof(CCEcpkiUserPublKey_t) + sizeof(CCEcdsaVerifyUserContext_t) +
                                                signature_size);
    if (pPub_key == NULL)
    {
    ...
    {
        ret = MBEDTLS_ERR_ECP_VERIFY_FAILED;
        CC_PAL_LOG_ERR("Error - r & s are not in range (1 to n-1)\n");
        goto end;
    }

    status = CC_EcdsaVerify(pTemp_context, pPub_key, hash_mode, pSignature, signature_size, (uint8_t *) buf, blen);
    ...
    mbedtls_platform_zeroize(pPub_key,
                             sizeof(CCEcpkiUserPublKey_t) + sizeof(CCEcdsaVerifyUserContext_t) + signature_size);
