 ...
 */
static uint32_t mbedtls_hashUpdate (void * pHashUserCtx, uint8_t * pDataIn, size_t dataInSize)
{
    uint32_t        rc         = 0;
    ...
    bytesToAdd = CC_MIN(((pHashCtx->blockSizeInBytes - pHashCtx->prevDataInSize) % pHashCtx->blockSizeInBytes),
                        dataInSize);
    if (bytesToAdd > 0)
    {
    ...
    if (pHashCtx->prevDataInSize == pHashCtx->blockSizeInBytes)
    {
        ...
        CC_PalMemCopy(localPrevDataIn, pHashCtx->prevDataIn,
                      CC_MIN(HASH_SHA512_BLOCK_SIZE_IN_WORDS * sizeof(uint32_t), pHashCtx->prevDataInSize));

        rc = SetDataBuffersInfo((uint8_t *) localPrevDataIn, pHashCtx->blockSizeInBytes, &inBuffInfo, NULL, 0, NULL);
        if (rc != 0)
        {
    ...
    if (bytesToAdd > 0)
    {
        rc = SetDataBuffersInfo(pDataIn, bytesToAdd, &inBuffInfo, NULL, 0, NULL);
        if (rc != 0)
        {
            CC_PAL_LOG_ERR("illegal data buffers\n");
...
int mbedtls_sha_finish_internal (void * ctx)
{
    ...
    drvRc = SetDataBuffersInfo((uint8_t *) localPrevDataIn, dataInSize, &inBuffInfo, NULL, 0, NULL);
    if (drvRc != 0)
    {
        CC_PAL_LOG_ERR("illegal data buffers\n");
