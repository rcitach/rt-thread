...
static int cmac_init (AesContext_t * ctx)
{
    ...
    mbedtls_platform_zeroize(ctx, sizeof(AesContext_t));
...
static int cmac_setkey (AesContext_t * ctx, const unsigned char * key, unsigned int keybits)
{
...
int mbedtls_cipher_cmac_starts (mbedtls_cipher_context_t * ctx, const unsigned char * key, size_t keybits)
{
    ...
    mbedtls_platform_zeroize(((mbedtls_cmac_private_context_t *) cmac_ctx)->cmac_ctrl_ctx.state,
...
int mbedtls_cipher_cmac_update (mbedtls_cipher_context_t * ctx, const unsigned char * input, size_t ilen)
{
    ...
    {
        CC_PalMemCopy(&((mbedtls_cmac_private_context_t *) cmac_ctx)->cmac_ctrl_ctx.unprocessed_block[((
                                                                                                           mbedtls_cmac_private_context_t
                                                                                                           *) cmac_ctx)
                                                                                                      ->cmac_ctrl_ctx.
                                                                                                      unprocessed_len],
        ...
        ret = SetDataBuffersInfo(
            (const uint8_t *) &((mbedtls_cmac_private_context_t *) cmac_ctx)->cmac_ctrl_ctx.unprocessed_block,
            block_size,
            &inBuffInfo,
            NULL,
            0,
    ...
    if (1 < blocks_num)
    {
        ...
        ret = SetDataBuffersInfo(input, main_chunk_in_bytes, &inBuffInfo, NULL, 0, &outBuffInfo);
        if (ret != 0)
        {
            CC_PAL_LOG_ERR("illegal data buffers\n");
...
int mbedtls_cipher_cmac_finish (mbedtls_cipher_context_t * ctx, unsigned char * output)
{
    ...
    ret =
        FinishAesDrv(&((mbedtls_cmac_private_context_t *) cmac_ctx)->aes_ctx, &inBuffInfo, &outBuffInfo,
    ...
    mbedtls_platform_zeroize(ctx->cmac_ctx, sizeof(mbedtls_cmac_private_context_t));
...
int mbedtls_cipher_cmac_reset (mbedtls_cipher_context_t * ctx)
{
    ...
    ((mbedtls_cmac_private_context_t *) cmac_ctx)->cmac_ctrl_ctx.unprocessed_len = 0;
    mbedtls_platform_zeroize(((mbedtls_cmac_private_context_t *) cmac_ctx)->cmac_ctrl_ctx.unprocessed_block,
                             sizeof(((mbedtls_cmac_private_context_t *) cmac_ctx)->cmac_ctrl_ctx.unprocessed_block));
    mbedtls_platform_zeroize(((mbedtls_cmac_private_context_t *) cmac_ctx)->cmac_ctrl_ctx.state,
                             sizeof(((mbedtls_cmac_private_context_t *) cmac_ctx)->cmac_ctrl_ctx.state));

    /* Zeroize the IV in the context */
    mbedtls_platform_zeroize(((mbedtls_cmac_private_context_t *) cmac_ctx)->aes_ctx.ivBuf,
