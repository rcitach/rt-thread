                     ...
                     uint32_t              ccmMode)
{
    ...
    mbedtls_platform_zeroize(pAesCcmCtx->ivBuf, sizeof(pAesCcmCtx->ivBuf));

    /*clear intenal buffers*/
    mbedtls_platform_zeroize(pAesCcmCtx->ctrStateBuf, sizeof(pAesCcmCtx->ctrStateBuf));
    mbedtls_platform_zeroize(pAesCcmCtx->tempBuff, sizeof(pAesCcmCtx->tempBuff));
    ...
    rc = SetDataBuffersInfo(tempBuff, CC_AES_BLOCK_SIZE_IN_BYTES, &inBuffInfo, NULL, 0, &outBuffInfo);
    if (rc != 0)
    {
...
static int ccm_ass_data (mbedtls_ccm_context * ctx, const uint8_t * pAssocData, size_t assocDataSize)
{
    ...
    mbedtls_platform_zeroize(pAesCcmCtx->tempBuff, CC_AES_BLOCK_SIZE_IN_BYTES);
    ...
    /* set data buffers structures */
    rc = SetDataBuffersInfo(pAesCcmCtx->tempBuff, CC_AES_BLOCK_SIZE_IN_BYTES, &inBuffInfo, NULL, 0, &outBuffInfo);
    if (rc != 0)
    {
        CC_PAL_LOG_ERR("illegal data buffers\n");
    ...
    {
        /* set data buffers structures */
        rc = SetDataBuffersInfo(pAssocData, assocDataSize, &inBuffInfo, NULL, 0, &outBuffInfo);
        if (rc != 0)
        {
            CC_PAL_LOG_ERR("illegal data buffers\n");
...
static int ccm_text_data (mbedtls_ccm_context * ctx,
                          ...
                          uint8_t             * pTextDataOut)
{
    ...
    if (pAesCcmCtx->dir == CRYPTO_DIRECTION_DECRYPT)
    {
        ...
        /* set data buffers structures */
        rc = SetDataBuffersInfo(pTextDataOut, textDataSize, &inBuffInfo, NULL, 0, &outBuffInfo);
        if (rc != 0)
        {
            CC_PAL_LOG_ERR("illegal data buffers\n");
...
static int ccm_finish (mbedtls_ccm_context * ctx, unsigned char * macBuf, size_t * sizeOfT)
{
    ...
    mbedtls_platform_zeroize((uint8_t *) pAesCcmCtx->ctrStateBuf + CC_AES_BLOCK_SIZE_IN_BYTES - qFieldSize, qFieldSize);

    /* on CCM encrypt mode*/
    if (pAesCcmCtx->dir == CRYPTO_DIRECTION_ENCRYPT)
    {
        /* set data buffers structures */
        rc = SetDataBuffersInfo((uint8_t *) pAesCcmCtx->ivBuf,
