...
static int gcm_calc_h (mbedtls_gcm_context * ctx)
{
    ...
    rc =
        SetDataBuffersInfo((uint8_t *) (pAesGcmCtx->tempBuf),
                           CC_AESGCM_GHASH_DIGEST_SIZE_BYTES,
                           &inBuffInfo,
                           (uint8_t *) (pAesGcmCtx->H),
...
static int gcm_process_j0 (mbedtls_gcm_context * ctx, const uint8_t * pIv)
{
    ...
    else
    {
        ...
        /* set data buffers structures */
        rc = SetDataBuffersInfo(pIv, pAesGcmCtx->ivSize, &inBuffInfo, NULL, 0, &outBuffInfo);
        if (rc != 0)
        {
            CC_PAL_LOG_ERR("illegal data buffers\n");
        ...
        /* set data buffers structures */
        rc = SetDataBuffersInfo((uint8_t *) (pAesGcmCtx->tempBuf),
                                CC_AESGCM_GHASH_DIGEST_SIZE_BYTES,
                                &inBuffInfo,
                                NULL,
                                0,
...
static int gcm_process_aad (mbedtls_gcm_context * ctx, const uint8_t * pAad)
{
    ...
    /* set data buffers structures */
    rc = SetDataBuffersInfo(pAad, pAesGcmCtx->aadSize, &inBuffInfo, NULL, 0, &outBuffInfo);
    if (rc != 0)
    {
        CC_PAL_LOG_ERR("illegal data buffers\n");
...
static int gcm_process_cipher (mbedtls_gcm_context * ctx, const uint8_t * pTextDataIn, uint8_t * pTextDataOut)
{
    ...
    /* set data buffers structures */
    rc = SetDataBuffersInfo(pTextDataIn,
                            pAesGcmCtx->dataSize,
                            &inBuffInfo,
                            pTextDataOut,
                            pAesGcmCtx->dataSize,
                              ...
                              unsigned char       * tag)
{
    ...
    rc = gcm_init(ctx, (cryptoDirection_t) mode, iv, iv_len, aad, aad_len, input, length, output, tag, tag_len);
                               ...
                               unsigned char       * tag)
{
    ...
    rc = gcm_crypt_and_tag(ctx, mode, length, iv, iv_len, add, add_len, input, output, tag_len, tag);
                              ...
                              unsigned char       * output)
{
    ...
    rc = gcm_crypt_and_tag(ctx, 0, length, iv, iv_len, add, add_len, input, output, tag_len, (unsigned char *) tag);

    return rc;
}

/* TIN-TODO: The following APIs have not been implemented. Either do so or remove them. */
 #if 0
