...
int mbedtls_mpi_exp_mod_hw (mbedtls_mpi       * X /*out*/,
                            const mbedtls_mpi * A /*in*/,
                            const mbedtls_mpi * E /*in*/,
                            const mbedtls_mpi * N /*in*/,
                            mbedtls_mpi       * _RR /*in*/)
{
    int         err = 0;
    ...
    /* allocate positive X  */
    err = mbedtls_mpi_grow(X, N->MBEDTLS_PRIVATE(n) /* + 1*/);
    ...
    {
        err = mbedtls_mpi_grow(&T, N->MBEDTLS_PRIVATE(n) /* + 1*/);
        if (err != 0)
        {
            goto End;
        }
        ...
        err = PkiLongNumDiv(A->MBEDTLS_PRIVATE(p),
                            a_len_words,
                            N->MBEDTLS_PRIVATE(p),
                            n_len_words,
                            T.MBEDTLS_PRIVATE(p) /*ModRes*/,
                            NULL /*DivRes*/);
        if (err != 0)
        {
            goto End;
        ...
        err = PkiExecModExpLeW(X->MBEDTLS_PRIVATE(p) /*out*/,
                               T.MBEDTLS_PRIVATE(p) /*in*/,
                               n_len_words,
                               N->MBEDTLS_PRIVATE(p) /*modulus*/,
                               n_size_bits,
                               E->MBEDTLS_PRIVATE(p),
                               E->MBEDTLS_PRIVATE(n));
        if (err != 0)
    ...
    if ((A->MBEDTLS_PRIVATE(s) == -1) && (E->MBEDTLS_PRIVATE(n) != 0) && ((E->MBEDTLS_PRIVATE(p)[0] & 1) != 0))
    {
        X->MBEDTLS_PRIVATE(s) = -1;
...
End:
    ...
    if (err)
    {
        mbedtls_platform_zeroize(X->p, X->n);
    }
                            ...
                            void * p_rng)
{
     ...
     */
    MBEDTLS_MPI_CHK(mbedtls_mpi_exp_mod_hw(&ctx->GX, &ctx->G, &ctx->X, &ctx->P, &ctx->RP));
                             ...
                             void * p_rng)
{
    ...
    if ((NULL == ctx) || (NULL == output) || (NULL == olen) || (NULL == f_rng))
    {
        return MBEDTLS_ERR_DHM_BAD_INPUT_DATA;
    }
                             ...
                             void * p_rng)
{
    ...
    if ((NULL == ctx) || (NULL == output) || (NULL == f_rng))
    {
        return MBEDTLS_ERR_DHM_BAD_INPUT_DATA;
    }
                             ...
                             void * p_rng)
{
    ...
    MBEDTLS_MPI_CHK(mbedtls_mpi_exp_mod_hw(&ctx->K, &GYb, &ctx->X, &ctx->P, &ctx->RP));
