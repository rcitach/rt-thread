...
// err fix: 'ecc_conv_scalar_to_mpi' defined but not used [-Werror=unused-function]
...
// err fix: 'ecc_conv_mpi_to_scalar' defined but not used [-Werror=unused-function]
static int ecc_conv_mpi_to_scalar (const mbedtls_mpi * X, uint8_t * scalar, size_t * scalarSize)
{
 ...
 #if defined(ECP_MONTGOMERY)
static int ecp_mont_mul (mbedtls_ecp_point * R, const mbedtls_mpi * m, const mbedtls_ecp_point * P)
{
    int                  ret;
    CCError_t            rc;
    ...
    mbedtls_platform_zeroize(px, CC_EC_MONT_MOD_SIZE_IN_BYTES);
    ...
    rc = CC_EcMontScalarmult(resPoint,
                             &resPointSize,
                             scalar,
                             scalarSize,
                             px,
    ...
    /*secure programing*/
    mbedtls_platform_zeroize(scalar, scalarSize);
...
static int ecp_wrst_mul (mbedtls_ecp_group       * grp,
                         mbedtls_ecp_point       * R,
                         const mbedtls_mpi       * m,
                         const mbedtls_ecp_point * P)
{
    ...
    rc = PkaEcWrstScalarMult(pDomain,
                             m->MBEDTLS_PRIVATE(p),
                             m->MBEDTLS_PRIVATE(n),
                             P->MBEDTLS_PRIVATE(X).MBEDTLS_PRIVATE(p),
                             P->MBEDTLS_PRIVATE(Y).MBEDTLS_PRIVATE(p),
                             R->MBEDTLS_PRIVATE(X).MBEDTLS_PRIVATE(p),
                             R->MBEDTLS_PRIVATE(Y).MBEDTLS_PRIVATE(p),
                             tmpBuf);
    mbedtls_free(tmpBuf);
    if (rc != CC_SUCCESS)
    {
                ...
                void * p_rng)
{
 ...
 #if defined(ECP_MONTGOMERY)
static int ecp_mont_gen_keypair_base (const mbedtls_ecp_point * G, /* can be NULL in order to use the curve generator*/
                                      mbedtls_mpi * d,
                                      mbedtls_ecp_point * Q,
                                      int (* f_rng)(void *, unsigned char *, size_t),
                                      void * p_rng)
{
    ...
    if (G != NULL)
    {
        rc = CC_EcMontKeyPairBase(resPoint,
                                  &resPointSize,
                                  scalar,
                                  &scalarSize,
                                  px,
                                      ...
                                      void * p_rng)
{
    ...
    pTempBuff =
        mbedtls_calloc(1,
                       (sizeof(CCEcpkiKgTempData_t) + sizeof(CCRndContext_t) + sizeof(CCEcpkiUserPrivKey_t) +
                        sizeof(CCEcpkiUserPublKey_t)));
    if (NULL == pTempBuff)
    {
        CC_PAL_LOG_ERR("Error - failed to allocate memory for temporary buffer\n");
    ...
    {
        rc = CC_EcpkiKeyPairGenerateBase(pRndContext,
                                         pDomain,
                                         G->MBEDTLS_PRIVATE(X).MBEDTLS_PRIVATE(p),
                                         G->MBEDTLS_PRIVATE(Y).MBEDTLS_PRIVATE(p),
                                         pUserPrivKey,
                                         pUserPublKey,
                                         pTempBuff,
                                         NULL);
    ...
    CC_PalMemCopy(Q->MBEDTLS_PRIVATE(X).MBEDTLS_PRIVATE(p), pPublicKey->MBEDTLS_PRIVATE(x),
                  CALC_FULL_BYTES(pDomain->modSizeInBits));
    CC_PalMemCopy(Q->MBEDTLS_PRIVATE(Y).MBEDTLS_PRIVATE(p), pPublicKey->MBEDTLS_PRIVATE(y),
                  CALC_FULL_BYTES(pDomain->modSizeInBits));
...
END:
    ...
    mbedtls_mpi_free(d);
SUCCESS:
    mbedtls_platform_zeroize(pTempBuff,
                             (sizeof(CCEcpkiKgTempData_t) + sizeof(CCRndContext_t) + sizeof(CCEcpkiUserPrivKey_t) +
                              sizeof(CCEcpkiUserPublKey_t)));
...
{
    /* Input parameters validation */
    if ((NULL == grp) || (NULL == Q) || (NULL == d) || (NULL == G) || (f_rng == NULL))
    {
        CC_PAL_LOG_ERR("Error - NULL pointer exception\n");
