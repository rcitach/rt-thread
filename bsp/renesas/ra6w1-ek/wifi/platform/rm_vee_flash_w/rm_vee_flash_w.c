...
#ifndef RM_VEE_FLASH_W_TIMEOUT_US
 #define RM_VEE_FLASH_W_TIMEOUT_US             (1000000)
#endif
#ifndef RM_VEE_FLASH_W_TIMEOUT_REFRESH_US
 #define RM_VEE_FLASH_W_TIMEOUT_REFRESH_US     (5000000)
#endif

#define RM_VEE_FLASH_W_OPEN                    (0x52564545U)
#define RM_VEE_FLASH_W_VALID_CODE              (0xBEAD)
#define RM_VEE_FLASH_W_ID_INVALID              (UINT16_MAX)
#define RM_VEE_FLASH_W_LOGICAL_END_ADDRESS     (p_ctrl->p_cfg->start_addr + p_ctrl->p_cfg->total_size)
#define RM_VEE_FLASH_W_PHYSICAL_END_ADDRESS    (BSP_FEATURE_FLASH_DATA_FLASH_START + BSP_DATA_FLASH_SIZE_BYTES)
#define RM_VEE_FLASH_W_REC_OVERHEAD            (sizeof(rm_vee_rec_hdr_t) + sizeof(rm_vee_rec_end_t))
#define RM_VEE_FLASH_W_REF_DATA_COUNT          (2)
#if RM_VEE_FLASH_W_CFG_REF_DATA_SUPPORT
 #define RM_VEE_FLASH_W_REF_DATA_AREA_SIZE     ((sizeof(rm_vee_ref_hdr_t) + \
                                               (p_ctrl->p_cfg->ref_data_size * RM_VEE_FLASH_W_REF_DATA_COUNT)))
#else
 #define RM_VEE_FLASH_W_REF_DATA_AREA_SIZE     (0)
#endif

/* Use appropriate macros for Flash HP or LP */
#if (BSP_FEATURE_BLOCK_MEDIA_SPI_W_BLOCK_SIZE > 0)
 #define RM_VEE_FLASH_W_DF_BLOCK_SIZE          (BSP_FEATURE_BLOCK_MEDIA_SPI_W_BLOCK_SIZE)
#elif (BSP_FEATURE_FLASH_HP_DF_BLOCK_SIZE > 0)
 #define RM_VEE_FLASH_W_DF_BLOCK_SIZE          (BSP_FEATURE_FLASH_HP_DF_BLOCK_SIZE)
#else
 #define RM_VEE_FLASH_W_DF_BLOCK_SIZE          (BSP_FEATURE_FLASH_LP_DF_BLOCK_SIZE)
#endif

#define RM_VEE_FLASH_W_DF_WRITE_MASK           (RM_VEE_FLASH_W_DF_WRITE_SIZE - 1U)

#define RM_VEE_ADDRESS_ALIGN(x)    ((x + RM_VEE_FLASH_W_DF_WRITE_MASK) & (~RM_VEE_FLASH_W_DF_WRITE_MASK))

#define RM_VEE_FLASH_W_REC_DATA_MAX_SIZE       (p_ctrl->segment_size -                                           \
                                              (sizeof(rm_vee_seg_hdr_t) + (p_ctrl->p_cfg->ref_data_size * 2) + \
                                               sizeof(rm_vee_ref_hdr_t) + RM_VEE_FLASH_W_REC_OVERHEAD))

#define RM_CONV_XIP2MEDIA(x)              ((x) - (p_ctrl->p_cfg->start_addr))
#define RM_CONV_MEDIA2XIP(x)              ((x) + (p_ctrl->p_cfg->start_addr))

#define SUPPORT_MEDIA_BLOCK_OPERATION 0
#define ANALSYZER_INFO 0

#if ANALSYZER_INFO
 #define MEASURE_DEFINE(name)                                           \
                    uint32_t g_##name##_old_time, g_##name##_new_time;

 #define MEASURE_START(name)                                            \
                    g_##name##_old_time = R_BSP_SystemRtcCountGet();

 #define MEASURE_END(name)                                              \
                    g_##name##_new_time = R_BSP_SystemRtcCountGet();    \
                    printf( #name"_time = %ld\n ",                      \
                    (uint32_t)CLK2US(g_##name##_new_time - g_##name##_old_time));
  MEASURE_DEFINE(refresh);
#else
  #define MEASURE_DEFINE(...)
 ...
 **********************************************************************************************************************/
/* Driver Modes */
typedef enum e_rm_vee_flash_w_prv_mode
{
    RM_VEE_FLASH_W_PRV_MODE_NORMAL        = 0,
    RM_VEE_FLASH_W_PRV_MODE_REFRESH       = RM_VEE_STATE_REFRESH,
    RM_VEE_FLASH_W_PRV_MODE_OVERFLOW      = RM_VEE_STATE_OVERFLOW,
    RM_VEE_FLASH_W_PRV_MODE_FLASH_PE_FAIL = RM_VEE_STATE_HARDWARE_FAIL,
} rm_vee_flash_w_prv_mode_t;

/* Driver States (used in NORMAL and REFRESH modes) */
typedef enum e_rm_vee_flash_w_prv_states
{
    RM_VEE_FLASH_W_PRV_STATES_UNINITIALIZED = 0,
    RM_VEE_FLASH_W_PRV_STATES_READY,
    RM_VEE_FLASH_W_PRV_STATES_WRITE_REC_HDR,
    RM_VEE_FLASH_W_PRV_STATES_WRITE_REC_DATA,
    RM_VEE_FLASH_W_PRV_STATES_WRITE_REC_DATA_TAIL,
    RM_VEE_FLASH_W_PRV_STATES_WRITE_REC_END,
    RM_VEE_FLASH_W_PRV_STATES_WRITE_REC_REFRESH,
    RM_VEE_FLASH_W_PRV_STATES_WRITE_NEW_REFDATA,
    RM_VEE_FLASH_W_PRV_STATES_WRITE_NEW_REFDATA_HDR,
    RM_VEE_FLASH_W_PRV_STATES_WRITE_REFDATA,
    RM_VEE_FLASH_W_PRV_STATES_WRITE_SEG_HDR,
    RM_VEE_FLASH_W_PRV_STATES_ERASE_SEG,
} rm_vee_flash_w_prv_states_t;

typedef enum e_rm_vee_flash_w_refresh_refresh
{
    RM_VEE_FLASH_W_PRV_REFRESH_USER_REQ,
    RM_VEE_FLASH_W_PRV_REFRESH_REFDATA_OVFL,
    RM_VEE_FLASH_W_PRV_REFRESH_RECORD_OVFL
} rm_vee_flash_w_refresh_refresh_t;

#if defined(__ARMCC_VERSION) || defined(__ICCARM__)
typedef void (BSP_CMSE_NONSECURE_CALL * rm_vee_flash_w_prv_ns_callback)(rm_vee_callback_args_t * p_args);
#elif defined(__GNUC__)
typedef BSP_CMSE_NONSECURE_CALL void (*volatile rm_vee_flash_w_prv_ns_callback)(rm_vee_callback_args_t * p_args);
#endif
...
/* Internal functions */
static fsp_err_t rm_vee_internal_open(rm_vee_flash_w_instance_ctrl_t * const p_ctrl);
static fsp_err_t rm_vee_inspect_segments(rm_vee_flash_w_instance_ctrl_t * const p_ctrl);
static fsp_err_t rm_vee_refresh_next_data_source(rm_vee_flash_w_instance_ctrl_t * const p_ctrl);
static uint32_t  rm_vee_get_next_id(rm_vee_flash_w_instance_ctrl_t * const p_ctrl);
static fsp_err_t rm_vee_restore_previous_seg(rm_vee_flash_w_instance_ctrl_t * const p_ctrl);
static fsp_err_t rm_vee_write_seg_hdr(rm_vee_flash_w_instance_ctrl_t * const p_ctrl);
static fsp_err_t rm_vee_init_record_xfer(rm_vee_flash_w_instance_ctrl_t * const p_ctrl, uint32_t id);
static fsp_err_t rm_vee_xfer_next_chunk(rm_vee_flash_w_instance_ctrl_t * const p_ctrl, rm_vee_flash_w_prv_states_t state);
static void      rm_vee_flash_w_err_handle(rm_vee_flash_w_instance_ctrl_t * const p_ctrl, fsp_err_t err);
static fsp_err_t rm_vee_state_get(rm_vee_flash_w_instance_ctrl_t * const p_ctrl, rm_vee_state_t * p_state);
static fsp_err_t rm_vee_blocking_erase_segment(rm_vee_flash_w_instance_ctrl_t * const p_ctrl,
                                               uint32_t                             seg_addr,
                                               bool                                 contains_refdata);
static fsp_err_t rm_vee_load_record_table(rm_vee_flash_w_instance_ctrl_t * const p_ctrl, bool initial_load);
static fsp_err_t rm_vee_blocking_blankcheck(rm_vee_flash_w_instance_ctrl_t * const p_ctrl,
                                            uint32_t                             addr,
                                            uint32_t                             num_bytes,
                                            flash_event_t                      * p_event);
static fsp_err_t rm_vee_blocking_erase(rm_vee_flash_w_instance_ctrl_t * const p_ctrl, uint32_t addr, uint32_t num_blocks);
static fsp_err_t rm_vee_blocking_write(rm_vee_flash_w_instance_ctrl_t * const p_ctrl,
                                       uint32_t                             src_addr,
                                       uint32_t                             dst_addr,
                                       uint32_t                             num_bytes);
static fsp_err_t rm_vee_internal_write_rec(rm_vee_flash_w_instance_ctrl_t * const p_ctrl,
                                           uint32_t                             rec_id,
                                           uint8_t const * const                p_rec_data,
                                           uint32_t                             num_bytes);
static fsp_err_t rm_vee_start_seg_refresh(rm_vee_flash_w_instance_ctrl_t * const p_ctrl,
                                          rm_vee_flash_w_refresh_refresh_t       refresh_type,
                                          uint32_t                             rec_id,
                                          uint32_t                             addr,
                                          uint32_t                             num_bytes);

static fsp_err_t rm_vee_media_write(rm_vee_flash_w_instance_ctrl_t * const p_ctrl,
                        uint32_t const src_address, uint32_t const flash_address,
                        uint32_t const num_bytes);
static fsp_err_t rm_vee_media_erase(rm_vee_flash_w_instance_ctrl_t * const p_ctrl,
                        uint32_t const address, uint32_t const num_blocks);

#if RM_VEE_FLASH_W_CFG_REF_DATA_SUPPORT
static fsp_err_t rm_vee_inspect_new_refdata(rm_vee_flash_w_instance_ctrl_t * const p_ctrl);
static void      rm_vee_init_refdata_xfer(rm_vee_flash_w_instance_ctrl_t * const p_ctrl);

#endif

void rm_vee_flash_w_callback(flash_callback_args_t * p_args);
fsp_err_t rm_vee_media_callback_preprocess(rm_block_media_callback_args_t * p_margs, flash_callback_args_t * p_fargs);
void rm_vee_media_callback(rm_block_media_callback_args_t * p_args);

const rm_vee_api_t g_rm_vee_on_flash =
{
    .open          = RM_VEE_FLASH_W_Open,
    .recordWrite   = RM_VEE_FLASH_W_RecordWrite,
    .recordPtrGet  = RM_VEE_FLASH_W_RecordPtrGet,
    .refDataWrite  = RM_VEE_FLASH_W_RefDataWrite,
    .refDataPtrGet = RM_VEE_FLASH_W_RefDataPtrGet,
    .statusGet     = RM_VEE_FLASH_W_StatusGet,
    .refresh       = RM_VEE_FLASH_W_Refresh,
    .format        = RM_VEE_FLASH_W_Format,
    .close         = RM_VEE_FLASH_W_Close,
};
...
/*******************************************************************************************************************//**
 * Open the RM_VEE_FLASH_W driver module
 *
...
fsp_err_t RM_VEE_FLASH_W_Open (rm_vee_ctrl_t * const p_api_ctrl, rm_vee_cfg_t const * const p_cfg)
{
    uint32_t  i;
    fsp_err_t err;
    rm_vee_flash_w_instance_ctrl_t * const p_ctrl = (rm_vee_flash_w_instance_ctrl_t *) p_api_ctrl;

#if RM_VEE_FLASH_W_CFG_PARAM_CHECKING_ENABLE == 1
    FSP_ASSERT(p_ctrl);
    FSP_ASSERT(p_cfg);

    /* Interface isn't already open and is configured for more than one segment */
    FSP_ERROR_RETURN(RM_VEE_FLASH_W_OPEN != p_ctrl->open, FSP_ERR_ALREADY_OPEN);
    ...
    FSP_ERROR_RETURN(RM_VEE_FLASH_W_PHYSICAL_END_ADDRESS >= (p_cfg->start_addr + p_cfg->total_size),
                     FSP_ERR_INVALID_ARGUMENT);

    /* All memory segments are correctly aligned */
    FSP_ERROR_RETURN(0 == (p_cfg->start_addr - BSP_FEATURE_FLASH_DATA_FLASH_START) % RM_VEE_FLASH_W_DF_BLOCK_SIZE,
                     FSP_ERR_INVALID_ARGUMENT);
    FSP_ERROR_RETURN(0 == p_cfg->total_size % RM_VEE_FLASH_W_DF_BLOCK_SIZE, FSP_ERR_INVALID_ARGUMENT);
    FSP_ERROR_RETURN(0 == (p_cfg->total_size / RM_VEE_FLASH_W_DF_BLOCK_SIZE) % p_cfg->num_segments,
                     FSP_ERR_INVALID_ARGUMENT);
    ...
    FSP_ERROR_RETURN(0 == (p_cfg->ref_data_size % RM_VEE_FLASH_W_DF_WRITE_SIZE), FSP_ERR_INVALID_ARGUMENT);
#endif
    MEASURE_DEFINE(open);
    MEASURE_START(open);

    p_ctrl->p_cfg        = p_cfg;
    p_ctrl->p_flash      = ((rm_vee_flash_w_cfg_t *) p_ctrl->p_cfg->p_extend)->p_flash;
    p_ctrl->p_callback_memory = NULL;
    if (NULL ==  p_ctrl->p_flash)
    {
        p_ctrl->p_media      = ((rm_vee_flash_w_cfg_t *) p_ctrl->p_cfg->p_extend)->p_media;

        p_ctrl->delegation.length_update        = 0;
        p_ctrl->delegation.wr_in_progress       = false;
        p_ctrl->delegation.wr_in_continue       = false;
        p_ctrl->delegation.refseg_in_progress   = false;
        p_ctrl->delegation.media_error          = false;

        p_ctrl->delegation.pendevent.pended     = false;
        p_ctrl->delegation.pendevent.rdptr      = 0;
        p_ctrl->delegation.pendevent.wrptr      = 0;
    }
    else
    {
        p_ctrl->p_media      = NULL;
    ...
    p_ctrl->seg_hdr.valid_code  = RM_VEE_FLASH_W_VALID_CODE;

    p_ctrl->rec_end.id         = (uint16_t) RM_VEE_FLASH_W_ID_INVALID;
    p_ctrl->rec_end.valid_code = RM_VEE_FLASH_W_VALID_CODE;

    p_ctrl->ref_hdr.pad        = 0;
    p_ctrl->ref_hdr.valid_code = RM_VEE_FLASH_W_VALID_CODE;
    ...
    else if (NULL != p_ctrl->p_media)
    {
        err = p_ctrl->p_media->p_api->open(p_ctrl->p_media->p_ctrl, p_ctrl->p_media->p_cfg);
    }
    else
    {
    ...
    if (NULL != p_ctrl->p_media)
    {
        err = p_ctrl->p_media->p_api->mediaInit(p_ctrl->p_media->p_ctrl);
        FSP_ERROR_RETURN(FSP_SUCCESS == err, err);
    }

    p_ctrl->state = RM_VEE_FLASH_W_PRV_STATES_UNINITIALIZED;
    ...
    {
        err = rm_vee_start_seg_refresh(p_ctrl, RM_VEE_FLASH_W_PRV_REFRESH_USER_REQ, 0, 0, 0);

        if (FSP_SUCCESS == err)
        {
            /* Block until Refresh done (Refresh and/or error condition will clear driver lock in callback function) */
            for (i = 0; i < RM_VEE_FLASH_W_TIMEOUT_REFRESH_US; i++)
            {
                if (NULL != p_ctrl->p_media)
                {
                    /* rm_block_media cannot change the state using the HW interrupt callback. */
                    if (true == p_ctrl->delegation.media_error)
                    {
                        p_ctrl->delegation.media_error = false;
                        err = FSP_ERR_PE_FAILURE;
                    }
                    p_ctrl->mode = RM_VEE_FLASH_W_PRV_MODE_NORMAL;
                    break;
                }

                if (p_ctrl->state == RM_VEE_FLASH_W_PRV_STATES_READY)
                {
                    break;
                }
            ...
            if (RM_VEE_FLASH_W_TIMEOUT_REFRESH_US == i)
            {
                err = FSP_ERR_TIMEOUT;
            }
            else if (RM_VEE_FLASH_W_PRV_MODE_FLASH_PE_FAIL == p_ctrl->mode)
            {
                err = FSP_ERR_PE_FAILURE;
            }
            else
    ...
    if (FSP_SUCCESS != err)
    {
        ...
        {
            p_ctrl->p_media->p_api->close(p_ctrl->p_media->p_ctrl);
        }
    }
    else
    {
        ...
        p_ctrl->open = RM_VEE_FLASH_W_OPEN;
    }
    MEASURE_END(open);
                                    ...
                                    uint32_t const        num_bytes)
{
...
#if (RM_VEE_FLASH_W_CFG_PARAM_CHECKING_ENABLE)
    FSP_ASSERT(p_ctrl);
    FSP_ERROR_RETURN(RM_VEE_FLASH_W_OPEN == p_ctrl->open, FSP_ERR_NOT_OPEN);
    ...
    FSP_ERROR_RETURN(RM_VEE_FLASH_W_REC_DATA_MAX_SIZE >= num_bytes, FSP_ERR_INVALID_ARGUMENT);

    FSP_ASSERT(NULL != p_rec_data);

    FSP_ERROR_RETURN(RM_VEE_FLASH_W_PRV_MODE_OVERFLOW != p_ctrl->mode, FSP_ERR_INVALID_MODE);
    FSP_ERROR_RETURN(RM_VEE_FLASH_W_PRV_MODE_FLASH_PE_FAIL != p_ctrl->mode, FSP_ERR_INVALID_MODE);
    FSP_ERROR_RETURN(RM_VEE_FLASH_W_PRV_STATES_READY == p_ctrl->state, FSP_ERR_IN_USE);
#endif

    MEASURE_DEFINE(write);
    MEASURE_START(write);
    fsp_err_t err = rm_vee_internal_write_rec(p_ctrl, rec_id, p_rec_data, num_bytes);
    rm_vee_flash_w_err_handle(p_ctrl, err);

    if ((FSP_SUCCESS == err) && (NULL != p_ctrl->p_media))
    {
        if ((RM_VEE_FLASH_W_PRV_MODE_FLASH_PE_FAIL == p_ctrl->mode)
            && (RM_VEE_FLASH_W_PRV_STATES_READY == p_ctrl->state))
        {
            p_ctrl->delegation.refseg_in_progress = false;
        }
        else
        {
            /*Any approach that calls this function again without calling rm_vee_state_get() should be prohibited. */
            p_ctrl->state      = RM_VEE_FLASH_W_PRV_STATES_WRITE_REC_HDR;
            if (p_ctrl->delegation.refseg_in_progress == true)
            {
                if (RM_VEE_FLASH_W_PRV_MODE_NORMAL == p_ctrl->mode)
                {
                    p_ctrl->mode       = RM_VEE_FLASH_W_PRV_MODE_REFRESH;
                }
                p_ctrl->delegation.refseg_in_progress = false;
            }
            else
...
fsp_err_t RM_VEE_FLASH_W_RefDataWrite (rm_vee_ctrl_t * const p_api_ctrl, uint8_t const * const p_ref_data)
{
    ...
    if (false == p_ctrl->new_refdata_valid)
    {
        ...
        p_ctrl->state = RM_VEE_FLASH_W_PRV_STATES_WRITE_NEW_REFDATA;
        if (NULL != p_ctrl->p_flash)
        {
...
fsp_err_t RM_VEE_FLASH_W_Refresh (rm_vee_ctrl_t * const p_api_ctrl)
{
    ...
    fsp_err_t err = rm_vee_start_seg_refresh(p_ctrl, RM_VEE_FLASH_W_PRV_REFRESH_USER_REQ, 0, 0, 0);
    rm_vee_flash_w_err_handle(p_ctrl, err);
...
fsp_err_t RM_VEE_FLASH_W_Format (rm_vee_ctrl_t * const p_api_ctrl, uint8_t const * const p_ref_data)
{
    ...
    fsp_err_t err =
        rm_vee_blocking_erase(p_ctrl,
                              ...
                              (p_ctrl->p_cfg->total_size / RM_VEE_FLASH_W_DF_BLOCK_SIZE));
    ...
    if ((0 != p_ctrl->p_cfg->ref_data_size) && (NULL != p_ref_data) && (FSP_SUCCESS == err))
    {
        err =
            rm_vee_blocking_write(p_ctrl,
                                  (uint32_t) p_ref_data,
                                  (RM_VEE_FLASH_W_LOGICAL_END_ADDRESS - p_ctrl->p_cfg->ref_data_size),
                                  p_ctrl->p_cfg->ref_data_size);
    }
...
{
    rm_vee_flash_w_instance_ctrl_t * p_ctrl = (rm_vee_flash_w_instance_ctrl_t *) p_api_ctrl;
...
static fsp_err_t rm_vee_internal_open (rm_vee_flash_w_instance_ctrl_t * const p_ctrl)
{
    ...
    p_ctrl->last_id  = (uint16_t) RM_VEE_FLASH_W_ID_INVALID;
    ...
    p_ctrl->state = RM_VEE_FLASH_W_PRV_STATES_READY;
...
static fsp_err_t rm_vee_inspect_segments (rm_vee_flash_w_instance_ctrl_t * const p_ctrl)
{
    ...
    for (uint32_t i = 0; i < p_ctrl->p_cfg->num_segments; i++)
    {
        ...
        if (FLASH_EVENT_BLANK == event)
        {
            ...
            {
                num_bytes -= RM_VEE_FLASH_W_REF_DATA_AREA_SIZE;
            }
#endif
        ...
        else
        {
            ...
            if (RM_VEE_FLASH_W_VALID_CODE == p_current_segment->valid_code)
            {
                    ...
                    if (p_current_segment->refresh_cnt > p_active_segment->refresh_cnt)
                    {
                        ...
                        err =
                            rm_vee_blocking_erase(p_ctrl, p_ctrl->active_seg_addr,
                                                  (p_ctrl->segment_size / RM_VEE_FLASH_W_DF_BLOCK_SIZE));
                        p_ctrl->active_seg_addr = addr;
                    }
                    else
                    {
                        /* Else current segment is older */
                        err = rm_vee_blocking_erase(p_ctrl, addr, (p_ctrl->segment_size / RM_VEE_FLASH_W_DF_BLOCK_SIZE));
                    }
                }
                else
                {
    ...
    {
        p_ctrl->active_seg_addr = RM_VEE_FLASH_W_LOGICAL_END_ADDRESS - p_ctrl->segment_size;
        ...
        if (FSP_SUCCESS == err)
        {
            ...
            err = rm_vee_blocking_write(p_ctrl,
                                        ...
                                        RM_VEE_FLASH_W_LOGICAL_END_ADDRESS - p_ctrl->segment_size,
                                                ...
                                                bool                                 contains_refdata)
{
    ...
    {
        err = rm_vee_blocking_erase(p_ctrl, seg_addr, (p_ctrl->segment_size / RM_VEE_FLASH_W_DF_BLOCK_SIZE));
    }
    else
    {
 ...
 *                                      (called by Open() or VEE_CMD_FORMAT. State=RM_VEE_FLASH_W_PRV_STATES_UNITIALIZED)
...
static fsp_err_t rm_vee_load_record_table (rm_vee_flash_w_instance_ctrl_t * const p_ctrl, bool initial_load)
{
    ...
    p_ctrl->ref_hdr_addr -= RM_VEE_FLASH_W_REF_DATA_AREA_SIZE;
    ...
    while (addr < p_ctrl->ref_hdr_addr)
    {
        ...
        if (initial_load == true)
        {
            ...
            err = rm_vee_blocking_blankcheck(p_ctrl, addr, RM_VEE_FLASH_W_DF_WRITE_SIZE, &event);
            FSP_ERROR_RETURN(FSP_SUCCESS == err, err);
        ...
        if (RM_VEE_FLASH_W_REC_DATA_MAX_SIZE >= p_hdr->length)
        {
        ...
        if (RM_VEE_FLASH_W_VALID_CODE == p_end->valid_code)
        {
...
static fsp_err_t rm_vee_inspect_new_refdata (rm_vee_flash_w_instance_ctrl_t * const p_ctrl)
{
    ...
    addr = p_ctrl->active_seg_addr + (p_ctrl->segment_size - RM_VEE_FLASH_W_REF_DATA_AREA_SIZE);
    ...
    else
    {
        ...
        if (RM_VEE_FLASH_W_VALID_CODE == p_hdr->valid_code)
        {
    ...
    fsp_err_t err2 = rm_vee_blocking_blankcheck(p_ctrl, addr, RM_VEE_FLASH_W_DF_WRITE_SIZE, &event);
                                             ...
                                             flash_event_t                      * p_event)
{
    ...
    {
#if SUPPORT_MEDIA_BLOCK_OPERATION
        rm_block_media_info_t   mediainfo;

        p_ctrl->p_media->p_api->infoGet(p_ctrl->p_media->p_ctrl, &mediainfo);

        uint32_t blk_address = RM_CONV_XIP2MEDIA(addr) / mediainfo.sector_size_bytes;
        uint32_t num_blocks  = (num_bytes + (mediainfo.sector_size_bytes - 1)) / mediainfo.sector_size_bytes;

        FSP_ERROR_RETURN(num_blocks <= mediainfo.num_sectors, FSP_ERR_INVALID_BLOCKS);

        uint8_t *tmpbuffer = &(p_ctrl->media_buffer[0]);

        result = FLASH_RESULT_BLANK;

        uint32_t startoffset = (addr) % mediainfo.sector_size_bytes;
        uint32_t roundsize = num_bytes;

        for (uint32_t idx = 0; idx < num_blocks; idx++)
        {
            /* A special interrupt from blankcheck() is required to invoke the vee callback. */
            err = p_ctrl->p_media->p_api->read(p_ctrl->p_media->p_ctrl, tmpbuffer, (blk_address + idx), 1);
            FSP_ERROR_RETURN(FSP_SUCCESS == err, err);

            uint32_t endoffset = ((roundsize + startoffset) > mediainfo.sector_size_bytes) ?
                                            mediainfo.sector_size_bytes : (roundsize + startoffset);

            for (uint32_t doffset = startoffset; doffset < endoffset; doffset++)
            {
                if (tmpbuffer[doffset] != (255U))
                {
            ...
            roundsize = roundsize - endoffset;
            startoffset = 0;
        }
#else
        rm_block_media_info_t   mediainfo;

        p_ctrl->p_media->p_api->infoGet(p_ctrl->p_media->p_ctrl, (rm_block_media_info_t *) &mediainfo);
        ...
        for (uint32_t idx = 0; idx < num_blocks; idx++)
        {
            one_block = num_bytes / mediainfo.sector_size_bytes ? mediainfo.sector_size_bytes
                                                                : num_bytes % mediainfo.sector_size_bytes;
            memcpy((uint8_t *) tmpbuffer, (uint8_t *) (addr + (idx * mediainfo.sector_size_bytes)), one_block);
            FSP_ERROR_RETURN(FSP_SUCCESS == err, err);

            for (uint32_t doffset = 0; doffset < one_block; doffset++)
            {
                if (tmpbuffer[doffset] != (255U))
                {
                    result = FLASH_RESULT_NOT_BLANK;
        ...
        /* RM_BLOCK_MEDIA_SPI does not support the blank checking event. */
        if (FLASH_RESULT_BLANK == result)
        {
            p_ctrl->flash_event = FLASH_EVENT_BLANK; /* pseudo event */
        }
        else
        {
            p_ctrl->flash_event = FLASH_EVENT_NOT_BLANK; /* pseudo event */
        }
    }
    else
    {
     ...
     * Blankcheck is for RM_VEE_FLASH_W_DF_WRITE_SIZE to p_ctrl->segment_size bytes.
     */
    for (i = 0; i < RM_VEE_FLASH_W_TIMEOUT_US; i++)
    {
    ...
    *p_event = p_ctrl->flash_event;
    ...
    FSP_ERROR_RETURN(RM_VEE_FLASH_W_TIMEOUT_US != i, FSP_ERR_TIMEOUT);
...
static fsp_err_t rm_vee_blocking_erase (rm_vee_flash_w_instance_ctrl_t * const p_ctrl, uint32_t addr, uint32_t num_blocks)
{
    ...
    {
        err = rm_vee_media_erase(p_ctrl, addr, num_blocks);
    }
    else
    {
        err = FSP_ERR_ASSERTION;
    ...
    /* Poll for interrupt complete. Do timeout check in case app disabled interrupts.
     * Erase is for (p_ctrl->segment_size/RM_VEE_FLASH_W_DF_BLOCK_SIZE).
     */
    for (i = 0; (i < RM_VEE_FLASH_W_TIMEOUT_US) && (p_ctrl->irq_flag != true); i++)
    {
        R_BSP_SoftwareDelay(1, BSP_DELAY_UNITS_MICROSECONDS);
    }
                                        ...
                                        uint32_t                             num_bytes)
{
    ...
    {
        err = rm_vee_media_write(p_ctrl, src_addr, dst_addr, num_bytes);
    }
    else
    {
        err = FSP_ERR_ASSERTION;
                                            ...
                                            uint32_t                             num_bytes)
{
    ...
    if (RM_VEE_ADDRESS_ALIGN(p_ctrl->next_write_addr + RM_VEE_FLASH_W_REC_OVERHEAD + num_bytes) <= p_ctrl->ref_hdr_addr)
    {
    ...
    {
        /* Start refresh */
        p_ctrl->delegation.refseg_in_progress = true;
 ...
 * driver control structure and the driver mode is changed to RM_VEE_FLASH_W_PRV_MODE_REFRESH. If this function is called
 * due to RM_VEE_FLASH_W_PRV_REFRESH_RECORD_OVFL (lack of record space), the refresh begins with writing the record passed
 ...
 * @param  rec_id                   Record ID if refresh type is RM_VEE_FLASH_W_PRV_REFRESH_RECORD_OVFL. NULL otherwise.
 * @param  addr                     Address of data to write. NULL if refresh type is RM_VEE_FLASH_W_PRV_REFRESH_USER_REQ.
 * @param  num_bytes                Number of bytes to write. NULL if refresh type is RM_VEE_FLASH_W_PRV_REFRESH_USER_REQ.
 *
 * @retval FSP_SUCCESS              Successful.
 * @retval FSP_ERR_PE_FAILURE       This error indicates that a flash programming, erase, or blankcheck operation has failed
 * @retval FSP_ERR_TIMEOUT          Flash write timed out (Should not be possible when flash bgo is used).
                                           ...
                                           uint32_t                             num_bytes)
{
    ...
    MEASURE_START(refresh);
    /* Determine first record ID to refresh if not passed in */
    if (RM_VEE_FLASH_W_PRV_REFRESH_RECORD_OVFL != refresh_type)
    {
    ...
    if (0 != p_ctrl->p_cfg->ref_data_size)
    {
        ...
        if (RM_VEE_FLASH_W_PRV_REFRESH_REFDATA_OVFL == refresh_type)
        {
    ...
    if (RM_VEE_FLASH_W_LOGICAL_END_ADDRESS == new_seg_addr)
    {
    ...
    p_ctrl->ref_hdr_addr         = p_ctrl->active_seg_addr + (p_ctrl->segment_size - RM_VEE_FLASH_W_REF_DATA_AREA_SIZE);
    ...
    if (rec_id <= p_ctrl->p_cfg->record_max_id)
    {
        ...
        else
        {
            ...
            err = rm_vee_xfer_next_chunk(p_ctrl, RM_VEE_FLASH_W_PRV_STATES_WRITE_REC_REFRESH);
        }
    }
    else
    {
        ...
        if (RM_VEE_FLASH_W_PRV_REFRESH_REFDATA_OVFL == refresh_type)
        {
            ...
            p_ctrl->state = RM_VEE_FLASH_W_PRV_STATES_WRITE_REFDATA;
        ...
        else if ((true == p_ctrl->factory_refdata) || (true == p_ctrl->new_refdata_valid))
        {
            ...
            err = rm_vee_xfer_next_chunk(p_ctrl, RM_VEE_FLASH_W_PRV_STATES_WRITE_REFDATA);
        }
        else
#endif
        {
 ...
 **********************************************************************************************************************/
static void rm_vee_call_callback (rm_vee_flash_w_instance_ctrl_t * p_ctrl, rm_vee_callback_args_t * p_args)
{
    ...
    else
    {
        ...
        rm_vee_flash_w_prv_ns_callback p_callback = (rm_vee_flash_w_prv_ns_callback) (p_ctrl->p_callback);
 ...
 *
 * Normal Mode:
 * Write a record: [RM_VEE_FLASH_W_PRV_STATES_WRITE_REC_HDR], WRITE_REC_DATA, WRITE_REC_END
 *    (header only written with variable recs)
 * Write reference data: RM_VEE_FLASH_W_PRV_STATES_WRITE_NEW_REFDATA. WRITE_NEW_REFDATA_HDR
 *
 * Refresh Mode:
 * [RM_VEE_FLASH_W_PRV_STATES_WRITE_REC_HDR, WRITE_REC_DATA, WRITE_REC_END] if started with RECORD_OVFL
 * RM_VEE_FLASH_W_PRV_STATES_WRITE_REC_REFRESH (uses interim RAM buffer; and how start if not RECORD_OVFL)
 * RM_VEE_FLASH_W_PRV_STATES_WRITE_REF_DATA
 * RM_VEE_FLASH_W_PRV_STATES_WRITE_SEG_HDR
 * RM_VEE_FLASH_W_PRV_STATES_ERASE_SEG
 *
...
void rm_vee_flash_w_callback (flash_callback_args_t * p_args)
{
    ...
    rm_vee_flash_w_instance_ctrl_t * p_ctrl = (rm_vee_flash_w_instance_ctrl_t *) p_args->p_context;
    fsp_err_t err = FSP_SUCCESS;

    p_ctrl->flash_event = flash_event;

    /* Starting up. Blocking on various events. Just set flag. */
    if (RM_VEE_FLASH_W_PRV_STATES_UNINITIALIZED == p_ctrl->state)
    {
        p_ctrl->irq_flag = true;
    ...
    if (p_ctrl->delegation.wr_in_continue == true)
        return;
    ...
    if (FLASH_EVENT_WRITE_COMPLETE == p_ctrl->flash_event)
    {
        switch (p_ctrl->state)
        {
            case RM_VEE_FLASH_W_PRV_STATES_WRITE_REC_HDR:
            {
                ...
                p_ctrl->state = RM_VEE_FLASH_W_PRV_STATES_WRITE_REC_DATA;

                /* If the record can be written in one write set the next state to write end. */
                if (0 == (p_ctrl->rec_hdr.length % RM_VEE_FLASH_W_DF_WRITE_SIZE))
                {
                    p_ctrl->state = RM_VEE_FLASH_W_PRV_STATES_WRITE_REC_DATA;
                }
                else if (RM_VEE_FLASH_W_DF_WRITE_SIZE > p_ctrl->rec_hdr.length)
                {
                    /* The record is smaller than the DF write size and needs to be buffered before write. */
                    memset(&p_ctrl->data_buffer[0], 0, RM_VEE_FLASH_W_DF_WRITE_SIZE);
                    memcpy(&p_ctrl->data_buffer[0],
                           p_ctrl->p_rec_data,
                           p_ctrl->rec_hdr.length & RM_VEE_FLASH_W_DF_WRITE_MASK);
                    p_ctrl->state = RM_VEE_FLASH_W_PRV_STATES_WRITE_REC_DATA;
                    src           = (uint32_t) &p_ctrl->data_buffer[0];
                    length        = RM_VEE_FLASH_W_DF_WRITE_SIZE;
                }
                else
                {
                    /* The end of the record must be written seperately. */
                    length        = length & (~RM_VEE_FLASH_W_DF_WRITE_MASK);
                    p_ctrl->state = RM_VEE_FLASH_W_PRV_STATES_WRITE_REC_DATA_TAIL;
                }
                ...
                {
                    err = rm_vee_media_write(p_ctrl, src, p_ctrl->next_write_addr, length);
                }  
                else
                {
                    err = FSP_ERR_ASSERTION;
            ...
            case RM_VEE_FLASH_W_PRV_STATES_WRITE_REC_DATA_TAIL:
            {
                uint32_t written = p_ctrl->rec_hdr.length & (~RM_VEE_FLASH_W_DF_WRITE_MASK);
            ...
            case RM_VEE_FLASH_W_PRV_STATES_WRITE_REC_DATA:
            {
                /* Record data write complete */
                if (0 != (p_ctrl->rec_hdr.length % RM_VEE_FLASH_W_DF_WRITE_SIZE))
                {
                    p_ctrl->next_write_addr += RM_VEE_FLASH_W_DF_WRITE_SIZE;
                }
                else
                {
                ...
                p_ctrl->state = RM_VEE_FLASH_W_PRV_STATES_WRITE_REC_END;
                if (NULL != p_ctrl->p_flash)
                {
                    err           = p_ctrl->p_flash->p_api->write(p_ctrl->p_flash->p_ctrl,
            ...
            case RM_VEE_FLASH_W_PRV_STATES_WRITE_REC_END:
            {
            ...
            case RM_VEE_FLASH_W_PRV_STATES_WRITE_REC_REFRESH:
            {
                /* Update the transfer info which is delegated by tasks. */
                uint32_t length = p_ctrl->delegation.length_update;
...
#if RM_VEE_FLASH_W_CFG_REF_DATA_SUPPORT
            case RM_VEE_FLASH_W_PRV_STATES_WRITE_REFDATA:
            {
                /* Update the transfer info which is delegated by tasks. */
                uint32_t length = p_ctrl->delegation.length_update;
            ...
            case RM_VEE_FLASH_W_PRV_STATES_WRITE_NEW_REFDATA:
            {
                /* New refdata location written; write the header to show that it is used */
                p_ctrl->state = RM_VEE_FLASH_W_PRV_STATES_WRITE_NEW_REFDATA_HDR;
                if (NULL != p_ctrl->p_flash)
                {
                    err           = p_ctrl->p_flash->p_api->write(p_ctrl->p_flash->p_ctrl,
            ...
            case RM_VEE_FLASH_W_PRV_STATES_WRITE_NEW_REFDATA_HDR:
            {
            ...
            case RM_VEE_FLASH_W_PRV_STATES_WRITE_SEG_HDR:
            {
                 ...
                 * RM_VEE_FLASH_W_PRV_STATES_WRITE_REFDATA. But functionally there is no harm in waiting until now to do it.
                ...
                p_ctrl->ref_hdr_addr = p_ctrl->active_seg_addr +
                                       (p_ctrl->segment_size - RM_VEE_FLASH_W_REF_DATA_AREA_SIZE);
                p_ctrl->new_refdata_valid = false;
                ...
                p_ctrl->state = RM_VEE_FLASH_W_PRV_STATES_ERASE_SEG;
                p_ctrl->delegation.wr_in_progress = false;
                ...
                {
                        err = rm_vee_media_erase(p_ctrl, p_ctrl->refresh_src_seg_addr,
                                                    (p_ctrl->segment_size / RM_VEE_FLASH_W_DF_BLOCK_SIZE));
                } 
                else
                {
    ...
    else
    {
         ...
         */
        if ((RM_VEE_FLASH_W_PRV_MODE_REFRESH == p_ctrl->mode) && (RM_VEE_FLASH_W_PRV_STATES_ERASE_SEG != p_ctrl->state))
        {
            err = rm_vee_restore_previous_seg(p_ctrl);
        }

        p_ctrl->flash_err_event = p_ctrl->flash_event;
        p_ctrl->mode            = RM_VEE_FLASH_W_PRV_MODE_FLASH_PE_FAIL;
        p_ctrl->state           = RM_VEE_FLASH_W_PRV_STATES_READY;
    }

    rm_vee_flash_w_err_handle(p_ctrl, err);

    if ((RM_VEE_FLASH_W_PRV_STATES_READY == p_ctrl->state) && (NULL != p_ctrl->p_callback))
    {
        rm_vee_callback_args_t args;
...
{
    if ((FSP_SUCCESS != err) &&
        ((p_ctrl->mode == RM_VEE_FLASH_W_PRV_MODE_NORMAL) || (p_ctrl->mode == RM_VEE_FLASH_W_PRV_MODE_REFRESH)))
    {
        p_ctrl->mode  = RM_VEE_FLASH_W_PRV_MODE_FLASH_PE_FAIL;
        p_ctrl->state = RM_VEE_FLASH_W_PRV_STATES_READY;
    }
...
/***********************************************************************************************************************
 * Function Name: rm_vee_media_callback_preprocess
 * Description  : preprocessor for rm_vee_media_callback
 * @param  p_args    callback arguments used to process the flash event.
 **********************************************************************************************************************/
fsp_err_t rm_vee_media_callback_preprocess (rm_block_media_callback_args_t * p_margs, flash_callback_args_t * p_fargs)
{
    fsp_err_t    flag_callcb = FSP_ERR_IN_USE;
    rm_vee_flash_w_instance_ctrl_t * p_ctrl = (rm_vee_flash_w_instance_ctrl_t *) p_margs->p_context;
    rm_block_media_spi_w_instance_ctrl_t *p_bmctrl = (rm_block_media_spi_w_instance_ctrl_t *)p_ctrl->p_media->p_ctrl;

    p_fargs->p_context = p_margs->p_context;

    switch (p_margs->event)
    {
        case RM_BLOCK_MEDIA_EVENT_OPERATION_COMPLETE:
            if (true == p_bmctrl->erase_in_progress)
            {
                if (p_ctrl->delegation.wr_in_progress == false)
                {
                    p_fargs->event = FLASH_EVENT_ERASE_COMPLETE;
                    p_ctrl->irq_flag = true;
                    flag_callcb = FSP_SUCCESS;
                }
            }
            else if (true == p_bmctrl->write_in_progress)
            {
                p_fargs->event = FLASH_EVENT_WRITE_COMPLETE;
                p_ctrl->irq_flag = true;
                flag_callcb = FSP_SUCCESS;
            }
            else
            {
                //p_fargs->event = FLASH_EVENT_NOT_BLANK;
                p_ctrl->irq_flag = true;
                //flag_callcb = FSP_SUCCESS;
            }
            break;
        case RM_BLOCK_MEDIA_EVENT_ERROR:
            p_fargs->event = FLASH_EVENT_ERR_FAILURE;
            p_ctrl->irq_flag = true;
            p_ctrl->delegation.media_error = true;
            flag_callcb = FSP_SUCCESS;  
            break;          
        default:
            break;
    ...
    switch (p_ctrl->state)
    {
        case RM_VEE_FLASH_W_PRV_STATES_READY:
        {
            p_ctrl->irq_flag = true;
            break;
        }
...
/***********************************************************************************************************************
 * Function Name: rm_vee_media_callback
 * Description  : wrapper for rm_vee_flash_w_callback
 * @param  p_args    callback arguments used to process the flash event.
 **********************************************************************************************************************/
void rm_vee_media_callback (rm_block_media_callback_args_t * p_args)
{
    ...
    if (p_ctrl->delegation.pendevent.rdptr 
            == ((p_ctrl->delegation.pendevent.wrptr + 1) % RM_VEE_PEND_EVENT_LIST_SIZE) )
    {
        /* event overflow !!!! */
    ...
    {
        flash_callback_args_t *wrfargs;

        wrfargs = &(p_ctrl->delegation.pendevent.event[p_ctrl->delegation.pendevent.wrptr]);

        err = rm_vee_media_callback_preprocess(p_args, wrfargs);

        if (FSP_SUCCESS == err)
        {
            p_ctrl->delegation.pendevent.wrptr 
                = (uint8_t) ((p_ctrl->delegation.pendevent.wrptr  + 1) % RM_VEE_PEND_EVENT_LIST_SIZE);
        }
    }
    
    if (FSP_SUCCESS == err)
    {
        if (p_ctrl->delegation.pendevent.pended == false)
        {
            /* soft mutex */
            p_ctrl->delegation.pendevent.pended = true;

            while (p_ctrl->delegation.pendevent.rdptr !=  p_ctrl->delegation.pendevent.wrptr)
            {
                flash_callback_args_t *rdfargs;

                rdfargs = &(p_ctrl->delegation.pendevent.event[p_ctrl->delegation.pendevent.rdptr]);
                rm_vee_flash_w_callback(rdfargs);

                p_ctrl->delegation.pendevent.rdptr 
                    = (uint8_t) ((p_ctrl->delegation.pendevent.rdptr  + 1) % RM_VEE_PEND_EVENT_LIST_SIZE);
            }
...
/*******************************************************************************************************************//**
 * Function Name: rm_vee_media_write
 * Description  : wrapper for equivalent support to flash_api_t::write
 *
 * Example:
 *
 ...
 **********************************************************************************************************************/
BSP_PLACE_CODE_IN_RAM fsp_err_t rm_vee_media_write(rm_vee_flash_w_instance_ctrl_t * const p_ctrl,
                        uint32_t const src_address, uint32_t const flash_address,
                        uint32_t const num_bytes)
{
    fsp_err_t   err;
    ...
    p_ctrl->p_media->p_api->infoGet(p_ctrl->p_media->p_ctrl, &mediainfo);
#if SUPPORT_MEDIA_BLOCK_OPERATION
    uint32_t blk_address = RM_CONV_XIP2MEDIA(flash_address) / mediainfo.sector_size_bytes;
#endif
    uint32_t num_blocks  = (num_bytes + (mediainfo.sector_size_bytes - 1)) / mediainfo.sector_size_bytes;

    FSP_ERROR_RETURN(num_blocks <= mediainfo.num_sectors, FSP_ERR_INVALID_BLOCKS);

    #define  secrange(x) ((x)%mediainfo.sector_size_bytes)

    uint8_t  *srcmemory = (uint8_t *)src_address;
#if SUPPORT_MEDIA_BLOCK_OPERATION
    uint32_t startoffset = (flash_address) % mediainfo.sector_size_bytes;
    uint32_t roundsize = num_bytes ;
    uint32_t ridx = 0;
#endif
...
#if SUPPORT_MEDIA_BLOCK_OPERATION
    while (ridx < num_bytes)
    {
        uint8_t *tmpbuffer = &(p_ctrl->media_buffer[0]);

        /* dump the data directly using the automode address to avoid calling the callback. */
        memcpy(tmpbuffer, 
                (void *)RM_CONV_MEDIA2XIP(blk_address * mediainfo.sector_size_bytes), 
                mediainfo.sector_size_bytes);

        uint32_t endoffset = ((roundsize + startoffset) > mediainfo.sector_size_bytes) ? 
                                    mediainfo.sector_size_bytes : (roundsize + startoffset) ;
        
        uint32_t burstsize = (endoffset - startoffset);

        for (uint32_t idx = 0; idx < burstsize; idx++)
        {
            if ((tmpbuffer[secrange(startoffset+idx)]) != (255U))
            {
                return FSP_ERR_IN_USE;
            }

            tmpbuffer[secrange(startoffset+idx)] = srcmemory[(ridx+idx)];
        }

        p_ctrl->delegation.wr_in_progress = true;
        if (burstsize < num_bytes)
        {
            p_ctrl->delegation.wr_in_continue = true;
        }
        err = p_ctrl->p_media->p_api->write(p_ctrl->p_media->p_ctrl,
                                            tmpbuffer, blk_address, 1);
        p_ctrl->delegation.wr_in_progress = false;
        FSP_ERROR_RETURN(FSP_SUCCESS == err, err);

        roundsize = roundsize - burstsize;
        startoffset = 0;
        ridx = ridx + burstsize;
        blk_address = blk_address + 1;
    }
#else
    spi_flash_instance_t * p_spi_flash = ((rm_block_media_spi_w_instance_ctrl_t *) p_ctrl->p_media->p_ctrl)->p_spi_flash;
    spi_flash_status_t     status;

    FSP_CRITICAL_SECTION_DEFINE;

    FSP_CRITICAL_SECTION_ENTER;
    p_spi_flash->p_api->xipExit(p_spi_flash->p_ctrl);
    err = p_spi_flash->p_api->write(p_spi_flash->p_ctrl, srcmemory, (uint8_t * const) flash_address, num_bytes);
    p_spi_flash->p_api->xipEnter(p_spi_flash->p_ctrl);
    ...
    {
        .event     = event,
        .p_context = p_ctrl->p_media->p_cfg->p_context
    };

    ((rm_block_media_spi_w_instance_ctrl_t *)p_ctrl->p_media->p_ctrl)->write_in_progress = true;
    rm_vee_media_callback(&args);

    ((rm_block_media_spi_w_instance_ctrl_t *)p_ctrl->p_media->p_ctrl)->write_in_progress = false;
    p_ctrl->delegation.wr_in_progress = false;
    FSP_ERROR_RETURN(FSP_SUCCESS == err, err);
#endif
...
/*******************************************************************************************************************//**
 * Function Name: rm_vee_media_erase
 * Description  : wrapper for equivalent support to flash_api_t::erase
 *
 * Example:
 *
 ...
 * @retval     FSP_ERR_PE_FAILURE       Failed to enter or exit P/E mode.
 **********************************************************************************************************************/
fsp_err_t rm_vee_media_erase(rm_vee_flash_w_instance_ctrl_t * const p_ctrl, uint32_t const address, uint32_t const num_blocks)
{
        rm_block_media_info_t   mediainfo;

        p_ctrl->p_media->p_api->infoGet(p_ctrl->p_media->p_ctrl, &mediainfo);

        uint32_t blk_address = RM_CONV_XIP2MEDIA(address) / mediainfo.sector_size_bytes;
        uint32_t num_of_blks = num_blocks;

        FSP_ERROR_RETURN(num_blocks <= mediainfo.num_sectors, FSP_ERR_INVALID_BLOCKS);

        return p_ctrl->p_media->p_api->erase(p_ctrl->p_media->p_ctrl, blk_address, num_of_blks);
}
...
static fsp_err_t rm_vee_init_record_xfer (rm_vee_flash_w_instance_ctrl_t * const p_ctrl, uint32_t id)
{
    ...
    p_ctrl->refresh_xfer_bytes_left = RM_VEE_ADDRESS_ALIGN(p_hdr->length + RM_VEE_FLASH_W_REC_OVERHEAD);
    ...
    if ((p_ctrl->next_write_addr + p_ctrl->refresh_xfer_bytes_left) >
        (p_ctrl->active_seg_addr + (p_ctrl->segment_size - RM_VEE_FLASH_W_REF_DATA_AREA_SIZE)))
    {
        /* Make previous segment active again and set error mode */
        err = rm_vee_restore_previous_seg(p_ctrl);

        p_ctrl->mode  = RM_VEE_FLASH_W_PRV_MODE_OVERFLOW;
        p_ctrl->state = RM_VEE_FLASH_W_PRV_STATES_READY;
    }
...
static fsp_err_t rm_vee_xfer_next_chunk (rm_vee_flash_w_instance_ctrl_t * const p_ctrl, rm_vee_flash_w_prv_states_t state)
{
    ...
    if (RM_VEE_FLASH_W_PRV_MODE_OVERFLOW != p_ctrl->mode)
    {
        /* Determine number of bytes to transfer in this chunk */
        if (RM_VEE_FLASH_W_REFRESH_BUFFER_SIZE <= p_ctrl->refresh_xfer_bytes_left)
        {
            length = RM_VEE_FLASH_W_REFRESH_BUFFER_SIZE;
        }
        else
        {
        ...
        /* The approach, that manages the transfer info, will change from post-updated to delegated. */
        p_ctrl->delegation.length_update = length;
...
static fsp_err_t rm_vee_write_seg_hdr (rm_vee_flash_w_instance_ctrl_t * const p_ctrl)
{
    p_ctrl->state = RM_VEE_FLASH_W_PRV_STATES_WRITE_SEG_HDR;
    ...
    {
        return rm_vee_media_write(p_ctrl, 
                                 (uint32_t) &p_ctrl->seg_hdr,
                                 p_ctrl->active_seg_addr,
                                 sizeof(rm_vee_seg_hdr_t));
    }                                          
...
static fsp_err_t rm_vee_state_get (rm_vee_flash_w_instance_ctrl_t * const p_ctrl, rm_vee_state_t * p_state)
{
    if (RM_VEE_FLASH_W_PRV_MODE_NORMAL == p_ctrl->mode)
    {
        if (RM_VEE_FLASH_W_PRV_STATES_READY == p_ctrl->state)
        {
        ...
        else
        {
            ...
            else if (NULL != p_ctrl->p_media)
            {
                ...
                {
                    case RM_VEE_FLASH_W_PRV_STATES_WRITE_REC_HDR:
                    case RM_VEE_FLASH_W_PRV_STATES_ERASE_SEG:
                    {
                        if (RM_VEE_FLASH_W_PRV_MODE_FLASH_PE_FAIL != p_ctrl->state)
                        {
                            p_ctrl->state = RM_VEE_FLASH_W_PRV_STATES_READY;
                        }
                        break;
    ...
    else
    {
            ...
            else if (NULL != p_ctrl->p_media)
            {
                *p_state = (rm_vee_state_t) p_ctrl->mode;
                ...
                if (RM_VEE_FLASH_W_PRV_MODE_REFRESH == p_ctrl->mode)
                {
                    p_ctrl->mode  = RM_VEE_FLASH_W_PRV_MODE_NORMAL;
                    p_ctrl->state = RM_VEE_FLASH_W_PRV_STATES_READY;
                }                
