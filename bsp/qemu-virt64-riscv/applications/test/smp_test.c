/*
 * SMP smoke test for qemu-virt64-riscv.
 *
 * The test is intentionally exposed as a shell command so it can be repeated
 * without changing the normal BSP boot sequence.
 */
...
static int smp_test(int argc, char **argv)
{
    ...
    /* Create and bind all workers before starting any of them. */
    for (int i = 0; i < RT_CPUS_NR; i++)
    {
        worker_args[i].cpu = i;
    ...
    for (int i = 0; i < RT_CPUS_NR; i++)
    {
        if (rt_sem_take(done, RT_TICK_PER_SECOND * 5) != RT_EOK)
        {
    ...
    /* Synchronous SMP-call must execute once on every selected CPU. */
    for (int round = 0; round < rounds; round++)
    {
        for (int cpu = 0; cpu < RT_CPUS_NR; cpu++)
        {
...
MSH_CMD_EXPORT(smp_test, "run qemu-virt64-riscv SMP test");

#else

static int smp_test(int argc, char **argv)
{
    rt_kprintf("smp_test requires RT_USING_SMP\n");
    return -RT_ENOSYS;
}
MSH_CMD_EXPORT(smp_test, "run qemu-virt64-riscv SMP test");
